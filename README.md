...
This repository is an [Agent Plugins 1.0](https://agent-plugins.org/) plugin. It exposes the portable `setup-cao`, `add-cao-package`, `create-cao-package`, `analyze-cao`, `cao-cli`, and `codebase-model` skills.

Install this repository using any [compatible client's](https://agent-plugins.org/compatible-clients) plugin installer, then invoke `setup-cao` to create a control plane from scratch, `add-cao-package` to discover and safely install an existing catalog package, `create-cao-package` with an operational strategy and target-repository description, `analyze-cao` to download and query CAO activity data with the `cao` CLI, `cao-cli` for a general reference on using `cao` in local development and inside agentic workflow runs, or `codebase-model` to compile an existing `ARCHITECTURE.md` into `CODEBASE.yml`.
