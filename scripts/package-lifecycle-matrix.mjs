...
const suites = [
  ...
  },
  {
    ...
    prefixes: [".github/graders/aw-failures-", ".github/graders/aw-maintenance-", ".github/workflows/aw-", ".github/workflows/shared/"],
  },
  {
    name: "EU CRA",
  ...
  },
  {
    ...
    prefixes: [".github/workflows/shared/", ".github/workflows/uk-ai-advisory"],
  },
  {
    name: "SelfCare",
    manifest: "self-care/aw.yml",
    testPattern: "focused SelfCare package contract",
    prefixes: [".github/graders/self-care-", ".github/workflows/self-care", ".github/workflows/shared/"],
  },
  {
    ...
    prefixes: [".github/graders/software-development-practices-", ".github/workflows/shared/", ".github/workflows/software-development-practices"],
  },
  {
    name: "dashboard",
  ...
  },
  {
    ...
    prefixes: [".github/graders/dependabot-", ".github/workflows/dependabot", ".github/workflows/shared/"],
  },
];
...
export function selectPackageLifecycleSuites(changedFiles, root = process.cwd()) {
  if (changedFiles === null)
    return suites.map(({ name, testPattern }) => ({
      name,
      "test-pattern": testPattern,
    }));

  const normalized = changedFiles.map((file) => file.replaceAll("\\", "/"));
  if (normalized.some((file) => ["scripts/package-lifecycle-matrix.mjs", "tests/integration/package-lifecycle.test.mjs"].includes(file))) {
    return selectPackageLifecycleSuites(null, root);
  }
  return suites
    .filter((suite) => {
      const packageDirectory = posix.dirname(suite.manifest);
      const prefixes = packageDirectory === "." ? suite.prefixes : [`${packageDirectory}/`, ...suite.prefixes];
...
if (invokedPath === import.meta.url) {
  ...
  const files = process.argv[2] === "--all" ? null : changedFiles(process.argv[2], process.argv[3], root);
