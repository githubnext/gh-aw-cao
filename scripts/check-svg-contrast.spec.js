 ...
 *
 * Uses Playwright to render each tracked SVG file in a headless browser and
 * verify that every text element has sufficient WCAG contrast against its
 * effective background color. Visuals that opt into the brand contract
 * (`data-visual-kind` on the root <svg>, see scripts/check-svg-visual-language.mjs)
 * use WCAG AA text thresholds; unannotated legacy files retain the previous
...
const svgFiles = envFiles
  ...
  : [...findSvgFiles(path.join(repoRoot, "docs", "assets")), ...findSvgFiles(path.join(repoRoot, "public", "assets"))];
...
for (const svgPath of svgFiles) {
  ...
  test(`color contrast: ${relPath}`, async ({ page }) => {
    ...
    // Embed the SVG in a plain HTML page — this simulates how GitHub renders
    // SVG images in README files and docs. Dark-mode picture-source variants
    // (e.g. `*-dark-mode.svg`, selected via `prefers-color-scheme: dark`) omit
    // an opaque full-canvas background rect and rely on the host page's
    // surface color instead, so use the SVG's own declared `--surface` custom
    // property as the simulated page background when present.
    const surfaceMatch = /--surface\s*:\s*([^;}\s]+)/.exec(svgContent);
    const pageBackground = surfaceMatch ? surfaceMatch[1] : "#ffffff";
...
<body>${svgContent}</body>
    ...
    const measurements = await page.evaluate(() => {
      ...
      for (const textEl of textEls) {
        ...
        // Default background: the simulated host page's background color.
        const pageBg = window.getComputedStyle(document.body).backgroundColor;
        let bgColor = parseColorStr(pageBg) || { r: 255, g: 255, b: 255 };
        let bgSource = `page background (${pageBg})`;
        ...
        const candidates = Array.from(document.querySelectorAll("svg rect, svg circle, svg ellipse, svg path, svg polygon"));
        ...
        for (const bg of candidates) {
          ...
          // Prefer the computed fill so CSS attribute-selector re-theming (used by
          // this repo's light/dark SVG variants, e.g. `[fill="#ffffff"]{fill:var(--surface)}`)
          // is reflected instead of the literal `fill` attribute value.
          const computedFill = window.getComputedStyle(bg).fill;
          const fillAttr = (computedFill && computedFill !== "none" ? computedFill : bg.getAttribute("fill")) || "";
          ...
          const contains = textCx >= bgBbox.left && textCx <= bgBbox.right && textCy >= bgBbox.top && textCy <= bgBbox.bottom;
    ...
    const violations = measurements
      .map((measurement) => ({
        ...measurement,
        threshold: requiredTextContrast(measurement.fontSize, measurement.fontWeight, enforcesBrandContrast),
      }))
    ...
    if (violations.length > 0) {
      const report = violations
        ...
        .join("\n\n");
      expect(violations, `${violations.length} color contrast violation(s) found in ${relPath}:\n\n${report}`).toHaveLength(0);
    }
  });
}
