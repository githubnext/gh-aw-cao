...
function selectedFiles() {
  if (!process.env.SVG_FILES) {
    return [...findSvgFiles(path.join(root, "docs", "assets")), ...findSvgFiles(path.join(root, "public", "assets"))];
  }
...
function referencedText(svg, id) {
  ...
  const match = new RegExp(`<([\\w:-]+)\\b[^>]*\\bid\\s*=\\s*(?:"${escapedId}"|'${escapedId}')[^>]*>([\\s\\S]*?)<\\/\\1\\s*>`, "i").exec(svg);
...
function checkSvg(svgPath) {
  ...
  if (isThemeVariant) {
    ...
    const counterpart = svgPath.replace(
      /-(light|dark)(\.svg)$/i,
      (_, theme, extension) => `-${theme.toLowerCase() === "light" ? "dark" : "light"}${extension}`,
    );
