...
function checkSvg(svgPath) {
  ...
  const visualKind = (
    attribute(rootAttributes, 'data-visual-kind') || ''
  ).toLowerCase()
  ...
  if (!ariaLabel && !labelledBy) {
    ...
    const unresolved = labelledBy
      .split(/\s+/)
      .filter((id) => !referencedText(svg, id))
    if (unresolved.length > 0) {
      violations.push(
        `aria-labelledby references missing or empty elements: ${unresolved.join(', ')}.`,
      )
    }
  }
  ...
  if (!visualKinds.has(visualKind)) {
    ...
    if (!new RegExp(`\\b${childAttribute}\\s*=`, 'i').test(svg)) {
      violations.push(
        `Declared ${visualKind} has no ${childAttribute} attributes.`,
      )
    }
    if (
      visualKind !== 'infographic' &&
      /<(?:linearGradient|radialGradient)\b/i.test(svg)
    ) {
  ...
  if (isThemeVariant) {
    ...
    const dimensions =
      /^0(?:\.0+)?\s+0(?:\.0+)?\s+(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)$/.exec(
        viewBox,
      )
    if (!dimensions) {
      violations.push(
        `Theme variant has an invalid viewBox: ${viewBox || 'missing'}.`,
      )
    } else if (!approvedWidths.has(Number(dimensions[1]))) {
      violations.push(
        `Theme variant uses unsupported canvas width ${dimensions[1]}.`,
      )
    }

    const counterpart = svgPath.replace(
      /-(light|dark)(\.svg)$/i,
      (_, theme, extension) =>
    ...
    if (!fs.existsSync(counterpart))
      violations.push(
        `Missing theme counterpart ${path.basename(counterpart)}.`,
      )

    const theme = /-dark\.svg$/i.test(relativePath) ? 'dark' : 'light'
    for (const match of svg.matchAll(
      /<(?:rect|circle|ellipse|path|polygon)\b([^>]*)>/gi,
    )) {
      ...
      if (!expected)
        violations.push(`Unknown data-state ${JSON.stringify(state)}.`)
      else if (fill !== expected)
        violations.push(
          `data-state ${JSON.stringify(state)} must use ${expected} in ${theme} mode.`,
        )
    }
  }

  for (const match of svg.matchAll(
    /<(?:text|tspan)\b([^>]*)>([\s\S]*?)<\/(?:text|tspan)>/gi,
  )) {
    const text = stripTags(match[2])
    const fontSize = Number.parseFloat(attribute(match[1], 'font-size') || '0')
    if (text && fontSize > 0 && fontSize < 16) {
      violations.push(
        `Text ${JSON.stringify(text)} uses font-size ${fontSize}; diagram labels require at least 16px.`,
      )
    }
    if (text.length <= 20 && /[✓✔✗✕✘⚡⏰▶►]/u.test(text)) {
      violations.push(
        `Text ${JSON.stringify(text)} uses a Unicode icon instead of a Primer Octicon.`,
      )
    }
  }
...
process.stdout.write(
  `${files.length} SVG file(s) checked; no visual-language violations found.\n`,
)
