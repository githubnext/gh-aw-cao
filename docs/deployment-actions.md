...
No additional secrets are needed. The build job uses the automatic `github.token` with job-scoped `actions: read` and `contents: read`; the deploy job uses Pages OIDC with `pages: write` and `id-token: write`. Do not create a `REPORT_PAGES_TOKEN` secret.
...
3. In **Settings > Pages**, select **GitHub Actions** as the source and restrict visibility to the intended audience.
4. Optionally protect the `github-pages` environment in **Settings > Environments**.
5. Run **CAO Activity** from the **Actions** tab and wait for it to succeed. The dashboard build fails when no successful Activity run exists.
6. Run **CAO Dashboard** from the **Actions** tab.
7. Open the deployment URL from the `deploy` job and confirm the site shows data only from the intended control repository.
