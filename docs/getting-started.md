...
The script installs `gh-aw` when needed, adds the latest published core CAO campaign, initializes the minimal control policy, and makes the repository-local `./cao.sh` CLI executable. If an installed `gh-aw` is too old for the campaign, the installer asks before upgrading it. Pre-releases are included only when the campaign's `aw.yml` requires one. Declining or running without an interactive terminal stops setup without an error; run the upgrade command shown by the installer and rerun the CAO installer when ready. Rerunning it after those files are installed makes no changes. Use the individual `gh aw` and CAO CLI commands when you intentionally need an older campaign release.
