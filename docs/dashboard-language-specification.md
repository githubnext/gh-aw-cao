...
- **DLS-VIEW-008:** A field definition **MUST** contain `field` and **MAY** contain only `type`, `aggregate`, `time-unit`, `title`, `as`, `display`, `format`, and `unit` in addition; `as` is valid only when `aggregate` is not `none`. `display` is valid only on table columns and **MUST** be `text`, `status`, `grader-status`, `mode`, `active-state`, `label`, `digest`, `outcome-link`, `run-link`, or `evidence-link`. `format`, when present, **MUST** be `workflow-relative-path`, is valid only on nominal or ordinal fields, and **MUST** affect presentation only by removing a leading `.github/workflows/` while preserving the `.md` extension.
