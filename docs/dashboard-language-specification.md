...
| View `config` | `body`, `sections`, `navigation` |
...
- **DLS-DOC-009a:** `config`, when present, **MUST** appear only on `mark: element` views. Version 0.1.0 defines `config.body` only for the `workflow-route`, `workflow-route-page`, `package-route`, `outcome-detail-section`, `experiments-evaluation`, and `work-project-view` elements. For `workflow-route` and `workflow-route-page`, it **MUST** be one of `insights`, `reports`, or `runs`. For `package-route`, it **MUST** be one of `insights`, `workflows`, `dispatches`, or `reports`. For `outcome-detail-section`, it **MUST** be `discussion` or `metadata`. For `experiments-evaluation`, it **MUST** be `overview`, `table`, or `detail`. For `work-project-view`, it **MUST** be `board`, `tasks`, or `roadmap`. Version 0.1.0 also defines `config.sections` for `experiments-evaluation` and `work-project-view`; it **MUST** be a non-empty list using only the canonical section values for that element, in the order they should be composed. Version 0.1.0 defines `config.navigation` only for `work-project-view`; it **MUST** be a non-empty list of navigation items with canonical `key` values, non-empty `title`, canonical Octicon `icon`, non-empty `pageId`, and `href` equal to `#page-<pageId>`. A conforming presenter **MUST** derive work-view navigation from `config.navigation` instead of hard-coding page-specific identities outside the declarative composition.
...
- **DLS-VIEW-008:** A field definition **MUST** contain `field` and **MAY** contain only `type`, `aggregate`, `time-unit`, `title`, `as`, `display`, `format`, and `unit` in addition; `as` is valid only when `aggregate` is not `none`. `display` is valid only on table columns and **MUST** be `text`, `status`, `grader-status`, `mode`, `active-state`, `label`, `digest`, `outcome-link`, `run-link`, or `evidence-link`. `format`, when present, **MUST** be `workflow-relative-path` or `workflow-run-url` and is valid only on nominal or ordinal fields. `workflow-relative-path` **MUST** affect presentation only by removing a leading `.github/workflows/` while preserving the `.md` extension. `workflow-run-url` is valid only on table columns and **MUST** present a canonical github.com Actions run URL as an external link labeled with the run ID, while leaving non-matching values as plain text.
