...
| `transactions` | canonical database transaction | `transaction`, `transaction-kind`, `created-at`, `status`, `repository`, `workflow`, `run`, `run-attempt`, `event-name`, `request-id`, `ref`, `sha`, `records`, `committed-records`, `run-link` |
