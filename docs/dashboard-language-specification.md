...
A metric view may select the reusable card widget with a `metric` mapping. The mapping uses `style: card`, one canonical Octicon `icon`, one `tone` value of `attention`, `danger`, `neutral`, or `review`, and a canonical `navigation-page`. A metric card renders unavailable evidence as an absent value rather than zero and links to the declared dashboard page without deriving navigation from the view or source identity.
...
A list view declares `list.style: cards` and a canonical Octicon `list.icon`. It **MAY** declare one view-placed dashboard CLI action through `list.action`. It encodes a non-empty `columns` sequence: the first column is the card title and remaining columns are labeled details. Optional row actions use the same declarative action contract as tables. When present, the view-level action is rendered once beside the list description; each matching row action is rendered on its card. Empty and unavailable states remain distinct.
...
A view may set the structural `layout` hint to `full`, `full-view`, `half`, or `third`. `full-view` requests the full available row and viewport height; the other values describe the preferred share of an available row, not fixed dimensions. Presenters **MAY** collapse every hint to `full` when space, accessibility, or output media requires it; source order remains the reading and focus order.

A table view may set `controls` to `interactive` or `static`; an omitted value defaults to `interactive`. Interactive tables may expose filtering, sorting, summaries, and progressive row disclosure. An interactive table may set `lazy-list` to `true` to reveal rows in bounded batches as its load boundary approaches the viewport, while retaining an accessible manual load control. A list card view may also set `lazy-list` to progressively reveal additional cards with the same bounded behavior and manual control. Static tables expose all rows in source order without those controls and cannot enable `lazy-list`. A table or list may set `empty-message` to a non-empty textual description shown inside its zero-row region.
...
- **DLS-VIEW-004a:** `list` **MUST** declare `list.style: cards`, a canonical `list.icon`, and non-empty encoded `columns`. It **MAY** declare one view-placed dashboard CLI action through `list.action`, and **MAY** set `lazy-list: true` for progressive card disclosure with an accessible manual load control. The first column **MUST** supply each card title; remaining columns **MUST** supply labeled details. It **MAY** encode conditional row `actions` and **MUST NOT** encode `value`, `x`, `y`, `color`, or `href`.
...
- **DLS-VIEW-006:** A `chart` with temporal `x` **MUST** use the line time-series default when its widget is omitted; any other valid `chart` **MUST** use the bar default. An optional `layout` hint **MUST** be `full`, `full-view`, `half`, or `third`, **MUST NOT** change source order, and **MAY** be collapsed by a presenter. `full-view` **SHOULD** consume the available viewport height.
