...
- **DLS-PAGE-012:** The `operational-value` page **MUST** expose a time-ordered absolute-attainment series with definition, operational case, evaluator digest, subject, requested evidence time, effective evidence cutoff, maturity time and status, accepted evidence provenance, applicable experiment assignment, and separate baseline delta when available. Declared diagnostics **MUST** be presented separately as direction-aware change from each metric's first observation.
