...
| View | `id`, `title`, `description`, `intent`, `locked`, `data`, `mark`, `element`, `config`, `callout`, `chart`, `tree`, `layout`, `disclosure`, `controls`, `lazy-list`, `column-summaries`, `empty-message`, `title-link`, `encoding` |
...
| `database-schema` | one table in the presenter's canonical database | `table`, `records`, `indexes`, `index-structure`, `primary-key`, `generation`, `schema-version`, `database-version` |
...
- **DLS-SEM-033:** `database-schema` **MUST** be generated by the presenter from its active canonical database without scanning table records. Each row **MUST** identify one table, its active-generation record count, primary key, deterministic index structure and count, active generation, canonical schema version, and database version. The source **MUST NOT** expose record values, implementation paths, credentials, or secrets.
...
The `database-schema` source enables declarative introspection of the presenter's
canonical database. For example:

```yaml
queries:
  - name: database-table-inventory
    intent: List canonical database tables with record and schema statistics.
...
- **DLS-QUERY-020:** A presenter **MUST** materialize `database-schema` only when resolving a requested query that depends on it, **MUST** execute that query in the data worker under **DLS-QUERY-016**, and **MUST** return only the requested derived source to the main thread.
...
- Added request-scoped `database-schema` introspection for declarative queries through **DLS-SEM-033** and **DLS-QUERY-020**.
