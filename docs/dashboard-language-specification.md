...
- **DLS-VIEW-004a:** `list` **MUST** declare `list.style: cards`, a canonical `list.icon`, and non-empty encoded `columns`. It **MAY** declare one view-placed dashboard CLI action through `list.action`. The first column **MUST** supply each card title; remaining columns **MUST** supply labeled details. It **MAY** encode conditional row `actions` and **MUST NOT** encode `value`, `x`, `y`, `color`, or `href`.
