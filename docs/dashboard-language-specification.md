...
- **DLS-DOC-009a:** `config`, when present, **MUST** appear only on `mark: element` views. Version 0.1.0 defines `config.body` only for the `workflow-route`, `workflow-route-page`, `package-route`, `outcome-detail-section`, and `work-project-view` elements. For `workflow-route` and `workflow-route-page`, it **MUST** be one of `insights`, `reports`, or `runs`. For `package-route`, it **MUST** be one of `insights`, `workflows`, `dispatches`, or `reports`. For `outcome-detail-section`, it **MUST** be `discussion` or `metadata`. For `work-project-view`, it **MUST** be `board`, `tasks`, or `roadmap`. Version 0.1.0 also defines `config.sections` for `work-project-view`; it **MUST** be a non-empty list of unique values using only the selected element's canonical section values, in the order they should be composed. A conforming presenter **MUST** derive any companion navigation, route targets, labels, and icons for `work-project-view` from those canonical body values instead of hard-coding page-specific identities outside the declarative composition. Version 0.1.0 defines `config.labels` and `config.animate: number` for `factory-floor`; labels **MUST** be a non-empty mapping keyed by canonical kebab-case label identifiers whose values are plural text variables containing exactly the non-empty strings `singular` and `plural`.
