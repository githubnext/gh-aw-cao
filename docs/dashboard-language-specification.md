...
| `dashboard` | `id`, `title`, `description`, `github-url-base`, `repository`, `callouts`, `cli-actions`, `horizon`, `defaults`, `units`, `queries`, `card-templates`, `pages`, `navigation` |
...
The dashboard may declare reusable `card-templates`. Each template has a canonical `id`, a canonical Octicon `icon`, one title field definition, and sequences of label and detail field definitions. Views reuse these JSON-defined presentation contracts by identifier; presenters must not supply entity-specific field layouts from implementation code.
