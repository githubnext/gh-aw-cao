...
The AW Doctor compiler-security worker reports the exact compiler, validation, lint, image, and security-scanner findings that need remediation. To fix the same findings locally with a coding agent:
...
The activity action refreshes one coherent cache snapshot. Immediately after cache restore, one `gh aw logs --audit --artifacts usage` call downloads compact run data for compiled workflows in the checked-out control repository. The usage bundle includes run summaries, audit output, and agent information; report generation keeps bounded normalized audit aggregates and agent/runtime/model/version metadata, and derives tool-call events from summary data without downloading heavyweight agent transcripts. The indexer then performs no GitHub API operations: it combines those logs with local workflow sources, locks, and `inventory.mjs` output. Policy target repositories are not scanned for unrelated workflows. Missing usage-artifact fields are recorded in the index so gh-aw can expose the required metadata. The AI Credit, security, and operational-value collectors process the same JSON and artifacts without additional history scans. `records.mjs` joins run attribution from that logs-derived inventory and uses GitHub only to discover durable issues, pull requests, comments, and review artifacts, plus the Pages privacy check when private inventory is enabled. It does not request repository details, remote workflow inventories, lock sources, gh-aw releases, or Actions run details. The dashboard builder restores this snapshot without collecting or publishing cache data. `dashboard-language-sources.mjs` adapts the collected records into the canonical `sources.json` contract, and the builder copies the packaged Dashboard Language site to `site-path`, uploads the mergeable artifact, and optionally deploys it through the standalone publisher. Generated site files and source data are not committed to the repository.
