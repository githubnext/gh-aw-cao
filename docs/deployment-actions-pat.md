...
This deployment profile is experimental and is a consented fallback, not a recommended default. Secret names, setup commands, and fallback order may change between releases. Prefer [GitHub Apps](deployment-actions-github-app.md) whenever an App can be installed.
:::
...
2. The token owner already has access to every enrolled repository.
3. Every enrolled repository has **one resource owner**. A fine-grained PAT cannot span organizations.
4. Organization and enterprise policy permits fine-grained PATs, and any required approval can be obtained before the first run.
5. Every API the installed campaigns need supports fine-grained PATs. For example, the Checks API does not.
6. Each token can be limited to exact repositories and minimum permissions, with an expiration and a named rotation owner.
...
2. Create both tokens with the helper:
...
3. Enter each token only at its interactive `gh secret set` prompt. Tokens are never accepted as command arguments.
4. Do not configure the legacy `GH_AW_GITHUB_TOKEN` secret, and make sure no `GH_AW_GITHUB_*_APP_ID` variable or `GH_AW_GITHUB_*_APP_PRIVATE_KEY` secret is left half-configured.
5. Run **CAO Activity**, then **CAO Dashboard**, and continue with the base procedure.
6. [Validate](#validate) before enabling any campaign in `live`.
...
This profile uses the same telemetry as the [base profile](deployment-actions.md#telemetry-and-observability), with these differences:
...
When migrating from `GH_AW_GITHUB_TOKEN`, create two new independent tokens rather than copying the legacy token into both secrets. Delete the legacy secret afterwards and rerun a bounded review. See [Migrate from the legacy PAT](authentication.md#migrate-from-the-legacy-pat).
