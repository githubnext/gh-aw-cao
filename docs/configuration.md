...
Control repositories should declare `gh-aw-compiler-version` at the document root. Factory infrastructure reads this exact release when installing the CLI, and fleet maintenance can compare it with the expected release to identify repositories that need an upgrade. The field is optional for compatibility with existing and target-authority-only documents.
