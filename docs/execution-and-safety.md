...
2. Before activation, `.github/workflows/shared/control.md` fetches the package-installed `.github/aw/cao/src` runtime or the source-managed `.github/workflows/shared` runtime at the exact workflow commit and runs the `admit` command against policy from that same revision.
