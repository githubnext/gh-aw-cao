...
2. Before activation, `.github/workflows/shared/control.md` fetches the package-installed `.github/aw/cao/src` runtime, or the `.github/cao/src` runtime in a source-managed control repository, at the exact workflow commit and runs the `admit` command against policy from that same revision.
