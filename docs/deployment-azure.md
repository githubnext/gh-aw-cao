...
- **Debug logs.** Set `DEBUG` to `cao:server`, `cao:query`, `cao:ingest`, `cao:redis`, `cao:cli`, or a pattern such as `cao:*,-cao:redis`. Logs go to stderr and contain operation names, counts, timings, and fixed `oauth branch=<operation>.<outcome>` identifiers, never tokens, credentials, query payloads, or source records. Append `?debug=auth` in the browser to see matching client-side authentication events.
