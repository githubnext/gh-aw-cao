...
description: "Set up a Central Agentic Ops (CAO) control plane from scratch. Use when a user asks to create, bootstrap, initialize, install, or get started with CAO; asks which separately installable catalog operations they want and whether they want to author a custom package, creates or reuses a control repository, installs the operation-free root CAO package and selected operation packages with gh aw add, writes .github/workflows/cao.json, and proves the boundary with one user-selected review target."
...
- Install the operation-free root CAO package with `gh aw add githubnext/gh-aw-cao`, then install each selected operation from its package path, such as `githubnext/gh-aw-cao/dependabot` or `githubnext/gh-aw-cao/optimization`. Keep every package on the same published release; never install CAO from `main`, another branch, or by copying package files.
...
- Ask which separately installable catalog operation packages the user wants. Do not silently choose Dependabot or infer package intent from the target repository.
...
4. Ask these two package questions separately before choosing the first target. Use a multi-select question followed by a yes/no question when an interactive question tool is available:
  - **Catalog operations:** Ask, "Which operation packages do you want to add to CAO?" Present the current package display names and outcome-focused descriptions from their manifests and READMEs, allow more than one answer, and include `Not sure yet`. Explain that the root package installs the CAO foundation without operational workflows and each selected operation is added separately. If the user selects more than one operation, ask which one should prove setup first. Record the exact `initial-package`, `initial-orchestrator`, and worker-to-workflow mapping from the selected package's catalog policy. Never silently default the package to Dependabot.
...
6. Confirm prerequisites without changing repositories:
  ...
  - Confirm that the organization can authenticate the selected operation packages' Copilot-backed workflows:
...
- root or selected operation package installation fails; or
