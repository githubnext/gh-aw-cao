...
- Keep the catalog's committed root `aw.yml` free of `config` so ordinary installation remains non-interactive. When the user chooses automated App setup, run the campaign-installed `.github/workflows/shared/setup-github-apps.mjs` helper and target the control repository explicitly.
...
6. Confirm prerequisites without changing repositories:
  ...
  - Confirm that the organization can authenticate the root campaign's Copilot-backed workflows:
...
9. Confirm `.github/aw/default-AGENTS.md` was installed. If the repository has no root `AGENTS.md`, read the installed template and create `AGENTS.md` with exactly that content using a file-editing tool. If root `AGENTS.md` already exists, preserve it unchanged unless the user explicitly approves a merge; the bundled file remains the reference default and campaign updates must not overwrite consumer-owned ambient context.
...
- the installed root campaign does not contain `.github/aw/default-AGENTS.md`;
