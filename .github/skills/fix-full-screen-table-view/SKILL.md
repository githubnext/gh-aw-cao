...
1. Locate the page and view in `.github/cao/dashboard/site/dashboard.json`. Confirm the target is one essential table view; full-view pages should not mix unrelated essential views or wrap the table in a layout section.
...
3. Trace the existing full-view behavior in `.github/cao/dashboard/site/src/presenter.js` and `.github/cao/dashboard/site/src/styles.js`. Reuse `.dashboard-full-view` and `[data-view-layout="full-view"]`; do not add page-specific positioning when the shared shell can express the behavior.
...
10. Extend the shared `JSON full-view mode fills the viewport and hides chrome while scrolling` Playwright test for reusable behavior. Run that test, the target route smoke test, `npm run typecheck`, and `npm run lint` from `.github/cao/dashboard/site/`.
