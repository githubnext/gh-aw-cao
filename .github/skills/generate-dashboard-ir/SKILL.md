...
- the built-in dashboard configuration at `.github/cao/dashboard/site/dashboard.json`;
...
1. Read the specification and `.github/cao/dashboard/site/dashboard.json`, then identify the relevant root structure, logical sources and grains, fields, dimensions, measures, aggregates, marks, encodings, pages, filters, links, routing, data-state semantics, defaults, and validation constraints. Reuse an established built-in view pattern when it satisfies the intent, adapting only its scope and labels. When prior knowledge differs, follow the provided specification.
...
5. Run `npm --prefix .github/cao/dashboard/site run validate:corpus` from the repository root. Keep only candidates that pass dashboard-document and logical-source validation.
