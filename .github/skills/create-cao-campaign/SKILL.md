...
For Copilot-backed workflows, check the control repository owner's organization billing with `gh api orgs/<organization>/copilot/billing --jq '{seat_management_setting, total_seats: .seat_breakdown.total}'` before attempting a run. Use API evidence of an active entitlement or explicit organization-administrator confirmation when the endpoint is inaccessible or inconclusive. Treat `total_seats: 0` with `seat_management_setting: unconfigured` as unavailable for Copilot runs, not as a blocker to authoring campaigns for other providers. A Pi or Codex workflow using a `copilot/*` model is Copilot-backed. Add `copilot-requests: write` directly to every Copilot-backed orchestrator and worker; validate their generated locks use `${{ github.token }}` for `COPILOT_GITHUB_TOKEN` and do not declare the PAT secret. Do not use `aw.yml` bootstrap `config`, authentication-neutral workflow sources, `COPILOT_GITHUB_TOKEN`, or runtime token precedence as a workaround for Copilot inference.
