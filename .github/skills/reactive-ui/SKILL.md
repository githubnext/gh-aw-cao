...
- Register elements in `.github/aw/dashboard/site/src/components/ui-elements.js` and render only from the supplied `ElementRenderContext`.
...
- Reuse `.github/aw/dashboard/site/src/dom.js`, shared UI primitives, chart elements, badges, formatters, Octicons, and view chrome instead of duplicating markup or behavior.
...
Use `.github/aw/dashboard/site/src/reactive.js` for state-driven updates:
...
- Use stable keyed rendering from `.github/aw/dashboard/site/src/dom.js` for changing collections; do not rebuild an entire view when only state or list membership changed.
...
- Publish transient user feedback through `.github/aw/dashboard/site/src/notification-service.js`. Do not create view-specific toast containers or duplicate their lifecycle, accessibility, or responsive behavior.
...
- Put dashboard component styles in `.github/aw/dashboard/site/src/styles.js`; do not add inline styles or element-local style injection for ordinary views.
...
3. From `.github/aw/dashboard/site/`, run the focused tests, `npm test`, `npm run lint`, and `npm run typecheck`. Run focused end-to-end tests for browser-facing changes.
