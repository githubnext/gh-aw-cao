...
- Before metric records, it emits an `operational_value_definition` JSONL record listing the module's namespaced metric IDs. CAO uses that authoritative definition query to retire cached IDs that no longer exist; it is metadata, not an observation.
