...
2. **Protect an existing design.** Run `.github/skills/add-operational-value/scripts/value-function-path.mjs OWNER/REPO WORKFLOW-NAME`. If that file exists, verify it with `.github/skills/add-operational-value/scripts/verify-value-function.mjs <path>`, report that it was left unchanged, and continue at step 7. Do not inspect post-adoption evidence or redesign it.
...
6. **Implement and freeze.** For a confirmed measurable design:
  ...
  - Create the canonical function directly from the Function Interface below. Do not search ignored `functions/` or `reports/` directories for examples or conventions.
  - Create one canonical executable JavaScript function at the `.mjs` path from step 2. Run `.github/skills/add-operational-value/scripts/verify-value-function.mjs --collector <path>` and do not report success unless it passes.
  - Report the outcome contract, evidence shape, metrics and roles, agreement or disagreement, rejected metrics, limitations, selected model, generated file, and successful verification command. Do not create evaluation artifacts yet.
...
- The function is a self-contained executable ECMAScript module and uses only Node.js built-in modules plus the `gh` CLI.
- `--definition` prints the current JSON contract containing `schemaVersion`, repository, workflow, adoption, `evaluation.mode`, frozen evidence contract, model, summary, metrics, and validation examples. Mode is `baseline-comparable` or `attainment-only`. `schemaVersion` is the only interface version; do not add separate collector version or command metadata.
- `--collect-batch` reads every requested `{windowStart, windowEnd, observedAt}` object as one JSON array and returns one collection result per window in the same order.
...
- `--metric <id>` scores one evidence object; no arguments score the primary metric. Return a deterministic number in `0..1`, or `null` for missing or wholly malformed evidence. Clamp and round only at the output boundary.
...
- The collector acceptance gate must run through process exit and validate evidence and provenance. Cleanup traps must capture temporary paths when installed rather than reference function-local variables after scope exit.
