...
Format every inline comment and overall review body with `###` headings only; never use `#`, `##`, or `####` and deeper headings. Put every Markdown table inside a clearly named `<details>` element.
