...
safe-outputs:
  create-pull-request:
    ...
    allowed-files:
      ...
      - ".github/aw/dashboard/site/src/**/*.js"
      - ".github/aw/dashboard/site/src/**/*.json"
      - ".github/aw/dashboard/site/test/**/*"
...
1. Read `AGENTS.md`, `.github/aw/instructions.md`, `.github/aw/dashboard/site/PLAN.md`, `docs/dashboard-language-specification.md`, `.github/aw/dashboard/site/dashboard.json`, `.github/aw/dashboard/site/src/specification.js`, `.github/aw/dashboard/aw.yml`, and relevant renderer tests before editing.
...
3. Inspect `.github/aw/dashboard/site/src/` for view rendering that branches on a built-in page identity, route, view ID, or one-off element name. Select exactly one candidate backed by concrete source evidence.
...
2. Move view composition out of page- or view-specific JavaScript and into `.github/aw/dashboard/site/dashboard.json` using existing Dashboard Language vocabulary whenever possible.
3. If the reusable boundary needs new language vocabulary, make the smallest coherent normative update to `docs/dashboard-language-specification.md`, implement matching validation in `.github/aw/dashboard/site/src/specification.js`, and add positive and negative conformance tests. Do not add arbitrary scripts, expressions, templates, or executable content to Dashboard Language.
4. Extract small reusable subcomponents under `.github/aw/dashboard/site/src/components/`. Give them domain-neutral names and inputs. Update both `.github/aw/dashboard/aw.yml` and root `aw.yml` only when a new runtime file must be packaged.
...
1. `npm --prefix .github/aw/dashboard/site run typecheck`
2. `npm --prefix .github/aw/dashboard/site run lint`
3. `npm --prefix .github/aw/dashboard/site test`
4. `npm --prefix .github/aw/dashboard/site run validate:corpus`
5. `npm --prefix .github/aw/dashboard/site run test:e2e`
6. `npm test`
