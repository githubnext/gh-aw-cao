...
1. Read `AGENTS.md`, `.github/aw/instructions.md`, `.github/skills/reactive-ui/SKILL.md`, `.github/skills/migrate-dashboard-view/SKILL.md`, `docs/dashboard-language-specification.md`, `specs/dashboard-data.md`, `.github/cao/dashboard/aw.yml`, `.github/cao/dashboard/site/package.json`, `.github/cao/dashboard/site/dashboard.json`, `.github/cao/dashboard/site/src/reactive.js`, `.github/cao/dashboard/site/src/dom.js`, `.github/cao/dashboard/site/src/data/queries/index.js`, `.github/cao/dashboard/site/src/data/queries/view-sources.js`, `.github/cao/dashboard/site/src/data-processor.js`, `.github/cao/dashboard/site/src/data-worker.js`, and the relevant presenter, component, and test files.
...
3. Inspect at most the 20 most recent commits that touch `.github/skills/reactive-ui/SKILL.md` or dashboard JavaScript. Read at most the ten most recently merged pull requests that changed `.github/cao/dashboard/site/src/**/*.js`. Use them only to identify recent regressions, implementation changes, and already-completed work.
4. Build a bounded candidate list:
   ...
   - **View-source migration:** a view still receives rows shaped or materialized by JavaScript even though canonical entities can supply its fields through a request-scoped Dashboard Language query in `.github/cao/dashboard/site/dashboard.json`.
...
3. For a view-source migration, select one view from `.github/cao/dashboard/site/dashboard.json`, record every field its renderer consumes, and trace its JavaScript source only far enough to identify the canonical entities and indexed reads that replace it. Define the query in `dashboard.queries`; add the smallest request-scoped projection under `.github/cao/dashboard/site/src/data/queries/`; route it through `data-processor.js` and `data-worker.js`; preserve the source-shaped payload and legacy fallback for unmigrated views; and return only the requested view payload. Do not modify the published source producer.
...
5. Prefer existing named elements, shared components, `h`, keyed rendering, and reactive primitives. Create a new domain-neutral component only when reuse is concrete. Update `aw.yml` and `.github/cao/dashboard/aw.yml` only when a new runtime file must be packaged.
6. Update `.github/cao/dashboard/site/dashboard.json` only for the selected source migration or when the selected extraction requires declarative view composition or binding. Do not add executable expressions or infer behavior from page IDs, view IDs, source names, or source contents.
...
2. Run `npm --prefix .github/cao/dashboard/site run typecheck`.
3. Run `npm --prefix .github/cao/dashboard/site run lint`.
4. Run `npm --prefix .github/cao/dashboard/site test`.
5. Run `npm --prefix .github/cao/dashboard/site run validate:corpus` when `.github/cao/dashboard/site/dashboard.json` changes.
6. Run focused `npm --prefix .github/cao/dashboard/site run test:e2e -- <test-file>` coverage for browser-facing changes.
7. Run `npm test`.
