...
pre-agent-steps:
  ...
  - name: Fetch the axe-core accessibility engine
    ...
    run: |
      ...
      tar -xzf axe-core-4.13.0.tgz campaign/axe.min.js
      mv campaign/axe.min.js axe.min.js
      rm -rf campaign axe-core-4.13.0.tgz
  - name: Install the workspace Playwright browser build
    ...
    # the same shared cache, so a Node script that imports the workspace campaign can
  ...
  - name: Node Playwright campaign launch preflight
    if: ${{ inputs.target_repo == 'githubnext/gh-aw-cao' && (inputs.safe_output_mode || 'review') == 'live' }}
    env:
      EXPR_GITHUB_WORKSPACE: ${{ github.workspace }}
      PLAYWRIGHT_BROWSERS_PATH: ${{ runner.temp }}/gh-aw/playwright-browsers
    run: |
      ...
      (async () => {
        ...
        await browser.close();
        console.log('workspace Playwright campaign launched Chromium successfully');
      })().catch((err) => { console.error(err); process.exit(1); });
      " > "$EXPR_GITHUB_WORKSPACE/.playwright/preflight-node.log" 2>&1
      PREFLIGHT_STATUS=$?
      set -e
      if [ $PREFLIGHT_STATUS -ne 0 ]; then
        echo "Node Playwright campaign preflight failed; agent will report the infrastructure blocker."
      fi
---
...
`${{ github.workspace }}/.playwright/preflight-node.log` records a separate preflight that imported the workspace's own `@playwright/test` campaign and launched Chromium from the same preinstalled browser cache (`PLAYWRIGHT_BROWSERS_PATH`) before the agent started; it confirms a Node script can reuse that browser without downloading anything at run time, in case `playwright-cli` alone is insufficient.
