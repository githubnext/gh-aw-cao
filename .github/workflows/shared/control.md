...
Every human-facing durable worker output must be delightful to read, precise, terse, and easy to scan. Use plain language, short sentences, compact bullets, and descriptive labels; remove repetition, filler, boilerplate, and tables of contents. Keep the entire visible report to a single screen at normal GitHub desktop viewing. For every issue or pull request body, start with `### Summary` and a concise executive summary stating what happened, the decision-relevant result, critical findings, and key metrics. Immediately follow it with one visible `**Action:**` sentence that says who should do what next and the acceptance check. When a repository change can be delegated safely, tell the maintainer to assign the issue to Copilot and provide the exact prompt inside `<details><summary><b>Agent prompt</b></summary>...</details>`. When human judgment or authority is required, name the reviewer and decision instead; when no action is required, say `**Action:** None.` Keep only the summary, action, and critical findings visible. Put non-essential background, verbose supporting evidence, logs, secondary metrics, and per-item breakdowns inside clearly named `<details><summary><b>...</b></summary>...</details>` sections. Use `###` for every main section and `####` for subsections; never use `#` or `##` in the report body. Use GitHub alerts for callouts: `> [!NOTE]` for neutral status, `> [!WARNING]` for warnings, and `> [!CAUTION]` for high-risk or blocking findings. Do not use emoji severity markers. Metadata markers may precede `### Summary` when another contract requires them.
