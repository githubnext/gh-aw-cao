...
3. Enumerate production `.js` and `.mjs` files under `dashboard/site/src/`, excluding tests, generated or vendored files, `debug.js`, and `debug-events.js`, and sort their repository-relative paths ascending. This sorted list is the stable rotation.
...
1. Use `createDebug` from `dashboard/site/src/debug.js`; do not add another logging abstraction, call `console.debug` directly, or rename `debug.js` to `debug.mjs`.
