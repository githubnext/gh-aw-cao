...
3. Enumerate production `.js` files under `dashboard/site/src/`, excluding tests, generated or vendored files, `debug.js`, and `debug-events.js`, and sort their repository-relative paths ascending. This sorted list is the stable rotation.
...
2. Derive the primary category predictably from the selected filename: remove the final `.js`, lowercase it, and replace every run of non-alphanumeric characters with `-`. For example, `source-store.js` uses `source-store`. Only when that basename is duplicated elsewhere, prefix the nearest distinguishing parent directory and `:`, applying the same normalization. Use `:<event>` subcategories only when independently enabling distinct high-value transitions is useful.
