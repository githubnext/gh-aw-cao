...
pre-agent-steps:
  ...
  - name: Install Chromium
    if: ${{ inputs.target_repo == 'githubnext/gh-aw-cao' && (inputs.safe_output_mode || 'review') == 'live' }}
    run: timeout 10m npm exec --prefix .github/aw/dashboard/site -- playwright install --with-deps chromium
  - name: Collect deployed Pages health evidence
    ...
    run: |
      set +e
      node .github/aw/dashboard/site/test/performance/pages-health.mjs
