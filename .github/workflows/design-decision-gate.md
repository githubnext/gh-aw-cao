...
steps:
  - name: Prefetch decision gate context
    ...
    run: |
      ...
      CORE_ADDITIONS="$(jq '[
        .[]
        | select(.filename | test("^(.github/workflows/shared|activity|dashboard/report|dashboard/site/src|scripts)/"))
        | .additions
...
Format every comment body with `###` headings only; never use `#`, `##`, or `####` and deeper headings. Put every Markdown table inside a clearly named `<details>` element.
