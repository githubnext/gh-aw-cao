...
First prove that experimental views are enabled by repository policy and are present in both the assembled and deployed dashboards. Then systematically test the Operations page shell, every editable view on that page, and every editable view on pages belonging to navigation sections with `experimental: true` in `dashboard/site/dashboard.json`. Ignore every view with `locked: true`. Ignore all views on top-level pages in non-experimental navigation sections, except the Operations page shell. Do not stop after the first failure.

## Coverage

1. Read `AGENTS.md`, `.github/aw/instructions.md`, `.github/workflows/cao.json`, `dashboard/site/README.md`, `dashboard/site/package.json`, and the dashboard definition. Require `.github/workflows/cao.json` to set `control-plane.web.experimental` to the Boolean `true`; do not infer enablement from a default, source navigation, or a cached control-settings file. If it is not explicitly enabled, record that blocker and call `noop` exactly once.
2. Derive the exact in-scope page and view inventory from the source definition; do not hard-code it. The `operations` page is in scope even if its navigation classification changes. Build the dashboard from the checkout using the repository's resolved control settings, then require every source experimental navigation section and page to remain in the assembled `dashboard.json` and require every generated experimental route to load successfully. Do not infer availability from the policy setting alone.
3. Fetch the deployed dashboard definition from `https://githubnext.github.io/gh-aw-cao/cao/dashboard.json` and require every source experimental navigation section and page to be present. Navigate every deployed experimental route and require it to render the expected page without an HTTP, application, console, or page error. Treat an unavailable or ambiguous deployed definition or route as failed availability evidence, not as permission to test only the source checkout.
4. Before deeper inspection, query at most the 20 most recent open pull requests and confirm no pull request body contains `gh-aw-workflow-id: self-care-experimental-views`. The trigger normally performs this check, but fail closed with one `noop` if a matching PR is open or the result is ambiguous.
5. Exercise every in-scope view with Playwright in both Chromium and WebKit. In PR mode, test at most the first five in-scope views in the source-definition order of the derived inventory; otherwise test every in-scope view. Use temporary fixtures only under `/tmp`; do not add an audit harness or generated evidence to the repository.
6. For every view and browser, verify navigation, initial render, interaction controls, console and page errors, failed requests, visible empty and error states, keyboard operation, viewport overflow at desktop and 390 CSS pixels, and stable rendering after source refresh.
7. Test every data source consumed by each in-scope view with schema-valid empty, single-row, representative multi-row, missing-optional-field, and high-cardinality inputs. Verify filters, joins, derived values, links, units, ordering, grouping, and fallback labels against the source contract. Never infer correctness from a successful render alone.
8. After each render and meaningful interaction, record `document.querySelectorAll('*').length` and the largest repeated-container child counts. Compare empty, representative, and high-cardinality fixtures. Treat growth as evidence, not an automatic defect; identify the source and component responsible.
9. When a view creates too many DOM nodes, evaluate and explicitly rank these remedies against its interaction and accessibility needs: move secondary detail to `disclosure: supplemental`, cap the initially rendered records with an honest visible count and access to remaining records, or use an accessible lazy/virtualized list. Never silently truncate data, hide an operationally material state, or weaken keyboard and screen-reader access.
