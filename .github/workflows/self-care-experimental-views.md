...
5. Exercise every in-scope view with Playwright in both Chromium and WebKit. In PR mode, test at most the first five editable views in the derived inventory; otherwise test every in-scope view. Use temporary fixtures only under `/tmp`; do not add an audit harness or generated evidence to the repository.
