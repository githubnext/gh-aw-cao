...
2. Find every open Dependabot security alert visible to this workflow with `list_dependabot_alerts`, including alerts not represented by an open pull request.
   ...
   - If alert evidence is unavailable but other Dependabot evidence is sufficient to produce a bounded plan, continue with that plan, add a human-only blocker or visible evidence limitation for the unavailable alert inventory, and do not `noop`.
...
5. After direct Dependabot evidence has produced the update inventory, list Dependabot-authored dependency update pull requests as supplementary queue/status evidence for conflicts, CI failures, grouping, branch names, links, and stale duplicate cleanup. Do not treat pull requests as required input or the sole source of truth for the update list.
