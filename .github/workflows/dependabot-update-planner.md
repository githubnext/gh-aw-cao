...
1. Build the current update inventory from Dependabot-native service evidence before inspecting pull requests. Use every available read-only Dependabot tool or API exposed to the workflow for pending security updates, version updates, update jobs, repository-access blockers, and Dependabot-identified configuration failures.
   ...
   - If the workflow lacks a Dependabot-native read path for non-security version updates or update jobs, do not substitute pull-request search results as a complete inventory. When visible Dependabot-authored open pull requests, repository-access findings, or Dependabot configuration/failure evidence are sufficient to create a bounded queue-hygiene or blocker plan, continue and add an `Evidence limitation` entry for the unavailable native version-update path in the parent issue instead of calling `missing_tool` or `report_incomplete`.
   - Call `missing_tool` or `report_incomplete` for a missing Dependabot-native version-update path only when no visible Dependabot-authored open pull requests, repository-access findings, Dependabot configuration failures, or existing plan issue provide bounded actionable work.
2. Find every open Dependabot security alert visible to this workflow with `list_dependabot_alerts`, including alerts not represented by an open pull request.
   ...
   - If alert evidence is unavailable but other Dependabot evidence is sufficient to produce a bounded plan, continue with that plan, add an `Evidence limitation` entry for the unavailable alert inventory, and do not `noop`.
...
5. After direct Dependabot evidence has produced the update inventory, list Dependabot-authored dependency update pull requests only as supplementary queue/status evidence for conflicts, CI failures, grouping, branch names, links, and stale duplicate cleanup. Do not treat pull requests as required input or the sole source of truth for the update list.
   - When native non-security version-update or update-job evidence is unavailable, visible Dependabot-authored open dependency update pull requests may still seed a bounded queue-hygiene plan. Clearly state that the plan covers visible Dependabot pull requests and blockers rather than a complete Dependabot update inventory.
...
If the memory lookup cannot identify an open parent and `list_issues` is unavailable, denied, or fully filtered by policy, do not report incomplete solely because parent discovery is unavailable. When current Dependabot evidence or actionable evidence limitations require a parent, and the target repository is valid enough to construct the exact canonical unprefixed subject plus `dependabot-update-plan:repository=<owner>/<repository>` marker deterministically, call `create_issue` with that canonical subject and marker and rely on the configured `deduplicate-by-title` safe-output guard to avoid a duplicate. Add an `Evidence limitation` entry stating that open-issue discovery was unavailable. If the canonical subject and marker cannot be constructed deterministically, call `report_incomplete` instead of guessing. Persist the parent number on the next run after safe-output processing makes it visible.
...
- If no matching open parent exists, or parent discovery was unavailable, and at least one current update, visible Dependabot queue-hygiene item, or actionable Dependabot evidence blocker exists, call `create_issue` once for the parent with its canonical unprefixed subject, complete body, and a temporary ID. Then create the bounded child set with stable subjects and parent references. Never let a closed parent prevent this creation. Never let unavailable issue listing prevent canonical safe-output creation when actionable work exists.
