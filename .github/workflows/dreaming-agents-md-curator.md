...
Use the precomputed evidence as authoritative. Do not pretty-print or reread the entire prefetch file, repeat the same target-file reads, or broaden the investigation after the evidence supports a bounded change set. Use short, targeted git/grep/sed/wc checks only to verify proposed edits; do not use Python, Node, sh, broad recursive searches, or exploratory arithmetic. After the scope and gain gates pass, make one bounded open-issue lookup and publish the issue immediately. If the evidence does not support an issue, emit noop rather than continuing to investigate.
