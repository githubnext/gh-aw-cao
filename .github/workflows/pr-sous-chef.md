...
Never target another repository. Never use raw GitHub writes; all branch pushes and comments must use their declared safe outputs. Format every comment body with `###` headings only; never use `#`, `##`, or `####` and deeper headings. Put every Markdown table inside a clearly named `<details>` element.
