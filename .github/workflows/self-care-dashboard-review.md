...
pre-agent-steps:
  ...
  - name: Build expected control-plane inventory
    ...
    run: |
      ...
      REPORT_INVENTORY=/tmp/gh-aw/agent/self-care-dashboard-review/expected-inventory.json \
        node .github/cao/activity/inventory.mjs
  - name: Download and grade the live dashboard artifact
    ...
    run: |
      ...
      node .github/cao/dashboard/grader/view-grader.mjs \
...
`.github/cao/dashboard/aw.yml` (CAO Dashboard) and `.github/cao/activity/aw.yml` (CAO Activity) are internal control-plane packages, not user-facing catalog packages. They deploy the dashboard and its shared activity index rather than a repository-facing capability, so their absence from the rendered Overview/Packages inventory is expected and must not be reported as a defect.
