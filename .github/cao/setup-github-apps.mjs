...
export function validateAppName(name, flag) {
  if (!/^[A-Za-z0-9-]+$/.test(name) || name.length > APP_NAME_MAX_LENGTH) {
    throw new Error(
      `${flag} must contain only letters, numbers, or hyphens and be at most ${APP_NAME_MAX_LENGTH} characters`,
    )
  }
...
function splitRepo(repo) {
  const match =
    /^([A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?)\/([A-Za-z0-9_.-]+)$/.exec(repo)
  if (!match) {
...
async function createGitHubApp({
  ...
  const server = createServer(async (request, response) => {
    ...
    if (returnedState !== state) {
      response
        .writeHead(400)
        .end('State mismatch while creating the GitHub App.')
    ...
    try {
      ...
      response
        .writeHead(500)
        .end(
          'GitHub App creation completed, but the manifest code exchange failed.',
        )
      fail(error)
    }
  })
  ...
  const timeout = setTimeout(() => {
    if (!settled) {
      fail(
        new Error('timed out waiting for GitHub App creation in the browser'),
      )
    }
  }, MANIFEST_TIMEOUT_MS)
  try {
...
function matchingInstallation(app, owner) {
  return listOrganizationInstallations(owner).find(
    (installation) =>
...
export function validateInstallationScope(installation, owner) {
  if (installation.repositorySelection !== 'selected') {
    ...
    const error = new Error(
      `GitHub App is installed for all ${owner} repositories; select only approved repositories at ${settingsUrl}`,
    )
...
async function waitForInstallation(app, repo) {
  console.error(
    `Install ${app.name || app.slug} in the browser. ${installationInstruction(repo)}`,
  )
  ...
  while (Date.now() < deadline) {
    try {
      if (hasSelectedInstallation(app, repo)) {
        console.error(
          `Selected-repository GitHub App installation detected for ${repo}.`,
        )
        return
      }
      lastError = undefined
  ...
  throw new Error(
    `timed out waiting for GitHub App installation on ${repo}${suffix}`,
  )
}
...
async function main() {
  ...
  for (const profile of APP_PROFILES) {
    const complete =
      state.variables.has(profile.variable) && state.secrets.has(profile.secret)
    if (complete && !options.force) {
      const app = existingGitHubApp(
        appNames[profile.role],
        repositoryVariableValue(repo, profile.variable),
      )
      if (hasSelectedInstallation(app, repo)) {
        console.error(
          `${profile.label} App credentials and selected-repository installation already exist; skipping.`,
        )
        continue
      }
      console.error(
        `${profile.label} App credentials exist, but installation on ${repo} is incomplete; reopening it.`,
      )
    ...
    if (
      state.variables.has(profile.variable) !==
      state.secrets.has(profile.secret)
    ) {
      console.error(
        `${profile.label} App credentials are incomplete; creating a replacement pair.`,
      )
    }
  ...
  for (const profile of APP_PROFILES) {
    if (
      !finalState.variables.has(profile.variable) ||
      !finalState.secrets.has(profile.secret)
    ) {
      throw new Error(
        `credential verification failed for the ${profile.label} App`,
      )
    }
  }
