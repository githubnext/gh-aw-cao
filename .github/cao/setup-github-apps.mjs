...
function repositoryState(repo) {
  const variables = new Set(
    runGh(["api", `/repos/${repo}/actions/variables?per_page=100`, "--paginate", "--jq", ".variables[].name"])
      .split("\n")
      .filter(Boolean),
  );
  const secrets = new Set(
    runGh(["api", `/repos/${repo}/actions/secrets?per_page=100`, "--paginate", "--jq", ".secrets[].name"])
      .split("\n")
      .filter(Boolean),
  );
...
function repositoryVariableValue(repo, name) {
  return runGh(["api", `/repos/${repo}/actions/variables/${name}`, "--jq", ".value"]);
}
...
function exchangeManifestCode(code) {
  const payload = JSON.parse(runGh(["api", "-X", "POST", "-H", "Accept: application/vnd.github+json", `/app-manifests/${code}/conversions`]));
...
function matchingInstallation(app, owner) {
  return listOrganizationInstallations(owner).find(
    (installation) => installation.clientId === app.clientId || installation.slug === app.slug || (installation.appId && installation.appId === app.id),
  );
}
