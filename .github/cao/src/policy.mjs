...
export function effectivePolicy(
  ...
  let targetPolicies = Object.fromEntries(
    Object.entries(packagePolicy.targets ?? {}).map(([repository, targetPolicy]) => [repository.toLowerCase(), { mode: targetPolicy.mode }]),
  );
  ...
  if (role === "worker") {
    ...
    if ("max-mode" in worker) {
      ...
      targetPolicies = Object.fromEntries(
        Object.entries(targetPolicies).map(([repository, targetPolicy]) => [repository, { mode: lesserMode(targetPolicy.mode, worker["max-mode"]) }]),
      );
    }
  }
  ...
  if (requestedMode) {
    ...
    targetPolicies = Object.fromEntries(
      Object.entries(targetPolicies).map(([repository, targetPolicy]) => [repository, { mode: lesserMode(targetPolicy.mode, requestedMode) }]),
    );
  }
  ...
  const packages = Object.fromEntries(
    Object.entries(control.packages ?? {}).map(([name, policy]) => [
      name,
      {
        ...
        ...(policy.targets
          ? {
              target_policies: Object.fromEntries(Object.entries(policy.targets).map(([repository, targetPolicy]) => [repository.toLowerCase(), targetPolicy])),
            }
          : {}),
      },
    ]),
