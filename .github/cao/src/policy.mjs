...
export function parsePolicy(source) {
  ...
  if (!isRecord(document))
    throw new PolicyError('policy root must be a mapping')
...
function validateDocument(document) {
  ...
  if ('control-plane' in document)
    validateControlPlane(document['control-plane'])
  if ('target-authority' in document)
    validateTargetAuthority(document['target-authority'])
}
...
function validateControlPlane(control) {
  ...
  if ('defaults' in control)
    validateDefaults(control.defaults, 'control-plane.defaults')
...
function validateInventory(inventory) {
  ...
  if ('max-scan-repositories' in inventory) {
    assertInteger(
      inventory['max-scan-repositories'],
      'control-plane.inventory.max-scan-repositories',
      1,
      100_000,
    )
  }
  if ('cell-count' in inventory)
    assertInteger(
  ...
  if ('batch-index' in inventory)
    assertInteger(
      inventory['batch-index'],
...
function validateDefaults(defaults, path) {
  ...
  if ('max-repositories' in defaults)
    assertInteger(
      defaults['max-repositories'],
      `${path}.max-repositories`,
      1,
      1000,
    )
  if ('rollout-percent' in defaults)
    assertInteger(
  ...
  if ('monthly-ai-credit-budget' in defaults) {
    assertInteger(
      defaults['monthly-ai-credit-budget'],
      `${path}.monthly-ai-credit-budget`,
      0,
    )
  }
}
...
function validatePackages(packages) {
  ...
  for (const [packageName, packagePolicy] of Object.entries(packages)) {
    ...
    if ('enabled' in packagePolicy)
      assertBoolean(packagePolicy.enabled, `${path}.enabled`)
    if ('icon' in packagePolicy)
      assertOcticon(packagePolicy.icon, `${path}.icon`)
    ...
    if ('targets' in packagePolicy) {
      ...
      for (const [repository, targetPolicy] of Object.entries(targets)) {
        ...
        if (!('mode' in targetPolicy))
          throw new PolicyError(`${targetPath}.mode is required`)
    ...
    for (const [workerName, worker] of Object.entries(workers)) {
      ...
      if (workflows.has(worker.workflow)) {
        throw new PolicyError(
          `${path}.workers must declare unique workflow identities`,
        )
      }
      workflows.add(worker.workflow)
      if ('enabled' in worker)
        assertBoolean(worker.enabled, `${workerPath}.enabled`)
      if ('max-mode' in worker)
        assertMode(worker['max-mode'], `${workerPath}.max-mode`)
    }
  }
}
...
function validatePackageRepositoryScopes(control) {
  ...
  const ownerSet =
    allowedOwners && new Set(allowedOwners.map((owner) => owner.toLowerCase()))
  ...
  for (const packagePolicy of Object.values(control.packages ?? {})) {
    for (const repository of Object.keys(packagePolicy.targets ?? {})) {
      ...
      if (repositorySet && !repositorySet.has(normalized)) {
        throw new PolicyError(
          `package target ${repository} is outside control-plane.scope.allowed-repositories`,
        )
      }
      if (ownerSet && !ownerSet.has(normalized.split('/', 1)[0])) {
        throw new PolicyError(
          `package target ${repository} is outside control-plane.scope.allowed-owners`,
        )
      }
    }
  }
...
function validatePublishing(publishing) {
  ...
  if ('enabled' in publishing)
    assertBoolean(publishing.enabled, `${path}.enabled`)
  if ('control-repositories' in publishing) {
    assertUniqueStrings(
      publishing['control-repositories'],
      `${path}.control-repositories`,
      REPOSITORY_PATTERN,
    )
  }
  if ('reviewers' in publishing)
    assertUniqueStrings(
...
function rejectExpressions(value, path = 'policy') {
  if (Array.isArray(value)) {
    value.forEach((child, index) =>
      rejectExpressions(child, `${path}[${index}]`),
    )
    ...
    throw new PolicyError(
      `${path} must not contain a GitHub Actions expression`,
    )
  }
}
...
export function effectivePolicy(
  ...
  if (!['orchestrator', 'worker'].includes(role))
    throw new PolicyError('role must be orchestrator or worker')
  if (role === 'worker' && !workerName)
    throw new PolicyError('worker identity is required')
  if (role === 'orchestrator' && workerName)
    throw new PolicyError('worker identity is forbidden for orchestrators')

  const control = document['control-plane']
  if (!control)
    return denied('control-plane-absent', packageName, role, workerName)

  const packagePolicy = (control.packages ?? {})[packageName]
  if (!packagePolicy)
    return denied('package-undeclared', packageName, role, workerName)
  if (!(packagePolicy.enabled ?? true))
    return denied('package-disabled', packageName, role, workerName)
  const workers = packagePolicy.workers ?? {}
  ...
  let targetPolicies = Object.fromEntries(
    Object.entries(packagePolicy.targets ?? {}).map(
  ...
  if (targetRepository) {
    if (
      typeof targetRepository !== 'string' ||
  ...
  if (role === 'worker') {
    ...
    if (!(worker.enabled ?? true))
      return denied('worker-disabled', packageName, role, workerName)
    if ('max-mode' in worker) {
  ...
  const allowedOwners = scope['allowed-owners'] ?? [
    controlRepository.split('/', 1)[0],
  ]
  const inventory = {
...
export function controlSettings(document, controlRepository) {
  ...
  const packages = Object.fromEntries(
    Object.entries(control.packages ?? {}).map(([name, policy]) => [
      name,
      {
        enabled: policy.enabled ?? true,
        ...defaults,
        ...
        worker_policies: Object.fromEntries(
          Object.entries(policy.workers ?? {}).map(([worker, workerPolicy]) => [
            workerPolicy.workflow,
            {
              worker,
        ...
        ...(policy.targets
          ? {
              target_policies: Object.fromEntries(
                Object.entries(policy.targets).map(
                  ([repository, targetPolicy]) => [
                    repository.toLowerCase(),
                    targetPolicy,
                  ],
  ...
  return {
    allowed_owners: scope['allowed-owners'] ?? [
      controlRepository.split('/', 1)[0],
    ],
    allowed_repositories: scope['allowed-repositories'] ?? [],
    web: {
    ...
    publishing_control_repositories: publishing['control-repositories'] ?? [
      controlRepository,
    ],
...
function assertBoolean(value, path) {
  if (value !== true && value !== false)
    throw new PolicyError(`${path} must be a Boolean`)
}
...
function assertFavicon(value, path) {
  if (typeof value !== 'string' || value.length > 2048 || /\s/.test(value)) {
    throw new PolicyError(
      `${path} must be an absolute HTTPS URL or ./ relative path`,
    )
  }
  if (value.startsWith('./')) {
    if (
...
function assertMode(value, path) {
  if (!MODES.includes(value))
    throw new PolicyError(`${path} must be review or live`)
}

function assertString(value, path, pattern) {
  if (typeof value !== 'string' || !pattern.test(value))
    throw new PolicyError(`${path} has an invalid value`)
}
...
function assertUniqueStrings(value, path, pattern) {
  ...
  if (new Set(normalized).size !== normalized.length)
    throw new PolicyError(`${path} must contain unique values`)
}

function assertUniqueRepositoryKeys(value, path) {
  const normalized = Object.keys(value).map((repository) =>
    repository.toLowerCase(),
  )
  if (new Set(normalized).size !== normalized.length)
    throw new PolicyError(`${path} must contain unique repository names`)
}
...
function narrowInteger(effective, key, requested, path, maximum) {
  ...
  if (value > effective[key])
    throw new PolicyError(`${path} exceeds checked-in policy`)
  effective[key] = value
}
