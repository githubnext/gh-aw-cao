...
Run metadata and any available failure evidence come from the `gh aw logs` usage payload. `runHealth.usageArtifact` reports required fields omitted by those artifacts, with aggregate counts and bounded sample run IDs so gaps can be addressed in gh-aw. The downloader does not query workflow-run, run-detail, Jobs, job-log, or artifact APIs to fill missing run fields. Consumers must use the top-level completeness fields instead of inferring completeness from array length.

If `gh aw logs` fails, the downloader preserves a compatible cached snapshot, marks the refresh unavailable and incomplete in `gh-aw-logs-state.json`, and records no fallback acquisition. On a cold-cache failure, it writes an empty snapshot and marks run health unavailable.
