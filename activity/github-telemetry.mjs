...
export async function recordGithubTelemetry({
  ...
  const entry = {
    ...
    pairId: [process.env.GITHUB_RUN_ID || "local", process.env.GITHUB_RUN_ATTEMPT || "1", process.env.GITHUB_JOB || "unknown", operation].join(":"),
    phase,
    operation,
    outcome,
    tokenType,
  ...
  const { hit = false, key = null, matchedKey = null, entryCount = 0, folderCount = 0, bytes = 0 } = entry.activityCache || {};
