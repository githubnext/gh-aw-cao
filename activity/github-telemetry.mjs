...
export function normalizeGithubTelemetryStackTrace(
  stackTrace,
  limit = GITHUB_TELEMETRY_STACK_TRACE_LIMIT,
) {
...
export function defaultGithubTelemetryStackTrace(
  limit = GITHUB_TELEMETRY_STACK_TRACE_LIMIT,
) {
  const stack = new Error().stack
  if (typeof stack !== 'string') return []
  return normalizeGithubTelemetryStackTrace(
    stack.split(/\r?\n/).slice(2),
    limit,
  )
}

export async function collectActivityCacheState(
...
export function normalizeRateLimit(document) {
  ...
  for (const [name, value] of Object.entries(document?.resources || {})) {
    if (
      ![value?.limit, value?.remaining, value?.reset].every(
    ...
    resources[name] = {
      limit: value.limit,
      used: Number.isSafeInteger(value.used)
        ? value.used
        : Math.max(0, value.limit - value.remaining),
...
export async function recordGithubTelemetry({
  ...
  entry.activityCache.matchedKey =
    process.env.CAO_ACTIVITY_CACHE_MATCHED_KEY || null
  ...
  log.info`Activity cache ${phase} ${operation}: ${hit ? 'matched' : 'missed'}; keyConfigured=${Boolean(key)}; matchedKeyConfigured=${Boolean(matchedKey)}; entries=${entryCount}; folders=${folderCount}; bytes=${bytes}`
  return entry
}
...
export async function main(actions = {}, args = process.argv.slice(2)) {
  ...
  if (!['before', 'after'].includes(phase) || !operation) {
    throw new Error(
      'usage: github-telemetry.mjs prepare [prior-ledger] | <before|after> <operation> [outcome]',
    )
  }
