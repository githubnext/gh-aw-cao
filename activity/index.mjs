...
function workflowPath(run) {
  const value = String(firstValue(run.workflow_path, run.workflow_file, run.workflow_file_path, run.workflow, ""));
...
function summarizeRuns(records) {
  ...
  result.runRecords = records.sort((left, right) => Date.parse(right.createdAt || 0) - Date.parse(left.createdAt || 0) || right.runAttempt - left.runAttempt);
...
export async function main(actions = {}) {
  ...
  try {
    ...
    if (!usageArtifact.complete) {
      log.warning`gh aw logs usage artifacts omit activity-index fields: ${Object.entries(usageArtifact.missingFields)
    ...
    for (const run of logs.runs) {
      const aliases = [workflowPath(run), firstValue(run.workflow_id, run.workflow_name, run.workflow)]
        .filter(Boolean)
        .map((value) => String(value).toLowerCase());
    ...
    const bundles = (localInventory.bundles || []).map((bundle) => {
      ...
      const members = [orchestrator, ...(bundle.workers || []).map((worker) => workflowById.get(worker.id))].filter(Boolean);
