...
function runRecord(run, repository) {
  ...
  return {
    ...
    runAttempt:
      Number(firstValue(run.runAttempt, run.run_attempt, run.attempt)) || 1,
    ...
    ...(run.failure_message
      ? { failureMessage: String(run.failure_message) }
      : {}),
    ...(Array.isArray(run.jobs)
      ? { jobs: run.jobs.map(performanceJobRecord) }
      : {}),
  }
}
...
function summarizeRuns(records) {
  ...
  result.runRecords = records.sort(
    (left, right) =>
...
function usageArtifactGaps(runs) {
  const missing = Object.fromEntries(
    Object.keys(USAGE_FIELDS).map((field) => [field, 0]),
  )
  const samples = {}
...
async function discoverLocalInventory(root) {
  ...
  for (const entry of entries) {
    ...
    const role =
      source.match(
        /uses:\s+shared\/(?:cao|control)\.md[\s\S]*?role:\s+(orchestrator|worker)/,
      )?.[1] || 'standalone'
    const inlineWorkers = source.match(
      /^[ \t]+workflows:[ \t]*\[([^\]]*)\]/m,
    )?.[1]
    const blockWorkers = source.match(
      /^([ \t]+)workflows:[ \t]*\r?\n((?:\1[ \t]+-[^\r\n]+\r?\n?)*)/m,
    )?.[2]
    workflows.push({
      id,
      ...
      compiled: entries.some(
        (candidate) => candidate.name === `${id}.lock.yml`,
      ),
      workers:
        role === 'orchestrator'
          ? (inlineWorkers
...
export async function main(actions = {}) {
  ...
  try {
    ...
    const organization =
      process.env.REPORT_ORGANIZATION || repository.split('/')[0]
    ...
    const outputPath = path.resolve(
      process.env.REPORT_DEPLOYED_WORKFLOWS ||
        '_activity/deployed-workflows.json',
    )
    ...
    if (!repository || !logsPath || !logsStatePath) {
      throw new Error(
        'GITHUB_REPOSITORY, REPORT_GH_AW_LOGS, and REPORT_GH_AW_LOGS_STATE are required',
      )
    }
    if (!Number.isInteger(windowDays) || windowDays < 1 || windowDays > 31) {
      throw new Error(
        'REPORT_RUN_WINDOW_DAYS must be an integer from 1 through 31',
      )
    }
    ...
    if (
      localInventory.schemaVersion !== 1 ||
      !Array.isArray(localInventory.workflows)
    ) {
      throw new Error(
        `Unsupported or invalid control-plane inventory: ${inventoryPath}`,
      )
    }
    if (!Array.isArray(logs.runs))
      throw new Error(`Unsupported or invalid gh aw logs snapshot: ${logsPath}`)
    const usageArtifact = usageArtifactGaps(logs.runs)
    if (!usageArtifact.complete) {
      log.warning`gh aw logs usage artifacts omit activity-index fields: ${Object.entries(
        usageArtifact.missingFields,
      )
    ...
    await Promise.all(
      localInventory.workflows.map(async (workflow) => {
    ...
    for (const workflow of localInventory.workflows) {
      for (const alias of workflowAliases(workflow))
        workflowByAlias.set(alias, workflow)
    }
    const runsByWorkflow = new Map(
      localInventory.workflows.map((workflow) => [workflow.id, []]),
    )
    for (const run of logs.runs) {
      const aliases = [
        ...
        .filter(Boolean)
        .map((value) => String(value).toLowerCase())
      const workflow = aliases
        .map((alias) => workflowByAlias.get(alias))
        .find(Boolean)
      const record = runRecord(run, repository)
      if (workflow && record) runsByWorkflow.get(workflow.id).push(record)
    }

    const workflows = localInventory.workflows
      .map((workflow) => {
        const source = sourceByWorkflow.get(workflow.id) || ''
        const { ghAwMetadata, ghAwManifest } = parseMetadata(
          lockByWorkflow.get(workflow.id) || '',
        )
    ...
    const bundles = (localInventory.bundles || []).map((bundle) => {
      ...
      const members = [
        orchestrator,
        ...(bundle.workers || []).map((worker) => workflowById.get(worker.id)),
      ].filter(Boolean)
      const packagePath =
        localInventory.workflows.find((workflow) => workflow.id === bundle.id)
          ?.package?.path || ''
      return {
        repository,
        visibility: 'unknown',
    ...
    const packagedWorkflowIds = new Set(
      bundles.flatMap((bundle) =>
        bundle.workflows.map((workflow) => workflow.id),
      ),
    )
    const standaloneWorkflows = workflows.filter(
      (workflow) => !packagedWorkflowIds.has(workflow.id),
    )
    const generatedAt = new Date().toISOString()
    const windowStart = new Date(
      Date.parse(generatedAt) - windowDays * 86_400_000,
    ).toISOString()
    ...
    const fallback = {
      ...
      snapshotGeneratedAt: logsState.fallback
        ? logsState.snapshotObservedAt || null
        : null,
      snapshotAgeSeconds:
        logsState.fallback && logsState.snapshotObservedAt
          ? Math.max(
              0,
              Math.floor(
                (Date.parse(generatedAt) -
                  Date.parse(logsState.snapshotObservedAt)) /
                  1000,
              ),
            )
          : null,
    ...
    const result = {
      ...
      collections: [
        ...
        },
        {
          ...
          observed: available
            ? (logsState.actionsTargetsObserved ?? logsState.targetCount)
            : null,
