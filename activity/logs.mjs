...
function runGhAw(targets, outputDirectory, cachedJsonPath, windowDays, runLimit, execute = spawn) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code, signal) => {
      ...
      else reject(new Error(Buffer.concat(stderr).toString("utf8").trim() || `gh aw logs exited with ${signal || code}`));
    });
  });
}
...
function runGhApi(target, repository, windowStart, runLimit, execute = spawn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code, signal) => {
      if (code !== 0 || signal) {
        reject(new Error(Buffer.concat(stderr).toString("utf8").trim() || `gh api exited with ${signal || code}`));
        return;
      }
      try {
...
function runGhJobsApi(repository, runId, runAttempt, page, execute = spawn) {
  return new Promise((resolve, reject) => {
    const child = execute(
      "gh",
      ["api", "--method", "GET", `repos/${repository}/actions/runs/${runId}/attempts/${runAttempt}/jobs`, "-f", "per_page=100", "-f", `page=${page}`],
      { env: process.env, stdio: ["ignore", "pipe", "pipe"] },
    );
    const stdout = [];
    const stderr = [];
...
async function collectJobDetails(runs, repository, execute) {
  const pending = runs.filter((run) => {
    const identity = runIdentity(run, repository);
    return Number.isSafeInteger(identity.runId) && identity.repository && (run.jobs_collected !== true || run.status !== "completed");
  });
  let nextRun = 0;
  let observed = 0;
  let failed = 0;
    ...
    while (nextRun < pending.length) {
      ...
      try {
        ...
        for (let page = 1; page <= ACTIONS_JOB_PAGE_LIMIT; page += 1) {
          const pageJobs = await runGhJobsApi(identity.repository, identity.runId, identity.runAttempt, page, execute);
          jobs.push(...pageJobs);
  ...
  await Promise.all(Array.from({ length: Math.min(ACTIONS_API_CONCURRENCY, pending.length) }, () => worker()));
  return {
...
function mergeRunMetadata(cachedRuns, actionRuns) {
  ...
  for (const cached of cachedRuns) {
    ...
    byRunId.set(
      runId,
      Object.fromEntries(
        [...new Set([...Object.keys(actionRun), ...Object.keys(cached)])].map((key) => [
          key,
          cached[key] === undefined || cached[key] === null || cached[key] === "" ? actionRun[key] : cached[key],
        ]),
      ),
    );
  }
...
async function enrichFromActions(targets, repository, windowDays, runLimit, cachedRuns, execute) {
  ...
  const worker = async () => {
    while (nextTarget < targets.length) {
      ...
      try {
        results[index] = {
          status: "fulfilled",
          value: await runGhApi(targets[index], repository, windowStart, runLimit, execute),
        };
      } catch (reason) {
  ...
  await Promise.all(Array.from({ length: Math.min(ACTIONS_API_CONCURRENCY, targets.length) }, () => worker()));
...
export async function collectActivityLogs({ execute = spawn } = {}) {
  ...
  try {
    ...
    const enrichment = await enrichFromActions(targets, repository, windowDays, runLimit, cachedRuns, execute);
