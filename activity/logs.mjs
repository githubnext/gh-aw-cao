...
function runIdentity(run, repository) {
  return {
    repository: String(
      run.repository || run.repository_full_name || repository,
    ),
...
function reuseCachedJobs(runs, cachedRuns, repository) {
  const cachedByRun = new Map(
    cachedRuns.map((run) => {
      const identity = runIdentity(run, repository)
      return [
        `${identity.repository.toLowerCase()}:${identity.runId}`,
        { identity, run },
      ]
    }),
  )
...
function mergeRunMetadata(cachedRuns, actionRuns) {
  const byRunId = new Map(
    actionRuns.map((run) => [String(run.database_id), run]),
  )
  for (const cached of cachedRuns) {
    ...
    byRunId.set(
      runId,
      Object.fromEntries(
        [...new Set([...Object.keys(actionRun), ...Object.keys(cached)])].map(
          (key) => [
            key,
...
async function enrichFromActions(
  targets,
  repository,
  windowDays,
  runLimit,
  ...
  const actionRuns = results.flatMap((result) =>
    result.status === 'fulfilled' ? result.value : [],
  )
  return {
    runs: mergeRunMetadata(cachedRuns, actionRuns),
    observedTargets: results.filter((result) => result.status === 'fulfilled')
      .length,
    failedTargets: results.filter((result) => result.status === 'rejected')
      .length,
  }
}
...
export async function collectActivityLogs({ execute = spawn } = {}) {
  ...
  const logsPath = path.resolve(
    process.env.REPORT_GH_AW_LOGS || '_activity/gh-aw-logs.json',
  )
  const statePath = path.resolve(
    process.env.REPORT_GH_AW_LOGS_STATE || '_activity/gh-aw-logs-state.json',
  )
  const outputDirectory = path.resolve(
    process.env.REPORT_AIC_CACHE || '_activity/gh-aw-logs',
  )
  const windowDays = Number(
    process.env.REPORT_RUN_WINDOW_DAYS || DEFAULT_WINDOW_DAYS,
  )
  ...
  const cachedRuns = Array.isArray(cachedSnapshot.runs)
    ? cachedSnapshot.runs
    : []
  ...
  try {
    ...
    const { output: raw, stderr } = await runGhAw(
      targets,
      outputDirectory,
      logsPath,
      windowDays,
    ...
    if (!Array.isArray(snapshot.runs))
      throw new Error('gh aw logs returned invalid JSON')
    const reusedJobRuns = reuseCachedJobs(snapshot.runs, cachedRuns, repository)
    const jobDetails = await collectJobDetails(
      snapshot.runs,
      repository,
      execute,
    )
    ...
    const workflowCount = new Set(
      snapshot.runs.map(
        (run) => run.workflow_path || run.workflow_name || 'unknown',
      ),
    ).size
    ...
    const reusedJobRuns = reuseCachedJobs(
      enrichment.runs,
      cachedRuns,
      repository,
    )
      ...
      `${JSON.stringify(
        {
          ...
          snapshotObservedAt:
            previousState.snapshotObservedAt ||
