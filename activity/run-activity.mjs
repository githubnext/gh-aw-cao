...
export async function runActivity(actions = {}) {
  ...
  if (dashboardCollectionEnabled && !dashboardReportRoot) {
    throw new Error(
      'DASHBOARD_REPORT_ROOT is required when DASHBOARD_COLLECTION is true',
    )
  }
  ...
  if (dashboardCollectionEnabled) {
    ...
    const controlSettings = await importModule(
      path.join(dashboardReportRoot, 'control-settings.mjs'),
    )
    ...
    const inventory = await importModule(
      path.join(dashboardReportRoot, 'inventory.mjs'),
    )
  ...
  if (dashboardCollectionEnabled) {
    const collectors = await importModule(
      path.join(dashboardReportRoot, 'activity-collectors.mjs'),
    )
