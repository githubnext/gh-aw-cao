...
export async function runActivity(actions = {}) {
  ...
  const [logs, telemetry, indexer] = await Promise.all([importModule(logsModulePath), importModule(telemetryModulePath), importModule(indexerModulePath)]);
  ...
  await telemetry.main(actions, ["prepare", path.join(runnerTemp, "cao-activity", "cao-gh.jsonl")]);
  ...
  if (dashboardCollectionEnabled) {
    const controlSettingsPath = process.env.REPORT_CONTROL_SETTINGS || path.join(runnerTemp, "cao-activity", "control-settings.json");
