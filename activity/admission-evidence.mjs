...
export function normalizeAdmissionRecord(value, expected = {}) {
  ...
  if (
    ...
    (expected.repository !== undefined &&
      repository.toLowerCase() !== expected.repository.toLowerCase()) ||
  ...
  if (checks.some(({ check, status }) => !check || !CHECK_STATUSES.has(status)))
    return null
...
export function admissionRecordFromArchive(archivePath, expected = {}) {
  ...
  if (
    ...
    result.status !== 0 ||
    !result.stdout ||
    Buffer.byteLength(result.stdout) >= MAX_RECORD_BYTES
  )
    return null
  try {
