...
const API_LIMITED_STEP_PREFIX =
  'CAO admission blocked: GitHub API limited until '
const API_UNAVAILABLE_STEP =
  'CAO admission blocked: GitHub API capacity unavailable'
...
export function runFailureEvidence(jobs, now = Date.now()) {
  ...
  const failedStep = failedJob?.steps?.find((step) =>
  ...
  for (const step of jobs.flatMap((job) => job.steps || [])) {
    ...
    const resetTime = Date.parse(
      step.name.slice(API_LIMITED_STEP_PREFIX.length).trim(),
    )
    admission = {
      admissionStatus: 'resource-limited',
      ...
      ...(Number.isFinite(resetTime)
        ? {
            ...
            resourceWaitHours:
              Math.ceil(Math.max(0, resetTime - now) / 36_000) / 100,
          }
        : {}),
    }
    break
...
export function extractCaoFailureMessage(logText) {
  ...
  for (const line of log.split(/\r?\n/)) {
    ...
    if (marker >= 0) {
      return normalizeCaoFailureMessage(
        line.slice(marker + CAO_FAILURE_MARKER.length).trim(),
      )
    }
  }
  return ''
