...
const packageSource = process.env.CENTRAL_AGENTIC_OPS_PACKAGE_SOURCE || "githubnext/gh-aw-cao@main";
...
const dashboardExpectedFiles = [
  ...
  ...[...readFileSync(new URL("../../dashboard/aw.yml", import.meta.url), "utf8").matchAll(/^\s+destination: (.+)$/gm)].map((match) => match[1]),
];
...
function installPackage(source) {
  ...
  try {
    ...
    run("gh", ["aw", "add", source, "--force", "--no-security-scanner"], consumer);
    return consumer;
  } catch (error) {
...
test("gh aw add installs the root package without rewriting Copilot authentication", { timeout: 180_000 }, () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(readFileSync(join(consumer, ".github", "workflows", "aw.json"), "utf8")).auto_upgrade.options, ["--pre-releases"]);
...
test("gh aw add installs the focused activity package contract", { timeout: 180_000 }, () => {
  ...
  try {
    ...
    const installedManifest = JSON.parse(readFileSync(join(consumer, ".github", "aw", "packages", packageManifests[0]), "utf8"));
    assert.deepEqual(
...
test("gh aw add installs the focused EU CRA package contract", { timeout: 180_000 }, () => {
  ...
  try {
    ...
    assert.ok(!existsSync(join(consumer, ".github", "workflows", "dependabot.md")), "focused CRA package installed an unrelated orchestrator");
...
test("gh aw add installs the focused UK AI Advisory package contract", { timeout: 180_000 }, () => {
  ...
  try {
    ...
    assert.ok(!existsSync(join(consumer, ".github", "workflows", "dependabot.md")), "focused UK AI Advisory package installed an unrelated orchestrator");
...
test("gh aw add installs the focused Software Development Practices package contract", { timeout: 180_000 }, () => {
  ...
  try {
    for (const relativePath of softwareDevelopmentPracticesExpectedFiles) {
      assert.ok(existsSync(join(consumer, relativePath)), `focused Software Development Practices package omitted ${relativePath}`);
    }
    assert.ok(
...
test("gh aw add --force restores dashboard workflows, producers, and renderer assets", { timeout: 180_000 }, () => {
  ...
  try {
    ...
    const removedFiles = [".github/aw/dashboard/report/records.mjs", ".github/aw/dashboard/site/index.html", ".github/workflows/dashboard-build.yml"];
    for (const relativePath of removedFiles) {
      rmSync(join(consumer, relativePath));
    }

    run("gh", ["aw", "add", dashboardPackageSource, "--force", "--no-security-scanner"], consumer);
...
test("gh aw update replaces workflows and restores package-owned assets", { timeout: 180_000 }, () => {
  ...
  try {
    ...
    const removedFiles = [".github/workflows/dependabot-release-train-updater.md", ".github/workflows/shared/control.md"];
    for (const relativePath of removedFiles) {
      rmSync(join(consumer, relativePath));
    }

    run("gh", ["aw", "update", "--force", "--no-merge", "--no-compile", "--no-security-scanner", "--cool-down", "0"], consumer);

    const updatedOrchestrator = readFileSync(orchestratorPath, "utf8");
    assert.ok(!updatedOrchestrator.includes("# local integration-test change"), "gh aw update retained a local package workflow modification");
