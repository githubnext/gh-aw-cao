...
const packageSource =
  process.env.CENTRAL_AGENTIC_OPS_PACKAGE_SOURCE || 'githubnext/gh-aw-cao@main'
...
const softwareDevelopmentPracticesPackageSource = focusedPackageSource(
...
function installPackage(source) {
  ...
  try {
    ...
    run(
      'gh',
      ['aw', 'add', source, '--force', '--no-security-scanner'],
      consumer,
    )
    return consumer
  } catch (error) {
...
test(
  'gh aw add installs the root package without rewriting Copilot authentication',
  { timeout: 180_000 },
  () => {
    const consumer = installPackage(packageSource)
    try {
      assert.ok(
        existsSync(join(consumer, '.github', 'aw', 'default-AGENTS.md')),
      )
      assert.deepEqual(
        JSON.parse(
          readFileSync(
            join(consumer, '.github', 'workflows', 'aw.json'),
            'utf8',
          ),
        ).auto_upgrade.options,
        ['--pre-releases'],
      )
      for (const relativePath of activityExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `root package omitted activity file ${relativePath}`,
        )
      }
      for (const relativePath of dashboardExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `root package omitted dashboard file ${relativePath}`,
        )
      }
      for (const workflowId of ['aw-doctor', 'dependabot', 'optimization']) {
        const source = readFileSync(
          join(consumer, '.github', 'workflows', `${workflowId}.md`),
          'utf8',
        )
        const lock = readFileSync(
          join(consumer, '.github', 'workflows', `${workflowId}.lock.yml`),
          'utf8',
        )
...
test(
  'gh aw add installs the focused activity package contract',
  { timeout: 180_000 },
  () => {
    ...
    try {
      for (const relativePath of activityExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `activity package omitted ${relativePath}`,
        )
      }
      const packageManifests = readdirSync(
        join(consumer, '.github', 'aw', 'packages'),
      )
      assert.equal(
        packageManifests.length,
        1,
        'expected one installed activity package manifest',
      )
...
test(
  'gh aw add installs the focused EU CRA package contract',
  { timeout: 180_000 },
  () => {
    ...
    try {
      for (const relativePath of craExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `focused CRA package omitted ${relativePath}`,
        )
      }
      assert.ok(
      ...
      assert.equal(
        packageManifests.length,
        1,
        'expected one focused CRA package manifest',
      )
      const installedManifest = JSON.parse(
        readFileSync(
          join(consumer, '.github', 'aw', 'packages', packageManifests[0]),
...
test(
  'gh aw add installs the focused UK AI Advisory package contract',
  { timeout: 180_000 },
  () => {
    ...
    try {
      for (const relativePath of ukAiAdvisoryExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `focused UK AI Advisory package omitted ${relativePath}`,
        )
      }
      assert.ok(
        !existsSync(join(consumer, '.github', 'workflows', 'dependabot.md')),
      ...
      assert.equal(
        packageManifests.length,
        1,
        'expected one focused UK AI Advisory package manifest',
      )
      const installedManifest = JSON.parse(
        readFileSync(
          join(consumer, '.github', 'aw', 'packages', packageManifests[0]),
...
test(
  'gh aw add installs the focused SelfCare package contract',
  { timeout: 180_000 },
  () => {
    ...
    try {
      for (const relativePath of selfCareExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `focused SelfCare package omitted ${relativePath}`,
        )
      }
    } finally {
      rmSync(consumer, { recursive: true, force: true })
...
test(
  'gh aw add installs the focused AW Doctor package contract',
  { timeout: 180_000 },
  () => {
    ...
    try {
      for (const relativePath of awDoctorExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `focused AW Doctor package omitted ${relativePath}`,
        )
      }
    } finally {
      rmSync(consumer, { recursive: true, force: true })
...
test(
  'gh aw add installs the focused Software Development Practices package contract',
  { timeout: 180_000 },
  () => {
    ...
    try {
      ...
      assert.equal(
        packageManifests.length,
        1,
        'expected one focused Software Development Practices package manifest',
      )
      const installedManifest = JSON.parse(
        readFileSync(
          join(consumer, '.github', 'aw', 'packages', packageManifests[0]),
  ...
  () => {
    ...
    try {
      for (const relativePath of dashboardExpectedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `dashboard package omitted ${relativePath}`,
        )
      }
      ...
      assert.equal(
        packageManifests.length,
        1,
        'expected one installed dashboard package manifest',
      )
      const installedManifest = JSON.parse(
        readFileSync(
          join(consumer, '.github', 'aw', 'packages', packageManifests[0]),
      ...
      const buildWorkflow = readFileSync(
        join(consumer, '.github', 'workflows', 'dashboard-build.yml'),
        'utf8',
      )
      const deployWorkflow = readFileSync(
        join(consumer, '.github', 'workflows', 'dashboard.yml'),
        'utf8',
      )
      assert.doesNotMatch(buildWorkflow, /workflow_call:/)
      assert.match(
        buildWorkflow,
        /workflow_dispatch:[\s\S]*?site-path:[\s\S]*?request-id:/,
      )
...
test(
  'gh aw add --force restores dashboard workflows, producers, and renderer assets',
  { timeout: 180_000 },
  () => {
    const consumer = installPackage(dashboardPackageSource)
    ...
    try {
      ...
      assert.ok(
        !readFileSync(deployPath, 'utf8').includes(
          '# local integration-test change',
        ),
      ...
      for (const relativePath of removedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
          `gh aw add --force did not restore ${relativePath}`,
        )
      }
    } finally {
      rmSync(consumer, { recursive: true, force: true })
...
test(
  ...
  () => {
    ...
    try {
      ...
      assert.equal(
        workflowBody(updatedOrchestrator),
        workflowBody(orchestrator),
      )
      for (const relativePath of removedFiles) {
        assert.ok(
          existsSync(join(consumer, relativePath)),
