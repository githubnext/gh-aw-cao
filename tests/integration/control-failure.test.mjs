...
const failures = [
  [
    'malformed target repository',
    { TARGET_REPO: 'not-a-repository' },
    'target_repo must use owner/repository form',
  ],
  [
    'missing worker target repository',
    { TARGET_REPO: '' },
    'worker target_repo is required',
  ],
  [
    'worker target outside the exact repository allowlist',
    { TARGET_REPO: 'acme/other' },
    'worker target_repo is not allowed',
    controlPolicy({
      scope: { 'allowed-repositories': ['acme/target'] },
    }),
  ],
  [
    'disallowed target owner',
    { TARGET_REPO: 'outside/target' },
    'target_repo owner is outside control-plane.scope.allowed-owners',
  ],
  [
    'malformed review repository',
    { SAFE_OUTPUT_REPO: 'not-a-repository' },
    'safe_output_repo must use owner/repository form',
  ],
  [
    'disallowed review owner',
    { SAFE_OUTPUT_REPO: 'outside/review' },
    'safe_output_repo owner is outside control-plane.scope.allowed-owners',
  ],
  [
    'worker mode ceiling',
    { REQUESTED_MODE: 'live', SAFE_OUTPUT_REPO: 'acme/target' },
    'safe_output_mode exceeds checked-in policy',
  ],
  [
    'invalid safe-output mode',
  ...
  [
    'mismatched control repository',
    { CENTRAL_REPO: 'acme/other' },
    'central_repo must identify the current control repository',
  ],
  [
    'mismatched control run URL',
    {
      CONTROL_PLANE_RUN_URL: 'https://github.com/acme/control/actions/runs/999',
    },
    'control_plane_run_url must match correlation_id and central_repo',
  ],
  [
    'oversized repository request',
    { ROLE: 'orchestrator', TARGET_REPO: '', REQUESTED_MAX_REPOS: '1001' },
    'max_repositories must be an integer in 1..1000',
  ],
  [
    'invalid rollout request',
    { ROLE: 'orchestrator', TARGET_REPO: '', REQUESTED_ROLLOUT_PERCENT: '0' },
    'rollout_percent must be an integer in 1..100',
  ],
  [
    'invalid dispatch maximum',
    { ROLE: 'orchestrator', TARGET_REPO: '', DISPATCH_MAX: 'invalid' },
...
const invalidPolicies = [
  [
    'invalid package kill switch',
    controlPolicy({ packagePolicy: { enabled: 'invalid' } }),
    'control-plane.packages.dependabot.enabled must be a Boolean',
  ],
  [
    'invalid worker kill switch',
    controlPolicy({ workerPolicy: { enabled: 'False' } }),
    'control-plane.packages.dependabot.workers.release-train-updater.enabled must be a Boolean',
  ],
  [
    'removed worker ceiling',
    controlPolicy({ workerPolicy: { 'max-mode': 'preview' } }),
    'control-plane.packages.dependabot.workers.release-train-updater.max-mode must be review or live',
  ],
  [
    'oversized scan cap',
    controlPolicy({ inventory: { 'max-scan-repositories': 100001 } }),
    'control-plane.inventory.max-scan-repositories must be an integer in 1..100000',
  ],
  [
    'invalid cell count',
    controlPolicy({ inventory: { 'cell-count': 0 } }),
    'control-plane.inventory.cell-count must be an integer in 1..1000',
  ],
  [
    'invalid cell index',
    controlPolicy({ inventory: { 'cell-count': 4, 'cell-index': 4 } }),
    'control-plane.inventory.cell-index must be smaller than cell-count',
  ],
  [
    'oversized batch',
    controlPolicy({ inventory: { 'batch-size': 100001 } }),
    'control-plane.inventory.batch-size must be an integer in 1..100000',
  ],
  [
    'invalid batch index',
    controlPolicy({ inventory: { 'batch-index': -1 } }),
...
test('control precompute fails cleanly as an admission check when GitHub API rate limits are reached mid-run', () => {
  const directory = mkdtempSync(
    join(tmpdir(), 'central-ops-precompute-ratelimit-'),
  )
