...
import { controlEnvironment, controlPolicy, controlProgram, root } from "../helpers/control-precompute.mjs";
...
for (const role of ["orchestrator", "worker"]) {
  for (const safeOutputMode of ["review", "live"]) {
    test(`control precompute disables a ${role} in ${safeOutputMode} before validation or repository access`, () => {
      ...
      assert.equal(
        result.stdout,
        ["::group::Central Agentic Ops precompute", "[CAO] Precompute skipped because admission was denied.", "::endgroup::", ""].join("\n"),
      );
...
test("control precompute disables a worker before review repository access", () => {
  const result = runPrecompute({}, "echo 'GitHub must not be called for a disabled worker' >&2; exit 99", controlPolicy({ workerPolicy: { enabled: false } }));
...
test("control precompute rejects a public non-central review destination", () => {
  const result = runPrecompute({ SAFE_OUTPUT_REPO: "acme/review" }, "printf 'false\\n'");
