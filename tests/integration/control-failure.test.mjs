  ...
  for (const safeOutputMode of ["review", "live"]) {
    test(`control precompute disables a ${role} in ${safeOutputMode} before validation or repository access`, () => {
      ...
      assert.equal(result.stdout, [
        ...
        '[CAO decision] {"decision":"precompute","outcome":"skipped","reason":"campaign-disabled"}',
        "::endgroup::",
        "",
      ].join("\n"));
...
test("control precompute writes a complete review worker envelope", () => {
  ...
  assert.match(result.stdout, /\[CAO decision] {"decision":"safe-output-destination","outcome":"accepted","mode":"review","validation":"central-review-shortcut"}/);
  assert.match(result.stdout, /\[CAO decision] {"decision":"worker-dispatch-envelope","outcome":"accepted","mode":"review","worker_max_mode":"review"}/);
  assert.match(result.stdout, /\[CAO decision] {"decision":"precompute","outcome":"prepared","role":"worker","candidate_count":0}/);
});
