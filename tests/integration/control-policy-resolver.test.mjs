...
function expectFailure(result, expected) {
  ...
  assert.ok(result.stderr.includes(expected), `expected stderr to include ${JSON.stringify(expected)}, received:\n${result.stderr}`);
}
...
test("control.mjs always logs policy validation without changing JSON output", () => {
  ...
  assert.equal(result.stderr, ["[CAO policy] Parsing control policy.", "[CAO policy] Validated control policy.", ""].join("\n"));
});
...
test("control.mjs authority requires a declared package", () => {
  expectFailure(run(["authority", "-", "unknown"], JSON.stringify(policy())), "target authority does not declare package unknown");
});
