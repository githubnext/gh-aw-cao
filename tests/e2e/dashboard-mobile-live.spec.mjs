...
import { summarizeAccessibilityTree, summarizeDomTree, summarizeMobileAccessibility } from "./dashboard-tree-analysis.mjs";
...
test("latest dashboard data loads within the mobile DOM budget", async ({ page }, testInfo) => {
  ...
  const domTree = await page.evaluate(() => {
    ...
    const structures = [...document.querySelectorAll("[data-page-id], [data-section-id], [data-view-id], header, nav, main, footer, table, svg, details")].map(
      (element) => ({
        tag: element.tagName.toLowerCase(),
        id: element.id || null,
        classes: [...element.classList],
  ...
  const mobileAccessibility = await page.evaluate(() => {
    ...
    const targets = [...document.querySelectorAll(selector)].flatMap((element) => {
      ...
      const explicitLabel =
        !wrappingLabel && element.id && element.matches("input, select, textarea") ? document.querySelector(`label[for="${CSS.escape(element.id)}"]`) : null;
      ...
      if (style.visibility === "hidden" || style.display === "none" || style.pointerEvents === "none" || rectangle.width === 0 || rectangle.height === 0)
        return [];
  ...
  expect(analysis.mobileAccessibility.zoom, "The viewport metadata must allow at least 200% zoom").toMatchObject({
    passes: true,
  });
