...
function formatDomAnalysis(dom) {
  const formatDistribution = ({ mean, median, p95, maximum }) =>
    `mean ${mean}, median ${median}, p95 ${p95}, max ${maximum}`
  const formatFrequencies = (values) =>
    values
      .slice(0, 5)
  ...
  const formatStructures =
    dom.topStructures
      ...
      .map(({ tag, id, jsonPath, jsView, descendantElements }) => {
        const provenance = jsonPath
          ? ` [${jsonPath}${jsView ? ` -> ${jsView}` : ''}]`
          : ''
...
test.beforeAll(async () => {
  ...
  preview = await startDashboardServer({
    downloadData: async (destination) => {
      ...
      if (!response.ok)
        throw new Error(
          `Unable to download deployed dashboard data: HTTP ${response.status}.`,
        )
...
test('latest dashboard data loads within the mobile DOM budget', async ({
  page,
}, testInfo) => {
  const pageErrors = []
  let crashed = false
  ...
  const networkIsConstrained = Object.values(network).some(
    (value) => value !== null,
  )
  ...
  if (networkIsConstrained) {
    expect(
      ...
      'All network constraint values are required',
    ).not.toContain(null)
  ...
  await expect(dashboard).toHaveAttribute('data-json-path', '$.dashboard', {
    timeout: 30_000,
  })
  ...
  expect(
    sourceIndexResponse,
    'The dashboard must request the split source manifest',
  ).toBeDefined()
  expect(
    sourceIndexResponse?.ok(),
    `Dashboard source manifest returned ${sourceIndexResponse?.status()}`,
  ).toBe(true)
  expect(
    sourcesResponse,
    'The dashboard must avoid fetching the monolithic sources.json when the manifest is available',
  ).toBeUndefined()
  expect(
    crashed,
    'The mobile browser page crashed while rendering the dashboard',
  ).toBe(false)
  ...
  const mobileAccessibility = await page.evaluate(() => {
    ...
    const targets = [...document.querySelectorAll(selector)].flatMap(
      (element) => {
        ...
        const explicitLabel =
          ...
          element.matches('input, select, textarea')
            ...
            : null
        const hitArea =
          (wrappingLabel?.contains(element) ? wrappingLabel : explicitLabel) ??
          element
        ...
        if (
          ...
          return []
        const labelText =
          hitArea instanceof HTMLLabelElement ? hitArea.textContent : null
        return [
          {
            tag: element.tagName.toLowerCase(),
            name:
  ...
  const accessibilityPath = testInfo.outputPath(
    'mobile-dashboard-accessibility-tree.yml',
  )
  ...
  expect(
    analysis.dom.totalElements,
    `Dashboard rendered ${analysis.dom.totalElements} DOM elements`,
  ).toBeLessThanOrEqual(maximumDomNodes)
  expect(
  ...
  expect(
    analysis.mobileAccessibility.reflow,
    'The dashboard must reflow without horizontal page scrolling',
  ).toMatchObject({ passes: true })
  expect(
    analysis.mobileAccessibility.zoom,
    'The viewport metadata must allow at least 200% zoom',
  ).toMatchObject({ passes: true })
  expect(
    analysis.mobileAccessibility.accessibleNames.unnamedInteractive,
