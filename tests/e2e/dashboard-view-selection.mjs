...
export function changedDashboardPageIds(current, previous) {
  const previousPages = new Map((previous?.dashboard?.pages || []).map((page) => [page.id, JSON.stringify(page)]));
  return (current?.dashboard?.pages || []).filter((page) => previousPages.get(page.id) !== JSON.stringify(page)).map((page) => page.id);
}
...
function pagesUsingElement(dashboard, element) {
  return dashboard.dashboard.pages.filter((page) => JSON.stringify(page).includes(`"element":"${element}"`)).map((page) => page.id);
}
...
function canScopeComponent(path) {
  ...
  const exports = [...source.matchAll(/\bexport\s+(?:async\s+)?(?:function|class|const|let|var)\s+([A-Za-z_$][\w$]*)/g)];
  ...
  for (const candidate of candidates) {
    ...
    const importsTarget = [...readFileSync(candidate, "utf8").matchAll(importPattern)].some((match) => resolve(dirname(candidate), match[1]) === absolutePath);
