...
function composedDashboard(paths) {
  const [primary, ...additions] = paths
    .map((path) => readDashboard(path))
    .filter(Boolean)
...
export function changedDashboardPageIds(current, previous) {
  const previousPages = new Map(
    (previous?.dashboard?.pages || []).map((page) => [
      page.id,
      JSON.stringify(page),
    ]),
  )
...
function canScopeComponent(path) {
  ...
  for (const candidate of candidates) {
    ...
    const importsTarget = [
      ...readFileSync(candidate, 'utf8').matchAll(importPattern),
    ].some((match) => resolve(dirname(candidate), match[1]) === absolutePath)
    if (
      importsTarget &&
      candidate !== join(sourceRoot, 'components/ui-elements.js')
    )
      return false
  }
  return true
...
export function selectAffectedPageIds({ dashboard, changedFiles, baseRef }) {
  ...
  for (const path of changedFiles) {
    if (path.endsWith('/dashboard.json') || path === primaryDashboardPath) {
      ...
      if (sharedDashboardConfigurationChanged(current, previous))
        return allPageIds
      for (const pageId of changedDashboardPageIds(current, previous))
        selected.add(pageId)
      continue
    }
    if (
      path.startsWith('dashboard/site/src/components/') &&
      path.endsWith('.js')
    ) {
...
function main() {
  ...
  if (!baseRef)
    throw new Error('usage: dashboard-view-selection.mjs BASE_REF [HEAD_REF]')
