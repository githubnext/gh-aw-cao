...
export function summarizeDomTree(nodes, structures) {
  ...
  return {
    ...
    childElements: distribution(
      nodes.map(({ childElementCount }) => childElementCount),
    ),
...
export function summarizeAccessibilityTree(snapshot) {
  ...
  const headingLevels = nodes
    .filter(({ role, level }) => role === 'heading' && level > 0)
    .map(({ level }) => level)
  ...
  const topStructures = nodes
    .flatMap((node, index) => {
      ...
      return descendantNodes > 0
        ? [{ role: node.role, name: node.name || null, descendantNodes }]
        : []
    })
...
export function summarizeMobileAccessibility(
  { targets, scrollWidth, viewportWidth, viewportMeta, unnamedInteractive },
  ...
  const undersizedTargets = targets
    ...
    .sort(
      (left, right) =>
        Math.min(left.width, left.height) - Math.min(right.width, right.height),
    )
  ...
  const zoomAllowed =
    !['no', '0'].includes(viewportDirectives.get('user-scalable')) &&
