...
test('each selected dashboard view renders with live data', async ({
  browser,
}, testInfo) => {
  ...
  const summary = {
    ...
    maximumDomNodes,
    selectionMode:
      process.env.DASHBOARD_PAGE_IDS === undefined ? 'all' : 'affected',
    selectedPageIds: [],
    results: [],
  }
  let preview
  ...
  try {
    ...
    if (!dashboardResponse.ok) {
      throw new Error(
        `Unable to load composed dashboard.json: HTTP ${dashboardResponse.status}.`,
      )
    }
    ...
    for (const pageDefinition of pages) {
      ...
      try {
        await page.goto(
          `${preview.url}/#page-${encodeURIComponent(pageDefinition.id)}`,
          {
        ...
        const sourceProblems = missingDashboardSources(
          pageDefinition,
          effectiveSources,
        )
        result.missingData = [
  ...
  expect(
    summary.blocker,
    'The dashboard view assessment was blocked',
  ).toBeUndefined()
  expect(
