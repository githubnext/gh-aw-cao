...
import { effectiveDashboardSources, missingDashboardSources } from "./dashboard-view-sources.mjs";

const outputDirectory = resolve(process.env.DASHBOARD_VIEWS_OUTPUT_DIR || "test-results/dashboard-views");
  ...
  try {
    ...
    for (const pageDefinition of pages) {
      ...
      const result = {
        ...
        status: "incomplete",
        domNodes: null,
        declaredViews: (pageDefinition.views || pageDefinition.definition?.views || []).map((view, index) => view.id || `view-${index + 1}`),
        renderedViews: [],
        missingViews: [],
        missingData: [],
        crashed: false,
      ...
      try {
        ...
        await expect(dashboardRoot).not.toHaveAttribute("aria-busy", "true", {
          timeout: 120_000,
        });
        await expect(activePage).toBeVisible();
        await expect(activePage).not.toHaveAttribute("aria-busy", "true", {
          timeout: 120_000,
        });
        ...
        result.renderedViews = await views.evaluateAll((elements) => elements.map((element) => element.getAttribute("data-view-id")).filter(Boolean));
        result.missingViews = result.declaredViews.filter((viewId) => !result.renderedViews.includes(viewId));
        const sourceProblems = missingDashboardSources(pageDefinition, effectiveSources);
        result.missingData = [
          ...sourceProblems,
          ...(await activePage
            .locator('[aria-label^="Unable to load "]')
