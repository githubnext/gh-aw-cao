...
test("Copilot prompt saves a dashboard change, renders it, and correlates browser/server traces", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.locator(".dashboard-copilot-message-update")).toContainText("Checking the current dashboard");
    await expect(page.locator(".dashboard-copilot-message-refusal")).toContainText("Refused python3: shell command not allowed.");
    await expect(page.locator(".dashboard-copilot-message-reasoning")).toContainText("The title should make the requested change easy to see.");
    await expect(page.locator(".dashboard-copilot-message-response")).toContainText("Updated the active view title.");
    expect(await page.locator(".app-shell").evaluate((shell) => getComputedStyle(shell).gridTemplateColumns.split(" ")[0])).toBe("420px");
    expect(
      await page.locator(".dashboard-copilot-conversation").evaluate((conversation) => Number.parseFloat(getComputedStyle(conversation).maxHeight)),
    ).toBeGreaterThan(180);
    ...
    await expect
      ...
      .toBe(true);
    expect(JSON.parse(await readFile(dashboardPath, "utf8")).dashboard.pages[0].title).toBe("Updated by Copilot");

    const activePageId = await page.locator("[data-nav-page-id][aria-current=page]").getAttribute("data-nav-page-id");
    await page.evaluate(() => {
    ...
    await expect(page.locator("[data-nav-page-id][aria-current=page]")).toHaveAttribute("data-nav-page-id", activePageId);
    ...
    await expect
      .poll(async () => {
        ...
        return traces.find(({ event, traceId }) => event === "preview.render.failed" && traceId === "manual-render-failure");
      })
      .toEqual(
    ...
    expect(browserTraces.some((line) => line.includes(request.traceId) && line.includes("copilot.request.completed"))).toBe(true);
  } finally {
    await preview?.close();
