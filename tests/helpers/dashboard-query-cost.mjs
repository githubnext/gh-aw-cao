...
export function measureDashboardQueryCost(definition, sources, defect) {
  ...
  const peakHeapBytes = Math.max(executedHeapBytes, retainedHeapBytes);
  return {
...
export async function benchmarkDashboardQueryCost({ databasePath, document, limit }) {
  ...
  const indexedDB = await openDeployedDatabase(databasePath);
  const required = resolveDashboardQuerySources(definitions, candidates.map(({ name }) => name));
  const databaseStartedAt = performance.now();
  const databaseSources = await queryDatabaseSources(indexedDB, {}, required.filter((name) => !index.has(name)));
  ...
  return {
    ...
    database: {
      ...
      sources: Object.keys(databaseSources).length,
    },
...
export function dashboardQueryCostMarkdown(report) {
  ...
  return [
    ...
    "### Space cost",
    "",
    "| Rank | Query | Output rows | Result size | Bytes/row | Retained heap | Peak heap |",
    "| ---: | --- | ---: | ---: | ---: | ---: | ---: |",
    ...byMemory.map((measurement, index) => `| ${index + 1} | ${markdownCode(measurement.query)} | ${formatCount(measurement.rows)} | ${formatBytes(measurement["result-bytes"])} | ${formatBytes(measurement["bytes-per-row"])} | ${formatBytes(measurement["retained-heap-bytes"])} | ${formatBytes(measurement["peak-heap-bytes"])} |`),
