...
const packages = controlSettings(parsePolicy(readFileSync(join(root, ".github", "workflows", "cao.json"), "utf8")), "githubnext/gh-aw-cao").packages;
    ...
    [
      {
        ...
        issue: {
          ...event.issue,
          body: event.issue.body.replace("actions/runs/123)", "actions/runs/123/extra)"),
        },
      },
...
test("published issues retain review and run provenance", () => {
  ...
  assert.equal(publicationCommentMarker("acme/service", 84), "<!-- central-agentic-ops:published-to=acme/service#84 -->");
});
