...
test('ops publish recognizes every supported worker on the default branch', () => {
  ...
  for (const [workflowFile, packageName] of configuredWorkerPackages(
    packages,
  )) {
    const validated = validateWorkflowRun({
      inspection,
  ...
  assert.throws(
    () =>
      inspectPublishEvent({
        event: {
          ...event,
          issue: { ...event.issue, user: { login: 'octocat', type: 'User' } },
        },
        repository: 'acme/review',
        reviewers: 'octocat',
        controlRepositories: 'acme/control',
...
test('ops publish rejects malformed review events and provenance', () => {
  const inspect = (candidateEvent, reviewers = 'octocat') =>
    inspectPublishEvent({
  ...
  for (const [candidateEvent, message] of [
    ...
    [
      { ...event, repository: { full_name: 'acme/other' } },
    ...
    [
      { ...event, sender: { login: 'automation[bot]', type: 'Bot' } },
      /not an authorized ops publisher/,
    ],
    [
      { ...event, issue: { ...event.issue, body: 'No provenance' } },
      /missing generated workflow-run provenance/,
    ],
    [
      {
        ...event,
        issue: {
          ...event.issue,
          body: event.issue.body.replace('github.com', 'example.com'),
        },
      },
    ...
    [
      {
        ...
        issue: {
          ...event.issue,
          body: event.issue.body.replace(
            'actions/runs/123)',
            'actions/runs/123/extra)',
          ),
        },
      },
      /missing generated workflow-run provenance/,
  ...
  const generatedWith = inspect({
    ...
    issue: {
      ...event.issue,
      body: event.issue.body.replace('Generated from', 'Generated with'),
    },
  })
...
test('ops publish rejects non-review runs and destinations outside policy', () => {
  ...
  assert.throws(
    () =>
      validateWorkflowRun({
        run: {
          ...run,
          display_title: 'AW failure investigation · acme/service · live',
        },
        inspection,
        reviewRepository: 'acme/review',
        allowedOwners: 'acme',
  ...
  assert.throws(
    () =>
      validateWorkflowRun({
        run: {
          ...run,
          path: '.github/workflows/nested/aw-failures-investigator.lock.yml',
        },
        inspection,
        reviewRepository: 'acme/review',
        allowedOwners: 'acme',
  ...
  for (const [runOverride, message] of [
    ...
    [
      { repository: { full_name: 'acme/other', default_branch: 'main' } },
      /does not match review issue provenance/,
    ],
    [{ event: 'schedule' }, /successful dispatched worker run/],
    [
      { status: 'in_progress', conclusion: null },
      /successful dispatched worker run/,
    ],
    [{ conclusion: 'failure' }, /successful dispatched worker run/],
    [{ updated_at: '2026-08-27T10:00:30Z' }, /creation time must fall within/],
    [
      { created_at: 'invalid' },
      /workflow run creation time must be an ISO 8601 timestamp/,
    ],
  ]) {
    assert.throws(
      () =>
...
test('ops publish does not consult target-owned policy', () => {
  const source = readFileSync(
    join(root, 'ops-publish', 'ops-publish.mjs'),
    'utf8',
  )
  assert.doesNotMatch(
    source,
    /target-authority|contents\/\.github\/workflows\/cao\.json/,
  )
})
...
test('published issues retain review and run provenance', () => {
  ...
  assert.match(
    body,
    new RegExp(
      publicationMarker('acme/review', 42).replace(
        /[.*+?^${}()|[\]\\]/g,
        '\\$&',
      ),
    ),
...
test('Ops Publish remains an explicit least-privilege add-on', () => {
  ...
  const workflow = readFileSync(
    join(root, 'ops-publish', 'ops-publish.yml'),
    'utf8',
  )
  ...
  assert.match(
    workflow,
    /github\.event\.label\.name == 'ops:publish-to-target'/,
  )
  assert.match(workflow, /runs-on: ubuntu-latest\n\s+timeout-minutes: 10/)
  assert.match(
    workflow,
    /group: ops-publish-\$\{\{ github\.event\.issue\.number \}\}\n\s+cancel-in-progress: false/,
  )
  assert.match(workflow, /ref: \$\{\{ github\.workflow_sha \}\}/)
  assert.match(
    workflow,
    /control\.mjs control-settings \.github\/workflows\/cao\.json/,
  )
  ...
  assert.match(
    workflow,
    /APP_CLIENT_ID: \$\{\{ vars\.GH_AW_GITHUB_READ_APP_ID \}\}/,
  )
  assert.match(
    workflow,
    /APP_CLIENT_ID: \$\{\{ vars\.GH_AW_GITHUB_WRITE_APP_ID \}\}/,
  )
  assert.match(
    workflow,
    /APP_PRIVATE_KEY: \$\{\{ secrets\.GH_AW_GITHUB_READ_APP_PRIVATE_KEY \}\}/,
  )
  assert.match(
    workflow,
    /APP_PRIVATE_KEY: \$\{\{ secrets\.GH_AW_GITHUB_WRITE_APP_PRIVATE_KEY \}\}/,
  )
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /SOURCE_CONTENT_DIGEST: \$\{\{ steps\.inspect\.outputs\.source_content_digest \}\}/,
  )
  assert.match(
    workflow,
    /SOURCE_ISSUE_CREATED_AT: \$\{\{ steps\.inspect\.outputs\.source_created_at \}\}/,
  )
  assert.match(
    workflow,
    /CONTROL_TOKEN: \$\{\{ steps\.control-app-token\.outputs\.token \|\| secrets\.CENTRAL_AGENTIC_OPS_PUBLISH_CONTROL_TOKEN \|\| github\.token \}\}/,
  )
  assert.match(
    workflow,
    /TARGET_TOKEN: \$\{\{ steps\.target-app-token\.outputs\.token \|\| secrets\.CENTRAL_AGENTIC_OPS_PUBLISH_TARGET_TOKEN \}\}/,
  )
