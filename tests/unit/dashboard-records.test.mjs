...
test("dashboard records attribute issues from the gh-aw workflow XML marker", async () => {
  const issue = {
    ...
    body: ["A current report issue.", "", "<!-- gh-aw-workflow-id: maintenance-worker -->"].join("\n"),
...
test("dashboard records discover gh-aw workflows in allowed repositories", async () => {
  const lockSource = ['# gh-aw-metadata: {"compiler_version":"v0.88.7","strict":true}', '# gh-aw-manifest: {"version":1,"actions":[]}'].join("\n");
          ...
          workflows: [
            {
              ...
              state: "active",
              html_url: "https://github.com/acme/service/actions/workflows/remote-agent.lock.yml",
            },
            {
              name: "CI",
              path: ".github/workflows/ci.yml",
              state: "active",
              html_url: "https://github.com/acme/service/actions/workflows/ci.yml",
            },
          ],
        }),
        { status: 200 },
