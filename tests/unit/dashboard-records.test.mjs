...
test('dashboard records retain durable-output target and run attribution', async () => {
  ...
  const fetchImpl = async (input) => {
    ...
    let value
    if (
      url.pathname === '/repos/acme/control' ||
      url.pathname === '/repos/acme/service'
    )
      value = publicRepositoryMetadata
    else if (url.pathname === '/repos/github/gh-aw/releases/latest')
      value = { tag_name: 'v0.89.0' }
      ...
      value = { artifacts: [] }
    else if (url.pathname.endsWith('/actions/workflows'))
      value = { workflows: [] }
...
test('dashboard records retain report model and agent metadata when available', async () => {
  ...
  const fetchImpl = async (input) => {
    ...
    if (
      ...
      value = { workflows: [] }
    else if (url.pathname.endsWith('/actions/runs/43'))
      value = {
        name: 'Maintenance / Worker',
        path: '.github/workflows/maintenance-worker.lock.yml',
...
test('dashboard records discover gh-aw workflows in allowed repositories', async () => {
  ...
  const fetchImpl = async (input) => {
    ...
    if (url.pathname === '/repos/acme/control') {
      return new Response(JSON.stringify(publicRepositoryMetadata), {
        status: 200,
      })
    }
    ...
    if (url.pathname === '/repos/github/gh-aw/releases/latest') {
      return new Response(JSON.stringify({ tag_name: 'v0.89.0' }), {
        status: 200,
      })
    }
          ...
          workflows: [
            {
              ...
              state: 'active',
              html_url:
                'https://github.com/acme/service/actions/workflows/remote-agent.lock.yml',
            },
            {
              name: 'CI',
    ...
    if (url.hostname === 'raw.githubusercontent.com') {
      ...
      assert.equal(
        url.pathname,
        `/acme/service/${commitSha}/.github/workflows/remote-agent.lock.yml`,
      )
    ...
    if (
      ...
      url.pathname.endsWith('/issues/comments')
    ) {
  ...
  assert.deepEqual(
    output.remoteWorkflows.map((workflow) => ({
  ...
  assert.equal(
    rateLimitedRefresh.remoteWorkflows[0].lockSha,
    retriedRefresh.remoteWorkflows[0].lockSha,
  )
})

test('dashboard records refuse non-public remote workflow metadata on public Pages', async () => {
  const collectForVisibility = (visibility) =>
    collectDashboardRecords({
      repository: 'acme/control',
      token: 'test-token',
  ...
  await assert.rejects(
    () => collectForVisibility('private'),
    /Refusing to publish non-public repository data/,
  )
  await assert.rejects(
    () => collectForVisibility('internal'),
    /Refusing to publish non-public repository data/,
  )
})
...
test('dashboard records skip remote data when repository visibility is unavailable', async () => {
  ...
  const output = await collectDashboardRecords({
    ...
    fetchImpl: async (input) => {
      ...
      if (url.pathname.startsWith('/repos/acme/private/'))
...
test('dashboard records stop on a GitHub rate limit and return a renderable error', async () => {
  ...
  try {
    ...
    assert.ok(
      logs.some((line) =>
        line.startsWith('::warning::GitHub API rate limit exceeded'),
      ),
    )
...
test('dashboard records retain only confirmed-public snapshots when a refresh is rate limited', async () => {
  const refresh = (previousSnapshot) =>
    collectDashboardRecords({
      repository: 'acme/control',
      token: 'test-token',
      controlSettings: { allowed_repositories: ['acme/service'] },
