...
test('grades a dashboard view with interpretable normalized metrics', () => {
  const result = gradeDashboardView(
    ...
    },
    {
      ...
      screenshots: [
        { width: 1280, height: 720, clipped: false, overlap: false },
      ],
    },
  )
...
test('returns only focused low-effort findings for observable problems', () => {
  ...
  assert.doesNotMatch(
    ...
    /redesign|new page|replace the view/i,
  )
})
...
test('grades a document and the CLI writes the same report shape', async () => {
  ...
  try {
    await Promise.all([
      ...
      writeFile(
        sourcesPath,
        JSON.stringify({ summary: { rows: [{ value: 2 }] } }),
      ),
    ])
    await executeFile(
      process.execPath,
    ...
    assert.deepEqual(Object.keys(gradeDashboardDocument(document).summary), [
      'views',
      'average',
      'findings',
...
test('rejects malformed inputs', () => {
  ...
  assert.throws(
    () => gradeDashboardDocument({}),
    /dashboard\.pages must be an array/,
  )
})
