...
test('normalizes a gh-aw report into append-only CAO observation records', () => {
  ...
  assert.equal(
    operationalValueRecordIdentity(records[0]),
    'github/gh-aw:daily-file-diet:42:2:digest',
  )
  assert.equal(records[0].observation.subject.createdAt, '2026-08-30T10:00:00Z')
  assert.deepEqual(records[0].observation.provenance, [
    { repository: 'github/gh-aw', sha: 'abc123' },
  ])
  assert.deepEqual(records[0].diagnostics, { 'repository-health': 0.65 })
  assert.equal(records[0].observationSource, 'report')
  assert.deepEqual(
    definitionFromOperationalValueReport(report).diagnosticMetrics,
    [
      {
        id: 'repository-health',
        name: 'Repository health',
...
test('keeps retries and evaluator generations while preferring report observations', () => {
  ...
  const previousEvaluator = {
    ...
    evaluatorDigest: 'old-digest',
  }
  const records = mergeOperationalValueRecords(
    artifact,
    normalized,
    retry,
    previousEvaluator,
  ...
  assert.equal(
    records.find((record) => record.observationId === normalized.observationId)
      ?.value,
    0.8,
  )
  assert.ok(
    records.some((record) => record.observationId.endsWith(':3:digest')),
  )
  assert.ok(
    records.some((record) => record.observationId.endsWith(':2:old-digest')),
  )
})
...
test('run identities cover cached results even without nested observation payloads', () => {
  ...
  assert.equal(
    operationalValueRecordIdentity(cached),
    'github/gh-aw:daily-file-diet:42:2:unknown-evaluator',
  )
  assert.equal(
    operationalValueRunIdentity(cached),
    operationalValueRunIdentity(selected),
  )
})
