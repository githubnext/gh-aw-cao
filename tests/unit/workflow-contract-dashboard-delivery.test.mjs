...
test("Dashboard campaign builds artifacts and deploys Pages in one workflow", () => {
  ...
  const dashboardNotifyFailureJob = dashboardWorkflow.match(/\n  notify-failure:\n([\s\S]*)/)?.[1];
  ...
  const repositoryMemory = readFileSync(join(root, "activity", "repository-memory.mjs"), "utf8");
  ...
  assert.match(activityNotifyFailureJob, /needs: \[index, cache\]/);
  ...
  assert.ok(dashboardNotifyFailureJob);
  ...
  assert.match(dashboardNotifyFailureJob, /GITHUB_WORKFLOW_SHA[\s\S]*?githubnext\/gh-aw-cao\/blob\/main\/skills\/debug-cao\/SKILL\.md/);
...
test("Activity campaign owns the shared collected-data cache contract", () => {
  ...
  assert.equal((workflow.match(/issues: read/g) || []).length, 5);
  ...
  assert.match(workflow, /Download agentic workflow logs[\s\S]*?REPORT_DEFER_ISSUE_STATUS: "1"[\s\S]*?Refresh GitHub App token for issue status[\s\S]*?actions\/create-github-app-token@[0-9a-f]{40}[\s\S]*?Enrich issue statuses[\s\S]*?cao\.mjs issue-status[\s\S]*?Generate phased activity shards/);
