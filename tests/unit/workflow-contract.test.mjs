...
test("Dashboard package builds artifacts and deploys Pages in one workflow", () => {
  ...
  assert.match(dashboardWorkflow, /Restore collected activity data[\s\S]*?actions\/cache\/restore@[0-9a-f]{40}[\s\S]*?restore-keys: \|[\s\S]*?cao-activity-v4-[\s\S]*?fail-on-cache-miss: true/);
  assert.match(dashboardWorkflow, /Restore collected activity data[\s\S]*?path: \|[\s\S]*?gh-aw-logs\.jsonl[\s\S]*?gh-aw-logs\.sqlite[\s\S]*?payload-hashes\.txt[\s\S]*?control-settings\.json[\s\S]*?inventory-sources\.json/);
  ...
  assert.doesNotMatch(activityWorkflow, /(?:key|restore-keys): cao-activity-(?!v4-)/);
  ...
  assert.match(dashboardWorkflow, /const collectedFiles = \[[\s\S]*?'inventory-sources\.json'[\s\S]*?'gh-aw-logs\.jsonl'[\s\S]*?'gh-aw-logs\.sqlite'[\s\S]*?'payload-hashes\.txt'[\s\S]*?for \(const fileName of collectedFiles\)[\s\S]*?fs\.copyFileSync/);
  ...
  assert.match(activityWorkflow, /Hash activity payloads[\s\S]*?sha256sum gh-aw-logs\.jsonl gh-aw-logs\.sqlite > payload-hashes\.txt/);
