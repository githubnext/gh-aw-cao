...
test("CAO Evolution is review-first, role-scoped, and deduplicated", () => {
  ...
  const workers = [
    ["compiler-security", "aw-maintenance-compiler-security"],
...
test("live workers use central policy as the activation authority", () => {
  ...
  for (const [name, bundle] of [
    ...
    ["aw-failures-investigator.md", "cao-evolution"],
    ...
    ["aw-maintenance-compiler-security.md", "cao-evolution"],
...
test("every worker uses the standard dispatch envelope and safe mode vocabulary", () => {
  const workerNames = [
    ...
    ["aw-failures-investigator.md", "cao-evolution", "failures-investigator"],
    ["aw-maintenance-compiler-security.md", "cao-evolution", "compiler-security"],
...
test("clean-room compilation emits the expected GitHub Actions settings", { timeout: 120_000 }, () => {
  ...
  try {
    ...
    const workerGates = new Map([
      ...
      ["aw-failures-investigator.lock.yml", ["cao-evolution", "failures-investigator"]],
      ["aw-maintenance-compiler-security.lock.yml", ["cao-evolution", "compiler-security"]],
...
test("Dashboard package builds artifacts and deploys Pages in one workflow", () => {
  ...
  assert.match(dashboardWorkflow, /Validate restored activity data[\s\S]*?ACTIVITY_DATABASE: \$\{\{ runner\.temp \}\}\/cao-activity\/gh-aw-logs\.sqlite[\s\S]*?REPORT_GH_AW_LOGS: \$\{\{ runner\.temp \}\}\/cao-activity\/gh-aw-logs\.jsonl[\s\S]*?REPORT_CONTROL_SETTINGS: \$\{\{ runner\.temp \}\}\/cao-activity\/control-settings\.json[\s\S]*?REPORT_INVENTORY_SOURCES: \$\{\{ runner\.temp \}\}\/cao-activity\/inventory-sources\.json[\s\S]*?const activityFiles = \[[\s\S]*?process\.env\.REPORT_GH_AW_LOGS[\s\S]*?process\.env\.ACTIVITY_DATABASE[\s\S]*?process\.env\.REPORT_CONTROL_SETTINGS[\s\S]*?process\.env\.REPORT_INVENTORY_SOURCES/);
...
test("Dashboard inventory links multiline orchestrator worker lists", () => {
  ...
  try {
    ...
    assert.deepEqual(inventory.bundles.map((bundle) => ({
      ...
      { id: "cao-evolution", workers: ["cao-evolution-integrity", "cao-evolution-reliability", "cao-evolution-efficiency", "aw-failures-investigator", "aw-maintenance-compiler-security"] },
