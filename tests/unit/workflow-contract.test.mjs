...
test("operational workflows use the transitive CAO package bundle", () => {
  ...
  const operationWorkflows = readdirSync(workflowsDirectory).filter((name) => name.endsWith(".md") && workflow(name).includes("uses: shared/control.md"));
...
function generatedJobs(source) {
  ...
  return new Map(
    matches.map((match, index) => {
      ...
      const needs = inlineNeeds ? [inlineNeeds[1]] : [...(listNeeds?.[1].matchAll(/^      - ([A-Za-z0-9_-]+)$/gm) ?? [])].map((item) => item[1]);
...
function transitivelyNeeds(jobs, jobName, dependency, visited = new Set()) {
  ...
  return needs.includes(dependency) || needs.some((name) => transitivelyNeeds(jobs, name, dependency, visited));
}
...
function resolvePolicy({
  ...
  const requestedMode = eventName === "workflow_dispatch" ? manualMode || "review" : configuredMode || "review";
  ...
  const percentCap = totalRepositories === 0 ? 0 : Math.max(1, Math.ceil((totalRepositories * rolloutPercent) / 100));
  const dispatchCap = eligibleWorkers === 0 ? 0 : Math.floor(dispatchMax / eligibleWorkers);
  const creditCap = workerCreditsPerTarget === 0 ? maxRepos : Math.max(0, Math.floor((aggregateCreditLimit - orchestratorCredits) / workerCreditsPerTarget));
...
test("all scheduled configurations and manual selections route safely", () => {
  ...
  for (const scenario of cases) {
    ...
    const expectedMode = scenario.eventName === "workflow_dispatch" ? scenario.manualMode : scenario.configuredMode;
    ...
    assert.equal(policy.safeOutputRepo, scenario.packageEnabled && expectedMode === "review" ? expectedReviewRepo : "", scenario.id);
    assert.equal(policy.dispatchAllowed, scenario.packageEnabled, scenario.id);
    assert.equal(
      policy.effectiveMaxRepos,
      scenario.packageEnabled ? (scenario.maxRepos ? Math.min(scenario.maxRepos, percentageCap) : percentageCap) : 0,
      scenario.id,
    );
  }
});
  ...
  for (const scenario of userFacingScenarios) {
    const matrixCase = cases.find(
      ({ id, totalRepositories, ...inputs }) =>
        Object.entries(scenario.inputs).every(([name, value]) => inputs[name] === value) && inputs.packageEnabled === (scenario.inputs.packageEnabled ?? true),
    );
...
test("percentage rollout rejects invalid settings and handles an empty organization", () => {
  for (const maxRepos of [0, -1, 1.5, 1001, Number.NaN]) {
    assert.throws(() => resolvePolicy({ maxRepos, rolloutPercent: 100, totalRepositories: 10 }), RangeError);
  }

  for (const rolloutPercent of [0, 101, 10.5, Number.NaN]) {
    assert.throws(() => resolvePolicy({ rolloutPercent, totalRepositories: 10 }), RangeError);
  }
  ...
  for (const totalRepositories of inventorySizes) {
    for (const rolloutPercent of rolloutPercents) {
      for (const maxRepos of absoluteCaps) {
        ...
        const percentageCap = totalRepositories === 0 ? 0 : Math.max(1, Math.ceil((totalRepositories * rolloutPercent) / 100));
...
test("control workflows deny before activation through one shared admission contract", () => {
  ...
  assert.equal([...sharedControl.matchAll(/^\s+- name: Evaluate Central Agentic Ops admission$/gm)].length, 1);
  ...
  assert.match(
    stepBlock(sharedControl, '"CAO precompute blocked: GitHub API limited until ${{ steps.cao_precompute.outputs.github_api_reset_at }}"'),
  ...
  assert.match(
    stepBlock(sharedControl, '"CAO precompute blocked: GitHub API capacity unavailable"'),
  ...
  for (const [name, source] of controlled) {
    assert.equal([...source.matchAll(/^\s+- name: Evaluate Central Agentic Ops admission$/gm)].length, 0, name);
    assert.match(source, /on:[\s\S]*?permissions:\n\s+(?:actions: read\n\s+contents: read|contents: read\n\s+actions: read)/, name);
    assert.match(
      source,
    ...
    assert.match(
      stepBlock(preActivation, '"CAO precompute blocked: GitHub API capacity unavailable"'),
      /::warning title=CAO precompute could not verify GitHub API capacity/,
    );
...
test("worker workflows allow service-account dispatches", () => {
  ...
  assert.ok(workers.length > 0, "expected at least one worker workflow");
  for (const [name, source] of workers) {
    assert.match(source, /^on:\n  bots: \["github-actions\[bot\]", "cao-githubnext-gh-aw-cao-write\[bot\]"\]/m, name);
...
test("issue-creating workers use package and worker title prefixes and labels", () => {
  for (const name of readdirSync(workflowsDirectory).filter((entry) => entry.endsWith(".md"))) {
    ...
    assert.equal(config["safe-outputs"]["create-issue"]["title-prefix"], `[${controlImport.with.package}:${controlImport.with.worker}] `, name);
    assert.deepEqual(
...
test("CAO runtime is control-repository-owned outside package resources", () => {
  ...
  const policy = JSON.parse(
    execFileSync(process.execPath, [join(root, ".github", "cao", "src", "control.mjs"), "resolve-policy", join(root, ".github", "workflows", "cao.json")], {
      encoding: "utf8",
      env: {
        ...process.env,
...
test("compiled workflow expressions do not contain HTML-escaped operators", () => {
  ...
  for (const lockName of lockNames) {
    ...
    for (const expression of expressions) {
      assert.doesNotMatch(expression, /\\+u(?:0026|003c|003e)/i, `${lockName} contains an HTML-escaped operator in ${expression}`);
    }
  }
});
...
test("operational-value graders expose deterministic run-scoped contracts", () => {
  ...
  for (const name of ["aw-failures-investigator-operational-value.sh", "aw-maintenance-compiler-security-operational-value.sh"]) {
    assert.equal(script(name, join(root, "aw-doctor", ".github", "graders")), script(name, gradersDirectory), `focused AW Doctor package must mirror ${name}`);
  }
  for (const name of [
    "optimization-agents-md-curator-operational-value.sh",
    "optimization-ai-credit-auditor-operational-value.sh",
  ...
  const dependabotDefinition = JSON.parse(
    execFileSync(join(gradersDirectory, "dependabot-release-train-updater-operational-value.sh"), ["--definition"], {
      encoding: "utf8",
    }),
  );
...
test("operation workflows optionally load per-operation markdown steering", () => {
  ...
  for (const [name, operation] of [
    ...
    assert.match(workflow(name), new RegExp(`^\\{\\{#runtime-import\\? \\.github/cao/${operation}\\.md\\}\\}$`, "m"));
  }
});
...
test("blank manual runs preserve an empty target for allowlisted discovery", () => {
  ...
  assert.match(control, /CAO_TARGET_REPOSITORY: \$\{\{ inputs\.target_repo \|\| '' \}\}/);
...
test("every worker uses the standard dispatch envelope and safe mode vocabulary", () => {
  ...
  for (const [name, packageName, workerName] of workerNames) {
    ...
    assert.match(source, /environment: central-agentic-ops/);
    for (const input of ["target_repo", "safe_output_repo", "safe_output_mode", "correlation_id", "central_repo", "control_plane_run_url"]) {
...
test("multi-device docs tester runs daily and covers browser and appearance compatibility", () => {
  ...
  assert.equal((source.match(/PLAYWRIGHT_BROWSERS_PATH: \$\{\{ runner\.temp \}\}\/gh-aw\/playwright-browsers/g) || []).length, 2);
...
test("AW Doctor compiler security worker runs the full validation suite", () => {
  ...
  for (const viewId of ["aw-doctor-attainment", "aw-doctor-value-trend"]) {
    ...
    assert.deepEqual(view.data.filters.workflow, [".github/workflows/aw-failures-investigator.md", ".github/workflows/aw-maintenance-compiler-security.md"]);
  }
});
...
test("slower package orchestrators run hourly", () => {
  for (const name of ["dependabot.md", "eu-cra-compliance.md", "optimization.md", "software-development-practices.md", "uk-ai-advisory.md"]) {
...
test("dashboard authoring corpus workflow generates only validated training examples", () => {
  ...
  const dashboardIrSkill = readFileSync(join(root, ".github", "skills", "generate-dashboard-ir", "SKILL.md"), "utf8");
  const dashboardAuthoringSkill = readFileSync(join(root, ".github", "skills", "dashboard-authoring", "SKILL.md"), "utf8");
...
test("dashboard CI runs the package quality gates", () => {
  ...
  assert.deepEqual([...jobs.keys()], ["lint-unit", "playwright-integration", "lighthouse-performance", "lighthouse-comment"]);
...
test("clean-room compilation emits the expected GitHub Actions settings", { timeout: 120_000 }, () => {
  ...
  try {
    ...
    execFileSync("gh", ["aw", "compile", "--no-check-update", "--schedule-seed", "githubnext/gh-aw-cao"], {
      cwd: temporaryRoot,
      stdio: "pipe",
    });
    ...
    for (const name of packageLockNames) {
      ...
      assert.doesNotMatch(generated, /github\.event\.inputs\.(?:max_repos|rollout_percent|correlation_id|central_repo|control_plane_run_url)/);
...
test("Dashboard package supports embedded and explicit standalone deployment", () => {
  ...
  const normalizeInclude = (entry, sourcePrefix = "") =>
    typeof entry === "string" ? { source: entry, destination: entry, kind: "action-workflow" } : { ...entry, source: `${sourcePrefix}${entry.source}` };
  ...
  assert.match(activityLogs, /"--artifacts",\s*"usage,detection,evals,experiment,firewall,github-api,graders,mcp,agent"/);
  assert.match(aicUsage, /const FIREWALL_HORIZON_DAYS = 30/);
  assert.match(activityLogs, /"--start-date",\s*`-\$\{windowDays\}d`,\s*"--cache-before",\s*`-\$\{windowDays\}d`/);
  assert.match(activityLogs, /"--count",\s*String\(runLimit\),\s*"--timeout",\s*"15"/);
  assert.match(activityLogs, /"--max-github-api-rate-limit",\s*"-2000",\s*"--max-storage",\s*"1200"/);
  assert.match(activityLogs, /"--prune-older-runs"/);
  assert.equal((activityLogs.match(/"aw",\s*"logs"/g) || []).length, 1);
...
test("Activity package owns the shared collected-data cache contract", () => {
  ...
  assert.deepEqual(activityManifest.includes, [".github/workflows/activity.yml", ".github/workflows/cao-maintenance.yml"]);
...
test("Documentation site uses stock Starlight without external themes", () => {
  ...
  assert.deepEqual(
    Object.keys(dependencies).filter((name) => name.startsWith("starlight-theme-")),
    [],
  );
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [
        {
          id: "aw-doctor",
          workers: ["aw-maintenance-upgrade", "aw-failures-investigator", "aw-maintenance-compiler-security"],
        },
        ...
        {
          id: "optimization",
          workers: ["optimization-ai-credit-auditor", "optimization-ai-credit-optimizer", "optimization-agents-md-curator", "optimization-skills-curator"],
        },
        {
          id: "self-care",
          workers: [
        ...
        },
        {
          ...
          workers: ["software-development-practices-github-well-architected", "software-development-practices-nist-ssdf"],
        },
        {
          id: "uk-ai-advisory",
