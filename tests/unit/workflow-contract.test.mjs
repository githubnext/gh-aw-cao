...
test("packages and repository workflows pin the supported gh-aw version", () => {
  ...
  for (const name of ["activity.yml", "copilot-setup-steps.yml", "release.yml", "workflow-contracts.yml"]) {
    ...
    assert.match(source, /github\/gh-aw-actions\/setup-cli@[0-9a-f]{40} # v0\.88\.6/);
    assert.match(source, /version: v0\.88\.6/);
  }
});
...
test("Dashboard package supports embedded and explicit standalone deployment", () => {
  ...
  assert.match(activityWorkflow, /Install gh-aw CLI[\s\S]*?version: v0\.88\.6/);
...
test("Activity package owns the shared collected-data cache contract", () => {
  ...
  assert.deepEqual(activityManifest.resources, [
    ...
    { source: "run-activity.mjs", destination: ".github/aw/activity/run-activity.mjs" },
...
test("mobile dashboard integration downloads deployed dashboard data", () => {
  ...
  assert.match(workflow, /Test mobile dashboard with restricted memory[\s\S]*?MOBILE_MEMORY_MB: 512/);
  assert.match(workflow, /Test mobile dashboard with throttled network[\s\S]*?MOBILE_NETWORK_DOWNLOAD_KBPS: 5000/);
  assert.match(workflow, /Test mobile dashboard with restricted memory and network[\s\S]*?MOBILE_MEMORY_MB: 512[\s\S]*?MOBILE_NETWORK_DOWNLOAD_KBPS: 5000[\s\S]*?MOBILE_NETWORK_LATENCY_MS: 150/);
  ...
  assert.match(packageDocument.scripts["dashboard:local:mobile"], /MOBILE_MEMORY_MB=512/);
