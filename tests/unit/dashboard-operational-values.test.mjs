...
test('operational-value collection processes the shared gh-aw logs JSON', async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), 'dashboard-operational-values-'),
  )
      ...
      workflows: [
        {
          ...
          runHealth: {
            runIds: [42],
            runRecords: [
              { runId: 42, runAttempt: 2, createdAt: '2026-09-05T10:00:00Z' },
            ],
          },
        },
      ],
  ...
  try {
    ...
    assert.equal(
      output.definitions[0].operationalValue,
      'Ship a verified outcome.',
    )
    assert.deepEqual(output.definitions[0].diagnosticMetrics, [
      { id: 'quality' },
    ])
    ...
    workflows: [
      {
        ...
        runHealth: {
          runIds: [42],
          runRecords: [
            { runId: 42, runAttempt: 1, createdAt: '2026-09-05T10:00:00Z' },
          ],
        },
      },
    ],
