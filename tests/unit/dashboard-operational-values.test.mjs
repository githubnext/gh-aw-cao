...
test("operational-value collection processes the shared gh-aw logs JSON", async () => {
  ...
  try {
    await execFileAsync(process.execPath, [path.resolve("dashboard/report/operational-values.mjs")], {
      cwd: path.resolve("."),
      env: {
        ...process.env,
...
test("operational-value collection defaults its cache to the output path when REPORT_VALUE_CACHE is unset", async () => {
  ...
  const run = () =>
    execFileAsync(process.execPath, [path.resolve("dashboard/report/operational-values.mjs")], {
      cwd: path.resolve("."),
      env: {
        ...process.env,
        REPORT_DEPLOYED_WORKFLOWS: inventoryPath,
