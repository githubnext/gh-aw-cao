...
} from "../../.github/aw/activity/github-telemetry.mjs";
...
test("GitHub telemetry caps captured frames to keep ledger growth bounded", async () => {
  ...
  const frames = Array.from({ length: GITHUB_TELEMETRY_STACK_TRACE_LIMIT + 5 }, (_, index) => `at frame ${index} (.github/aw/activity/github-telemetry.mjs:${index}:1)`);
  try {
    const entry = await recordGithubTelemetry({
      phase: "before",
      operation: "refresh-activity",
