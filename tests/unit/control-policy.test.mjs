...
import {
  ...
  effectivePolicy,
  parsePolicy,
} from '../../.github/cao/src/policy.mjs'

const root = resolve(dirname(fileURLToPath(import.meta.url)), '../..')
const control = join(root, '.github', 'cao', 'src', 'control.mjs')
const schema = JSON.parse(
  readFileSync(join(root, '.github', 'cao', 'cao.schema.json'), 'utf8'),
)
...
test('control policy schema accepts config-defined package and worker catalogs', () => {
  ...
  assert.equal(
    schema.$defs.controlPackages.additionalProperties.$ref,
    '#/$defs/packagePolicy',
  )
  assert.equal(
    schema.$defs.targetPackages.additionalProperties.$ref,
    '#/$defs/targetPackage',
  )
...
test('checked-in control policy selects seven repositories with live Dependabot and local SelfCare authority', () => {
  ...
  assert.deepEqual(
    policy['control-plane'].scope['allowed-repositories'],
    repositories,
  )
  ...
  assert.equal(
    policy['control-plane'].packages['self-care'].workers[
  ...
  assert.equal(
    policy['control-plane'].packages['self-care'].workers['experimental-views']
      .workflow,
    'self-care-experimental-views',
  )
  assert.equal(
    policy['control-plane'].packages['self-care'].workers['pages-health']
      .workflow,
...
test('control policy exposes scope and publishing defaults to deterministic add-ons', () => {
  assert.deepEqual(
    controlSettings(parsePolicy(minimalPolicy), 'acme/control'),
    {
      allowed_owners: ['acme'],
...
test('control policy validates and exposes web presentation settings', () => {
  ...
  assert.equal(validate(JSON.stringify(policy)).status, 0)
  assert.deepEqual(
    controlSettings(parsePolicy(JSON.stringify(policy)), 'acme/control').web,
    {
      favicon: 'https://example.com/operations.svg',
    },
  )
...
test('control policy validates and exposes a package octicon', () => {
  ...
  assert.equal(
    validate(policyWithIcon).status,
    0,
    validate(policyWithIcon).stderr,
  )
  const invalidResult = validate(policyWithInvalidIcon)
  assert.notEqual(invalidResult.status, 0)
  assert.match(
    invalidResult.stderr,
    /control-plane\.packages\.dependabot\.icon must be one of/,
  )

  assert.equal(
    controlSettings(parsePolicy(policyWithIcon), 'acme/control').packages
      .dependabot.icon,
    'dependabot',
  )
})
...
test('control policy validates config-defined worker workflow identities', () => {
  ...
  delete missingWorkflow['control-plane'].packages.dependabot.workers[
    'release-train-updater'
  ].workflow
  ...
  assert.match(
    ...
    /workers\.release-train-updater\.workflow has an invalid value/,
  )
  ...
  assert.match(
    ...
    /workers must declare unique workflow identities/,
  )
})

test('control policy disables packages by absence and requires declared workers', () => {
  const absentPackage = effective(minimalPolicy, {
    packageName: 'optimization',
  })
  ...
  const disabledWorker = effective(
  ...
  disabledPackageWithoutWorker['control-plane'].packages.dependabot = {
    enabled: false,
  }
  const disabledPackageWorker = effective(
    JSON.stringify(disabledPackageWithoutWorker),
    {
      role: 'worker',
      worker: 'release-train-updater',
    },
  ...
  assert.match(
    undeclaredWorker.stderr,
    /unknown worker: dependabot\/release-train-updater/,
  )
  assert.equal(
    disabledWorkerOrchestrator.output.worker_policies[
      'dependabot-release-train-updater'
    ].enabled,
    false,
  )
...
test('control policy intersects package mode, dispatch request, and worker ceiling', () => {
  ...
  const reviewCeilingPolicy = minimalPolicy.replace(
    '"max-mode":"live"',
    '"max-mode":"review"',
  )
...
test('workers inherit the resolved mode when max-mode is omitted', () => {
  const policy = JSON.stringify({
    ...
    'control-plane': {
      scope: {
        'allowed-repositories': ['acme/payments-api', 'acme/storefront'],
      },
      packages: {
        dependabot: {
          mode: 'review',
...
test('control policy resolves exact package target modes', () => {
  ...
  const defaultTarget = effective(policy, {
    targetRepository: 'acme/storefront',
  })
  const liveTarget = effective(policy, {
    targetRepository: 'ACME/payments-api',
  })
  const liveWorker = effective(policy, {
    role: 'worker',
  ...
  const reviewCeilingWorker = effective(
    policy.replace('"max-mode":"live"', '"max-mode":"review"'),
    {
      role: 'worker',
      worker: 'release-train-updater',
      targetRepository: 'acme/payments-api',
...
test('control policy requires package targets to stay inside explicit scope', () => {
  ...
  assert.match(
    result.stderr,
    /package target acme\/payments-api is outside control-plane\.scope\.allowed-repositories/,
  )
})
...
for (const [name, targets, error] of [
  [
    'an array',
    [],
    /control-plane\.packages\.dependabot\.targets must be a mapping/,
  ],
  [
    'a missing mode',
    { 'acme/payments-api': {} },
    /control-plane\.packages\.dependabot\.targets\.acme\/payments-api\.mode is required/,
  ],
  [
    'an unsupported field',
    { 'acme/payments-api': { mode: 'live', percentage: 10 } },
    /unknown key control-plane\.packages\.dependabot\.targets\.acme\/payments-api\.percentage/,
  ],
  [
    'an invalid mode',
    { 'acme/payments-api': { mode: 'preview' } },
    /control-plane\.packages\.dependabot\.targets\.acme\/payments-api\.mode must be review or live/,
  ],
  [
    ...
    /control-plane\.packages\.dependabot\.targets must contain unique repository names/,
  ],
]) {
...
test('control policy requires package targets to stay inside allowed owners', () => {
  ...
  assert.match(
    result.stderr,
    /package target outside\/payments-api is outside control-plane\.scope\.allowed-owners/,
  )
})
...
test('control policy accepts target-only authority in the version 1 shape', () => {
  const result = validate(
    JSON.stringify({
      version: 1,
      'target-authority': {
        packages: { optimization: { authority: 'acme/control' } },
      },
    }),
  )
...
for (const [name, policy, error] of [
  ...
  [
    'unknown schema URI',
    '{"$schema":"https://example.com/policy.schema.json","version":1,"control-plane":{}}',
    /\$schema must be https:\/\/raw\.githubusercontent\.com/,
  ],
  [
    'unknown nested keys',
    '{"version":1,"control-plane":{"packages":{"dependabot":{"surprise":true}}}}',
    /unknown key control-plane.packages.dependabot.surprise/,
  ],
  [
    'duplicate keys',
  ...
  [
    ...
    /must not contain a GitHub Actions expression/,
  ],
]) {
