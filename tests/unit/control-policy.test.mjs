...
function effective(policy, { packageName = "dependabot", role = "orchestrator", worker = "", requestedMode = "", targetRepository = "" } = {}) {
  try {
    return {
      status: 0,
      stderr: "",
...
test("checked-in control policy selects seven repositories with live Dependabot and local SelfCare authority", () => {
  ...
  assert.equal(policy["control-plane"].packages["self-care"].workers["open-source-failures"].workflow, "self-care-open-source-failures");
  assert.equal(policy["control-plane"].packages["self-care"].workers["dashboard-performance"].workflow, "self-care-dashboard-performance");
  assert.equal(policy["control-plane"].packages["self-care"].workers["experimental-views"].workflow, "self-care-experimental-views");
  assert.equal(policy["control-plane"].packages["self-care"].workers["pages-health"].workflow, "self-care-pages-health");
  assert.equal(policy["control-plane"].packages["self-care"].workers.glossary.workflow, "self-care-glossary");
...
test("control policy validates and exposes a package octicon", () => {
  ...
  assert.equal(controlSettings(parsePolicy(policyWithIcon), "acme/control").packages.dependabot.icon, "dependabot");
});
...
test("control policy disables packages by absence and requires declared workers", () => {
  ...
  const disabledWorkerOrchestrator = effective(minimalPolicy.replace('"max-mode":"live"', '"enabled":false'));
  ...
  assert.equal(disabledWorkerOrchestrator.output.worker_policies["dependabot-release-train-updater"].enabled, false);
