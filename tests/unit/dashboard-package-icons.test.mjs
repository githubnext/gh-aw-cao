...
test("configured package icons are used by package dashboard menu entries", async () => {
  ...
  for (const [packageName, packagePolicy] of Object.entries(packages)) {
    ...
    const packageDashboard = JSON.parse(await readFile(new URL(`${packageName}/dashboard.json`, root), "utf8"));
    assert.equal(packageDashboard.dashboard.pages[0]?.icon, packagePolicy.icon, `${packageName} dashboard menu must use its configured Octicon`);
  }
});
