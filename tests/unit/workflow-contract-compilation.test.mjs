...
test("clean-room compilation emits the expected GitHub Actions settings", { timeout: 120_000 }, () => {
  ...
  try {
    ...
    for (const packageDirectory of ["activity", "cao-evolution", "dashboard", "dependabot"]) {
      cpSync(join(root, packageDirectory), join(temporaryRoot, packageDirectory), { recursive: true });
    }
    for (const manifest of ["aw.yml", "activity/aw.yml", "cao-evolution/aw.yml", "dashboard/aw.yml", "dependabot/aw.yml"]) {
    ...
    const packageLockNames = [
      ...
      "cao-evolution-agents-md-curator.lock.yml",
      "cao-evolution-skills-curator.lock.yml",
      ...
      "cao-evolution-ai-credit-auditor.lock.yml",
      "cao-evolution-ai-credit-optimizer.lock.yml",
    ...
    const workerGates = new Map([
      ...
      ["cao-evolution-agents-md-curator.lock.yml", ["optimization", "agents-md-curator"]],
      ["cao-evolution-skills-curator.lock.yml", ["optimization", "skills-curator"]],
      ...
      ["cao-evolution-ai-credit-auditor.lock.yml", ["optimization", "ai-credit-auditor"]],
      ["cao-evolution-ai-credit-optimizer.lock.yml", ["optimization", "ai-credit-optimizer"]],
