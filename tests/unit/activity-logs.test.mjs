...
`);
  ...
  try {
    ...
    const collection = await execFileAsync("bash", [path.resolve(".github/aw/activity/collect-logs.sh")], { env });
    const { stdout } = await execFileAsync(process.execPath, [path.resolve(".github/aw/activity/logs.mjs")], { env });
...
test("activity logs preserves cached runs and records collection failure", async () => {
  ...
  try {
    await execFileAsync(process.execPath, [path.resolve(".github/aw/activity/logs.mjs")], {
      env: {
        ...process.env,
        PATH: `${item.bin}:${process.env.PATH}`,
        GITHUB_REPOSITORY: "githubnext/gh-aw-cao",
