...
test('dashboard telemetry extracts bounded security aggregates without retaining denial details', async () => {
  ...
  try {
    await writeFile(
      path.join(runDirectory, 'run_summary.json'),
      JSON.stringify({
        cli_version: '0.88.0',
        agentic_assessments: [
          {
    ...
    await writeFile(
      path.join(runDirectory, 'audit.json'),
      JSON.stringify({
        agentic_assessments: [
          {
    ...
    await writeFile(
      path.join(runDirectory, 'agent', 'agent-stdio.log'),
      [
    ...
    await writeFile(
      path.join(runDirectory, 'detection', 'detection_result.json'),
      JSON.stringify({
    ...
    assert.equal(
      telemetry.firewall.firewallEvidenceSource,
      'run-summary-legacy',
    )
    assert.match(
      telemetry.firewall.firewallEvidenceError,
      /Legacy summary only/,
    )
    ...
    assert.doesNotMatch(
      ...
      /private\/path|sensitive diagnostic|sensitive MCP error|secret detail|unrelated output/,
    )
  } finally {
...
test('dashboard telemetry classifies authoritative, empty, malformed, and disabled firewall artifacts', async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(root, 'run-50', 'aw_info.json'),
      JSON.stringify({
        firewall: 'squid',
        awf_version: '0.28.12',
      }),
    ...
    await writeFile(
      path.join(authoritative, 'policy-manifest.json'),
      JSON.stringify({
        version: 1,
    ...
    await writeFile(
      path.join(authoritative, 'audit.jsonl'),
      [
        JSON.stringify({
          _schema: 'audit/v0.28.12',
          ts: 1788411660,
    ...
    assert.deepEqual(
      telemetry.firewall.observations.map(
    ...
    assert.equal(
      telemetry.firewall.firewallEvidenceReference,
      'sandbox/firewall/audit/audit.jsonl',
    )

    const noTraffic = path.join(root, 'run-51', 'sandbox', 'firewall', 'audit')
    await mkdir(noTraffic, { recursive: true })
    await writeFile(
      path.join(root, 'run-51', 'aw_info.json'),
      JSON.stringify({ firewall: 'squid' }),
    )
    await writeFile(path.join(noTraffic, 'audit.jsonl'), '')
    assert.equal(
      (await readRunSecurityTelemetry(root, 51)).firewall.firewallEvidenceState,
      'no-traffic',
    )
    ...
    await writeFile(
      path.join(root, 'run-52', 'aw_info.json'),
      JSON.stringify({ firewall: 'squid' }),
    )
    ...
    await writeFile(
      path.join(root, 'run-54', 'aw_info.json'),
      JSON.stringify({ firewall: 'squid' }),
    )
    await writeFile(
      path.join(partial, 'audit.jsonl'),
      [
        JSON.stringify({
          ts: 1788411660,
          host: 'api.github.com:443',
    ...
    assert.equal(
      partialTelemetry.firewall.firewallEvidenceCompleteness,
      'partial',
    )
    ...
    await writeFile(
      path.join(root, 'run-53', 'aw_info.json'),
      JSON.stringify({ firewall: 'disabled' }),
    )
