...
test("SelfCare dashboard debug logging worker preserves the logging privacy boundary", () => {
  ...
  assert.match(source, /remove the final `\.js`, lowercase it/);
  assert.match(source, /source-store\.js` uses `source-store`/);
  ...
  assert.match(source, /dashboard\/site\/src\/\*\*\/\*\.js/);
  assert.match(source, /dashboard\/site\/src\/debug\.mjs/);
