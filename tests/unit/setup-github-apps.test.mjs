...
test('dry run emits both exact manifests without requiring GitHub access', () => {
  const result = spawnSync(
    process.execPath,
    [script, '--repo', 'githubnext/gh-aw-cao', '--dry-run'],
    {
      encoding: 'utf8',
    },
  )
  ...
  assert.deepEqual(
    output.apps.map((app) => app.manifest.name),
    ['cao-githubnext-gh-aw-cao-read', 'cao-githubnext-gh-aw-cao-write'],
  )
  assert.deepEqual(
    output.apps.map((app) => app.manifest.redirect_url),
    ['http://127.0.0.1:0/callback', 'http://127.0.0.1:0/callback'],
  )
})
...
test('App setup validates callback codes and bounded generated names', () => {
  ...
  assert.equal(
    deriveAppName('githubnext/gh-aw-cao', 'read'),
    'cao-githubnext-gh-aw-cao-read',
  )
  assert.ok(
    deriveAppName('very-long-organization/example-control-repository', 'write')
      .length <= 34,
  )
  assert.throws(
    () => validateAppName('GitHub-control-read', '--read-app-name'),
    /must not begin with GitHub or Gist/,
  )
  assert.throws(
    () => validateAppName('gist-control-write', '--write-app-name'),
    /must not begin with GitHub or Gist/,
  )
})
...
test('App setup rejects all-repository installations', () => {
  assert.doesNotThrow(() =>
    validateInstallationScope(
      { id: '123', repositorySelection: 'selected' },
      'octo',
    ),
  )
  let error
  ...
  assert.match(
    error.message,
    /organizations\/octo\/settings\/installations\/123/,
  )
})
