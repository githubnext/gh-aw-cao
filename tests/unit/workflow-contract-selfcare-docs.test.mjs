...
test("docs maintainer uses daily merged-change and ADR evidence", () => {
  ...
  assert.match(source, /query inclusively from each cursor timestamp/);
  ...
  assert.match(source, /Clear it only when a merged `docs-maintainer` pull request cites every pending/);
