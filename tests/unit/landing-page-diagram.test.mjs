...
test("landing wizard prompt references the raw setup skill", () => {
  assert.match(wizard, /https:\/\/raw\.githubusercontent\.com\/githubnext\/gh-aw-cao\/main\/\.github\/skills\/setup-central-agentic-ops\/SKILL\.md/);
});

test("landing wizard client imports its prompt generation dependencies", () => {
  assert.equal(wizard.match(/import controlPolicy from "\.\.\/\.\.\/\.github\/workflows\/cao\.json";/g)?.length, 2);
  assert.equal(wizard.match(/import \{ buildWizardPolicy \} from "\.\.\/lib\/configured-operations\.mjs";/g)?.length, 2);
});
...
test("configured wizard operations require a package map", () => {
  assert.throws(() => selectConfiguredOperations({}, []), /must define control-plane\.packages as an object/);
});

test("configured wizard operations require a matching catalog manifest", () => {
  const policy = { "control-plane": { packages: { missing: {} } } };

  assert.throws(() => selectConfiguredOperations(policy, []), /Configured package missing must have a catalog manifest/);
});
