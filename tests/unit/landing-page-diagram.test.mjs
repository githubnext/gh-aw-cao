...
const controlPolicy = JSON.parse(
  readFileSync('.github/workflows/cao.json', 'utf8'),
)
...
test('landing diagram fallbacks use concrete light and dark palettes', () => {
  for (const layout of ['fallback', 'mobile']) {
    const light = readFileSync(
      `docs/assets/control-plane-dispatch-${layout}.svg`,
      'utf8',
    )
    const dark = readFileSync(
      `docs/assets/control-plane-dispatch-${layout}-dark-mode.svg`,
      'utf8',
    )
...
test('landing animations use SVG and CSS without a JavaScript player', () => {
  ...
  assert.match(
    hero,
    /@media \(prefers-reduced-motion: reduce\) \{\s+\.dispatch-motion \{\s+display: none;/,
  )

  for (const layout of ['', '-mobile']) {
    const motion = readFileSync(
      `docs/assets/control-plane-dispatch${layout}-motion.svg`,
      'utf8',
    )
...
test('landing wizard client imports its prompt generation dependencies', () => {
  assert.equal(
    wizard.match(
      /import controlPolicy from "\.\.\/\.\.\/\.github\/workflows\/cao\.json";/g,
    )?.length,
    2,
  )
  assert.equal(
    wizard.match(
      /import \{ buildWizardPolicy \} from "\.\.\/lib\/configured-operations\.mjs";/g,
    )?.length,
    2,
  )
})
...
test('landing wizard operations come from the checked-in control policy', () => {
  ...
  assert.match(
    catalog,
    /selectConfiguredOperations\(controlPolicy, catalogEntries\)/,
  )
...
test('configured wizard operations follow policy package order', () => {
  ...
  assert.deepEqual(selectConfiguredOperations(policy, [first, second]), [
    second,
    first,
  ])
})
...
test('configured wizard operations exclude the repository-local self-care package', () => {
  ...
  const policy = {
    'control-plane': { packages: { 'self-care': {}, first: {} } },
  }

  assert.deepEqual(selectConfiguredOperations(policy, [first, selfCare]), [
    first,
  ])
})
...
test('wizard policy keeps the checked-in package configuration', () => {
  ...
  const { icon, ...expectedPackage } =
    controlPolicy['control-plane'].packages.dependabot
