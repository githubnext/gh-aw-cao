...
test('activity index combines local inventory and gh aw logs without GitHub API access', async () => {
  ...
  await writeFile(
    path.join(workflowDirectory, 'sample.md'),
    `---
name: Sample
graders:
  operational-value:
...
`,
  ...
  await writeFile(
    ...
    '# gh-aw-metadata: {"compiler_version":"v0.88.4"}\n',
  )
  await writeFile(
    inventoryPath,
    JSON.stringify({
  ...
  try {
    await execFileAsync(
      process.execPath,
      [path.resolve('activity/index.mjs')],
      {
        cwd: path.resolve('.'),
        env: {
          ...process.env,
...
test('activity index reports fields missing from gh aw usage artifacts', async () => {
  ...
  await writeFile(
    logsPath,
    '{"runs":[{"database_id":42,"workflow_name":"Sample"}]}\n',
  )
  await writeFile(
    statePath,
    '{"available":true,"complete":true,"targetCount":1,"fallback":false}\n',
  )
  try {
    await execFileAsync(
      process.execPath,
    ...
    assert.deepEqual(result.runHealth.usageArtifact.sampleRunIds.conclusion, [
      '42',
    ])
  } finally {
    await rm(root, { recursive: true, force: true })
...
test('activity index accepts null usage fields and discovers block-list workers', async () => {
  ...
  await mkdir(workflowDirectory, { recursive: true })
  await writeFile(
    path.join(workflowDirectory, 'orchestrator.md'),
    `---
name: Orchestrator
safe-outputs:
  dispatch-workflow:
...
`,
  ...
  await writeFile(
    path.join(workflowDirectory, 'orchestrator.lock.yml'),
    'name: Orchestrator\n',
  )
  await writeFile(
    logsPath,
