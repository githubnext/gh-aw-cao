...
test('detection observations preserve verdict, warning, tooling, skipped, and unknown states', () => {
  ...
  assert.equal(
    byRun.get('8')['detection-signal'],
    'Prompt injection, Secret leak, Malicious patch',
  )
  assert.equal(
    byRun.get('2')['run-link'].href,
...
test('detection observations normalize conclusions and keep usable verdicts independent of job failures', () => {
  ...
  const rows = detectionObservationRows(
    {
      securityAvailable: true,
      securityRuns: [detectionRun(10, null), detectionRun(11, clear)],
    },
    [detectionJob(10, 'startup_failure'), detectionJob(11, 'failure')],
  )
  const byRun = new Map(rows.map((row) => [row.run, row]))
...
test('detection source metadata exposes incomplete evidence', () => {
  ...
  assert.equal(
    sources['detection-observations'].metadata.completeness,
    'partial',
  )
  assert.equal(
  ...
  assert.equal(
    sources['detection-observations'].metadata['coverage-end'],
...
test('dashboard source bridge derives work-oriented sources from run, admission, and outcome telemetry', () => {
  ...
  const workItems = new Map(
    sources['work-items'].rows.map((row) => [row['work-item-id'], row]),
  )
  ...
  const outcomes = new Map(
    sources.outcomes.rows.map((row) => [row['safe-output'], row]),
  )
  assert.equal(
    outcomes.get('worker-outcome')['safe-output-kind'],
    'create-issue',
  )
  assert.equal(
    outcomes.get('reviewer-outcome')['safe-output-kind'],
    'create-pull-request',
  )

  assert.equal(sources['attention-signals'].metadata.availability, 'available')
  assert.deepEqual(
    sources['attention-signals'].rows.map((row) => row['work-item-id']),
    [dependabotKey, reviewerKey],
  )
  ...
  const assignments = new Map(
    sources['agent-assignments'].rows.map((row) => [row['work-item-id'], row]),
  )
  ...
  assert.deepEqual(
    sources['evidence-records'].rows.map((row) => row['work-item-id']),
    [workerKey, reviewerKey, workerKey, reviewerKey],
  )
  assert.deepEqual(
    new Set(
      sources['evidence-records'].rows.map((row) => row['evidence-class']),
    ),
    new Set(['outcome', 'finding']),
  )
  assert.ok(
    sources['evidence-records'].rows.every(
      (row) =>
        row['verification-state'] === 'accepted' ||
        row['verification-state'] === 'pending',
    ),
  )
})
...
test('dashboard source bridge classifies safe-output performance and diagnostics', () => {
  ...
  assert.equal(
    sources['safe-output-performance'].metadata.completeness,
    'complete',
  )
  assert.equal(
...
test('dashboard source bridge expands GitHub telemetry resources', () => {
  ...
  test('dashboard source bridge explains configuration health and proposes bounded rollout actions', () => {
    ...
    assert.match(
      sources['configuration-actions'].rows[0].prompt,
  ...
  assert.equal(
    sources['github-api-rate-limits'].metadata.availability,
    'available',
  )
  assert.equal(
    sources['github-api-rate-limits'].metadata.completeness,
    'complete',
  )
})
...
test('dashboard source bridge derives reset-safe rate-limit forecasts and correlated attribution', () => {
  const checkpoint = (
    ...
    rateLimit: {
      core: { limit: 100, used: 100 - remaining, remaining, resetAt },
    },
    rateLimitError: null,
    activityCache: {},
  })
...
test('dashboard source bridge exposes stale and partial rate-limit evidence as unknown', () => {
  ...
  assert.equal(
    sources['github-api-rate-limits'].rows[0]['risk-status'],
    'unknown',
  )
  assert.equal(
    sources['github-api-rate-limits'].rows[1]['burn-rate-per-minute'],
    null,
  )
  assert.equal(
    sources['github-api-rate-limits'].rows[1]['projected-exhaustion-at'],
    null,
  )
  assert.equal(
    sources['github-api-rate-limits'].rows[1]['history-series'],
    'search · app · segment 2',
  )
  assert.equal(
    sources['github-api-collector-health'].rows[1]['rate-limit-error'],
    'unavailable',
  )
})
...
test('dashboard source bridge carries API capacity admission blocks into run rows', () => {
            ...
            runRecords: [
              {
                ...
                failureStep:
                  'CAO admission blocked: GitHub API limited until 2026-09-02T22:04:33.000Z',
  ...
  test('dashboard source bridge exposes structured admission decisions and gate checks', () => {
    ...
    assert.deepEqual(
      sources['admission-checks'].rows.map((row) => ({
        check: row.check,
  ...
  assert.deepEqual(
    Object.fromEntries(
      Object.entries(sources.runs.rows[0]).filter(([key]) =>
        [
          'admission-status',
          'admission-reason',
          'failure-job',
...
test('dashboard source bridge attaches usage data payloads to every run row', () => {
  ...
  assert.deepEqual(
    sources.runs.rows.map((run) => run.data),
    [
...
test('dashboard source bridge detects rollout mode from run titles with punctuation separators', () => {
  ...
  test('dashboard source bridge exposes chart summaries and in-depth security observations', () => {
    ...
    assert.equal(
      sources['security-observations'].metadata.completeness,
      'complete',
    )
    assert.ok(
      rows.some(
        (row) =>
          row['security-feature'] === 'access-control' &&
    ...
    assert.ok(
      sources['firewall-observations'].rows.some(
        (row) =>
    ...
    assert.ok(
      rows.some(
        (row) =>
          row['security-feature'] === 'integrity-filtering' &&
    ...
    assert.equal(
      sources['detection-observations'].rows[0]['detection-state'],
      'threat',
    )
    assert.equal(
      sources['detection-observations'].rows[0]['inspection-warning'],
      'patch: ERR_VALIDATION',
    )
    assert.deepEqual(
      sources['mcp-calls'].rows.map((row) => ({
...
test('dashboard source bridge omits mcp-calls rows when mcp telemetry is unavailable and correctly counts mcp failures', () => {
  ...
  assert.equal(
    sources['mcp-calls'].rows.filter((row) => row.run === '101').length,
    0,
  )
  ...
  const aggregatedFailureRow = sources['mcp-servers'].rows.find(
    (row) => row.run === '103',
  )
...
test('dashboard source bridge exposes run and job performance dimensions', () => {
  ...
  assert.match(
    sources['run-performance'].metadata['coverage-start'],
    /^2026-09-02/,
  )
  ...
  const { 'run-duration-seconds': _runDuration, ...commonPerformance } =
    sources['run-performance'].rows[0]
...
test('dashboard source bridge exposes authoritative coverage and structured collection provenance', () => {
  ...
  assert.equal(
    sources.runs.metadata['requested-coverage-start'],
...
test('dashboard source bridge carries package memberships, allowance, and inventory readiness into workflow rows', () => {
  ...
  const sources = buildDashboardLanguageSources({
    ...
    inventory: {
      workflows: [
        { lockPath: workflowPath, maxAiCredits: 500, compiled: true },
      ],
      bundles: [
        {
          id: 'ambient-context',
  ...
  assert.deepEqual(
    ...
    },
    {
      ...
      packageMemberships: [{ id: 'ambient-context', name: 'Ambient Context' }],
      maxAiCredits: 500,
...
test('dashboard source bridge merges remotely discovered allowed-repository workflows', () => {
  ...
  assert.deepEqual(
    sources.workflows.rows.map((row) => ({
...
test('dashboard source bridge maps a legacy manifest-derived package identity to the canonical inventory bundle id', () => {
  ...
  const workerPath =
    '.github/workflows/uk-ai-advisory-operational-resilience.lock.yml'
  const standalonePath =
    '.github/workflows/uk-ai-advisory-package-maintainer.lock.yml'
  const sources = buildDashboardLanguageSources({
    deployed: {
  ...
  const packagesById = new Map(
    sources.workflows.rows.map((row) => [row.workflow, row.package]),
  )
  assert.equal(
    packagesById.get('.github/workflows/uk-ai-advisory.md'),
    'uk-ai-advisory',
  )
  assert.equal(
    packagesById.get(
  ...
  assert.equal(
    packagesById.get('.github/workflows/uk-ai-advisory-package-maintainer.md'),
    'uk-ai-advisory',
  )
  assert.deepEqual(
...
test('dashboard source bridge carries canonical coverage diagnostics', () => {
  const input = {
    deployed: {
      ...
      workflows: [
        {
          repository: 'githubnext/public',
          visibility: 'public',
          path: '.github/workflows/public.lock.yml',
        },
        {
          repository: 'githubnext/unknown',
          visibility: 'unknown',
          path: '.github/workflows/unknown.lock.yml',
        },
      ],
    },
...
test('dashboard source bridge exposes rate-limit details for retained records', () => {
  const sources = buildDashboardLanguageSources({
    ...
    report: {
      ...
      records: [
        { repository: 'githubnext/service', updatedAt: '2026-09-02T23:00:00Z' },
      ],
      remoteWorkflows: [
        {
      ...
      workflowDiscovery: {
        ...
        workflowsObserved: 1,
        failures: [
          { repository: '', reason: 'GitHub API rate limit exceeded' },
        ],
      },
  ...
  assert.deepEqual(sources['coverage-diagnostics'].rows, [
    ...
    },
    {
      ...
      effect:
        'Retained the last successful snapshot from 2026-09-02T23:00:00Z.',
      'snapshot-age-seconds': 3600,
    },
  ])
  ...
  assert.equal(
    sources.workflows.metadata['fallback-source-as-of'],
...
test('dashboard source bridge derives admission gates from resolved control policy', () => {
  const sources = buildDashboardLanguageSources({
    deployed: {
      ...
      workflows: [
        {
          repository: 'acme/control',
          path: '.github/workflows/operations.lock.yml',
          name: 'Operations',
          role: 'orchestrator',
          state: 'active',
          runHealth: { runRecords: [] },
        ...
        {
          repository: 'acme/control',
          path: '.github/workflows/enabled-worker.lock.yml',
          name: 'Enabled worker',
          role: 'worker',
          state: 'active',
          runHealth: { runRecords: [] },
        ...
        {
          repository: 'acme/control',
          path: '.github/workflows/disabled-worker.lock.yml',
        ...
        {
          repository: 'acme/control',
          path: '.github/workflows/undeclared-worker.lock.yml',
        ...
        {
          repository: 'acme/control',
          path: '.github/workflows/disabled-package.lock.yml',
        ...
        {
          repository: 'acme/control',
          path: '.github/workflows/undeclared-package.lock.yml',
      ...
      bundles: [
        {
          ...
          workers: [
            {
              id: 'enabled-worker',
              sourcePath: '.github/workflows/enabled-worker.md',
              lockPath: '.github/workflows/enabled-worker.lock.yml',
              compiled: true,
            },
            {
              id: 'disabled-worker',
              sourcePath: '.github/workflows/disabled-worker.md',
              lockPath: '.github/workflows/disabled-worker.lock.yml',
              compiled: true,
            },
            {
              id: 'undeclared-worker',
              sourcePath: '.github/workflows/undeclared-worker.md',
              lockPath: '.github/workflows/undeclared-worker.lock.yml',
              compiled: true,
            },
          ],
        },
  ...
  assert.deepEqual(
    ...
    [
      ...
      {
        workflow: '.github/workflows/enabled-worker.md',
        status: 'authorized',
        reason: 'authorized',
      },
      {
        workflow: '.github/workflows/disabled-worker.md',
        status: 'blocked',
        reason: 'worker-disabled',
      },
      {
        workflow: '.github/workflows/undeclared-worker.md',
        status: 'blocked',
        reason: 'worker-undeclared',
      },
      {
        workflow: '.github/workflows/disabled-package.md',
        status: 'blocked',
...
test('dashboard source bridge preserves report observation identity, diagnostics, and historical coverage', () => {
      ...
      records: [
        {
          ...
          observation: {
            ...
            provenance: [
              { repository: 'github/gh-aw', sha: 'abc123', path: 'pkg/cli' },
            ],
          },
        },
      ],
  ...
  assert.deepEqual(sources['operational-values'].rows[0], {
    organization: 'github',
    repository: 'gh-aw',
...
test('dashboard source bridge preserves firewall evidence states, policy attribution, and workflow-scoped drift', () => {
      ...
      securityRuns: [
        {
          ...
          security: security(
            firewall({
              observations: [
                {
                  observedAt: '2026-09-01T05:01:00Z',
                ...
                {
                  observedAt: '2026-09-01T05:02:00Z',
                  domain: 'old.example',
                  host: 'old.example',
                  port: 443,
                  protocol: 'https',
        ...
        },
        {
          ...
          security: security(
            firewall({
              observations: [
                {
                  observedAt: '2026-09-02T05:01:00Z',
                  domain: 'api.github.com',
                  host: 'api.github.com',
                  port: 443,
                  protocol: 'https',
                ...
                {
                  observedAt: '2026-09-02T05:02:00Z',
                  domain: 'new.example',
                  host: 'new.example',
                  port: 443,
                  protocol: 'https',
        ...
        },
        {
          ...
          security: security(
            firewall({
              observations: [
                {
                  observedAt: '2026-09-02T06:01:00Z',
                  domain: 'new.example',
                  host: 'new.example',
                  port: 443,
                  protocol: 'https',
  ...
  assert.ok(
    sources['firewall-policy-rules'].rows.some(
      (row) =>
        row['rule-id'] === 'new-domain' &&
