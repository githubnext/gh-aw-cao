...
import { buildDashboardLanguageSources, detectionObservationRows } from "../../dashboard/report/dashboard-language-sources.mjs";
...
function detectionRun(runId, verdict, overrides = {}) {
  return {
    ...
    mode: "review",
    security: {
      threatDetection: verdict === null ? { available: false, verdict: null } : { available: true, verdict },
    },
    ...overrides,
  };
}
    ...
    githubTelemetry: [
      {
        ...
        rateLimitError: null,
        stackTrace: ["at recordGithubTelemetry (activity/github-telemetry.mjs:100:16)", "at main (activity/github-telemetry.mjs:150:9)"],
        activityCache: {
          hydrated: true,
          bytes: 1024,
          entryCount: 6,
...
test("dashboard source bridge diagnoses telemetry without valid rate-limit data", () => {
  ...
  assert.equal(sources["github-api-rate-limits"].metadata.completeness, "partial");
  assert.equal(sources["github-api-collector-health"].rows[0]["rate-limit-error"], "GitHub API returned no valid rate-limit resources.");
});
...
test("dashboard source bridge attaches usage data payloads to every run row", () => {
  ...
  assert.deepEqual(
    ...
    [{ findings: [{ severity: "high", total: 3 }] }, { summary: { total: 7 } }, null],
  );
});
  ...
  test("dashboard source bridge exposes chart summaries and in-depth security observations", () => {
    ...
    assert.ok(
      sources["firewall-observations"].rows.some(
        (row) => row.domain === "api.github.com" && row.port === 443 && row.protocol === "https" && row.decision === "allowed" && row["request-count"] === 8,
      ),
    );
    assert.ok(rows.some((row) => row["security-feature"] === "integrity-filtering" && row["security-subject"] === "create_issue"));
    assert.ok(
      rows.some((row) => row["security-feature"] === "integrity-filtering" && row["security-signal"] === "Passed interactions" && row["security-count"] === 4),
    );
    assert.ok(
      rows.some(
        (row) => row["security-feature"] === "threat-detection" && row["security-signal"] === "Prompt injection" && row["security-status"] === "detected",
      ),
    );
...
test("dashboard source bridge maps a legacy manifest-derived package identity to the canonical inventory bundle id", () => {
      ...
      bundles: [
        {
          ...
          name: "UK AI Advisory",
          workflows: [{ lockPath: orchestratorPath }, { lockPath: workerPath }, { lockPath: standalonePath }],
        },
      ],
      workflows: [
        {
  ...
  assert.deepEqual(new Set(sources.workflows.rows.map((row) => row.package)), new Set(["uk-ai-advisory"]));
});
...
test("dashboard source bridge preserves firewall evidence states, policy attribution, and workflow-scoped drift", () => {
  ...
  assert.ok(
    observations.rows.some(
      (row) => row.run === "41" && row.domain === "new.example" && row["drift-state"] === "newly-allowed" && row["policy-rule-id"] === "new-domain",
    ),
  );
  assert.ok(observations.rows.some((row) => row.run === "41" && row.domain === "old.example" && row["drift-state"] === "removed"));
  assert.ok(observations.rows.some((row) => row.run === "42" && row.domain === "new.example" && row["drift-state"] === "unknown"));
  assert.ok(observations.rows.some((row) => row.run === "43" && row["review-state"] === "evidence-missing" && row["request-count"] === null));
  assert.ok(
    sources["firewall-policy-rules"].rows.some((row) => row["rule-id"] === "new-domain" && row["domain-pattern"] === "new.example" && row["hit-count"] === 1),
  );
});
