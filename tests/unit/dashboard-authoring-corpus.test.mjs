...
test('generate-dashboard-ir corpus is indexed and valid', () => {
  execFileSync(
    'npm',
    ['--prefix', 'dashboard/site', 'run', 'validate:corpus'],
    {
      cwd: root,
      encoding: 'utf8',
      stdio: 'pipe',
...
test('every production dashboard page starts with an executive summary or prescribed attention view', () => {
  const executiveSummaryCharts = new Set([
    'pie',
    'line',
    'dot',
    'histogram',
  ...
  for (const path of dashboardFiles) {
    ...
    for (const page of document.dashboard.pages) {
      const views =
        page.kind === 'built-in' ? page.definition?.views : page.views
      const summary = views?.[0]
      assert.ok(summary, `${path}: page "${page.id}" must contain a view`)
      const isSummaryTable =
        summary.mark === 'table' &&
        summary.encoding?.columns?.some(
          (column) => typeof column.aggregate === 'string',
        )
      const isSummaryGrid =
        summary.mark === 'element' && summary.element === 'summary-grid'
      const isExperimentsEvaluation =
        page.id === 'experiments' &&
      ...
      const isWorkProjectView =
        ['work', 'work-tasks', 'work-roadmap'].includes(page.id) &&
        summary.mark === 'element' &&
        summary.element === 'work-project-view'
      const isInsightsOverview =
        page.id === 'insights' &&
        summary.mark === 'element' &&
        summary.element === 'insights-overview'
      const isOverviewDrillDown =
        page.route?.['navigation-page'] === 'overview' &&
        summary.mark === 'table'
      const isDataHealthConfidenceSummary =
        ...
        summary.encoding?.columns?.some(
          (column) =>
            column.field === 'confidence' && column.display === 'status',
        )
      ...
      assert.ok(
        (summary.mark === 'chart' &&
          executiveSummaryCharts.has(summary.chart)) ||
          isSummaryTable ||
          isSummaryGrid ||
