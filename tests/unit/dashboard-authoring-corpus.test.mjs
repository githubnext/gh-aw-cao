...
test("every production dashboard page starts with an executive summary or prescribed attention view", () => {
  ...
  for (const path of dashboardFiles) {
    ...
    for (const page of document.dashboard.pages) {
      ...
      const isSummaryTable = summary.mark === "table" && summary.encoding?.columns?.some((column) => typeof column.aggregate === "string");
      const isSummaryGrid = summary.mark === "element" && summary.element === "summary-grid";
      const isExperimentsEvaluation = page.id === "experiments" && summary.mark === "element" && summary.element === "experiments-evaluation";
      const isConfigurationPolicy = page.id === "configuration" && summary.mark === "element" && summary.element === "configuration-policy";
      const isAgentsMarketplace = page.id === "agents" && summary.mark === "element" && summary.element === "agent-marketplace-view";
      const isPackageDetail = page.id === "package-detail" && summary.mark === "element" && summary.element === "package-route";
      const isWorkProjectView =
        ["work", "work-tasks", "work-roadmap"].includes(page.id) && summary.mark === "element" && summary.element === "work-project-view";
      const isInsightsOverview = page.id === "insights" && summary.mark === "element" && summary.element === "insights-overview";
      const isOverviewDrillDown = page.route?.["navigation-page"] === "overview" && summary.mark === "table";
      const isDataHealthConfidenceSummary =
        page.id === "data-health" &&
        summary.mark === "table" &&
        summary.id === "data-health-summary" &&
        summary.encoding?.columns?.some((column) => column.field === "confidence" && column.display === "status");
      const isFullViewTable =
        views.length === 1 && summary.mark === "table" && summary.controls === "interactive" && summary["lazy-list"] === true && summary.layout === "full-view";
      const isAttentionFirstHome =
        page.id === "home" &&
