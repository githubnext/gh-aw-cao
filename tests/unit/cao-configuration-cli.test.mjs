...
test("cao init writes the minimal control-plane policy", async () => {
  ...
  try {
    ...
    await writeFile(path.join(root, ".github", "aw", "packages", "root.json"), JSON.stringify({
      schemaVersion: 1,
    ...
    for (const workflow of ["activity", "dashboard"]) {
      ...
      assert.match(installedWorkflow, /# Managed by cao init\/update; do not edit this file directly\./);
    ...
    const ownership = JSON.parse(await readFile(path.join(root, ".github", "aw", "packages", "root.json"), "utf8"));
