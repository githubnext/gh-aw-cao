...
function shellPermission(fullCommandText, identifiers) {
  return {
    ...
    commandSegments: identifiers.map((identifier) => ({
      identifier,
      fullCommandText,
    })),
  }
...
test('Copilot shell policy allows safe text tools and rejects mutating sed', () => {
  assert.equal(
    shellPermissionRejection(shellPermission('echo ready', ['echo'])),
    null,
  )
  assert.equal(
    shellPermissionRejection(shellPermission('cat dashboard.json', ['cat'])),
    null,
  )
  assert.equal(
    shellPermissionRejection(
      shellPermission("sed -n '1,20p' dashboard.json", ['sed']),
    ),
    null,
  )
  assert.match(
    shellPermissionRejection(
      shellPermission("sed -i 's/old/new/' dashboard.json", ['sed']),
    ),
    /in-place/,
  )
  assert.match(
    shellPermissionRejection(
      shellPermission("sed 's/old/new/w output.json' dashboard.json", ['sed']),
    ),
    /file-writing/,
  )
  assert.match(
...
test('Copilot file roots include the workspace and system temporary directory', () => {
  assert.equal(
    isWithinCopilotFileRoots('/workspace', '/tmp', '/workspace/dashboard.json'),
    true,
  )
  assert.equal(
    isWithinCopilotFileRoots('/workspace', '/tmp', '/tmp/copilot-notes.json'),
    true,
  )
  assert.equal(
    isWithinCopilotFileRoots('/workspace', '/tmp', '/etc/passwd'),
    false,
  )
})
...
test('local dashboard server composes package dashboards and reloads after updates', async () => {
  ...
  const stalePreviewDirectory = path.join(
    packageRoot,
    '.cao-dashboard-preview-stale',
  )
  ...
  await writeFile(
    ...
    `<!doctype html><body>preview ${syntheticToken}</body>`,
  )
  await writeFile(
    path.join(siteRoot, 'guide.md'),
    `# Guide\n\n${syntheticToken}\n`,
  )
  await writeFile(
    path.join(siteRoot, 'image.png'),
  ...
  await writeFile(
    path.join(siteRoot, 'dashboard.json'),
    JSON.stringify(builtInDashboard),
  )
  await writeFile(
  ...
  await writeFile(
    path.join(stalePreviewDirectory, 'dashboard.json'),
    dashboard('built-in'),
  )
          ...
          rows: [
            {
              ...
              note: ['github', 'pat', 'abcdefghijklmnopqrstuvwxyz123456'].join(
                '_',
              ),
  ...
  try {
    ...
    assert.equal(
      new URL(indexResponse.url).searchParams.get('local-preview'),
      'enabled',
    )
    ...
    assert.ok(
      requestLogs.some((message) =>
        /^GET \/dashboard\.json 200 \d+ms$/.test(message),
      ),
    )
    assert.deepEqual(
    ...
    const splitSourceResponse = await fetch(
      `${preview.url}/sources/repositories.json`,
    )
    assert.equal(splitSourceResponse.headers.get('content-encoding'), 'gzip')
    assert.deepEqual(await splitSourceResponse.json(), {
      rows: [
        { token: '[REDACTED]', note: '[REDACTED]', detail: 'x'.repeat(2_048) },
      ],
    })
    ...
    test('repository skill discovery includes supported workspace skill directories', async () => {
      const root = await mkdtemp(
        path.join(tmpdir(), 'dashboard-local-server-skills-'),
      )
      await mkdir(path.join(root, '.github', 'skills'), { recursive: true })
      await mkdir(path.join(root, '.agents', 'skills'), { recursive: true })
      try {
        const { repositorySkillDirectories } =
          await import('../../dashboard/local-server.mjs')
    ...
    assert.equal(
      await requestWithHost(`${preview.url}/sources.json`, 'attacker.example'),
      400,
    )
...
test('local dashboard server fails when dashboard data cannot be downloaded', async () => {
  ...
  await writeFile(
    ...
    '<!doctype html><body>preview</body>',
  )
...
test('local dashboard server optionally prompts Copilot to update the active view', async () => {
    ...
    createCopilotRuntime: async () => ({
      prompt: async (payload) => {
        ...
        if (payload.request === 'Produce invalid JSON') {
          ...
          updatedDashboard.dashboard.pages[0].title =
  ...
  try {
    ...
    const concurrentError = nextSocketMessage(
      socket,
      (message) =>
    ...
    const stoppedMessage = nextSocketMessage(
      socket,
    ...
    const expectedViewDashboardPath = await realpath(
      path.join(packageDirectory, 'dashboard.json'),
    )
    ...
    assert.match(
      prompts[0].bundledDashboardPath,
      /cao-dashboard-preview-.*dashboard\.json$/,
    )
    assert.equal(
    ...
    const invalidJsonError = nextSocketMessage(
      socket,
      (message) =>
    ...
    const invalidDashboardError = nextSocketMessage(
      socket,
      (message) =>
        message.type === 'error' && /could not update/.test(message.message),
    )
    ...
    const recoveredUpdate = nextSocketMessage(
      socket,
      (message) => message.type === 'dashboard-update',
    )
    const recoveredDone = nextSocketMessage(
      socket,
    ...
    const dashboardUpdate = nextSocketMessage(
      socket,
      (message) => message.type === 'dashboard-update',
    )
    const reloadError = nextSocketMessage(
      socket,
      (message) =>
        message.type === 'error' &&
        /preview could not reload/.test(message.message),
    )
    socket.send(
      JSON.stringify({
        type: 'copilot.start',
    ...
    assert.ok(
      prompts.every((prompt) => prompt.sessionKey === prompts[0].sessionKey),
    )
...
test('local dashboard CLI runs directly without a permission sandbox relaunch', () => {
  ...
  const result = spawnSync(
    process.execPath,
    ['dashboard/local-server.mjs', '--help'],
    {
      cwd: repositoryRoot,
      encoding: 'utf8',
    },
