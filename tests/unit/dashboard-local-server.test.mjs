...
import { isWithinCopilotFileRoots, shellPermissionRejection, startDashboardServer } from "../../dashboard/local-server.mjs";
...
test("Copilot shell policy allows safe text tools and rejects mutating sed", () => {
  ...
  assert.equal(shellPermissionRejection(shellPermission("sed -n '1,20p' dashboard.json", ["sed"])), null);
  assert.match(shellPermissionRejection(shellPermission("sed -i 's/old/new/' dashboard.json", ["sed"])), /in-place/);
  assert.match(shellPermissionRejection(shellPermission("sed 's/old/new/w output.json' dashboard.json", ["sed"])), /file-writing/);
  assert.match(
  ...
  try {
    ...
    test("repository skill discovery includes supported workspace skill directories", async () => {
      ...
      try {
        ...
        assert.deepEqual(await repositorySkillDirectories(root), [path.join(root, ".github", "skills"), path.join(root, ".agents", "skills")]);
      } finally {
...
test("local dashboard server optionally prompts Copilot to update the active view", async () => {
  ...
  try {
    ...
    const concurrentError = nextSocketMessage(socket, (message) => message.type === "error" && /already running/.test(message.message));
    socket.send(
      JSON.stringify({
        type: "copilot.start",
        view: "package-one",
    ...
    assert.deepEqual(prompts[0].editableDashboardPaths, [await realpath(path.join(root, "dashboard.json")), expectedViewDashboardPath]);
    assert.match(prompts[0].bundledDashboardPath, /cao-dashboard-preview-.*dashboard\.json$/);
    assert.equal(await readFile(path.join(packageDirectory, "dashboard.json"), "utf8"), dashboard("package-one"));

    runtimeStopped = false;
    const invalidJsonError = nextSocketMessage(socket, (message) => message.type === "error" && /could not update/.test(message.message));
    socket.send(
      JSON.stringify({
        type: "copilot.start",
        view: "package-one",
    ...
    const invalidDashboardError = nextSocketMessage(socket, (message) => message.type === "error" && /could not update/.test(message.message));
    socket.send(
      JSON.stringify({
        type: "copilot.start",
        view: "package-one",
    ...
    const recoveredUpdate = nextSocketMessage(socket, (message) => message.type === "dashboard-update");
    ...
    assert.equal(JSON.parse(await readFile(path.join(packageDirectory, "dashboard.json"), "utf8")).dashboard.pages[0].title, "Updated despite SDK failure");
    await writeFile(path.join(packageDirectory, "dashboard.json"), dashboard("package-one"));

    const dashboardUpdate = nextSocketMessage(socket, (message) => message.type === "dashboard-update");
    const reloadError = nextSocketMessage(socket, (message) => message.type === "error" && /preview could not reload/.test(message.message));
    socket.send(
      JSON.stringify({
        type: "copilot.start",
        view: "package-one",
