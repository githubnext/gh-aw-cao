...
test("runActivity requires the logs, telemetry, and indexer module paths", async () => {
  await assert.rejects(() => runActivity({}), /ACTIVITY_LOGS, GITHUB_TELEMETRY, ACTIVITY_INDEXER are required/);
});
...
test("runActivity skips dashboard-only steps when collection is disabled", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ["logs", "telemetry", "telemetry", "indexer"],
    );
...
test("runActivity removes cached agent directories before collecting logs", async () => {
  ...
  try {
    await runActivity({
      core: {
        info: () => {
          throw new Error("core.info should not be called");
        },
      },
    });
...
test("runActivity runs dashboard collection steps in order when enabled", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      calls.map((call) => call.name),
      ["logs", "telemetry", "telemetry", "control-settings", "inventory", "indexer", "activity-collectors"],
    );
  } finally {
    for (const key of Object.keys(process.env)) {
      if (!(key in originalEnv)) delete process.env[key];
