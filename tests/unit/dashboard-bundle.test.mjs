...
test('bundles installed package dashboards into one deterministic document', async () => {
  ...
  await writeFile(
    output,
    `${JSON.stringify(document('overview', 'Explore'))}\n`,
  )
  await writeFile(
    path.join(additions, 'zeta.json'),
    `${JSON.stringify(document('zeta', 'Package operations'))}\n`,
  )
  await writeFile(
    path.join(additions, 'alpha.json'),
    `${JSON.stringify(document('alpha', 'Package operations'))}\n`,
  )
