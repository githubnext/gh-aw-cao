...
export function configuredWorkerPackages(packages) {
  return new Map(
    Object.entries(packages || {}).flatMap(([packageName, policy]) =>
      Object.keys(policy.worker_policies || {}).map((workflow) => [
        `${workflow}.lock.yml`,
        packageName,
      ]),
    ),
...
function controlSettings() {
  if (!process.env.CONTROL_SETTINGS)
    throw new Error('CONTROL_SETTINGS is required')
  const settings = JSON.parse(
    readFileSync(process.env.CONTROL_SETTINGS, 'utf8'),
  )
...
function generatedRun(body, serverUrl) {
  const match = String(body || '').match(
    /Generated (?:from|with) \[[^\]]+\]\((https?:\/\/[^\s)]+\/actions\/runs\/(\d+))\)/i,
  )
  if (!match)
    throw new Error('review issue is missing generated workflow-run provenance')
  const runUrl = new URL(match[1])
  const server = new URL(serverUrl)
  if (runUrl.origin !== server.origin)
    throw new Error('workflow-run provenance uses a different GitHub host')
  ...
  const controlRepository = requireRepository(
...
export function inspectPublishEvent({
  ...
  if (event.issue?.pull_request)
    throw new Error(`${PUBLISH_LABEL} supports issues only`)
  if (event.issue?.state !== 'open')
    throw new Error('review issue must be open')
  if (event.issue?.user?.type !== 'Bot')
    throw new Error(
      'review issue must be created by a GitHub App or Actions bot',
    )

  const allowedReviewers = configuredSet(reviewers)
  if (allowedReviewers.size === 0) {
    throw new Error(
      'control-plane.publishing.reviewers must list at least one GitHub login',
    )
  }
  const reviewer = event.sender?.login || ''
  if (
    event.sender?.type !== 'User' ||
    !allowedReviewers.has(normalized(reviewer))
  ) {
    throw new Error(
      `${reviewer || 'event sender'} is not an authorized ops publisher`,
    )
  }

  const provenance = generatedRun(event.issue?.body, serverUrl)
  const allowedControlRepositories = configuredSet(
    controlRepositories || repository,
  )
  if (
    !allowedControlRepositories.has(normalized(provenance.controlRepository))
  ) {
    throw new Error(
      'generated workflow run is outside control-plane.publishing.control-repositories',
    )
  }
...
export function validateWorkflowRun({
  ...
  if (
    ...
    normalized(run.repository?.full_name) !==
      normalized(inspection.controlRepository) ||
  ...
  if (
    ...
    throw new Error(
      'review issue must originate from a successful dispatched worker run',
    )
  }
  const defaultBranch = String(run.repository?.default_branch || '')
  if (!defaultBranch || run.head_branch !== defaultBranch) {
    throw new Error(
      'workflow run must execute from the control repository default branch',
    )
  }
  const sourceCreatedAt = requireTimestamp(
    inspection.sourceIssueCreatedAt,
    'review issue creation time',
  )
  const runCreatedAt = requireTimestamp(
    run.created_at,
    'workflow run creation time',
  )
  const runUpdatedAt = requireTimestamp(
    run.updated_at,
    'workflow run update time',
  )
  if (sourceCreatedAt < runCreatedAt || sourceCreatedAt > runUpdatedAt) {
    throw new Error(
      'review issue creation time must fall within the originating workflow run',
    )
  }
  ...
  if (!packageName || workflowPath !== `.github/workflows/${workflowFile}`) {
    throw new Error(
      'workflow run is not from a supported Central Agentic Ops worker',
    )
  }
  const targetMatch = String(run.display_title || '').match(
    / · ([A-Za-z0-9][A-Za-z0-9-]*\/[A-Za-z0-9._-]+) · review$/,
  )
  if (!targetMatch)
    throw new Error(
      'workflow run does not identify a review-mode target repository',
    )
  const targetRepository = requireRepository(
    targetMatch[1],
    'target repository',
  )
  ...
  if (!ownerAllowlist.has(owner))
    throw new Error(
      'target repository owner is outside control-plane.scope.allowed-owners',
    )
  const repositoryAllowlist = configuredSet(allowedRepositories)
  if (
    repositoryAllowlist.size > 0 &&
    !repositoryAllowlist.has(normalized(targetRepository))
  ) {
    throw new Error(
      'target repository is outside control-plane.scope.allowed-repositories',
    )
  }
...
export function publishedIssueBody(
  sourceBody,
  { sourceRepository, sourceIssueNumber, sourceRunUrl, reviewer, serverUrl },
) {
...
async function apiRequest(token, apiUrl, pathname, options = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `GitHub API ${response.status} for ${pathname}: ${detail.slice(0, 500)}`,
    )
  }
...
async function inspectCommand() {
  ...
  if (settings.publishing_enabled !== true)
    throw new Error('Ops Publish is disabled by control policy')
...
async function findPublishedIssue(
  targetToken,
  apiUrl,
  targetRepository,
  marker,
  ...
  for (let page = 1; page <= 100; page += 1) {
    const issues = await apiRequest(
      targetToken,
      apiUrl,
      `/repos/${targetRepository}/issues?state=all&sort=created&direction=desc&per_page=100&page=${page}`,
    )
    ...
    if (
      oldestCreatedAt &&
      Date.parse(oldestCreatedAt) < Date.parse(sourceCreatedAt)
    )
      return null
  }
  throw new Error(
...
async function findPublicationComment(
  sourceToken,
  apiUrl,
  sourceRepository,
  sourceIssueNumber,
  ...
  throw new Error(
    'could not prove source comment idempotency within 100 comment pages',
  )
}
...
async function publishCommand() {
  ...
  if (!/^[a-f0-9]{64}$/.test(expectedContentDigest))
    throw new Error('approved source content digest is invalid')
  if (
    ![...configuredWorkerPackages(settings.packages).values()].includes(
      packageName,
    )
  ) {
  ...
  const sourceIssue = await apiRequest(
    process.env.SOURCE_TOKEN,
    apiUrl,
    `/repos/${sourceRepository}/issues/${sourceIssueNumber}`,
  )
  if (
  ...
  if (
    issueContentDigest(sourceIssue.title, sourceIssue.body) !==
    expectedContentDigest
  ) {
    throw new Error(
      `review issue content changed after ${PUBLISH_LABEL} approval; remove and reapply the label`,
    )
  }
  const currentProvenance = generatedRun(sourceIssue.body, serverUrl)
  if (
    currentProvenance.runUrl !== process.env.SOURCE_RUN_URL ||
    normalized(currentProvenance.controlRepository) !==
      normalized(process.env.CONTROL_REPOSITORY)
  ) {
  ...
  if (target.archived || target.disabled || !target.has_issues)
    throw new Error('target repository cannot accept published issues')
  ...
  const commentMarker = publicationCommentMarker(
    targetRepository,
    targetIssue.number,
  )
  ...
  if (!existingComment) {
    await apiRequest(
      process.env.SOURCE_TOKEN,
      apiUrl,
      `/repos/${sourceRepository}/issues/${sourceIssueNumber}/comments`,
      {
        method: 'POST',
        body: JSON.stringify({
