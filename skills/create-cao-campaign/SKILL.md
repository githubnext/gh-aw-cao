...
description: "Create a CAO campaign from an agentic strategy, operational idea, or deterministic add-on. Use when adding a CAO campaign, orchestrator/worker workflow family, organization-wide agentic automation, or the dashboard campaign; follows the repository's operational-campaign and add-on contracts."
argument-hint: "Describe the agentic strategy, target repositories, and desired outcomes"
---

# Create a CAO Campaign

Turn an operational idea into a complete campaign of GitHub Agentic Workflows. An operational campaign always contains one orchestrator and at least one worker. Never finish an operational campaign with a standalone workflow. The deterministic dashboard follows the explicit add-on exception below.
...
CAO does not require organization-billed Copilot inference. Choose a supported gh-aw engine and model for each workflow and configure its provider credentials as Actions secrets before running it. Do not infer a provider from available secrets or silently change the engine of an installed workflow. Target-repository credentials remain a separate authentication boundary.

For Copilot-backed workflows, checking the control repository owner's organization billing with `gh api orgs/<organization>/copilot/billing --jq '{seat_management_setting, total_seats: .seat_breakdown.total}'` is completely optional: the user's token may not have access to billing information. Never require that evidence or administrator confirmation, and never block campaign authoring on it; when the call fails or is inconclusive, note it once and continue. Treat an affirmative `total_seats: 0` with `seat_management_setting: unconfigured` as Copilot runs being unavailable, not as a blocker to authoring campaigns for other providers. A Pi or Codex workflow using a `copilot/*` model is Copilot-backed. Add `copilot-requests: write` directly to every Copilot-backed orchestrator and worker; validate their generated locks use `${{ github.token }}` for `COPILOT_GITHUB_TOKEN` and do not declare the PAT secret. Do not use `aw.yml` bootstrap `config`, authentication-neutral workflow sources, `COPILOT_GITHUB_TOKEN`, or runtime token precedence as a workaround for Copilot inference.

## Procedure

1. Load `.github/skills/agentic-workflows/SKILL.md`. For each orchestrator and worker, follow its `github/gh-aw` new-workflow route through `.github/aw/create-agentic-workflow.md`, then apply this repository-specific campaign contract as an overlay. Workflow creation is an agent workflow; use `gh aw compile` to validate the authored files.
2. Inspect `.github/workflows/shared/control.md` and the source `.md` files for the nearest existing campaign. Prefer a recently maintained campaign with behavior similar to the request. Do not copy generated `.lock.yml` files.
3. Establish the campaign contract from the user's idea:
  - campaign slug and short display name
...
- Keep the unified builder and publisher manually dispatchable, always upload a reusable dashboard artifact, honor `control-plane.campaigns.dashboard.deploy` for standalone Pages publication, pass `enablement: false` to `actions/configure-pages`, and require Pages access control before use. Do not add a schedule or a second enable variable.
- Keep canonical report modules under `dashboard/report/`; the CAO materializer preserves that path in control repositories.
- Keep the Dashboard Language renderer under `dashboard/site/`; it is owned and installed by the deterministic `dashboard/` campaign.

For this exception, validate manifest workflow ownership, canonical runtime materialization, both action workflows, safe relative `site-path` handling, standalone Pages prerequisites, and clean-room `gh aw add` and `gh aw add --force` restoration. The remaining Campaign Contract and Validation sections apply to operational campaigns.

## Campaign Contract

### Campaign metadata

Every campaign manifest must declare whether it is installable and production-ready:

- Set `private: true` only for campaigns that must not be installed. `gh aw add` rejects private campaigns, so never mark a public catalog campaign private.
- Set `experimental: true` while a campaign is being evaluated or its contract may change. `gh aw add` warns before installing experimental campaigns; remove the flag only after the campaign is ready for general use.

Keep these fields in `aw.yml` alongside `name`, `description`, and `min-version`, and review them whenever campaign maturity or distribution changes.
...
- Issues: configure `deduplicate-by-title: true`, a bounded `max`, and an explicit `expires`; use a canonical stable subject and search all open campaign-worker issues before creation. Default to `max: 1`. Permit a larger bound only when the worker can produce independently actionable items with distinct stable identities.
...
- Comments and reviews: inspect prior campaign-worker output on the target item and do not post the same finding or status again. Update or supersede an existing output when the configured safe output supports it; otherwise return `noop`. Hiding older comments does not make duplicate posting idempotent.
...
- a prompt headed with the campaign display name and containing `Discovery`, `Workers`, and `Completion` sections
...
`shared/control.md` owns the exact `## Orchestrator Report` format used by every campaign. Inspect its current report contract when creating the orchestrator; do not copy the template into the campaign because duplicated formats drift.
...
- add campaign-specific findings only after or alongside the standard fields; never rename, replace, or omit them

### Workers

Create at least one `.github/workflows/<campaign>-<worker>.md`. Every worker must include:

- `name` set to the exact `<Campaign Name> / <Worker Name>` hierarchy, where `<Campaign Name>` exactly matches the orchestrator's `name`
...
- `shared/control.md` imported with static `campaign`, `role: worker`, and `worker` identities
...
- when `safe-outputs.create-issue` or `safe-outputs.create-pull-request` is enabled, require every created issue or pull request body to follow the complete Worker Report Formatting contract below; configure `labels: [<campaign-slug>, <campaign-slug>:<worker-slug>]` so every created issue or pull request identifies both its owning operation and worker, and configure `title-prefix: "[<campaign-slug>:<worker-slug>] "`; instruct the worker to provide only the unprefixed subject because the safe output adds the configured prefix automatically, without repeating it or adding a semantically equivalent category prefix
- when `safe-outputs.create-issue` is enabled, configure `deduplicate-by-title: true`, an explicit `expires`, and a bounded `max`; require a canonical unprefixed subject that remains identical for the same unresolved repository work across reruns; derive it only from stable work identity such as the target repository, finding or blocking condition, affected component, and relevant path, while keeping versions, dates, run or correlation IDs, counts, severity, and status wording in the body; require the worker to search all open campaign-worker issues in the safe-output repository and reuse or comment on matching work, or call `noop`, instead of creating another issue even when an older matching issue used a different title
- when a comment or review safe output is enabled, require every comment and overall review body to follow the complete Worker Report Formatting contract below
...
Use a dedicated `target/` checkout when the worker must inspect a target repository while safe outputs land elsewhere. Add campaign-specific inputs only after the standard envelope.

### Worker Report Formatting

Follow the GitHub/gh-aw report conventions for every human-facing durable worker output. This contract is mandatory for every worker that creates issues, pull requests, comments, or reviews and applies to the complete output body:
...
3. Start directly with a concise executive-summary paragraph that states what happened, the decision-relevant result, critical findings, and key metrics. Do not add a heading before this opening paragraph because the first paragraph is always the executive summary. After the opening paragraph, use `###` for every heading; never use `#`, `##`, or `####` and deeper headings.
...
5. Keep only the summary, action, and critical findings visible. Put non-essential background, verbose evidence, logs, secondary metrics, per-item breakdowns, and every Markdown table in clearly named `<details><summary><b>...</b></summary>...</details>` sections.
...
- Always import `shared/control.md` with the correct role. Its import schema requires `campaign` and `role` and accepts the standard narrowing inputs.
- When an orchestrator or worker needs recent GitHub Agentic Workflow run history, restore the core activity cache before model execution and prefer `activity/`'s schema-versioned evidence over downloading the same run pages again. Workers that download `gh aw logs` or reason about agentic-workflow traces should import `shared/activity-cache.md` so both activation and agent jobs can reuse predownloaded evidence. Validate `schemaVersion`, `generatedAt`, repository scope, evidence window, and completeness, and fetch only missing or stale evidence. Treat a cache miss as a fallback condition, never as authority to widen scope, and do not copy the activity indexer into an operational campaign.
- When a worker optimizes a campaign or campaign portfolio, require a bounded campaign-health query over the authoritative activity, safe-output, review-item, and operational-value evidence that feeds the dashboard. Map campaign membership from checked-in CAO policy and workflow identity; measure run health, review backlog and age, review dispositions and decision latency, cost, and operational value over complete current and comparison windows. A campaign workflow has no dashboard browser session: never add browser automation or Pages access merely to query IndexedDB, and never depend on it as a workflow data service or authority. During dashboard implementation or debugging, coding agents may inspect the disposable cache only through the canonical storage/query APIs or Playwright and must confirm conclusions against authoritative upstream evidence.
- Every orchestrator inherits the dedicated `central-agentic-ops.dispatcher.run` OTEL span from `shared/control.md`; do not duplicate dispatcher telemetry in campaign workflows.
- `shared/sentry.md`, `shared/grafana.md`, and `shared/datadog.md` configure OTLP exporters only. Import them only when a campaign explicitly requires provider-specific routing; otherwise use the gh-aw organization defaults `GH_AW_DEFAULT_OTLP_ENDPOINT` and `GH_AW_DEFAULT_OTLP_HEADERS`.
...
- Extend a shared component only for behavior genuinely common to multiple campaigns; do not hide campaign policy in shared workflow files.

## Naming and Structure

- Use lowercase kebab-case for campaign, worker, and tracker slugs.
...
- Limit campaign names to the fewest distinctive words. Limit worker names to the specific responsibility, omitting redundant role words such as `Advisor`, `Auditor`, `Checker`, `Curator`, `Investigator`, `Maintainer`, or `Updater` when the responsibility remains clear.
...
- Name the orchestrator file `<campaign-slug>.md` and set its `name` to `<Campaign Name>`.
- Name each worker file `<campaign-slug>-<worker-slug>.md` and set its `name` to `<Campaign Name> / <Worker Name>`.
- Prefix every worker slug with the campaign slug. The display-name prefix before ` / ` must exactly equal the orchestrator display name.
- Do not add a role suffix to the orchestrator name or give a worker an independent top-level name.
- Keep frontmatter ordered like the nearest current campaign; do not normalize unrelated files.
- Keep campaign selection policy in the orchestrator and execution policy in workers.
...
2. Confirm the orchestrator `name` is exactly `<Campaign Name>`, every worker `name` is exactly `<Campaign Name> / <Worker Name>`, and the orchestrator `run-name` distinguishes scheduled runs from manual target-and-mode runs without unresolved placeholders.
...
5. Confirm the orchestrator imports `shared/control.md` with static campaign identity, reads policy only through the shared JSON resolver, and defaults safely to review mode.
6. Confirm the orchestrator has a `Completion` section that preserves the exact standard report contract provided by `shared/control.md`; campaign-specific reporting must be additive.
