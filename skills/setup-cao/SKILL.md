...
8. In a separate control repository, run the CAO Bash installer:
    ...
    Existing complete credential pairs are left unchanged. Use `--dry-run` before creation when reviewing custom App names or permissions. The helper creates private Apps only: organization-owned Apps for one organization, or enterprise-owned Apps installed separately on selected repositories in each enrolled organization. For a fine-grained PAT fallback, use `./cao.sh setup-auth token --repo <organization>/<control-repository>`; the command opens separate host-aware read and write PAT forms, prints each role's exact repositories, and stores the tokens as `GH_AW_GITHUB_READ_PAT` and `GH_AW_GITHUB_WRITE_PAT` through interactive prompts. Use `--write-repository` for each approved live-output repository; without it, write access defaults only to the control repository for review outputs. Confirm the read App has no write permission, the read PAT is read-only, and the write App or PAT is scoped only to repositories approved for safe outputs.
