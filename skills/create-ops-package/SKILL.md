...
- Issues: configure `deduplicate-by-title: true`, a bounded `max`, and either an explicit `expires` or a constrained `close-issue` output with interaction-aware cleanup instructions; use a canonical stable subject and search all open package-worker issues before creation. Default to `max: 1`. Permit a larger bound only when the worker can produce independently actionable items with distinct stable identities.
