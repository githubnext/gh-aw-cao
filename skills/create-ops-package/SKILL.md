...
The top-level `.github/cao/dashboard/` package is conventional GitHub Actions automation, not an agentic operation. Do not create an orchestrator, workers, runtime steering, rollout variables, or operational-value evaluators for it.

- Install the dashboard from root `aw.yml` by default, keep `.github/cao/dashboard/aw.yml` available for focused dashboard-only installations, and keep both manifests' dashboard destinations in sync. Never fold the dashboard into an operational package.
...
- Keep canonical report modules under `.github/cao/dashboard/report/` and install them under `.github/cao/dashboard/report/` as package resources.
- Keep the Dashboard Language renderer under `.github/cao/dashboard/site/`; it is owned and installed by the deterministic `.github/cao/dashboard/` package.
