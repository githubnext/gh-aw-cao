...
8. When an adopted worker already has an operational-value evaluator, preserve it under `.github/aw/<package>/graders/`, declare its source and destination in the package manifest, and keep its `graders.operational-value` registration. Evaluator design remains a separate post-adoption maintenance task.
...
- Keep the canonical evaluator at `.github/aw/<package>/graders/<worker-stem>-operational-value.sh`, explicitly map that source and destination in the package manifest, and register the same path under `graders.operational-value.run`.
...
11. Confirm every existing operational-value evaluator remains under `.github/aw/<package>/graders/`, is explicitly mapped by the package manifest, and is registered by its worker, or identify each new worker whose value design is pending adoption.
