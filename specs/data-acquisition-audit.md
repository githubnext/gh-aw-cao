...
**Version:** 1.0.0  
**Status:** Working Draft  
**Audit date:** 2026-09-05 (refreshed)
...
| `dashboard/report/aic-usage.mjs` | One call passing the workflow targets of every deployed workflow with recorded runs; 30-day security-evidence window | Raw JSON and downloaded artifacts in the shared `cao-activity` cache | Logs for all selected workflow targets are downloaded at once. AI Credit, security, and operational-value collection process the same bounded logs snapshot. |
...
1. **Run history is collected by multiple independent systems.** `activity/index.mjs` collects Actions run metadata, the activity snapshot makes one shared gh-aw logs download, worker predownloads collect overlapping target logs, and graders later collect overlapping evidence windows.
