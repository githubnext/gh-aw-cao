...
**Version:** 1.0.0  
**Status:** Working Draft  
**Audit date:** 2026-09-08 (refreshed: `design-decision-gate.md` and `mattpocock-skills-reviewer.md` were converted from automatic `pull_request` triggers (`opened`/`reopened`/`synchronize`/`labeled`/`ready_for_review`) to `slash_command`-only triggers (`/design-gate`, `/matt`); each now issues its per-event `gh pr view`/diff/paginated-files-or-comments prefetch only when explicitly invoked via slash command instead of on every qualifying push or review-readiness change, removing their prior per-push re-fetch pressure while leaving repeated-invocation re-fetching on the same pull request unchanged)
