...
| `.github/workflows/optimization-ai-credit-auditor.md` | Target repository, two days, at most 100 runs; locally filtered to the preceding 24 hours | `/tmp` for the current run | Overlaps the dashboard usage window and the evaluator's later evidence window. A separate API call first reads the current run's creation time. Now also imports `shared/activity-cache.md`, restoring the `cao-activity` snapshot into `runner.temp` before the agent runs, but its `gh aw logs` step is unchanged and still performs its own fresh, unconditional download regardless of cache contents. |
| `.github/workflows/optimization-ai-credit-optimizer.md` | Target repository, seven days, at most 50 runs | `/tmp` for the current run | Overlaps the auditor and dashboard collections. Monitoring workflows are filtered only after download. Now also imports `shared/activity-cache.md` for the same restored-but-unconsumed snapshot; the `gh aw logs` step still runs unconditionally. |
...
| `.github/workflows/aw-failures-investigator.md` | Now imports `shared/activity-cache.md`, restoring the `cao-activity` snapshot into `runner.temp` before activation and the agent step. | The restored snapshot is not read anywhere in this workflow's steps or prompt; only `self-care-open-source-failures.md` currently consumes `deployed-workflows.json` from the same restore. The remaining three importers restore an unused cache each run. |
...
12. **New `shared/activity-cache.md` restores are mostly unconsumed.** `aw-failures-investigator.md`, `optimization-ai-credit-auditor.md`, and `optimization-ai-credit-optimizer.md` now import `shared/activity-cache.md` and restore the `cao-activity` snapshot each run, but none of their steps or prompts read it; the two AI-credit workflows still issue their own unconditional `gh aw logs` downloads regardless of cache contents. Only `self-care-open-source-failures.md` (whose own inline restore step was consolidated into this shared import, with no functional change) consumes the snapshot's `deployed-workflows.json`.
...
11. **Consume the activity cache where it is already restored.** `aw-failures-investigator.md`, `optimization-ai-credit-auditor.md`, and `optimization-ai-credit-optimizer.md` now restore `cao-activity` via `shared/activity-cache.md` but do not read it. Either use `deployed-workflows.json` to bound or skip their own `gh aw logs` calls, or remove the unused restore step until a consumer is added.
