...
**Version:** 1.0.0  
**Status:** Working Draft  
**Audit date:** 2026-09-08 (refreshed: new `self-care-dashboard-data-schema.md` worker (SelfCare's thirteenth) fetches the already-built Pages dashboard-data manifest and its advertised JSON files by plain `curl` — no `gh api`/`gh aw logs` calls — and `self-care.md` now also gates that worker's dispatch behind the same 10-run/24-hour cadence check already used for `self-care-glossary`, duplicating a run-history read pattern rather than introducing a new one)
