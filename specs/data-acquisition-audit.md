...
**Version:** 1.0.0  
**Status:** Working Draft  
**Audit date:** 2026-09-08 (refreshed: new `dashboard-views.yml` PR comment-upsert workflow; `activity/logs.mjs` now logs its full `gh aw logs` argument list without changing request behavior; `activity/run-activity.mjs` now prunes cached per-run agent-log directories before collection, a local disk cleanup with no new GitHub request)
