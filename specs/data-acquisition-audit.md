...
| `activity/logs.mjs` | One call for all compiled workflows checked out in the control repository; 30-day security-evidence window | Raw JSON, collection state, and downloaded artifacts in the shared `cao-activity` cache | Runs immediately after cache restore with `--artifacts all`. AI Credit, security, operational-value, and index collection process the same bounded snapshot. |
