...
**Version:** 1.0.0  
**Status:** Working Draft  
**Audit date:** 2026-09-07 (refreshed again)
...
| `.github/workflows/self-care.md` (orchestrator) | Before dispatching the new `self-care-glossary` worker, inspects at most the ten most recent `self-care-glossary` workflow runs (via the `actions` `gh-proxy` toolset) to enforce a 24-hour dispatch cadence; fails closed (skips dispatch) when run history is unavailable or ambiguous. | Bounded to 10 runs and only gates one worker's dispatch; this is a small, independent run-history read separate from the shared `cao-activity` snapshot and from the worker's own prior-run lookup described above, so the same "most recent successful run" fact can be queried twice (once by the orchestrator's cadence check, once by the worker's evidence-window bootstrap) in the same dispatch cycle. |
