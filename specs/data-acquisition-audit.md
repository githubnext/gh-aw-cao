...
**Version:** 1.0.0  
**Status:** Working Draft  
**Audit date:** 2026-09-08 (refreshed: new `self-care-experimental-views.md` worker (SelfCare's twelfth) declares a `pull_requests`/`repos`/`actions` `gh-proxy` toolset but issues no `gh api`/`gh aw logs` calls in its prefetch or instructions, matching the no-material-collection pattern of `self-care-accessibility-checker.md`/`self-care-primer-brand-checker.md`)
...
| `activity/logs.mjs` | One call for all compiled workflows checked out in the control repository; 30-day security-evidence window | Raw JSON, collection state, and downloaded artifacts in the shared `cao-activity` cache | Runs immediately after cache restore with bounded usage, detection, evaluation, experiment, firewall, GitHub API telemetry, grader, and MCP artifact sets. Heavy agent artifacts are excluded. AI Credit, security, operational-value, and index collection process the same snapshot. When the single `gh aw logs` invocation fails, a new fallback path (`enrichFromActions`/`runGhApi`) issues one `gh api repos/{repo}/actions/workflows/{workflow}/runs` request per compiled workflow target (4-way concurrency, same 30-day window, up to 100 runs each), merging Actions run metadata onto any cached snapshot instead of writing an empty one. This converts a single bounded `gh aw logs` failure into up to *N* (one per `.lock.yml` workflow) separate Actions API list calls in the same run. The full `gh aw logs` argument list is now logged for diagnosability; this is an observability change with no effect on request count or targets. `activity/run-activity.mjs` also now deletes cached per-run `agent/` directories under the local `_activity/gh-aw-logs` cache tree immediately before invoking `logs.main`, a local disk-space cleanup step that issues no GitHub requests and does not alter the collection window or count. |
...
| `dashboard/report/records.mjs` | For every report repository, fetches up to ten pages each of all issues and issue comments, the first 100 Actions artifacts, and run metadata referenced by outputs. Additionally, for every enrolled repository other than the control repository (`allowedRepositories` minus the control repo), paginates up to 10 pages of `GET /repos/{repo}/actions/workflows` to discover deployed `.lock.yml` workflows for cross-repository package/marketplace views. | Promise memoization deduplicates run metadata only within one process. A prior snapshot is retained on rate limit, but successful runs always rescan issue/comment history and artifact metadata. The artifact list is not paginated, so traffic is bounded at the cost of incomplete results beyond 100 artifacts. Issue bodies now also carry a `gh-aw-workflow-id` marker that lets bundle/workflow attribution be resolved from the already-fetched issue body instead of an extra run-metadata lookup, but this only reduces per-record work when the marker is present; it does not change the fetch pattern above. The new remote-workflow discovery runs at 4-way concurrency alongside `repositoryReportSources`, is skipped on a confirmed rate limit (retaining the previous snapshot's `remoteWorkflows`/`workflowDiscovery`), and otherwise adds one additional Workflows-API scan per enrolled remote repository on every run, independent of the issue/comment scan already listed above. |
...
12. **The `self-care-glossary` worker and its orchestrator cadence check independently query the same run history.** `self-care.md` inspects up to 10 recent `self-care-glossary` runs to enforce a 24-hour dispatch gate, and the dispatched worker separately re-derives its own evidence-window lower bound from the most recent successful run; neither reuses the other's already-fetched run metadata, and the worker additionally fetches up to 50 merged-PR diffs/file lists and up to 100 commits per run with no persisted cursor beyond run history itself. `self-care-pages-health` adds a second, independent cadence check (up to 20 recent runs, six-hour gate) but has no worker-side prior-run lookup of its own, so it does not duplicate a run-history read the way the glossary worker does.
...
| P2 | Duplicate glossary run-history lookups | `self-care.md`'s cadence gate and `self-care-glossary.md`'s own evidence-window bootstrap both query recent `self-care-glossary` run history independently within the same dispatch cycle; low absolute cost (bounded to 10 and one prior-run lookup respectively) but avoidable duplication. `self-care-pages-health.md`'s six-hour cadence gate (up to 20 runs) is a second, similarly bounded but non-overlapping run-history read; it has no worker-side counterpart to duplicate. |
