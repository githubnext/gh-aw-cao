...
| `.github/workflows/optimization-ai-credit-auditor.md` | Target repository, two days, at most 100 runs; locally filtered to the preceding 24 hours | `/tmp` for the current run | Overlaps the dashboard usage window and the evaluator's later evidence window. Now imports `shared/activity-cache.md` and restores `cao-activity`. Reuses cached `workflow-logs.json` and `deployed-workflows.json` directly when complete for the target repository, skipping `gh aw logs` downloads unless the cache is incomplete or unavailable. |
| `.github/workflows/optimization-ai-credit-optimizer.md` | Target repository, seven days, at most 50 runs | `/tmp` for the current run | Overlaps the auditor and dashboard collections. Now imports `shared/activity-cache.md` and restores `cao-activity`. Reuses cached `workflow-logs.json` and `deployed-workflows.json` directly when complete for the target repository, falling back to `gh aw logs` when the cache is unavailable or incomplete. |
...
| `.github/workflows/aw-failures-investigator.md` | Imports `shared/activity-cache.md`, restoring the `cao-activity` snapshot into `runner.temp` before activation and the agent step. | Consumes `deployed-workflows.json` (`cachedFailedAgenticRuns`) when complete for the target repository to identify failed agentic runs, falling back to `listFailedAgenticRuns` via GitHub API when the snapshot is incomplete or unavailable. |
...
12. **`shared/activity-cache.md` restores are now consumed by audit and failure workers.** `aw-failures-investigator.md` consumes `deployed-workflows.json` from the restored `cao-activity` snapshot to identify failed agentic runs, while `optimization-ai-credit-auditor.md` and `optimization-ai-credit-optimizer.md` consume the cached `workflow-logs.json` directly to skip redundant `gh aw logs` downloads when the cache is complete for the target repository and window.
...
11. **~~Consume the activity cache where it is already restored.~~ (Implemented)** `aw-failures-investigator.md` reads `deployed-workflows.json` from the restored `cao-activity` snapshot to identify failed agentic runs, while `optimization-ai-credit-auditor.md` and `optimization-ai-credit-optimizer.md` read cached `workflow-logs.json` directly to skip redundant `gh aw logs` calls when the snapshot is complete for the target repository.
