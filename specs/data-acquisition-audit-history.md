...
| --- | --- | --- | --- |
| 2026-09-06 | API-free activity index | The activity workflow now updates its cache with one `gh aw logs --json --artifacts all` call immediately after restore. `activity/index.mjs` reads only checked-out metadata and that snapshot, performs no GitHub API operations, and records missing usage-artifact fields for follow-up in gh-aw. | Policy targets are no longer treated as repositories whose unrelated workflows should be scanned. |
