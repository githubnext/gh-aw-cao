...
| --- | --- | --- | --- |
| 2026-09-06 | Shared activity log cache | The activity workflow now invokes `gh aw logs --json` once without workflow arguments and persists all repository logs plus requested artifacts. AIC, security, and operational-value collection process that shared snapshot; per-workflow operational-value reports and fallback artifact downloads were removed. | Core quota remaining is emitted alongside each telemetry observation without exposing credential values. |
