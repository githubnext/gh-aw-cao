...
| --- | --- | --- | --- |
| 2026-09-06 | Shared activity log cache | The activity workflow now persists one bounded multi-target `gh aw logs --json` result and its requested artifacts. AIC, security, and operational-value collection process that shared snapshot; per-workflow operational-value reports and fallback artifact downloads were removed. | Core quota remaining is emitted alongside each telemetry observation without exposing credential values. |
