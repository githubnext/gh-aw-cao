...
| --- | --- | --- | --- |
| 2026-09-06 | Fourth material refresh | The activity workflow now retains `dashboard/report/aic-usage.mjs`'s gh-aw output under the shared `cao-activity` cache. Later `gh aw logs` runs can reuse per-repository, per-workflow query watermarks and artifacts instead of rescanning the full thirty-day horizon. | The collector's selected repositories, workflows, artifacts, count limits, rate-limit reserve, and thirty-day retention horizon are unchanged. |
