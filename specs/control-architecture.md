...
**CAO-CFG-006:** A control repository MUST declare `gh-aw-compiler-version` as a `v`-prefixed semantic version. Infrastructure that installs the gh-aw CLI MUST use this value as its compiler-version source of truth. The field does not grant execution capability or rollout authority.
