  ...
  metrics: [
    {
      ...
      direction: "decrease",
      presentation: { name: "Largest-file health", legendLabel: "Largest-file health", transform: "complement" },
    },
    {
...
function score(metricId, evidence) {
...
async function collectBatch() {
  let requests;
  try {
    requests = JSON.parse(await readStdin());
  } catch {
    fail("invalid batch collection request");
  }
  ...
  const range = ghJsonPages(`repos/github/gh-aw/commits?sha=main&since=${minimum}&until=${maximum}&per_page=100`);
...
if (args[0] === "--definition" && args.length === 1) {
  console.log(JSON.stringify(definition, null, 2));
} else if (args[0] === "--collect-batch" && args.length === 1) {
  console.log(JSON.stringify(await collectBatch(), null, 2));
} else if (args[0] === "--metric" && args.length === 2) {
  console.log(JSON.stringify(score(args[1], JSON.parse(await readStdin()))));
} else if (args.length === 0) {
  console.log(JSON.stringify(score("largest-file-health", JSON.parse(await readStdin()))));
} else {
  fail(`usage: ${process.argv[1]} [--definition|--collect-batch|--metric <id>]`);
}
