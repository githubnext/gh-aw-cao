...
func TestRedisPersistenceFailsWhenInfoReturnsNoData(t *testing.T) {
	doctor := testDoctor(fakeClient{do: func(arguments ...string) (any, error) {
		if len(arguments) == 2 && arguments[0] == "INFO" && arguments[1] == "persistence" {
			return nil, nil
		}
...
func TestRedisPersistencePreservesReportedMetricsChecks(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			status:  StatusPass,
			summary: "healthy writes",
		},
		{
			name:    "failed RDB save",
			reply:   "# Persistence\r\naof_enabled:0\r\nrdb_last_bgsave_status:err\r\nrdb_changes_since_last_save:4\r\n",
			status:  StatusWarn,
			summary: "background save did not succeed",
		},
		{
			name:    "failed AOF write",
			reply:   "# Persistence\r\naof_enabled:1\r\naof_last_write_status:err\r\nrdb_last_bgsave_status:ok\r\nrdb_changes_since_last_save:0\r\n",
			status:  StatusWarn,
			summary: "append-only-file write did not succeed",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if check.Status != tc.status {
				t.Fatalf("status = %s, want %s: %+v", check.Status, tc.status, check)
			}
			if !strings.Contains(check.Summary, tc.summary) {
				t.Fatalf("summary = %q, want to contain %q", check.Summary, tc.summary)
			}
		})
	}
}
