...
function viewSources(view, sources) {
  const names = [view?.data?.source, ...(Array.isArray(view?.data?.sources) ? view.data.sources : [])].filter((name) => typeof name === "string");
...
function categoricalSignal(view, rows) {
  const encoding = encodedFields(view).find(({ type }) => type === "nominal" || type === "ordinal");
...
function screenshotScore(observations) {
  ...
  const valid = observations.filter(({ width, height }) => Number.isFinite(width) && width > 0 && Number.isFinite(height) && height > 0);
  if (valid.length === 0) return { score: 0.5, status: "invalid-observations", viewports: 0 };
  const defects = valid.reduce((count, item) => count + Number(item.clipped === true) + Number(item.overlap === true), 0);
  return {
...
export function gradeDashboardView(view, options = {}) {
  ...
  const rows = sourceRecords.flatMap((source) => (Array.isArray(source.rows) ? source.rows.slice(0, MAX_ROWS_PER_SOURCE) : []));
  ...
  const contextParts = [textPresent(view.title), textPresent(view.description), sourceRecords.length > 0, textPresent(view.mark)];
  ...
  const overall = round(Object.entries(WEIGHTS).reduce((total, [metric, weight]) => total + scores[metric] * weight, 0) * 100, 1);
  ...
  return {
    ...
    findings: findings.toSorted((left, right) => ["high", "medium", "low"].indexOf(left.severity) - ["high", "medium", "low"].indexOf(right.severity)),
  };
}

function pageViews(page) {
  return page?.kind === "built-in" ? page?.definition?.views || [] : page?.views || [];
}
...
export function gradeDashboardDocument(document, options = {}) {
  ...
  return {
    ...
    summary: {
      ...
      average: results.length ? round(results.reduce((total, result) => total + result.overall, 0) / results.length, 1) : null,
