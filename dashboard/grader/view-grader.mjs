...
const REFERENCES = [
  {
    ...
    adaptation:
      'Normalized categorical entropy measures whether a visual channel carries a useful distribution of signal.',
  },
  {
    ...
    adaptation:
      'Position and length encodings are favored for quantitative comparisons.',
  },
  {
    id: 'mackinlay-1986',
    ...
    adaptation:
      'Encoding fields and types are checked for expressiveness and effectiveness.',
  },
  {
    id: 'purchase-1997',
    ...
    adaptation:
      'Observable overlap and crossing evidence lowers graphical readability.',
  },
  {
    id: 'ngo-2003',
    ...
    adaptation:
      'Layout balance, density, and regularity provide limited structural proxies for aesthetics.',
  },
]
...
function categoricalSignal(view, rows) {
  const encoding = encodedFields(view).find(
    ({ type }) => type === 'nominal' || type === 'ordinal',
  )
  if (!encoding?.field || !rows.length)
    return { cardinality: null, entropy: null }
...
function screenshotScore(observations) {
  ...
  if (valid.length === 0)
    return { score: 0.5, status: 'invalid-observations', viewports: 0 }
  const defects = valid.reduce(
    (count, item) =>
      count + Number(item.clipped === true) + Number(item.overlap === true),
    0,
  )
  return {
...
export function gradeDashboardView(view, options = {}) {
  ...
  if (!textPresent(view.title)) {
    findings.push(
      finding(
        'clarity',
        'high',
        'The view has no title.',
        'Add a short, decision-oriented title.',
      ),
    )
  }
  if (!textPresent(view.description)) {
    findings.push(
      finding(
        'clarity',
        'low',
        'The view has no explanatory description.',
        'Add one sentence stating the question this view answers.',
      ),
    )
  }
  ...
  if (categorical.entropy != null)
    informationSignal = (informationSignal + categorical.entropy) / 2
  ...
  const complexity =
    encodings.length +
    Number(view.mark === 'table') *
      Math.max(0, (view.encoding?.columns?.length || 0) - 4) +
  ...
  const density = clamp(
    (encodings.length + Number(view.mark === 'element')) / 6,
  )
  const visualBalance = clamp(
    1 -
      Math.abs(0.55 - density) * 0.8 -
      (width < 1 && complexity > 6 ? 0.15 : 0),
  )
  ...
  return {
    ...
    observations: {
      rows: rows.length,
      sources: sourceRecords.map(({ name, metadata }) => ({
        name,
...
export function gradeDashboardDocument(document, options = {}) {
  ...
  if (!Array.isArray(pages))
    throw new TypeError('dashboard.pages must be an array')
  const screenshots = options.screenshots || {}
  const results = pages.flatMap((page) =>
    pageViews(page)
      .filter((view) => textPresent(view.title))
      .map((view) => ({
  ...
  return {
    ...
    methodology: {
      purpose:
        'Decision-support heuristic for triaging small dashboard improvements; not a usability study or redesign mandate.',
      scoreRange: '0–100',
      weights: WEIGHTS,
...
async function main() {
  ...
  if (!dashboardPath || !outputPath) {
    throw new Error(
      'Usage: view-grader.mjs --dashboard <dashboard.json> --output <report.json> [--sources <sources.json>] [--screenshots <observations.json>]',
    )
  }
  const [document, sources, screenshots] = await Promise.all([
    readFile(dashboardPath, 'utf8').then(JSON.parse),
    argument('--sources')
      ? readFile(argument('--sources'), 'utf8').then(JSON.parse)
      : {},
    argument('--screenshots')
      ? readFile(argument('--screenshots'), 'utf8').then(JSON.parse)
      : {},
  ])
