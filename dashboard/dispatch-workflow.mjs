...
async function main() {
  ...
  const dispatch = await api(`/repos/${owner}/${repository}/actions/workflows/${encodeURIComponent(workflow)}/dispatches`, {
    method: "POST",
    body: JSON.stringify({
      ref,
      inputs: parseInputs(),
  ...
  await appendFile(
    ...
    [`run-id=${completedRun.id}`, `run-attempt=${completedRun.run_attempt}`, `run-url=${completedRun.html_url}`, ""].join("\n"),
  );
}
