...
The activity action reads trusted workflow, issue, pull request, and value-artifact data and writes a bounded cache snapshot. Local workflow metadata comes from the installed control repository; the dashboard report collector supplements it by listing generated `.lock.yml` Actions workflows in repositories explicitly named by `control-plane.scope.allowed-repositories`. Those remote entries appear as standalone Agents because package membership remains control-repository-owned metadata. Inaccessible repositories make workflow discovery partial rather than widening scope or fabricating inventory. The dashboard publisher restores the snapshot, `dashboard-language-sources.mjs` creates `sources.json`, and the packaged renderer serves it at the configured `site-path`. AI agents do not receive `pages: write`, `id-token: write`, or deployment authority.
