...
`GITHUB_TOKEN` as `GH_TOKEN` to that process. It also supplies an ephemeral
GitHub Copilot author and committer identity so pull-request actions can create
commits without changing global or repository Git configuration. When `gh aw`
is unavailable, an approved action first attempts
