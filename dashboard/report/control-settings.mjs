...
export function resolveDashboardControlSettings({
  ...
  try {
    result = execute(
      process.execPath,
      [controlProgram, 'control-settings', policyPath],
      {
        encoding: 'utf8',
    ...
    return unavailableSettings(
      repository,
      diagnostic(error?.message, 'control policy resolver crashed'),
      snapshot,
    )
  }
  ...
  try {
    ...
    return unavailableSettings(
      ...
      diagnostic(
        error?.message,
        'control policy resolver returned invalid JSON',
      ),
      snapshot,
    )
  }
...
export async function main(actions = {}, args) {
  ...
  if (!controlProgram || !policyPath || !outputPath) {
    throw new Error(
      'usage: control-settings.mjs <control.mjs> <policy.json> <output.json>',
    )
  }
