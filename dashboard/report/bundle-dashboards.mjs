...
function dashboard(value, source) {
  ...
  if (
    typeof candidate['language-version'] !== 'string' ||
    candidate['language-version'].length === 0 ||
    !candidate.dashboard ||
  ...
  const dashboardCandidate = /** @type {Record<string, unknown>} */ (
    candidate.dashboard
  )
  if (
    !Array.isArray(dashboardCandidate.pages) ||
    dashboardCandidate.pages.length === 0
  ) {
    throw new Error(`${source} is not a dashboard document`)
  }
...
export async function bundleDashboards(outputPath, dashboardsDirectory) {
  const entries = await readdir(dashboardsDirectory, {
    withFileTypes: true,
  }).catch((error) => {
...
export async function bundleDashboardFiles(outputPath, dashboardPaths) {
  ...
  await writeFile(
    outputPath,
    `${JSON.stringify(composeDashboardDocuments(primary, additions), null, 2)}\n`,
  )
}
...
async function main() {
  ...
  if (!outputPath || !dashboardsDirectory) {
    throw new Error(
      'usage: bundle-dashboards.mjs OUTPUT_DASHBOARD DASHBOARDS_DIRECTORY',
    )
  }
  ...
  try {
    await bundleDashboards(
      ...
      path.resolve(dashboardsDirectory),
    )
