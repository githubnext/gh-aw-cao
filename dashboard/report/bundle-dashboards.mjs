...
export async function bundleDashboards(outputPath, dashboardsDirectory) {
  ...
  await bundleDashboardFiles(
    outputPath,
    entries.filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => path.join(dashboardsDirectory, entry.name)),
  );
}
...
export async function bundleDashboardFiles(outputPath, dashboardPaths) {
  ...
  const additions = await Promise.all(dashboardPaths.toSorted().map(async (source) => dashboard(JSON.parse(await readFile(source, "utf8")), source)));
