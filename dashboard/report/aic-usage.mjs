...
import {
  runId as canonicalRunId,
  sourceId,
} from '../site/src/data/model/ids.js'
...
export async function readRunTimeline(outputDirectory, runId, sessionId) {
  ...
  const selected = files.filter((file) => {
    ...
    return (
      /(^|\/)gateway\.jsonl$/.test(relativePath) ||
...
function agenticAssessments(summary) {
  return Array.isArray(summary?.agentic_assessments)
    ? summary.agentic_assessments
        .filter(
          (assessment) =>
            AGENTIC_ASSESSMENT_KINDS.has(assessment?.kind) &&
...
function parseFirewallHost(value) {
  ...
  if (!text || text === '-')
    return { domain: 'unknown', host: 'unknown', port: null }
...
function firewallDecision(entry) {
  ...
  if (
    [200, 206, 304].includes(status) ||
    /TCP_(?:TUNNEL|HIT|MISS)/.test(decision)
  )
    return 'allowed'
  if ([403, 407].includes(status) || /TCP_DENIED/.test(decision))
    return 'denied'
  return 'unknown'
}

function firewallProtocol(entry, port) {
  if (String(entry?.method || '').toUpperCase() === 'CONNECT' || port === 443)
    return 'https'
...
async function parseFirewallAudit(file) {
  ...
  if (content === null)
    return {
      entries: [],
      malformed: 0,
      error: 'Firewall audit artifact could not be read.',
    }
  const entries = []
  let malformed = 0
...
function firewallEnabledFromInfo(info) {
  ...
  if (value)
    return value.toLowerCase() !== 'none' && value.toLowerCase() !== 'disabled'
  return null
}
...
async function readFirewallTelemetry(runRoot, files, summary) {
  ...
  if (infoFile) {
    ...
    if (content !== null)
      try {
        ...
        firewall.awfVersion =
          firstText(info.awf_version, info.firewall_version) || 'unknown'
      } catch {
        firewall.firewallEvidenceError =
          'Firewall configuration metadata is malformed.'
      }
  }

  const manifestFile = files.find(
    (file) => path.basename(file) === 'policy-manifest.json',
  )
  if (manifestFile) {
    ...
    if (content !== null)
      try {
        ...
        firewall.firewallEvidenceError =
          'Firewall policy manifest is malformed.'
      }
  }
  ...
  if (auditFile) {
    ...
    firewall.firewallEvidenceReference = relativeEvidencePath(
      runRoot,
      auditFile,
    )
    ...
    if (parsed.error) {
      ...
      firewall.firewallEvidenceCompleteness = 'unknown'
      firewall.firewallEvidenceError =
        'Firewall audit artifact contains no valid records.'
    ...
    const timestamps = parsed.entries
      .map((entry) => Date.parse(entry.observedAt))
      .filter(Number.isFinite)
    if (timestamps.length > 0) {
      firewall.firewallEvidenceHorizonStart = new Date(
        Math.min(...timestamps),
      ).toISOString()
      firewall.firewallEvidenceHorizonEnd = new Date(
        Math.max(...timestamps),
      ).toISOString()
  ...
  if (legacy && typeof legacy === 'object') {
    ...
    if (!auditFile) {
      ...
      firewall.firewallEvidenceError ||=
        'Legacy summary only; authoritative request and policy attribution may be incomplete.'
    }
  }
  ...
  if (firewall.firewallEnabled === false) {
    ...
    firewall.firewallEvidenceCompleteness = 'complete'
    firewall.firewallEvidenceSource = infoFile
      ? 'workflow-metadata'
      : firewall.firewallEvidenceSource
  } else if (
    firewall.firewallEnabled === true &&
    firewall.firewallEvidenceState === 'unknown'
  ) {
    firewall.firewallEvidenceState = 'unavailable'
    firewall.firewallEvidenceCompleteness = 'unknown'
    firewall.firewallEvidenceError ||=
      'Firewall was enabled but no firewall artifact was collected.'
  }
  return firewall
}
...
function countPermissionDenials(content, telemetry) {
  const pattern =
    /\[sdk-driver\].*permission denied by workflow tool permissions:\s*(read|write|shell|mcp|url|custom-tool)\(/gi
...
export async function readRunSecurityTelemetry(outputDirectory, runId) {
  ...
  const summaryFile = files.find(
    (file) => path.basename(file) === 'run_summary.json',
  )
  let summary = null
  if (summaryFile) {
      ...
      try {
        ...
        if (toolUsage && typeof toolUsage === 'object') {
            ...
            ? toolUsage.servers
                .map((server) => ({
                  ...
                  toolCallCount: Math.max(
                    0,
                    Number(server?.tool_call_count ?? server?.request_count) ||
                      0,
                  ),
                  errorCount: Math.max(0, Number(server?.error_count) || 0),
                  totalOutputSize: Math.max(
                    0,
                    Number(server?.total_output_size) || 0,
                  ),
                  maxOutputSize: Math.max(
                    0,
                    Number(server?.max_output_size) || 0,
                  ),
                }))
                .filter((server) => server.serverName)
            : []
          telemetry.mcp.calls = Array.isArray(toolUsage.tool_calls)
            ? toolUsage.tool_calls
                .map((call) => ({
                  ...
                  outputSize:
                    call?.output_size != null &&
                    Number.isFinite(Number(call.output_size))
          ...
          telemetry.integrity.totalToolCalls = Array.isArray(toolUsage.summary)
            ? toolUsage.summary.reduce(
                (total, tool) =>
                  total + Math.max(0, Number(tool.call_count) || 0),
                0,
              )
            : 0
        ...
        if (Array.isArray(summary.mcp_failures)) {
          telemetry.mcp.failures = summary.mcp_failures
            .map((failure) => ({
  ...
  const agentLogs = files.filter(
    (file) => path.basename(file) === 'agent-stdio.log',
  )
  ...
  const detectionFile = files.find(
    (file) => path.basename(file) === 'detection_result.json',
  )
  if (detectionFile) {
...
function securityTelemetryComplete(telemetry) {
  return (
    telemetry.accessControl.available &&
...
async function readRunEvals(outputDirectory, runId) {
  const runRoot = path.join(outputDirectory, `run-${runId}`)
  const files = await securityFiles(runRoot)
  const evalFiles = files.filter(
    (file) => path.basename(file) === 'evals.jsonl',
  )
  const observations = []
...
export async function collectAicUsage() {
  ...
  try {
    ...
    const outputPath = path.resolve(
      process.env.REPORT_AIC_USAGE || '_inventory/aic-usage.json',
    )
    const logsPath = process.env.REPORT_GH_AW_LOGS
      ? path.resolve(process.env.REPORT_GH_AW_LOGS)
      : ''
    const logsStatePath = process.env.REPORT_GH_AW_LOGS_STATE
      ? path.resolve(process.env.REPORT_GH_AW_LOGS_STATE)
      : ''
    const configuredCacheRoot = process.env.REPORT_AIC_CACHE
      ? path.resolve(process.env.REPORT_AIC_CACHE)
      : ''
    ...
    try {
      ...
      previousRunsByKey = new Map(
        (previousUsage.runs || []).map((run) => [
          `${run.repository}:${run.runId}`,
          run,
        ]),
      )
      previousSecurityRunsByKey = new Map(
        (previousUsage.securityRuns || []).map((run) => [
          `${run.repository}:${run.runId}`,
          run,
        ]),
      )
      ...
      if (error.code !== 'ENOENT')
        log.warning`Ignoring previous AI Credit usage at ${outputPath}: ${error.message}`
    }
    ...
    for (const [runId, metadata] of workflowByRunId) {
      ...
      securityRuns.set(
        key,
        previousSecurityRunsByKey.get(key) || {
          repository,
          runId,
    ...
    const temporaryRoot =
      configuredCacheRoot ||
      (await mkdtemp(path.join(os.tmpdir(), 'pages-aic-')))
    ...
    try {
      ...
      try {
        ...
        if (!Array.isArray(result.runs))
          throw new Error('gh aw logs snapshot has no runs array')
        ...
        for (const run of result.runs) {
          ...
          const aic =
            run.aic === null || run.aic === undefined || run.aic === ''
              ? null
              : Number(run.aic)
          ...
          const common = {
            ...
            runId,
            runAttempt:
              Number(run.run_attempt ?? run.runAttempt ?? run.attempt) || 1,
            workflowName:
              run.workflow_name ||
              run.workflow ||
          ...
          if (Number.isFinite(aic) || common.tokenUsage)
          ...
          try {
            ...
            security.firewall.firewallEvidenceError =
              'Firewall artifact parsing failed.'
            log.warning`Firewall evidence unavailable for ${repository} run ${runId}: ${error.message}`
          }
          if (
            common.createdAt &&
            ['available', 'partial', 'disabled', 'no-traffic'].includes(
              security.firewall.firewallEvidenceState,
            ) &&
          ...
          if (security.firewall.firewallEvidenceSource !== 'none') {
            security.firewall.firewallEvidenceFreshness = collectionAvailable
              ? 'fresh'
              : 'stale'
          }
      ...
      const reportedRunsByRepository = Object.groupBy(
        [...runs.values()],
        (run) => run.repository,
      )
      const repositories = [...runIdsByRepository].map(
        ([repository, runIds]) => {
      ...
      const firewallEvidenceTimes = [...securityRuns.values()]
        .flatMap((run) => [
      ...
      const usage = {
        ...
        firewallEvidenceHorizonStart:
          firewallEvidenceTimes.length > 0
            ? new Date(Math.min(...firewallEvidenceTimes)).toISOString()
            : null,
        firewallEvidenceHorizonEnd:
          firewallEvidenceTimes.length > 0
        ...
        securityComplete:
          collectionAvailable &&
          [...securityRuns.values()].every((run) =>
            securityTelemetryComplete(run.security),
          ),
