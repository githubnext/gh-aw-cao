...
function agenticAssessments(summary) {
  return Array.isArray(summary?.agentic_assessments)
    ? summary.agentic_assessments
        .filter((assessment) => AGENTIC_ASSESSMENT_KINDS.has(assessment?.kind) && ["low", "medium", "high"].includes(assessment?.severity))
        .map((assessment) => ({
...
function countPermissionDenials(content, telemetry) {
  ...
  for (const match of content.matchAll(pattern)) {
    ...
    const target = kind === "read" || kind === "write" ? telemetry.accessControl.fileDenials : telemetry.accessControl.toolDenials;
          ...
          telemetry.mcp.calls = Array.isArray(toolUsage.tool_calls)
            ? toolUsage.tool_calls
                .map((call) => ({
                  ...
                  outputSize: call?.output_size != null && Number.isFinite(Number(call.output_size)) ? Math.max(0, Number(call.output_size)) : null,
                }))
...
function securityTelemetryComplete(telemetry) {
  return telemetry.accessControl.available && telemetry.firewall.available && telemetry.integrity.available && telemetry.threatDetection.available;
}
...
function tokenUsage(run) {
  ...
  return summary && typeof summary === "object"
    ? {
        ...
        reasoningTokens: Object.values(summary.by_model || {}).reduce((total, model) => total + (Number(model?.reasoning_tokens) || 0), 0),
      }
    : null;
}
...
export async function collectAicUsage() {
  ...
  try {
    ...
    try {
      ...
      previousSecurityRunsByKey = new Map((previousUsage.securityRuns || []).map((run) => [`${run.repository}:${run.runId}`, run]));
    ...
    try {
      ...
      try {
        ...
        for (const run of result.runs) {
          ...
          const aic = run.aic === null || run.aic === undefined || run.aic === "" ? null : Number(run.aic);
          ...
          try {
            [security, evals, timeline] = await Promise.all([
              ...
              readRunTimeline(temporaryRoot, runId, sourceId("session", "gh-aw-logs", `${canonicalRunId(runId, common.runAttempt)}:unified`)),
            ]);
          } catch (error) {
            security = emptySecurityTelemetry();
      ...
      const firewallEvidenceTimes = [...securityRuns.values()]
        .flatMap((run) => [Date.parse(run.security.firewall.firewallEvidenceHorizonStart), Date.parse(run.security.firewall.firewallEvidenceHorizonEnd)])
        .filter(Number.isFinite);
      const requestedFirewallStart = new Date(Date.parse(generatedAt) - FIREWALL_HORIZON_DAYS * 86_400_000).toISOString();
      const usage = {
        schemaVersion: 5,
        generatedAt,
        ...
        firewallEvidenceHorizonStart: firewallEvidenceTimes.length > 0 ? new Date(Math.min(...firewallEvidenceTimes)).toISOString() : null,
        firewallEvidenceHorizonEnd: firewallEvidenceTimes.length > 0 ? new Date(Math.max(...firewallEvidenceTimes)).toISOString() : null,
        firewallLastSuccessfulCollectionAt: collectionAvailable ? generatedAt : previousUsage?.firewallLastSuccessfulCollectionAt || null,
        ...
        securityComplete: collectionAvailable && [...securityRuns.values()].every((run) => securityTelemetryComplete(run.security)),
