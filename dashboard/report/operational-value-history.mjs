...
function runAttempt(record) {
  const attempt = Number(
    record.runAttempt ?? record.run?.runAttempt ?? record.run?.attempt ?? 1,
  )
  return Number.isInteger(attempt) && attempt > 0 ? attempt : 1
}

export function operationalValueRecordIdentity(record) {
  if (typeof record.observationId === 'string' && record.observationId)
...
export function recordsFromOperationalValueReport(report) {
  ...
  return report.observations.map((observation) => {
    const record = {
      ...
      deltaFromBaseline: Number.isFinite(observation.deltaFromBaseline)
        ? observation.deltaFromBaseline
        : null,
      evaluatorDigest:
        observation.evaluatorDigest || report.evaluator?.sha256 || null,
      observation: {
        subject: {
      ...
      error:
        observation.status === 'error'
          ? observation.message || 'evaluator replay failed'
          : null,
    }
    record.observationId =
      observation.id || operationalValueRecordIdentity(record)
    return record
  })
}
...
export function definitionFromOperationalValueReport(report) {
  return {
    ...
    diagnosticMetrics: (report.diagnostics || []).map(
...
export function mergeOperationalValueRecords(...recordSets) {
  ...
  for (const record of recordSets.flat()) {
    ...
    if (
      existing?.observationSource === 'report' &&
      record.observationSource !== 'report'
    )
      continue
  ...
  for (const [key, record] of records) {
    if (
      ...
      observedRunKeys.has(operationalValueRunIdentity(record))
    ) {
      records.delete(key)
    }
  }
  return [...records.values()].sort((left, right) => {
    const leftTime = Date.parse(
      left.observation?.evidenceAt || left.run?.createdAt || '',
    )
    const rightTime = Date.parse(
      right.observation?.evidenceAt || right.run?.createdAt || '',
    )
    return (
      (Number.isFinite(leftTime) ? leftTime : 0) -
        (Number.isFinite(rightTime) ? rightTime : 0) ||
      operationalValueRecordIdentity(left).localeCompare(
        operationalValueRecordIdentity(right),
      )
    )
  })
}
