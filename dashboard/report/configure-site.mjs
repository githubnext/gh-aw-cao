...
export function configureSite(html, settings) {
  ...
  if (typeof favicon !== 'string')
    throw new Error('web.favicon must be a string')
  if (!html.includes(DEFAULT_FAVICON_LINK))
    throw new Error('dashboard favicon declaration is missing')
  return html.replace(
...
async function main([htmlPath, settingsPath]) {
  if (!htmlPath || !settingsPath) {
    throw new Error(
      'usage: configure-site.mjs <index.html> <control-settings.json>',
    )
  }
