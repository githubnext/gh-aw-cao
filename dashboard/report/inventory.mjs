...
export async function main(actions = {}) {
  ...
  try {
    ...
    const outputPath = path.resolve(
      process.env.REPORT_INVENTORY || '_inventory/control-plane.json',
    )
    ...
    function scalar(source, key) {
      return unquote(
        source.match(new RegExp(`^${key}:[ \\t]*(.+)$`, 'm'))?.[1] || '',
      )
    }
    ...
    function controlPackage(source) {
      return (
        source.match(
          /uses:\s+shared\/control\.md[\s\S]*?package:\s+([a-z0-9][a-z0-9-]*)/,
        )?.[1] || ''
      )
    }
    ...
    function discoverInventory() {
      ...
      for (const manifest of manifests.sort(
        ...
        for (const include of manifest.includes) {
          if (
            include.startsWith('.github/workflows/') &&
            include.endsWith('.md')
          )
            packageByWorkflow.set(include, manifest)
        }
      }
      ...
      const workflows = readdirSync(workflowDirectory, { withFileTypes: true })
        ...
        .map((entry) => {
          ...
          const role =
            source.match(
              /uses:\s+shared\/control\.md[\s\S]*?role:\s+(orchestrator|worker)/,
            )?.[1] || 'standalone'
          ...
          return {
            ...
            compiled: existsSync(
              path.join(workflowDirectory, `${stem}.lock.yml`),
            ),
            workers:
              role === 'orchestrator' ? inlineList(source, 'workflows') : [],
      ...
      const assignedWorkers = new Set(
        workflows.flatMap((workflow) => workflow.workers),
      )
      const bundles = workflows
        ...
        .map((orchestrator) => ({
          ...
          description:
            orchestrator.package?.description || orchestrator.description,
          ...
          workers: orchestrator.workers
            .map((workerId) => workflowById.get(workerId))
            .filter(Boolean),
          missingWorkers: orchestrator.workers.filter(
            (workerId) => !workflowById.has(workerId),
          ),
        }))
      const standalone = workflows.filter(
        (workflow) =>
          workflow.role === 'standalone' && !assignedWorkers.has(workflow.id),
      )
