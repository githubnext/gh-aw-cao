...
import { mergeOperationalValueRecords, operationalValueRunIdentity } from "./operational-value-history.mjs";
...
function operationalValueResult(run) {
  ...
  const matches = (Array.isArray(results) ? results : []).filter((result) => result.id === "operational-value" && result.source === "operational-value");
...
export async function collectOperationalValues() {
  ...
  try {
    ...
    const runsById = new Map(logs.runs.map((run) => [logsRunId(run), run]).filter(([runId]) => Number.isFinite(runId)));
