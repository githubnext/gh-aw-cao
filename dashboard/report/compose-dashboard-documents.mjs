...
export function composeDashboardDocuments(primary, additions) {
  ...
  const sections = new Map(
    (result.dashboard.navigation || []).map((section) => [
      section.label,
      section,
    ]),
  )
  ...
  for (const addition of additionalDocuments) {
    if (addition['language-version'] !== result['language-version']) {
      throw new Error(
        `dashboard language version mismatch: expected ${result['language-version']}, got ${addition['language-version']}`,
      )
    }
    for (const page of addition.dashboard.pages) {
      if (pageIds.has(page.id))
        throw new Error(`duplicate dashboard page id: ${page.id}`)
      pageIds.add(page.id)
