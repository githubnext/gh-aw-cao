  ...
  for (const workflow of deployed.workflows || []) {
    ...
    for (const run of workflow.runHealth?.runRecords || []) {
      ...
      rows.set(key, {
        ...
        "agent-id": firstText(usageRun.agentId) || "copilot",
        "agent-version": firstText(usageRun.agentVersion, usageRun.engineVersion) || "unknown",
        "model-id": firstText(usageRun.modelId, usageRun.resolvedModel, usageRun.requestedModel) || "auto",
        "gh-aw-version": firstText(usageRun.ghAwVersion) || "unknown",
        "aic-total": finite(usageRun.aic),
        "input-tokens": finite(usageRun.tokenUsage?.inputTokens),
        "output-tokens": finite(usageRun.tokenUsage?.outputTokens),
        "cache-read-tokens": finite(usageRun.tokenUsage?.cacheReadTokens),
        "cache-write-tokens": finite(usageRun.tokenUsage?.cacheWriteTokens),
        "reasoning-tokens": finite(usageRun.tokenUsage?.reasoningTokens),
...
function firewallRunBase(run, usage) {
  ...
  return {
    ...
    "run-conclusion": runConclusion(run.conclusion),
    "rollout-mode": rolloutMode(run.mode),
    "agent-id": run.agentId || run.engine || "unknown",
    "agent-version": run.agentVersion || run.engineVersion || "unknown",
    engine: run.engine || run.agentId || "unknown",
    "engine-version": run.engineVersion || run.agentVersion || "unknown",
    "requested-model": run.requestedModel || run.modelId || "unknown",
    "resolved-model": run.resolvedModel || run.modelId || run.requestedModel || "unknown",
    "agent-runtime": run.agentRuntime || "unknown",
