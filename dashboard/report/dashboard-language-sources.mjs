...
function runConclusion(value) {
  ...
  return ["success", "failure", "cancelled", "timed-out", "action-required", "neutral", "skipped", "stale", "startup-failure"].includes(normalized)
    ? normalized
    : "unknown";
}

function link(relation, href, label) {
  return typeof href === "string" && href.startsWith("https://") ? { relation, href, label } : undefined;
}

function workflowRunUrl(repository, runId) {
  const parts = String(repository || "").split("/");
  const id = String(runId ?? "");
  return parts.length === 2 && parts.every(Boolean) && /^\d+$/.test(id) ? `https://github.com/${parts[0]}/${parts[1]}/actions/runs/${id}` : undefined;
}
...
function uniqueStableRuns(rows) {
  return new Set((rows || []).flatMap((row) => (row.organization && row.repository && row.run ? [`${row.organization}/${row.repository}:${row.run}`] : [])))
    .size;
}
  ...
  const attachCollection = (names, operation) => {
    ...
    for (const name of names) {
      ...
      metadata["collection-state"] =
        collection?.state || (metadata.availability === "unavailable" ? "failed" : metadata.completeness === "partial" ? "partial" : "complete");
    ...
    for (let index = 0; index < history.length; index += 1) {
      ...
      if (complete && freshness === "fresh" && intervals.length >= GITHUB_RATE_LIMIT_THRESHOLDS.minimumBurnIntervals) {
        ...
        if (burn !== null && burn > 0) {
          ...
          row["runway-ratio"] = row["minutes-to-reset"] > 0 ? rounded(minutesToExhaustion / row["minutes-to-reset"]) : null;
        }
      }
...
function repositoryCoverageRows(deployed) {
  ...
  for (const item of [...(deployed.workflows || []), ...(deployed.bundles || [])]) {
    ...
    discovered.set(repository, ["public", "private", "internal"].includes(item.visibility) ? item.visibility : "unknown");
  }
  ...
  return [
    {
      ...
      value: deployed.repositoryScope === "allowlist" ? "Configured allowlist" : "Organization",
    },
...
function workflowAdmission(controlSettings, packageName, role, workflowId) {
  ...
  if (controlSettings.policy_resolution?.status === "unavailable") {
    return {
      status: "unavailable",
      reason: controlSettings.policy_resolution.reason || "policy-resolution-unavailable",
    };
  }
...
function inventoryWorkflowDetails(inventory = {}, controlSettings = {}) {
  ...
  for (const bundle of inventory.bundles || []) {
    const policyId = String(bundle.controlPackage || bundle.id || String(bundle.path || "").replace(/\/aw\.yml$|^aw\.yml$/g, "")).trim();
    const packagePolicy = controlSettings.packages?.[policyId] || controlSettings.packages?.[bundle.id];
    ...
    const targetPolicies = new Map(
      Object.entries(packagePolicy?.targets ?? packagePolicy?.target_policies ?? {}).map(([repository, targetPolicy]) => [
    ...
    const ready = bundle.compiled === true && (bundle.missingWorkers || []).length === 0 && workers.every((worker) => worker.compiled !== false);
...
function workflowRows(deployed, generatedAt, inventory, controlSettings) {
  ...
  return (deployed.workflows || []).map((workflow) => {
    ...
    const workflowMemberships = details?.packageMembership ? [details.packageMembership] : discoveredMemberships;
    const membership = workflowMemberships.at(-1);
    const packageIcon = details?.packageIcon || controlSettings.packages?.[membership?.id]?.icon || "package";
    ...
    return {
      ...
      "workflow-active": workflow.state === "active" ? "true" : String(workflow.state).startsWith("disabled") ? "false" : "unknown",
      ...
      "rollout-mode":
        details?.packageTargets?.find((target) => target.repository.toLowerCase() === workflowRepository)?.mode || details?.configuredMode || recentMode,
...
function runRows(deployed, usage) {
  ...
  const dataByRun = new Map(
    [...(usage.securityRuns || []), ...(usage.runs || [])].map((run) => [`${String(run.repository || "").toLowerCase()}:${run.runId}`, run.data ?? null]),
  );
  const metadataByRun = new Map(
    [...(usage.securityRuns || []), ...(usage.runs || [])].map((run) => [`${String(run.repository || "").toLowerCase()}:${run.runId}`, run]),
  );
  for (const workflow of deployed.workflows || []) {
    ...
    for (const run of workflow.runHealth?.runRecords || []) {
      ...
      rows.set(key, {
        ...
        "requested-model":
          firstText(run.requestedModel, run.requested_model, run.model, usageRun.requestedModel, usageRun.requested_model, usageRun.model) || "unknown",
        "resolved-model":
          firstText(run.resolvedModel, run.resolved_model, run.model, usageRun.resolvedModel, usageRun.resolved_model, usageRun.model) || "unknown",
...
function usageRows(usage) {
  return (usage.runs || []).map((run, index) => ({
    ...
    aic: run.aic,
    "estimated-usd": run.aic !== null && run.aic !== undefined && run.aic !== "" && Number.isFinite(Number(run.aic)) ? Number(run.aic) * AIC_TO_USD : null,
...
function transactionLogRows(usage) {
  ...
  for (const run of usage.securityRuns || []) {
    ...
    const timestamps = timeline
      ...
      .filter(Boolean)
      .sort();
    const agentJob = Array.isArray(run.logsPayload?.jobs) ? run.logsPayload.jobs.find((job) => /agent/i.test(firstText(job?.name))) : null;
    const common = {
      ...names,
      workflow: run.workflowPath?.replace(/\.lock\.yml$/, ".md") || run.workflowName || "",
      run: String(run.runId),
  ...
  for (const run of collectedLogRuns(usage)) {
    for (const result of run.evals || []) {
      ...
      observations.push({
        ...
        eval: evalId,
        "eval-result": answer,
        status: answer === "YES" ? "pass" : answer === "NO" ? "fail" : "unavailable",
        included: "true",
        "exclusion-reason": "",
        role: "metric",
        direction: "maximize",
...
function accessControlRows(run) {
  ...
  const toolTotal = toolDenials.reduce((total, [, count]) => total + count, 0) + Object.values(guardDenials).reduce((total, count) => total + count, 0);
  rows.push(
...
function firewallCoverage(usage) {
  ...
  return requestedDuration > 0 && Number.isFinite(observedDuration) ? rounded((observedDuration / requestedDuration) * 100, 1) : null;
}
      ...
      const reviewState =
        base["firewall-enabled"] === "disabled"
          ...
          : base["evidence-state"] === "no-traffic"
            ? "stable"
            : ["unavailable", "malformed", "unknown"].includes(base["evidence-state"])
              ? "evidence-missing"
              : "unknown";
      return [
        {
...
function firewallObservationRows(usage) {
  ...
  for (const scopedRows of Object.values(scopes)) {
    const runs = Object.values(Object.groupBy(scopedRows, (row) => row.run)).sort(
      (left, right) => Date.parse(left[0]["observed-at"]) - Date.parse(right[0]["observed-at"]),
    );
    let previous = null;
    for (const current of runs) {
      const previousByDomain = new Map(
        (previous || []).filter((row) => row.domain !== "unknown").map((row) => [`${row.domain}:${row.port ?? ""}:${row.protocol}`, row]),
      );
      ...
      for (const row of current) {
        ...
        row["request-volume-change"] = baseline && Number.isFinite(row["request-count"]) ? row["request-count"] - baseline["request-count"] : null;
        ...
        row["review-state"] ||=
          row.decision === "denied" && row["drift-state"] === "stable" && row["run-conclusion"] === "failure"
            ? "repeated-denial"
      ...
      previous = current[0]?.["evidence-completeness"] === "complete" ? current.filter((row) => row.domain !== "unknown") : null;
    }
  }
  const reviewLabels = {
  ...
  return result.map((row, index) => ({
    ...
    "enforcement-label": row["firewall-enabled"] === "enabled" ? "Enabled" : row["firewall-enabled"] === "disabled" ? "Enforcement disabled" : "Unknown",
    "evidence-label":
      row["evidence-state"] === "no-traffic"
        ? "No observed traffic"
        : row["evidence-state"] === "disabled"
          ? "Enforcement disabled"
          : ["unavailable", "malformed", "unknown"].includes(row["evidence-state"])
            ? "Evidence missing"
            : row["evidence-state"] === "partial"
              ? "Partial evidence"
              : "Evidence available",
    "decision-label": row.decision === "allowed" ? "Allowed by policy" : row.decision === "denied" ? "Denied by policy" : "Unknown",
...
function threatDetectionRows(run) {
  ...
  for (const warning of verdict.warnings || []) {
    rows.push(
      securityObservation(run, "threat-detection", "detail", "Inspection warning", "warning", 1, [warning.field, warning.code].filter(Boolean).join(": ")),
    );
  }
  return rows;
}
...
function securityObservationRows(usage) {
  return (usage.securityRuns || []).flatMap((run) => [...accessControlRows(run), ...firewallRows(run), ...integrityRows(run), ...threatDetectionRows(run)]);
}
...
function securityFindingRows(securityObservations) {
  return securityObservations
    .filter((row) => row["security-feature"] === "threat-detection" && row["security-analysis"] === "summary" && row["security-status"] === "detected")
    .map((row) => ({
...
function agentSmellRows(securityRuns = []) {
  return securityRuns.flatMap((run) =>
    (run.security?.agenticAssessments || []).map((assessment) => ({
      ...
      "smell-category": ["resource_heavy_for_domain", "model_downgrade_available"].includes(assessment.kind) ? "cost-memory-and-value" : "design",
...
function workflowSmellRows(workflows) {
  return workflows.flatMap((workflow) => {
    const rows = [];
    const metadata = workflow["gh-aw-metadata"] && typeof workflow["gh-aw-metadata"] === "object" ? workflow["gh-aw-metadata"] : {};
    const manifest = workflow["gh-aw-manifest"] && typeof workflow["gh-aw-manifest"] === "object" ? workflow["gh-aw-manifest"] : {};
    ...
    const unpinnedContainers = Array.isArray(manifest.containers)
      ? manifest.containers.filter((container) => !container?.digest && !container?.pinned_image).length
      : 0;
...
const DETECTION_FAILURE_CONCLUSIONS = new Set(["failure", "cancelled", "timed-out", "action-required", "stale", "startup-failure"]);
...
function detectionSignal(state, verdict, warningText) {
  if (state === "threat") {
    return [verdict.promptInjection && "Prompt injection", verdict.secretLeak && "Secret leak", verdict.maliciousPatch && "Malicious patch"]
      .filter(Boolean)
      .join(", ");
  }
...
export function detectionObservationRows(usage, jobs = []) {
  ...
  const securityByRun = new Map(securityRuns.map((run) => [observationKey(run.repository, run.runId), run]));
  const detectionJobs = jobs.filter((job) => String(job.job || "").toLowerCase() === "detection");
  const jobByRun = new Map(detectionJobs.map((job) => [observationKey([job.organization, job.repository].filter(Boolean).join("/"), job.run), job]));
  ...
  return [...keys].map((key) => {
    ...
    const threatsDetected = verdictAvailable && Boolean(verdict.promptInjection || verdict.secretLeak || verdict.maliciousPatch);
...
function mcpServerRows(usage) {
  return (usage.securityRuns || []).flatMap((run) => {
    ...
    for (const failure of mcp.failures || []) {
      ...
      const alreadyAggregatedFailure = reportedServers.has(failure.serverName) && positiveCount(server.errorCount) > 0;
...
function recordWorkflowRoleResolver(workflows) {
  const roleByRuntimeWorkflow = new Map(workflows.map((row) => [`${row.organization}/${row.repository}:${row.workflow}`.toLowerCase(), row["workflow-role"]]));
  return (record) => {
...
function outcomeRows(records, workflowRoleFor = () => "unknown") {
  return records.map((record) => ({
    ...
    "safe-output-kind": record.kind === "issue" ? "create-issue" : record.kind === "pull-request" ? "create-pull-request" : record.kind || "unknown",
    "outcome-status": record.state || "unknown",
    "outcome-state": record.state === "closed" ? "lifecycle-close" : record.kind === "noop" ? "ignored" : "pending",
...
function workItemRows(workflows, runs, outcomes) {
  const runsByWorkItem = latestByWorkItemKey(runs, (run) => workItemKey(run.organization, run.repository, run.workflow), "started-at");
  const outcomesByWorkItem = latestByWorkItemKey(
    outcomes,
    (outcome) => workItemKey(...(outcome["runtime-repository"] || `${outcome.organization}/${outcome.repository}`).split("/"), outcome.workflow),
    "observed-at",
  );
  return workflows.flatMap((workflow) => {
    ...
    return runCandidates.map((run, index) => {
      const matchedOutcome = run
        ? workflowOutcomes.find((outcome) => outcome.run && outcome.run === run.run) || (index === 0 ? workflowOutcomes[0] : undefined)
        : workflowOutcomes[0];
      const lifecycleState = workItemLifecycle(run, matchedOutcome);
      return {
        "work-item-id": key,
        name: run ? `${workflow["workflow-name"] || workflow.workflow} · ${run["run-title"]}` : workflow["workflow-name"] || workflow.workflow,
  ...
  return workItems
    ...
    .map((item) => {
      ...
      return {
        ...
        priority: item["lifecycle-state"] === "blocked" ? (item["consequence-tier"] === "high" ? 0 : 1) : item["lifecycle-state"] === "review" ? 1 : 2,
  ...
  for (const workflow of workflows) {
    ...
    rows.push({
      ...
      row: {
        ...
        "handoff-state": lifecycleState === "completed" ? "completed" : lifecycleState === "waiting" || lifecycleState === "review" ? "pending" : "in-progress",
...
function evidenceRecordRows(outcomes, findings, workItems) {
  ...
  const findingRecords = findings.map((finding) => {
    ...
    return {
      ...
      "verification-state": finding["finding-status"] === "resolved" ? "accepted" : finding["finding-status"] === "dismissed" ? "rejected" : "pending",
...
function operationalValueRows(values) {
  const definitions = new Map(
    (values.definitions || []).map((definition) => [`${definition.repository}:${definition.workflowId}:${definition.evaluatorDigest || ""}`, definition]),
  );
  return (values.records || [])
    ...
    .map((record) => {
      ...
      const observationId =
        ...
        ([record.repository, record.workflowId, record.runId, runAttempt, record.evaluatorDigest].every(
...
function operationalValueGraderRows(values) {
  return (values.records || []).map((record) => {
    ...
    return {
      ...
      value: record.value,
      "maturity-status": !record.observation ? "unavailable" : record.observation.mature ? "matured" : "interim",
      "baseline-value": record.baselineValue,
      "delta-from-baseline": record.deltaFromBaseline,
      "evaluator-digest": record.evaluatorDigest || "",
      "observed-at": record.observation?.evidenceAt || record.run?.createdAt,
      "run-link": link("run", record.runUrl || workflowRunUrl(record.repository, record.runId), `Run ${record.runId}`),
    };
  });
}
...
export function buildDashboardLanguageSources({ deployed, usage, operationalValues, report, inventory = {}, controlSettings = {}, githubTelemetry = [] }) {
  ...
  const workflowsByIdentity = new Map(
    [...(deployed.workflows || []), ...(report.remoteWorkflows || [])].map((workflow) => [
      `${String(workflow.repository).toLowerCase()}:${String(workflow.path).toLowerCase()}`,
      workflow,
    ]),
  );
  const workflowInventoryComplete = deployed.discovery?.complete === true && report.workflowDiscovery?.complete !== false;
  ...
  const graderObservations = [...operationalValueGraderRows(operationalValues), ...graders.observations];
  ...
  const controlRepository =
    deployed.workflows?.find((workflow) => workflow.repository)?.repository || deployed.bundles?.find((bundle) => bundle.repository)?.repository || "";
  ...
  sources.sessions = source("sessions", transactionLogs.sessions, generatedAt, usage.securityAvailable === true, usage.securityComplete === true);
  sources.events = source("events", transactionLogs.events, generatedAt, usage.securityAvailable === true, usage.securityComplete === true);
  ...
  const admissionComplete =
    runComplete && deployed.runHealth?.admissionEvidence?.complete !== false && admissionExpected.every((run) => Boolean(run.admission));
  sources.admissions = source("admissions", admission.admissions, generatedAt, admissionAvailable, admissionComplete);
  sources["admission-checks"] = source("admission-checks", admission.checks, generatedAt, admissionAvailable, admissionComplete);
  sources["run-performance"] = source("run-performance", performance.runs, generatedAt, runAvailable, runComplete && usageComplete);
  sources["job-performance"] = source("job-performance", performance.jobs, generatedAt, runAvailable, runComplete && usageComplete);
  ...
  if (Number.isFinite(deployed.runHealth?.windowHours) && deployed.runHealth.windowHours > 0) {
    ...
    sources.runs.metadata["coverage-start"] = new Date(Date.parse(generatedAt) - deployed.runHealth.windowHours * 3_600_000).toISOString();
  ...
  sources.experiments = source("experiments", experiments.definitions, generatedAt, usageAvailable, usageComplete);
  sources["experiment-assignments"] = source("experiment-assignments", experiments.assignments, generatedAt, usageAvailable, usageComplete);
  ...
  sources["eval-observations"] = source("eval-observations", evals.observations, generatedAt, usageAvailable, usageComplete);
  sources["mcp-calls"] = source("mcp-calls", mcpCallRows(usage), generatedAt, usage.mcpAvailable === true, usage.mcpComplete === true);
  sources["mcp-servers"] = source("mcp-servers", mcpServerRows(usage), generatedAt, usage.mcpAvailable === true, usage.mcpComplete === true);
  ...
  const firewallComplete = firewallRuns.length > 0 && firewallRuns.every((run) => ["complete"].includes(run.security?.firewall?.firewallEvidenceCompleteness));
  sources["firewall-observations"] = source(
    ...
    firewallComplete,
    firewallRuns.some((run) => run.security?.firewall?.firewallEvidenceFreshness === "stale")
      ? "stale"
      : firewallRuns.some((run) => run.security?.firewall?.firewallEvidenceFreshness === "fresh")
        ? "fresh"
        : "unknown",
  ...
  sources["firewall-policy-rules"] = source(
    ...
    firewallRuns.length > 0 &&
      firewallRuns.every((run) => run.security?.firewall?.firewallEnabled === false || Boolean(run.security?.firewall?.policyManifest)),
  );
  ...
  if (Number.isFinite(usage.windowHours) && usage.windowHours > 0) {
    ...
    sources.usage.metadata["coverage-start"] = new Date(Date.parse(generatedAt) - usage.windowHours * 3_600_000).toISOString();
  ...
  sources["coverage-diagnostics"] = source("coverage-diagnostics", coverageDiagnosticRows(deployed, usage, controlSettings, report), generatedAt);
  ...
  for (const [name, logicalSource] of Object.entries(sources)) {
    const redactedSource =
      name === "runs"
        ? {
            ...logicalSource,
            rows: logicalSource.rows.map(({ "logs-payload": _logsPayload, ...row }) => row),
          }
        : logicalSource;
    const browserSource = {
      ...redactedSource,
