...
import { appendFile, copyFile, mkdir, mkdtemp, readFile, readdir, realpath, rm, stat, writeFile } from "node:fs/promises";
...
const defaultCatalogRoot = basename(resolve(scriptDirectory, "..", "..")) === ".github" ? null : resolve(scriptDirectory, "..");
...
const trustedDashboardWorkflowPaths = new Set([".github/workflows/dashboard-build.yml", ".github/workflows/dashboard.yml"]);
...
async function loadLocalViewer(ghExecutable) {
  try {
    const result = await executeFile(ghExecutable, ["api", "user", "--jq", "{login: .login, name: .name, avatarUrl: .avatar_url}"]);
...
function sendContent(request, response, contentType, content) {
  ...
  if (request.method === "HEAD") {
    response.writeHead(200, compressibleContentTypes.has(contentType) ? { ...headers, Vary: "Accept-Encoding" } : headers);
    response.end();
    return;
  }
...
export async function repositorySkillDirectories(workingDirectory) {
  return existingDirectories([join(workingDirectory, ".github", "skills"), join(workingDirectory, ".agents", "skills")]);
}
...
async function dashboardSourceForView(view, dashboardPaths) {
  for (const path of dashboardPaths) {
    try {
      ...
      const matches = document?.dashboard?.pages?.some((page) => [page?.id, page?.title, page?.["navigation-label"]].includes(view));
...
async function downloadDashboardData(destination, repository, ghExecutable) {
  ...
  let runId;
  try {
    const repositoryResult = await executeFile(ghExecutable, ["api", `repos/${repositoryPath}`, "--jq", ".default_branch"]);
  ...
  if (provenance[0] !== "success" || provenance[1] !== defaultBranch || !trustedDashboardWorkflowPaths.has(provenance[2])) {
...
function shellCommandIdentifiers(permission) {
  const segmentIdentifiers = (permission.commandSegments ?? []).map((segment) => segment.identifier).filter(Boolean);
  return segmentIdentifiers.length > 0 ? segmentIdentifiers : permission.commands.map((command) => command.identifier);
}
...
export function shellPermissionRejection(permission) {
  ...
  const deniedIdentifiers = identifiers.filter((identifier) => !copilotReadOnlyShellCommands.has(identifier));
...
function dashboardPageIndex(document, view) {
  return document?.dashboard?.pages?.findIndex((page) => [page?.id, page?.title, page?.["navigation-label"]].includes(view)) ?? -1;
}
...
function redactSecretValues(value) {
  return secretValuePatterns.reduce((redacted, pattern) => redacted.replace(pattern, "[REDACTED]"), value);
}
...
function truncatedLogText(value, maximumLength = 1000) {
  ...
  return redacted.length <= maximumLength ? redacted : `${redacted.slice(0, maximumLength)}… [truncated ${redacted.length - maximumLength} chars]`;
}
...
async function isWorkspaceDashboardServer(pid, workingDirectory) {
  try {
    const [commandLine, processDirectory] = await Promise.all([readFile(`/proc/${pid}/cmdline`, "utf8"), realpath(`/proc/${pid}/cwd`)]);
...
async function listeningProcessIds(port) {
  try {
    const result = await executeFile("lsof", ["-nP", `-iTCP:${port}`, "-sTCP:LISTEN", "-t"]);
...
async function readDevServerPid(pidFile) {
  try {
    ...
    return Number.isInteger(record?.pid) && record.pid > 0 ? { pid: record.pid, port: record.port } : null;
  } catch (error) {
...
export async function replaceExistingDashboardDevServer({ workingDirectory, port, output = console.log }) {
...
async function startCopilotRuntime({ workingDirectory, copilotExecutable }) {
  ...
  return {
    async prompt({ sessionKey, view, request, bundledDashboardPath, editableDashboardPaths, viewDashboardPath, onEvent = () => {}, signal }) {
      ...
      const tools = [
        ...
        defineTool("validate_current_dashboard_view", {
          ...
          parameters: {
            type: "object",
            properties: {
              source: stringParameter("Complete candidate dashboard page JSON object."),
            },
            required: ["source"],
            additionalProperties: false,
          },
        ...
        defineTool("save_current_dashboard_view", {
          ...
          parameters: {
            type: "object",
            properties: {
              source: stringParameter("Complete validated dashboard page JSON object."),
            },
            required: ["source"],
            additionalProperties: false,
          },
      ...
      const handleSessionEvent = (event) => {
        ...
        if (event.type === "session.skills_loaded") {
          const skills = event.data.skills.filter((skill) => skill.enabled).map((skill) => skill.commandName || skill.name);
          console.log("Copilot dashboard skills loaded.", { skills });
          context.onEvent({
            type: "status",
            message: skills.includes("generate-dashboard-ir") ? "Dashboard authoring skill loaded." : "Repository skills loaded.",
          });
          ...
          const shellExit = event.data.result?.contents?.find((content) => content.type === "shell_exit" || content.type === "terminal");
          console.log("Copilot tool execution completed.", {
            sessionId,
            toolCallId: event.data.toolCallId,
            toolName: execution?.name ?? event.data.toolDescription?.name,
            success: event.data.success,
            durationMs: execution ? Math.max(0, Math.round(performance.now() - execution.startedAt)) : undefined,
            ...
            shellExit: shellExit
              ? {
                  ...
                  cwd: shellExit.cwd,
                  output: truncatedLogText(shellExit.type === "shell_exit" ? shellExit.outputPreview : shellExit.text),
                  outputTruncated: shellExit.type === "shell_exit" ? shellExit.outputTruncated : undefined,
                }
              : undefined,
            result: truncatedLogText(event.data.result?.detailedContent ?? event.data.result?.content),
            telemetry: redactedLogValue(event.data.toolTelemetry),
          });
          context.onEvent({
            type: "status",
            message: event.data.success ? "Applying dashboard update…" : "Copilot is retrying after a tool error…",
          });
      ...
      if (!entry)
        try {
          ...
          const workspacePath = (path) => canonicalPath(isAbsolute(path) ? path : join(workspaceRoot, path));
          session = await client.createSession({
            ...
            onPermissionRequest: async (permission) => {
              ...
              try {
                if (permission.requestSandboxBypass) {
                  ...
                  if (rejection) {
                    ...
                    const possiblePaths = await Promise.all(shellAbsolutePaths(permission).map(workspacePath));
                    if (possiblePaths.every((path) => isWithinCopilotFileRoots(workspaceRoot, temporaryRoot, path))) {
                      decision = { kind: "approve-once" };
              ...
              if (decision.kind === "reject") {
                const tool = permission.kind === "shell" ? shellCommandIdentifiers(permission).join(" | ") || "bash" : permission.kind;
                context.onEvent({
                  type: "tool-refused",
...
After saving the validated dashboard page, respond with a short, plain-language summary of what changed in the dashboard and what the user will now see. Avoid implementation details, JSON field names, schema terminology, file paths, and developer-oriented language. Complete the edit rather than only describing it.`,
        ...
        for (let repairAttempt = 1; ; repairAttempt += 1) {
          ...
          if (validation.ok) {
            onEvent({
              type: "status",
              message: repairAttempt === 1 ? "Dashboard validation passed." : "Dashboard repaired and validation passed.",
            });
            break;
          }
          ...
          if (repairAttempt > maxCopilotDashboardRepairAttempts) {
            throw new Error(`Dashboard validation still failed after ${maxCopilotDashboardRepairAttempts} repair attempts:\n${validationErrors}`);
          }
          try {
            await session.sendAndWait({
...
export async function startDashboardServer({
  ...
  try {
    ...
    sourcesContent = redactJsonSecrets(await readFile(join(dashboardDataDirectory, "sources.json"), "utf8"));
  ...
  const runCopilotRequest = async (socket, payload) => {
    ...
    const traceId = typeof payload?.traceId === "string" && /^[A-Za-z0-9-]{8,80}$/.test(payload.traceId) ? payload.traceId : randomBytes(16).toString("hex");
    if (!copilotRuntime) {
    ...
    try {
      const editableDashboardPaths = [baseDashboardPath, ...(await packageDashboardPaths(resolvedCatalogRoot, resolvedInstalledDashboardsDirectory))];
      ...
      const renderTimeout = setTimeout(() => rendered.reject(new Error("The browser did not confirm the dashboard update.")), 5000);
      ...
      try {
        ...
        const errorLog = [
          ...
          recoveryError
            ? `Dashboard source recovery failed:\n${recoveryError instanceof Error && recoveryError.stack ? recoveryError.stack : String(recoveryError)}`
            : undefined,
        ]
          .filter(Boolean)
          .join("\n\n");
      ...
      sendSocketEvent(socket, {
        type: "error",
        traceId,
        message: error?.code === "DASHBOARD_PREVIEW_RELOAD_FAILED" ? error.message : "Copilot could not update the view.",
        details:
    ...
    if (command?.type === "copilot.start") {
      ...
      if (command.event === "preview.rendered") {
        ...
        if (acknowledgement?.socket === socket) {
          const message = typeof command.details?.message === "string" ? command.details.message : "The browser could not render the updated dashboard.";
  ...
  const rebuild = async (notify = true, traceId, forceNotify = false) => {
    ...
    const packagePaths = await packageDashboardPaths(resolvedCatalogRoot, resolvedInstalledDashboardsDirectory);
  ...
  const refreshWatchers = async (packagePaths) => {
    if (closed) return;
    const candidates = new Set([resolvedSiteRoot, resolvedInstalledDashboardsDirectory, ...packagePaths.map(dirname)]);
  ...
  const isAllowedHost = (host) =>
    host === expectedAuthority || (localhostAuthority && host === localhostAuthority) || (codespaceAuthority && host === codespaceAuthority);
  const isAllowedOrigin = (origin) => {
    if (typeof origin !== "string") return false;
    try {
      const parsed = new URL(origin);
      return (parsed.protocol === "http:" || parsed.protocol === "https:") && isAllowedHost(parsed.host);
    } catch {
      return false;
    }
  };
  const server = createServer(async (request, response) => {
    ...
    try {
      if (!expectedAuthority || !request.headers.host || !isAllowedHost(request.headers.host) || /^[A-Za-z][A-Za-z0-9+.-]*:\/\//.test(request.url || "")) {
      ...
      if ((pathname === "/" || pathname === "/index.html") && !url.searchParams.has("local-preview")) {
