...
const defaultCatalogRoot =
  basename(resolve(scriptDirectory, '..', '..')) === '.github'
    ? null
...
function normalizeLocalViewer(viewer) {
  ...
  const avatarUrl =
    typeof viewer.avatarUrl === 'string' ? viewer.avatarUrl.trim() : ''
...
function compressPayload(body) {
  ...
  for (const staleKey of [...compressedPayloads.keys()].slice(
    0,
    -maximumCachedCompressions,
  )) {
...
function sendContent(request, response, contentType, content) {
  ...
  const acceptsGzip = /(^|,)\s*gzip\s*(;|,|$)/.test(
    String(request.headers['accept-encoding'] ?? ''),
  )
  if (request.method === 'HEAD') {
    response.writeHead(
      200,
      compressibleContentTypes.has(contentType)
  ...
  if (
    acceptsGzip &&
    compressibleContentTypes.has(contentType) &&
    body.byteLength >= minimumCompressedBytes
  ) {
...
async function canonicalPath(path) {
  ...
  try {
    ...
    if (error?.code !== 'ENOENT' || dirname(absolutePath) === absolutePath)
      throw error
    return join(
      await canonicalPath(dirname(absolutePath)),
...
async function packageDashboardPaths(
  catalogRoot,
  installedDashboardsDirectory,
) {
  const paths = []
  const installed = await readdir(installedDashboardsDirectory, {
    withFileTypes: true,
  }).catch((error) => {
  ...
  if (catalogRoot) {
    const catalogEntries = await readdir(catalogRoot, {
      withFileTypes: true,
    }).catch((error) => {
      if (error?.code === 'ENOENT') return []
      throw error
...
async function downloadDashboardData(destination, repository, ghExecutable) {
  ...
  try {
    ...
    throw new Error(
      'Unable to query the dashboard data artifact. Authenticate GitHub CLI with Actions read access.',
    )
  }
  if (!/^[1-9][0-9]*$/.test(runId)) {
    throw new Error(
      `No current ${dataArtifactName} artifact is available. Run the dashboard action first.`,
    )
  }
  if (!defaultBranch)
  ...
  try {
    ...
    throw new Error(
      `Unable to verify ${dataArtifactName} from workflow run ${runId}.`,
    )
  }
  if (
    provenance[0] !== 'success' ||
    provenance[1] !== defaultBranch ||
    !trustedDashboardWorkflowPaths.has(provenance[2])
  ) {
    throw new Error(
      `The latest ${dataArtifactName} artifact is not from a successful trusted dashboard workflow run.`,
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Unable to download ${dataArtifactName} from workflow run ${runId}.`,
    )
  }
}
...
async function sourceSignature(paths) {
  const entries = await Promise.all(
    paths.map(async (path) => `${path}\0${await readFile(path, 'utf8')}`),
  )
  return entries.join('\n')
}
...
function shellAbsolutePaths(permission) {
  const commandPaths =
    permission.fullCommandText.match(/\/[^\s"'|;&<>]+/g) ?? []
...
export function shellPermissionRejection(permission) {
  ...
  if (permission.possibleUrls.length > 0)
    return 'shell command may access a URL'
  ...
  if (identifiers.includes('sed')) {
    ...
    if (
      /(?:^|[;/'"\s])(?:e|w|W)(?:\s|['"]|$)/.test(permission.fullCommandText)
    ) {
      return 'sed execute and file-writing commands are not allowed'
    }
  }
  const deniedIdentifiers = identifiers.filter(
    (identifier) => !copilotReadOnlyShellCommands.has(identifier),
  )
...
function dashboardPageIndex(document, view) {
  return (
    document?.dashboard?.pages?.findIndex((page) =>
      [page?.id, page?.title, page?.['navigation-label']].includes(view),
    ) ?? -1
  )
}
...
const secretKeyPattern =
  /(?:^|[-_])(api[-_]?key|authorization|client[-_]?secret|password|private[-_]?key|secret|token)(?:$|[-_])/i
  ...
  for (const pid of candidates) {
    ...
    if (!(await isWorkspaceDashboardServer(pid, resolvedWorkingDirectory))) {
      if ((await listeningProcessIds(port)).includes(pid)) {
        throw new Error(
          `Port ${port} is used by another process; refusing to stop it.`,
        )
      }
      continue
    }
    ...
    if (!(await waitForProcessExit(pid))) {
      output(
        `Previous dashboard dev server did not stop gracefully; terminating PID ${pid}.`,
      )
      process.kill(pid, 'SIGKILL')
      if (!(await waitForProcessExit(pid, 2000))) {
        throw new Error(
          `Unable to stop previous dashboard dev server PID ${pid}.`,
        )
      }
    }
  }
...
async function startCopilotRuntime({ workingDirectory, copilotExecutable }) {
  ...
  try {
    ...
    console.log(
      'Copilot SDK runtime is unavailable. Install @github/copilot-sdk.',
    )
  ...
  try {
    ...
    console.log(
      'Copilot headless server failed to start.',
      errorMetadata(error),
    )
    await client.stop().catch((stopError) => {
      console.log(
        'Copilot cleanup after startup failure failed.',
        errorMetadata(stopError),
      )
    })
    throw error
  }
  ...
  return {
    async prompt({
      ...
      console.log(
        entry
          ? 'Reusing Copilot dashboard session.'
          : 'Creating Copilot dashboard session.',
        {
          sessionKey,
      ...
      const readViewDocument = async () => {
        const document = JSON.parse(
          await readFile(context.viewDashboardPath, 'utf8'),
        )
      ...
      const tools = [
        defineTool('read_dashboard_language_reference', {
          description:
            'Read the canonical Dashboard Language vocabulary used by the local renderer.',
          parameters: {
            type: 'object',
            properties: {},
        ...
        defineTool('read_current_dashboard_view', {
          description:
            'Read only the current dashboard page that the user asked to modify.',
          parameters: {
            type: 'object',
            properties: {},
            additionalProperties: false,
          ...
          handler: async () => {
            ...
            const source = JSON.stringify(
              document.dashboard.pages[pageIndex],
              null,
              2,
            )
        ...
        defineTool('validate_current_dashboard_view', {
          description:
            'Validate the complete candidate JSON object for the current dashboard page.',
          parameters: {
            type: 'object',
            properties: {
              source: stringParameter(
                'Complete candidate dashboard page JSON object.',
              ),
            },
            required: ['source'],
            additionalProperties: false,
          ...
          handler: async ({ source }) => {
            try {
              ...
              console.log(
                'Rejected invalid Copilot dashboard view candidate.',
                errorMetadata(error),
              )
              return {
                ok: false,
                error:
                  error instanceof Error
                    ? error.message
                    : 'Dashboard page must be valid JSON.',
              }
            }
          },
        }),
        defineTool('validate_dashboard_json', {
          description:
            'Run the existing dashboard validator.js against the selected dashboard.json.',
          parameters: {
            type: 'object',
            properties: {},
            additionalProperties: false,
        ...
        defineTool('save_current_dashboard_view', {
          description:
            'Save the complete validated JSON object for the current dashboard page.',
          parameters: {
            type: 'object',
            properties: {
              source: stringParameter(
                'Complete validated dashboard page JSON object.',
              ),
            },
            required: ['source'],
            additionalProperties: false,
          ...
          handler: async ({ source }) => {
            try {
              const { candidate, document, pageIndex, normalized } =
                await validateViewCandidate(source)
              document.dashboard.pages[pageIndex] = candidate
      ...
      const handleSessionEvent = (event) => {
        ...
        if (event.type === 'session.skills_loaded') {
            ...
            shell: event.data.shellToolInfo
              ? {
                  hasWriteFileRedirection:
                    event.data.shellToolInfo.hasWriteFileRedirection,
          ...
          const shellExit = event.data.result?.contents?.find(
            (content) =>
            ...
            shellExit: shellExit
              ? {
                  ...
                  output: truncatedLogText(
                    shellExit.type === 'shell_exit'
                      ? shellExit.outputPreview
                      : shellExit.text,
                  ),
                  outputTruncated:
                    shellExit.type === 'shell_exit'
          ...
          console.log('Copilot permission requested.', {
            ...
            permission:
              event.data.permissionRequest.kind === 'shell'
                ? {
          ...
          context.onEvent({
            type: 'status',
            message:
              'Copilot reported a session issue; checking the saved dashboard…',
          })
          context.onEvent({
            type: 'debug',
      ...
      if (!entry)
        try {
          ...
          const workspacePath = (path) =>
            canonicalPath(isAbsolute(path) ? path : join(workspaceRoot, path))
          session = await client.createSession({
            ...
            onPermissionRequest: async (permission) => {
              let decision = {
                kind: 'reject',
                feedback:
                  'This session permits file reads and writes in the workspace and temporary directory, plus common safe shell commands.',
              }
              ...
              try {
                if (permission.requestSandboxBypass) {
                  ...
                  if (
                    ...
                    reason = isWithin(temporaryRoot, requestedPath)
                      ? 'temporary directory read'
                      : 'workspace read'
                  } else {
                    reason =
                      'read path is outside workspace and temporary directory'
                  }
                  ...
                  if (
                    ...
                    reason =
                      'write path is outside workspace and temporary directory'
                  }
                  ...
                  if (rejection) {
                    ...
                    if (
                      ...
                      reason =
                        'shell path is outside workspace and temporary directory'
                    }
                  }
                }
              } catch (error) {
          ...
          console.log(
            'Copilot dashboard session creation failed.',
            errorMetadata(error),
          )
          throw error
        }
...
After saving the validated dashboard page, respond with a short, plain-language summary of what changed in the dashboard and what the user will now see. Avoid implementation details, JSON field names, schema terminology, file paths, and developer-oriented language. Complete the edit rather than only describing it.`,
        ...
        for (let repairAttempt = 1; ; repairAttempt += 1) {
          ...
          const validationErrors = formatDashboardValidationErrors(
            validation.errors,
          )
          console.log(
            'Copilot dashboard validation failed; continuing the session for repair.',
            {
              sessionId: session.sessionId,
              view,
              repairAttempt,
          ...
          onEvent({
            type: 'debug',
            message:
              'Authoritative dashboard validation failed; continuing the same Copilot session.',
            details: {
              view,
              repairAttempt,
              errorCount: validation.errors.length,
...
${validationErrors}`,
        ...
        console.log('Copilot dashboard request completed.', {
          sessionId: session.sessionId,
          view,
        })
    ...
    async close() {
      await Promise.all(
        [...sessions.keys()].map(async (sessionKey) => {
          await this.disconnect(sessionKey)
        }),
      )
      const errors = await client.stop()
      for (const error of errors)
        console.log('Copilot shutdown error.', errorMetadata(error))
    },
  }
}
...
export function isWithinCopilotFileRoots(
  workspaceRoot,
  temporaryRoot,
  candidate,
) {
  return (
    isWithin(workspaceRoot, candidate) || isWithin(temporaryRoot, candidate)
  )
}
...
function readWebsocketFrames(buffer) {
  ...
  while (buffer.length - offset >= 2) {
    ...
    for (let index = 0; index < length; index += 1) {
      payload[index] =
        buffer[payloadOffset + index] ^ buffer[maskOffset + (index % 4)]
    }
...
export async function startDashboardServer({
  ...
  if (copilot && !isLoopbackHost(host)) {
    ...
    throw new Error(
      'Copilot mode requires a loopback --host such as 127.0.0.1 or ::1.',
    )
  }
  const resolvedWorkingDirectory = await realpath(workingDirectory)
  const resolvedTraceFile = traceFile
    ? resolve(resolvedWorkingDirectory, traceFile)
    : null
  if (
    resolvedTraceFile &&
    !isWithin(resolvedWorkingDirectory, resolvedTraceFile)
  ) {
  ...
  const resolvedCatalogRoot = catalogRoot
    ? await canonicalPath(catalogRoot)
    : null
  const resolvedInstalledDashboardsDirectory = await canonicalPath(
    installedDashboardsDirectory,
  )
  if (
    !isWithin(resolvedWorkingDirectory, resolvedSiteRoot) ||
    (resolvedCatalogRoot &&
      !isWithin(resolvedWorkingDirectory, resolvedCatalogRoot)) ||
  ...
  const temporaryDirectory = await mkdtemp(
    join(resolvedWorkingDirectory, '.cao-dashboard-preview-'),
  )
  ...
  try {
    ...
    sourceManifestContent = JSON.stringify({
      version: 1,
      sources: [...splitSourceContent.keys()],
    })
    viewerContent = JSON.stringify(
      normalizeLocalViewer(await loadViewer(ghExecutable).catch(() => null)),
    )
  } catch (error) {
  ...
  const broadcastDashboard = (traceId) => {
    console.log('Broadcasting dashboard preview update.', {
  ...
  const runCopilotRequest = async (socket, payload) => {
    ...
    if (!copilotRuntime) {
      sendSocketEvent(socket, {
        type: 'error',
        traceId,
        message: 'Copilot mode is not available.',
      })
      return
    }
    ...
    try {
      const editableDashboardPaths = [
        baseDashboardPath,
        ...(await packageDashboardPaths(
          resolvedCatalogRoot,
          resolvedInstalledDashboardsDirectory,
        )),
      ]
      const viewDashboardPath = await dashboardSourceForView(
        payload.view,
        editableDashboardPaths,
      )
      ...
      if (promptError) {
        ...
        if (candidateSource === previousDashboardSource || !candidateValid)
          throw promptError
        console.log(
          'Copilot session failed after saving a valid dashboard; continuing.',
          {
            view: payload.view,
            viewDashboardPath,
        ...
        onEvent({
          type: 'status',
          message:
            'The dashboard change was saved; finishing the preview update…',
        })
      }
      onEvent({
        type: 'debug',
        message:
          'Copilot session completed; validating the saved dashboard source.',
        details: { view: payload.view },
      })
      const savedSource = await readFile(viewDashboardPath, 'utf8')
      ...
      try {
        ...
        console.log(
          'Copilot dashboard request left an invalid dashboard source.',
          {
            view: payload.view,
            viewDashboardPath,
            ...errorMetadata(error),
      ...
      sendSocketEvent(socket, {
        ...
        traceId,
        message:
          error?.code === 'DASHBOARD_PREVIEW_RELOAD_FAILED'
            ? error.message
    ...
    if (command?.type === 'copilot.start') {
      ...
      if (command.event === 'preview.rendered') {
        ...
        if (acknowledgement?.socket === socket) {
          const message =
            typeof command.details?.message === 'string'
          ...
          renderError.errorLog = truncatedLogText(
            command.details?.errorLog,
            6000,
          )
  ...
  const rebuild = async (notify = true, traceId, forceNotify = false) => {
    ...
    dashboardContent = redactJsonSecrets(
      await readFile(bundledDashboardPath, 'utf8'),
    )
  ...
  const refreshWatchers = async (packagePaths) => {
    ...
    for (const directory of await existingDirectories(candidates)) {
      ...
      watcher.on('error', (error) => {
        console.log(
          `Dashboard watcher failed for ${directory}: ${error.message}`,
        )
        watcher.close()
  ...
  const refresh = async () => {
    ...
    try {
      ...
      console.log(
        `Dashboard update failed: ${error instanceof Error ? error.message : String(error)}`,
      )
  ...
  const server = createServer(async (request, response) => {
    ...
    response.once('finish', () => {
      ...
      requestOutput(
        `${request.method || 'GET'} ${requestPath} ${response.statusCode} ${duration}ms`,
      )
    })
    try {
      if (
      ...
      if (
        ...
        url.searchParams.set(
          'local-preview',
          copilotRuntime ? 'copilot' : 'enabled',
        )
        response
          .writeHead(302, {
            Location: `${routePrefix}/${url.search}`,
      ...
      if (!contentTypes.has(extension)) {
        console.log('Refused unsupported dashboard file type.', {
      ...
      if (pathname === '/dashboard.json') content = dashboardContent
        ...
        content = browserSafeFileContent(
          canonicalFilePath,
          await readFile(canonicalFilePath),
        )
      sendContent(request, response, contentTypes.get(extension), content)
    } catch (error) {
      console.log(
        `Dashboard request failed: ${error instanceof Error ? error.message : String(error)}`,
      )
  ...
  server.on('upgrade', (request, socket) => {
    ...
    console.log('Dashboard preview socket connected.', {
      socketCount: sockets.size,
    })
      ...
      for (const [traceId, acknowledgement] of renderAcknowledgements) {
        if (acknowledgement.socket === socket) {
          acknowledgement.reject(
            new Error(
              'The browser disconnected before rendering the dashboard update.',
            ),
          )
          renderAcknowledgements.delete(traceId)
        }
      }
      console.log('Dashboard preview socket disconnected.', {
        socketCount: sockets.size,
      })
    }
    socket.on('data', (data) => {
      try {
        ...
        console.log(
          'Dashboard preview socket command failed.',
          errorMetadata(error),
        )
  ...
  return {
    ...
    ...(codespaceAuthority
      ? { codespaceUrl: `https://${codespaceAuthority}${routePrefix}` }
      : {}),
    async close() {
      if (closed) return
      closed = true
...
function parseArguments(arguments_) {
  ...
  if (
    !Number.isInteger(options.port ?? 4173) ||
    (options.port ?? 4173) < 1 ||
    (options.port ?? 4173) > 65535
  ) {
...
async function main() {
  ...
  if (options.help) {
    console.log(
      'usage: local-server.mjs [--copilot] [--replace-existing] [--trace-file PATH] [--repo OWNER/REPOSITORY] [--host HOST] [--port PORT]',
    )
    return
  }
  ...
  if (options.replaceExisting) {
    pidFile = await replaceExistingDashboardDevServer({
      workingDirectory,
      port,
    })
  }
