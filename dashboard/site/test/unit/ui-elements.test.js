...
describe('UI elements', () => {
          ...
          rows: [
            {
              ...
              repository: 'mona-tools',
              workflow: '.github/workflows/upgrade.lock.yml',
              'smell-id': 'overkill-for-agentic',
              'smell-name': 'Agent by default',
        ...
        workflows: {
          ...
          rows: [
            ...
            },
            {
              ...
              workflow: '.github/workflows/aw-doctor-failures.md',
              'workflow-name': 'AW Doctor / Failures',
              'workflow-active': 'true',
            },
            {
              organization: 'githubnext',
          ...
          rows: [
            {
              ...
              'agent-description': 'Reviews changes before merge.',
            },
            {
              'agent-id': 'absolute-path',
              'agent-name': '/tmp/review-agent.md',
              'agent-description': 'Invalid absolute path.',
            },
            {
              'agent-id': 'workflow-path',
              'agent-name': '.github/workflows/review.md',
              'agent-description': 'Invalid workflow path.',
            },
            {
              'agent-id': 'bracketed',
              'agent-name': '[aw] Failure Investigator',
              'agent-description': 'Invalid bracketed name.',
            },
            {
              'agent-id': 'numbered',
              'agent-name': '1. List all packages',
              'agent-description': 'Invalid numbered instruction.',
            },
          ],
        },
      },
        ...
        'operational-values': source([
          {
            ...
            'operational-value-definition': 'Accepted change',
          ...
          },
          {
            ...
            'operational-value-definition': 'Issue resolved',
            'maturity-status': 'matured',
            'observed-at': '2026-08-29T10:00:00Z',
          },
          {
        ...
        'detection-observations': source([
          {
            ...
            'detection-state-label': 'Clean',
            'usable-verdict-percent': 100,
          },
          {
            'detection-state': 'threat',
            'detection-state-label': 'Threat',
            'usable-verdict-percent': 100,
          },
        ]),
  ...
  it('groups packages in Board while keeping every Tasks and Roadmap item visible', () => {
    ...
    const render = (body) =>
      renderUiElement('work-project-view', {
        pageId: `work-${body}`,
        title: body,
        sourceNames: ['work-items'],
      ...
      sources: {
          ...
          rows: [
            {
              ...
              'workflow-link': {
                relation: 'workflow',
                href: 'https://github.com/github/mona-tools/actions/workflows/upgrade.yml',
                label: 'View workflow',
              },
        ...
        runs: {
          ...
          rows: [
            {
              run: '1',
              'started-at': new Date(Date.now() - 3_600_000).toISOString(),
              'run-status': 'completed',
              'run-conclusion': 'failure',
            },
            {
        ...
        'work-items': {
          ...
          rows: [
            {
              'work-item-id': 'blocked',
              'lifecycle-state': 'blocked',
              'waiting-since': new Date(Date.now() - 172_800_000).toISOString(),
            },
            {
              'work-item-id': 'review',
              'lifecycle-state': 'review',
              'waiting-since': new Date(Date.now() - 7_200_000).toISOString(),
            },
          ],
          metadata,
        },
  ...
  it('clusters repeated notification causes while leaving unique notifications visible', () => {
    ...
    const repeated = (id, scope) => ({
      'attention-signal-id': id,
      'signal-type': 'workflow-output',
      objective: 'Daily scan',
      scope,
      reason: 'Workflow completed with no safe outputs',
      'consequence-tier': 'medium',
      priority: 3,
      'age-seconds': 60,
    })
    const rendered = renderNotificationsInbox([
      repeated('repeat:1', 'github/one'),
      repeated('repeat:2', 'github/two'),
      {
        'attention-signal-id': 'unique',
        'signal-type': 'authority-gate',
  ...
  it('shows a positive quiet state when every attention class was evaluated', () => {
    ...
    const rendered = renderUiElement('home-attention-summary', {
      pageId: 'overview',
      title: 'Notifications',
      sourceNames: ['attention-signals', 'runs'],
      sources: {
        'attention-signals': {
          source: 'attention-signals',
          rows: [],
  ...
  it('qualifies a quiet state when source evidence is partial', () => {
    ...
    const rendered = renderUiElement('home-attention-summary', {
      ...
      sourceNames: ['runs', 'work-items', 'security-findings'],
      sources: {
        ...
        rows: [
          {
            ...
            'observed-at': '2026-08-28T10:05:00Z',
          },
        ],
        metadata,
      },
  ...
  it('renders workflow-route with declarative body selection', () => {
    ...
    rendered?.dispatchEvent(
      new CustomEvent('dashboard-route-change', {
        detail: {
          parameter: 'workflow',
          value: 'githubnext/gh-aw-cao:.github/workflows/sample.md',
        },
      }),
    )
