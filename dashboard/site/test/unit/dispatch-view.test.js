...
const dispatchPage = dashboard.dashboard.pages.find(
  (/** @type {{ id: string }} */ page) => page.id === 'dispatches',
)
const metadata = {
  'source-id': 'fixture',
  'source-kind': 'fixture',
...
describe('declarative dispatch view', () => {
  it('renders dispatch JSON through the generic table and cell elements', () => {
    ...
    const dispatchTable = dispatchPage.views.find(
      (/** @type {{ id: string }} */ view) =>
        view.id === 'package-worker-dispatches',
    )
    ...
    const pageFilter = /** @type {HTMLInputElement | null} */ (
      rendered.querySelector('.filter-bar input')
    )
    expect(pageFilter?.value).toBe('')
    const tables = rendered.querySelectorAll('table')
    expect(tables).toHaveLength(1)
    expect(
      [...tables[0].querySelectorAll('thead tr:first-child th')].map(
    ...
    expect(
      rendered.querySelector('.table-summary-temporal')?.textContent,
    ).toContain('StartAug 30, 2026, 7:00 AM')
    expect(
      rendered.querySelector('.table-summary-temporal')?.textContent,
    ).toContain('StopAug 30, 2026, 7:00 AM')
    expect(
      rendered.querySelector('.table-summary-temporal')?.textContent,
    ).toContain('Duration0s')
    expect(
      rendered
        .querySelector('.table-filter input[type="search"]')
    ...
    expect(
      rendered.querySelector(
        'tbody td a[href="https://github.com/githubnext/control/actions/runs/3"]',
      ),
    ).not.toBeNull()
    expect(
      rendered.querySelector(
        'a[href="https://github.com/githubnext/control/blob/HEAD/worker.yml"]',
      )?.textContent,
    ).toBe('Dependency updater')
    expect(
      rendered.querySelector('a[href="https://github.com/githubnext/control"]')
        ?.textContent,
    ).toBe('githubnext/control')
  })
})
