...
describe('table cell display helper', () => {
  it('DLS-VIEW-004 renders JSON-selected display types through one generic helper', () => {
    ...
    const repositoryStatus = renderCellDisplay(
      'status',
      'Needs attention',
      toText,
    )
    const graderPass = renderCellDisplay('grader-status', 'pass', toText)
    const graderUnavailable = renderCellDisplay(
      'grader-status',
      'unavailable',
      toText,
    )
    ...
    expect(/** @type {HTMLElement} */ (mode).className).toBe(
      'mode-badge mode-live',
    )
    expect(activeState).toBeInstanceOf(HTMLElement)
    expect(/** @type {HTMLElement} */ (activeState).className).toBe(
      'status status-success',
    )
    expect(status).toBeInstanceOf(HTMLElement)
    expect(/** @type {HTMLElement} */ (status).className).toBe(
      'status status-danger',
    )
    expect(/** @type {HTMLElement} */ (repositoryStatus).className).toBe(
      'status status-danger',
    )
    expect(/** @type {HTMLElement} */ (graderPass).className).toBe(
      'status status-success',
    )
    expect(/** @type {HTMLElement} */ (graderUnavailable).className).toBe(
    ...
    expect(
      /** @type {HTMLElement} */ (
        renderCellDisplay('digest', '1234567890abcdef', toText)
      ).textContent,
    ).toBe('1234567890ab')
    ...
    const workflowRun = /** @type {HTMLAnchorElement} */ (
      renderCellDisplay(
        undefined,
    ...
    expect(
      renderCellDisplay(undefined, 2.5, toText, {
        name: 'AI Credits',
        symbol: 'AIC',
        significant: 1,
      }),
