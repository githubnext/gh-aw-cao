...
describe('canonical ingestion errors', () => {
  it('classifies quota, incomplete, validation, and transaction failures', () => {
    ...
    expect(
      classifyIngestionError(
        new Error('Generation a is incomplete'),
        'activating',
      ),
    ).toBe(INGESTION_ERROR_CODES.generationIncomplete)
    expect(
      classifyIngestionError(
        new Error('Generation relationship validation failed'),
        'activating',
      ),
    ).toBe(INGESTION_ERROR_CODES.generationValidationFailed)
    expect(classifyIngestionError(new Error('write failed'), 'staging')).toBe(
      INGESTION_ERROR_CODES.transactionAborted,
    )
  })
  ...
  it('retains the underlying cause and safe diagnostic fields', () => {
    ...
    const error = new CanonicalIngestionError(
      'TRANSACTION_ABORTED',
      'staging',
      'generation-a',
      cause,
