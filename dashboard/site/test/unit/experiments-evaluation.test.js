...
describe('experiments and evaluation', () => {
  it('defines reusable experiments view compositions by declarative body', () => {
    expect(experimentsViewCompositionForBody('overview')).toEqual({
      key: 'overview',
      className: 'experiment-overview',
    })
    expect(experimentsViewCompositionForBody('table')).toEqual({
      key: 'table',
      className: 'experiment-decision-table',
    })
    expect(experimentsViewCompositionForBody('detail')).toEqual({
      key: 'detail',
      className: 'experiment-detail',
    })
    expect(experimentsViewCompositionForBody('unknown')).toEqual({
      key: 'detail',
      className: 'experiment-detail',
    })
  ...
  it('renders reusable decision surface sections by declarative composition key', () => {
    ...
    expect(
      renderExperimentDecisionSurfaceSection('overview', {
        experiments: [{ id: 'one' }],
    ...
    expect(
      renderExperimentDecisionSurfaceSection('table', {
        experiments: [{ id: 'one' }],
        model: /** @type {any} */ ({ experiments: [] }),
        selectedExperiment: 'one',
        onSelect: () => {},
    ...
    expect(
      renderExperimentDecisionSurfaceSection('detail', {
        experiments: [{ id: 'one' }],
  ...
  it('keeps decisions, observations, producers, runs, and evidence distinct', () => {
    ...
    const evidenceLink = {
      href: 'https://github.com/acme/tools/actions/runs/101/artifacts/5',
      label: 'Open grader evidence',
    }
    const sources = {
      ...
      'grader-observations': source('grader-observations', [
        ...
        },
        {
          ...
          grader: 'quality',
          status: 'missing',
          'exclusion-reason': 'grader missing',
        },
      ]),
      evals: source('evals', [
        {
          eval: 'answer-correct',
          'eval-question': 'Is the answer correct?',
          role: 'SECONDARY',
        },
      ]),
