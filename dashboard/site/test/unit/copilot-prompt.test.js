...
describe('Copilot dashboard prompt', () => {
  it('renders an integrated chat and streams the Copilot response', async () => {
    document.body.innerHTML =
      '<a data-nav-page-id="overview" aria-current="page" aria-label="Overview"></a>'
    const debugMock = vi.spyOn(console, 'debug').mockImplementation(() => {})
    const socket = new MockSocket()
    const prompt = renderCopilotPrompt(
      /** @type {WebSocket} */ (/** @type {unknown} */ (socket)),
    )
    ...
    expect(prompt.querySelector('#dashboard-copilot-status')?.textContent).toBe(
      'Working…',
    )
    socket.emit({
      type: 'tool-refused',
    ...
    expect(prompt.querySelector('label')).toBeNull()
    expect(prompt.querySelector('#dashboard-copilot-title')?.textContent).toBe(
      'Modify this view',
    )
    expect(
      prompt
        .querySelector('#dashboard-copilot-title')
        ?.classList.contains('sr-only'),
    ).toBe(true)
    expect(prompt.querySelector('dialog')).toBeNull()
    expect(
      prompt.querySelector('.dashboard-copilot-message-user')?.textContent,
    ).toContain('Add a trend')
    expect(
      prompt.querySelector('.dashboard-copilot-message-assistant strong'),
    ).toBeNull()
    expect(
      prompt.querySelector('.dashboard-copilot-message-update')?.textContent,
    ).toBe('Reading the current view…')
    expect(
      prompt.querySelector('.dashboard-copilot-message-refusal')?.textContent,
    ).toBe('Refused python3: shell command not allowed.')
    expect(
      prompt.querySelector('.dashboard-copilot-message-reasoning')?.textContent,
    ).toBe('A trend card will make the change clearer.')
    expect(
      prompt.querySelectorAll('.dashboard-copilot-message-reasoning'),
    ).toHaveLength(1)
    expect(
      [...prompt.querySelectorAll('.dashboard-copilot-message-content')].every(
        (message) => message.textContent?.trim(),
      ),
    ).toBe(true)
    expect(
      prompt.querySelector('.dashboard-copilot-message-response')?.textContent,
    ).toBe('Added a trend.')
    expect(prompt.querySelector('#dashboard-copilot-status')?.textContent).toBe(
      'Updated.',
    )
    ...
    const start = socket.sent.find(
      (message) => message.type === 'copilot.start',
    )
    ...
    expect(prompt.querySelectorAll('.dashboard-copilot-message')).toHaveLength(
      9,
    )
    expect(
      prompt.querySelector('.dashboard-copilot-message-error')?.textContent,
    ...
    expect(
      prompt.querySelector('.dashboard-copilot-conversation')?.textContent,
  ...
  it('changes the action icon to cancel while a session is active', async () => {
    ...
    if (!(actionButton instanceof HTMLButtonElement))
      throw new Error('Expected Copilot action button')
