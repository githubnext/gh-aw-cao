...
describe('notifications inbox large data', () => {
  ...
  it('keeps search, grouping, bulk actions, and deep links working with normalized stories', () => {
    ...
    selectAll.click();
    /** @type {HTMLButtonElement} */ (rendered.querySelector('[aria-label="Acknowledge selected locally"]')).click();
...
describe('catch up queue', () => {
  ...
  it('separates human decisions from automated watching and exposes supervision context', () => {
    ...
    expect(rendered.querySelector('[data-responsibility="automated"]')?.textContent).toContain('Expected actorCoordinator');
  ...
  it('routes completion to Later in Notifications while excluding Done stories after refresh', () => {
    ...
    /** @type {HTMLButtonElement} */ (refreshed.querySelector('.notifications-main [aria-label="Acknowledge locally (does not resolve work)"]')).click();
