...
describe('live Dashboard Language sources', () => {
  ...
  it('maps the operations report inputs into canonical logical sources', () => {
    ...
    try {
      ...
      expect(sources['security-findings'].rows).toContainEqual(
      ...
      expect(sources['agent-smells'].rows).toContainEqual(
        expect.objectContaining({
          workflow: '.github/workflows/dependabot.md',
          run: '42',
      ...
      expect(sources['workflow-smells'].rows).toEqual(
      ...
      expect(sources['control-plane-smells'].rows).toContainEqual(
        expect.objectContaining({
      ...
      expect(sources['control-plane-smells'].rows).toContainEqual(
        expect.objectContaining({
          ...
          'repository-link': expect.objectContaining({
