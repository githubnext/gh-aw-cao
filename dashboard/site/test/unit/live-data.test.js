...
describe("live Dashboard Language sources", () => {
  it("loads generated sources progressively and requires an explicit fixture opt-in", () => {
    ...
    expect(preview.indexOf('fetch(dashboardUrl, { cache: "no-store" })')).toBeLessThan(preview.indexOf("await loadCanonicalDashboardSources("));
    expect(preview.indexOf("startLoadingProgress(document)")).toBeLessThan(preview.indexOf('fetch(dashboardUrl, { cache: "no-store" })'));
