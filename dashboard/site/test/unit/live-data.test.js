...
describe("live Dashboard Language sources", () => {
  it("loads generated sources progressively and requires an explicit fixture opt-in", () => {
    ...
    expect(startup).toContain("bindContinuations(initialPageId, sources, initialLazySources)");
    expect(startup).toMatch(/loadCanonicalDashboardPage\(DATABASE_COUNT_SOURCE_NAMES,/);
    expect(startup.indexOf("cachedSources = await loadInitialSources(")).toBeLessThan(startup.indexOf("loadCanonicalDashboardSources("));
    expect(startup).toContain('render(displayedSources, "cached", loadPageSources, loadHorizonSources)');
    expect(startup).toContain('render(displayedSources, "stale", loadPageSources, loadHorizonSources, refreshSources)');
    expect(startup.indexOf('render(displayedSources, "cached"')).toBeLessThan(startup.indexOf("await settleUi()"));
