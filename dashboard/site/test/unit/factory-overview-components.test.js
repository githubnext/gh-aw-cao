...
describe('Overview component boundaries', () => {
  ...
  it('floor composes selected stations and owns their aggregate accessible summary', () => {
    ...
    const rendered = renderFactoryFloor(sources, false, { signal: controller.signal }, ['campaigns', 'repositories']);
  ...
  it('header presents heading priority, outcome summary, and rhythm composition', () => {
    const controller = new AbortController();
    const sources = {
      'overview-header-presentation': binding({ rows: [{
        heading: 'Your campaigns are delivering value.',
        summary: '5 retained issue and pull request outputs are backed by Actions evidence across 4 repositories.'
      }] }),
