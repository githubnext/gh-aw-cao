...
it('renders the factory header from only its declared JSON sources', () => {
  const rendered = renderUiElement('factory-header', context('factory-header', {
    'overview-header-presentation': source('overview-header-presentation', [{
      heading: 'Your campaigns are delivering value.',
      summary: '2 retained issue and pull request outputs are backed by Actions evidence across 3 repositories.'
    }]),
...
it('stops updating an element after its rendered root is removed', async () => {
  const first = renderUiElement('factory-header', context('factory-header', {
    'overview-header-presentation': source('overview-header-presentation', [{ heading: 'First status', summary: '' }]),
    'overview-rhythm': source('overview-rhythm', [{ rhythm }])
  }));
  ...
  renderUiElement('factory-header', context('factory-header', {
    'overview-header-presentation': source('overview-header-presentation', [{ heading: 'Second status', summary: '' }]),
    'overview-rhythm': source('overview-rhythm', [{ rhythm }])
  }));
