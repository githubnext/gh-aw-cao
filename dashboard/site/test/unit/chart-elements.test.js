...
import {
  ...
  renderChartWidget,
  renderPieLegend,
} from '../../src/components/chart-elements.js'
...
describe('chart element helpers', () => {
  it('DLS-SAFE-009 groups chart series deterministically and lists reusable class names', () => {
    ...
    expect(listChartSeries(points)).toEqual([
      {
        name: 'fail',
        className: 'chart-series-1 chart-series-semantic-failure',
      },
      {
        name: 'pass',
        className: 'chart-series-2 chart-series-semantic-success',
      },
    ...
    expect(
      listChartSeries(expandedSeries).map(({ className }) => className),
    ).toEqual([
      'chart-series-1',
      'chart-series-2',
      'chart-series-3',
  ...
  it('assigns color-blind-safe semantic colors before falling back to the palette', () => {
    expect(chartSeriesClassName('startup-failure', 0)).toBe(
      'chart-series-1 chart-series-semantic-failure',
    )
    expect(chartSeriesClassName('timed_out', 1)).toBe(
      'chart-series-2 chart-series-semantic-failure',
    )
    expect(chartSeriesClassName('Success', 2)).toBe(
      'chart-series-3 chart-series-semantic-success',
    )
    expect(chartSeriesClassName('waiting for approval', 3)).toBe(
      'chart-series-4 chart-series-semantic-waiting',
    )
    expect(chartSeriesClassName('action_required', 4)).toBe(
      'chart-series-5 chart-series-semantic-waiting',
    )
    expect(chartSeriesClassName('in-progress', 5)).toBe(
      'chart-series-6 chart-series-semantic-waiting',
    )
    expect(chartSeriesClassName('cancelled', 6)).toBe(
      'chart-series-7 chart-series-semantic-attention',
    )
    expect(chartSeriesClassName('skipped', 7)).toBe(
      'chart-series-8 chart-series-semantic-neutral',
    )
  ...
  it('explains scatter chart scale guides in the legend', () => {
    ...
    expect(legend.querySelectorAll('li')).toHaveLength(2)
    expect(legend.querySelector('li:last-child i')?.className).toBe(
      'chart-grid-key',
    )
    expect(legend.querySelector('li:last-child')?.textContent).toBe(
      'Dashed lines show scale guides',
    )
  })
  ...
  it('DLS-SAFE-009 renders reusable pie legends including zero-total fallback percentages', () => {
    ...
    expect(populated.querySelector('a')?.getAttribute('href')).toBe(
      'https://github.com/octo-org/open',
    )
    expect(populated.querySelector('a')?.getAttribute('aria-label')).toBe(
      'View octo-org/open on GitHub',
    )
    ...
    expect(expanded.querySelector('li:last-child i')?.className).toBe(
      'chart-series-12',
    )
    ...
    expect(
      semantic
        .querySelector('li:nth-child(1) i')
        ?.classList.contains('chart-series-semantic-failure'),
    ).toBe(true)
    expect(
      semantic
        .querySelector('li:nth-child(2) i')
        ?.classList.contains('chart-series-semantic-success'),
    ).toBe(true)
    expect(
      semantic
        .querySelector('li:nth-child(3) i')
        ?.classList.contains('chart-series-semantic-waiting'),
    ).toBe(true)
  })
  ...
  it('renders a single-category pie distribution', () => {
    const singleCategoryPie = renderChartWidget(
      'pie',
      [
    ...
    expect(
      singleCategoryPie.querySelector('[data-chart-category="only"]'),
    ).not.toBeNull()
    expect(
      singleCategoryPie.querySelector('.pie-chart-total-value')?.textContent,
    ).toBe('3')
  })
  ...
  it('renders compact heatmaps as accessible labeled tables without relying on color', () => {
    ...
    expect(chart.querySelector('.heatmap-chart caption')?.textContent).toBe(
      'Heatmap of Mean job time',
    )
    expect(
      [...chart.querySelectorAll('thead th')].map((cell) => cell.textContent),
    ).toEqual(['build', 'test'])
    expect(
      [...chart.querySelectorAll('tbody th')].map((cell) => cell.textContent),
    ).toEqual(['macos', 'ubuntu'])
    expect(chart.querySelectorAll('.heatmap-cell')).toHaveLength(4)
    expect(
      [...chart.querySelectorAll('.heatmap-cell[tabindex="0"]')].map((cell) =>
        cell.getAttribute('aria-label'),
      ),
    ).toContain('build, ubuntu, Mean job time: 62 s')
    expect(
      chart.querySelector('.heatmap-cell-empty')?.getAttribute('aria-label'),
    ).toBe('test, macos: no observation')
    expect(
      chart.querySelector('.heatmap-cell-empty')?.getAttribute('tabindex'),
    ).toBe('0')
    expect(
      [...chart.querySelectorAll('.heatmap-cell[tabindex="0"]')].map(
  ...
  it('rejects oversized heatmaps with a visible status instead of rendering a dense matrix', () => {
    ...
    expect(chart.querySelector('[role="status"]')?.textContent).toContain(
      '12 categories per axis',
    )
  })
  ...
  it('renders categorical workflow runs as accessible swimlanes without connecting marks', () => {
    const points = [
      {
        ...
        source: {
          run: '1840',
          'started-at': '2026-08-28T08:00:00Z',
          'ended-at': '2026-08-28T08:03:18Z',
        },
      },
      {
        ...
        source: {
          run: '1842',
          'started-at': '2026-08-29T12:48:37Z',
          'ended-at': '2026-08-29T12:51:55Z',
          branch: 'main',
        },
      },
      {
    ...
    expect(
      [...chart.querySelectorAll('.swimlane-label')].map(
        (label) => label.textContent,
      ),
    ).toEqual(['Action required', 'Failure', 'Cancelled', 'Skipped', 'Success'])
    expect(chart.querySelectorAll('.swimlane-mark')).toHaveLength(5)
    expect(chart.querySelector('polyline')).toBeNull()
    expect(chart.querySelector('.swimlane-summary')?.textContent).toContain(
      '5 runs',
    )
    expect(chart.querySelector('.swimlane-summary')?.textContent).toContain(
      '20.0% success',
    ...
    expect(
      chart.querySelector('.swimlane-mark-failure')?.getAttribute('aria-label'),
    ).toContain('Conclusion: failure')
    expect(
      chart.querySelector('.swimlane-mark-failure')?.getAttribute('aria-label'),
    ).toContain('Run #1842')
    expect(
  ...
  it('renders 100,000 swimlane observations as bounded line intervals', () => {
    ...
    expect(
      [...marks].reduce(
        (total, mark) =>
          total + Number(mark.getAttribute('data-swimlane-count')),
        0,
      ),
    ).toBe(100_000)
  })
  ...
  it('DLS-VIEW-005 DLS-VIEW-006 DLS-VIEW-007 renders JSON-selected chart marks through one generic helper', () => {
    ...
    const bar = renderChartWidget(
      'bar',
      [...points, { x: 'invalid', y: Number.NaN, color: null }],
      series,
    )
    const line = renderChartWidget('line', points, series)
    const pie = renderChartWidget('pie', points, series)
    const histogram = renderChartWidget(
      'histogram',
      [
    ...
    expect(bar.querySelector('.bar-chart-bar')?.getAttribute('height')).toBe(
      chartHeight,
    )
    expect(bar.querySelector('.bar-chart-bar')?.getAttribute('rx')).toBe('0.75')
    expect(
      bar.querySelector('.bar-chart-bar')?.getAttribute('style'),
    ).toContain('--chart-entry-index: 0')
    expect(
      [...bar.querySelectorAll('.bar-chart-bar')]
        .at(-1)
        ?.getAttribute('height'),
    ).toBe('1')
    expect(
      [...bar.querySelectorAll('.bar-chart-y-axis text')].map(
        (tick) => tick.textContent,
      ),
    ).toEqual(['3', '1.50', '0'])
    expect(
      [...bar.querySelectorAll('.bar-chart-x-axis text')].map(
        (tick) => tick.lastChild?.textContent,
      ),
    ).toEqual(['Aug 29', 'Aug 30', 'invalid'])
    ...
    expect(
      line.querySelector('.line-chart-series')?.getAttribute('pathLength'),
    ).toBe('1')
    expect(
      line.querySelector('.line-chart-series')?.getAttribute('style'),
    ).toContain('--chart-entry-index: 0')
    expect(
      line.querySelector('.line-chart-point')?.getAttribute('style'),
    ).toContain('--chart-point-size: 6px')
    expect(line.querySelector('.chart-point')?.getAttribute('style')).toContain(
      '--chart-entry-index: 0',
    )
    expect(
      [...line.querySelectorAll('.timeline-chart-axis span')].map(
        (tick) => tick.textContent,
      ),
    ).toEqual(['Aug 29', 'Aug 30'])
    expect(pie.getAttribute('data-chart-widget')).toBe('pie')
    expect(pie.querySelectorAll('.pie-chart-segment')).toHaveLength(2)
    expect(
      pie.querySelector('.pie-chart-mark')?.getAttribute('style'),
    ).toContain('--chart-entry-index: 0')
    expect(
      pie.querySelector('.pie-chart-track')?.getAttribute('stroke-width'),
    ).toBe('6')
    expect(pie.querySelector('.pie-chart-segment')?.tagName).toBe('path')
    expect(
      pie.querySelector('.pie-chart-segment')?.getAttribute('stroke-width'),
    ).toBe('6')
    expect(
      pie.querySelector('.pie-chart-segment')?.getAttribute('stroke-linecap'),
    ).toBe('round')
    expect(
      [...pie.querySelectorAll('.pie-chart-segment')].map((segment) =>
    ...
    expect(
      pie.querySelector('.pie-chart-segment')?.hasAttribute('stroke-dasharray'),
    ).toBe(false)
    expect(pie.querySelectorAll('.pie-chart-mark .point-tooltip')).toHaveLength(
      2,
    )
    expect(
      pie.querySelector('.pie-chart-mark')?.getAttribute('aria-label'),
    ).toBe('2026-08-29: 3')
    expect(
      pie.querySelector('.pie-chart-tooltip rect')?.getAttribute('width'),
    ).toBe('21.25')
    ...
    expect(
      histogram.querySelector('.histogram-chart-mark')?.getAttribute('style'),
    ).toContain('--chart-entry-index: 0')
    expect(
      histogram.querySelectorAll('.histogram-chart-mark .point-tooltip'),
    ).toHaveLength(3)
    expect(histogram.querySelectorAll('.histogram-chart-grid')).toHaveLength(3)
    expect(
      [...histogram.querySelectorAll('.histogram-chart-y-label')].map(
        (tick) => tick.textContent,
      ),
    ).toEqual(['2', '1', '0'])
    expect(
      [...histogram.querySelectorAll('[data-chart-axis="histogram"] span')].map(
        (tick) => tick.textContent,
      ),
    ).toEqual(['3 AIC', '5 AIC', '7 AIC', '9 AIC'])
    expect(
      histogram
        .querySelector('.histogram-chart-bar')
        ?.classList.contains('chart-series-1'),
    ).toBe(true)
    expect(
      histogram.querySelector('.histogram-chart-bar')?.getAttribute('rx'),
    ).toBe('0.75')
    expect(
      histogram
        .querySelector('.histogram-chart-tooltip text')
        ?.getAttribute('lengthAdjust'),
    ).toBe('spacingAndGlyphs')
    expect(
      histogram.querySelector('svg')?.getAttribute('aria-label'),
    ).toContain('automatically calculated bins')
    expect(
      histogram
        .querySelector('.histogram-chart-mark')
    ...
    expect(histogram.querySelector('.histogram-chart-mark:last-child')).toBe(
      firstHistogramMark,
    )
    expect(
      unitPie.querySelector('.pie-chart-mark')?.getAttribute('aria-label'),
    ).toBe('2026-08-29: 3 AIC')
    expect(unitPie.querySelector('.pie-chart-total-value')?.textContent).toBe(
      '4',
    )
  ...
  it('renders temporal dot observations with per-series reference lines', () => {
    ...
    expect(
      dot
        .querySelector('[data-chart-reference="core"]')
        ?.getAttribute('data-chart-reference-value'),
    ).toBe('5000')
    expect(dot.querySelector('.line-chart-series')).toBeNull()
    expect(dot.querySelector('svg')?.getAttribute('aria-label')).toBe(
      'Dot chart with 3 points and 2 reference lines',
    )
  })
  ...
  it('renders temporal scatter observations at proportional timestamps without connecting them', () => {
    ...
    const scatter = renderChartWidget(
      'scatter',
      points,
      listChartSeries(points),
      null,
    ...
    expect(
      [...scatter.querySelectorAll('.timeline-chart-axis span')].map((tick) =>
        tick.getAttribute('title'),
      ),
    ).toEqual([
    ...
    expect(scatter.querySelector('svg')?.getAttribute('aria-label')).toBe(
      'Scatter chart with 3 points',
    )
  })
  ...
  it('exposes scatter cluster sizes in accessible point labels', () => {
    const scatter = renderChartWidget(
      ...
      [{ name: 'max 5000', className: 'chart-series-1' }],
      null,
      'Remaining',
      {
        name: 'Percent',
    ...
    expect(
      scatter.querySelector('.chart-point')?.getAttribute('aria-label'),
    ).toContain('cluster of 250 observations')
  })
  ...
  it('dims historical line context and emphasizes the selected window', () => {
    ...
    expect(
      Number(
        line.querySelector('.line-chart-window-band')?.getAttribute('width'),
      ),
    ).toBeGreaterThan(0)
    expect(
      line.querySelector('.line-chart-current')?.getAttribute('points'),
    ).not.toBe('')
    expect(line.querySelectorAll('.chart-point-context')).toHaveLength(1)
    expect(
      line
        .querySelector('.chart-point-context .line-chart-point')
        ?.getAttribute('style'),
    ).toContain('--chart-point-size: 4px')
    expect(
      line
        .querySelector('.chart-point-current .line-chart-point')
        ?.getAttribute('style'),
    ).toContain('--chart-point-size: 4px')
    expect(line.querySelector('.chart-window-key')?.textContent).toContain(
      'Selected window',
    )
  })
  ...
  it('reduces non-scaling line-chart point sizes as the number of points increases', () => {
    ...
    expect(
      renderPoints(2).querySelector('.line-chart-point')?.getAttribute('style'),
    ).toContain('--chart-point-size: 6px')
    expect(
      renderPoints(51)
        .querySelector('.line-chart-point')
        ?.getAttribute('style'),
    ).toContain('--chart-point-size: 4px')
    expect(
      renderPoints(100)
        .querySelector('.line-chart-point')
        ?.getAttribute('style'),
    ).toContain('--chart-point-size: 2px')
    expect(
      renderPoints(150)
        .querySelector('.line-chart-point')
        ?.getAttribute('style'),
  ...
  it('renders 100,000 line-chart points in bounded time and SVG size', () => {
    ...
    const renderedPoints =
      chart
        .querySelector('.line-chart-series')
        ?.getAttribute('points')
        ?.split(' ') ?? []
  ...
  it('shares the rendered point budget across line series', () => {
    ...
    const renderedPointCount = [
      ...chart.querySelectorAll('.line-chart-series'),
    ].reduce(
      (total, line) =>
        total + (line.getAttribute('points')?.split(' ').length ?? 0),
      0,
    )
  ...
  it('caps each series to its two endpoints when the series count collapses the per-series budget', () => {
    ...
    const points = Array.from(
      { length: seriesCount * pointsPerSeries },
      (_, index) => ({
    ...
    expect(
      seriesLines.every(
        (line) => (line.getAttribute('points')?.split(' ').length ?? 0) <= 2,
      ),
    ).toBe(true)
    expect(chart.querySelectorAll('.chart-point')).toHaveLength(0)
  })
  ...
  it('renders concise, evenly sampled bar axes while preserving exact category values', () => {
    ...
    expect(
      [...chart.querySelectorAll('.bar-chart-x-axis text > title')].map(
        (title) => title.textContent,
      ),
    ).toEqual([points[0].x, points[2].x, points[4].x, points[6].x, points[8].x])
    expect(
      [...chart.querySelectorAll('.bar-chart-y-axis text')].map(
        (tick) => tick.textContent,
      ),
    ).toEqual(['200 AIC', '100 AIC', '0 AIC'])
  })
})
