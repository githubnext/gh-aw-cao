...
describe('chart element helpers', () => {
  ...
  it('explains scatter chart scale guides in the legend', () => {
    const legend = renderChartLegend([{ name: 'core · app · max 5000', className: 'chart-series-1' }], 'scatter')
  ...
  it('renders compact heatmaps as accessible labeled tables without relying on color', () => {
    ...
    expect([...chart.querySelectorAll('.heatmap-cell[tabindex="0"]')].map((cell) => cell.getAttribute('aria-label'))).toContain(
      'build, ubuntu, Mean job time: 62 s',
    )
  ...
  it('renders categorical workflow runs as accessible swimlanes without connecting marks', () => {
    ...
    const chart = renderChartWidget('swimlane', points, listChartSeries(points), null, 'Total', null, {
  ...
  it('DLS-VIEW-005 DLS-VIEW-006 DLS-VIEW-007 renders JSON-selected chart marks through one generic helper', () => {
    ...
    expect([...bar.querySelectorAll('.bar-chart-x-axis text')].map((tick) => tick.lastChild?.textContent)).toEqual(['Aug 29', 'Aug 30', 'invalid'])
    ...
    expect([...line.querySelectorAll('.timeline-chart-axis span')].map((tick) => tick.textContent)).toEqual(['Aug 29', 'Aug 30'])
    ...
    expect([...histogram.querySelectorAll('.histogram-chart-y-label')].map((tick) => tick.textContent)).toEqual(['2', '1', '0'])
    expect([...histogram.querySelectorAll('[data-chart-axis="histogram"] span')].map((tick) => tick.textContent)).toEqual(['3 AIC', '5 AIC', '7 AIC', '9 AIC'])
  ...
  it('shares the rendered point budget across line series', () => {
    ...
    const chart = renderChartWidget('line', points, listChartSeries(points))
    const renderedPointCount = [...chart.querySelectorAll('.line-chart-series')].reduce(
      (total, line) => total + (line.getAttribute('points')?.split(' ').length ?? 0),
      0,
    )
  ...
  it('renders a concise, evenly sampled timeline axis while preserving exact values', () => {
    ...
    expect(ticks.map((tick) => tick.textContent)).toEqual(['Sep 1, 00:15 UTC', 'Sep 3, 02:15 UTC', 'Sep 5, 04:15 UTC', 'Sep 7, 06:15 UTC', 'Sep 9, 08:15 UTC'])
    expect(ticks.map((tick) => tick.getAttribute('title'))).toEqual([points[0].x, points[2].x, points[4].x, points[6].x, points[8].x])
  })
  ...
  it('renders concise, evenly sampled bar axes while preserving exact category values', () => {
    ...
    expect(xTicks.map((tick) => tick.lastChild?.textContent)).toEqual(['category-wi…', 'category-wi…', 'category-wi…', 'category-wi…', 'category-wi…'])
    expect(xTicks.map((tick) => tick.getAttribute('title'))).toEqual([points[0].x, points[2].x, points[4].x, points[6].x, points[8].x])
    ...
    expect([...chart.querySelectorAll('.bar-chart-y-axis text')].map((tick) => tick.textContent)).toEqual(['200 AIC', '100 AIC', '0 AIC'])
  })
})
