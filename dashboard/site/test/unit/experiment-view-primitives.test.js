...
import {
  numericObservation,
  safeExperimentLink,
} from '../../src/components/experiment-view-primitives.js'
...
describe('numericObservation', () => {
  it('maps eval-sourced YES/NO results to 1/0 and unknown results to NaN', () => {
    ...
    expect(
      numericObservation({ sourceType: 'eval', result: 'MAYBE' }),
    ).toBeNaN()
  })

  it('parses non-eval results as finite numbers, otherwise NaN', () => {
    expect(numericObservation({ sourceType: 'metric', result: '3.5' })).toBe(
      3.5,
    )
    expect(
      numericObservation({ sourceType: 'metric', result: 'not-a-number' }),
    ).toBeNaN()
  })
})
...
describe('safeExperimentLink', () => {
  it('returns null for non-object, missing, or unsafe-scheme values', () => {
    ...
    expect(
      safeExperimentLink({ href: 'javascript:alert(1)', label: 'bad' }),
    ).toBeNull()
  })
