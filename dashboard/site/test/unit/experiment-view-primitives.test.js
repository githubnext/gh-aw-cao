...
describe('safeExperimentLink', () => {
  ...
  it('returns a trimmed href/label pair for a safe https link', () => {
    expect(safeExperimentLink({ href: 'https://example.com/run', label: '  Run 1  ' })).toEqual({
  ...
  it('defaults the label to an empty string when absent', () => {
    expect(safeExperimentLink({ href: 'https://example.com/run' })).toEqual({ href: 'https://example.com/run', label: '' });
  });
});
