...
describe('renderTableRegion', () => {
  ...
  it('adds reusable data summaries to the table header', () => {
    const rendered = renderTableRegion({
      ...
      bodyRows: [h('tr', null, h('td', null, 'open'), h('td', null, '1'))],
    })

    expect(rendered.querySelectorAll('thead tr')).toHaveLength(2)
    expect(
      rendered.querySelector('.table-summary-categories')?.textContent,
    ).toContain('open66.7%')
  ...
  it('preserves the custom table view data attribute', () => {
    ...
    expect(
      table.querySelector('table')?.getAttribute('data-custom-view-mark'),
    ).toBe('table')
  })
  ...
  it('accepts keyed-list descriptors as populated body rows', async () => {
    ...
    const rendered = renderTableRegion({
      ...
      bodyRows: keyed(
        ...
        (item) => {
          ...
          return h(
            'tr',
            { 'data-key': keyedItem.key },
            h('td', null, keyedItem.key),
            h('td', null, 'YES: 1'),
          )
        },
        /** @param {unknown} item */
    ...
    expect(rendered.querySelector('tbody')?.textContent).toContain(
      'release-risk',
    )
    expect(rendered.querySelector('tbody')?.textContent).not.toContain(
      'No eval definitions available.',
    )
  })
  ...
  it('filters rows and announces the visible result count', () => {
    ...
    const input = /** @type {HTMLInputElement} */ (
      rendered.querySelector('[data-table-filter]')
    )
  ...
  it('ports report-style facets, URL state, and progressive disclosure generically', () => {
    ...
    const mode = /** @type {HTMLSelectElement} */ (
      rendered.querySelector('[data-table-facet="mode"]')
    )
    const more = /** @type {HTMLButtonElement} */ (
      rendered.querySelector('[data-table-more]')
    )
  ...
  it('supports report-style filter labels, placeholders, and always-visible facets', () => {
    ...
    expect(rendered.querySelector('input')?.getAttribute('placeholder')).toBe(
      'Run, package, worker, status, or repository',
    )
    expect(
      [...rendered.querySelectorAll('select option')].map(
        (option) => option.textContent,
      ),
    ).toEqual(['All packages', 'Dependabot'])
    expect(rendered.querySelector('output')?.textContent).toBe(
      'Showing 2 of 2 dispatches',
    )
  })
    ...
    const rendered = renderTableRegion({
      ...
      bodyRows: [
        h(
          ...
          null,
          h('td', null, '9'),
          h('td', null, '2026-08-27T10:00:00Z'),
        ),
        h(
          'tr',
          null,
          h('td', null, '10'),
          h('td', null, '2026-08-29T10:00:00Z'),
        ),
        h(
          'tr',
          null,
          h('td', null, '2'),
          h('td', null, '2026-08-28T10:00:00Z'),
        ),
      ],
    })

    const headers = [...rendered.querySelectorAll('th[aria-sort]')]
    const runSort = /** @type {HTMLButtonElement} */ (
      rendered.querySelector('[data-table-sort="0"]')
    )
    const startedSort = /** @type {HTMLButtonElement} */ (
      rendered.querySelector('[data-table-sort="1"]')
    )
    const runValues = () =>
      [...rendered.querySelectorAll('tbody tr')].map(
        (row) => /** @type {HTMLTableRowElement} */ (row).cells[0]?.textContent,
      )
  ...
  it('keeps pagination consistent after sorting', () => {
    const rows = Array.from({ length: 30 }, (_, index) =>
      h('tr', null, h('td', null, String(index + 1))),
    )
    const rendered = renderTableRegion({
      tableClassName: 'custom-table',
      emptyMessage: 'No runs available.',
    ...
    const visible = [...rendered.querySelectorAll('tbody tr')].filter(
      (row) => !(/** @type {HTMLTableRowElement} */ (row).hidden),
    )
  ...
  it('defers table summary computation to the data worker', async () => {
    ...
    expect(
      rendered.querySelector('.table-summary-row')?.hasAttribute('aria-busy'),
    ).toBe(false)
    expect(
      rendered.querySelector('.table-summary-cell')?.getAttribute('aria-busy'),
    ).toBe('true')
    ...
    await vi.waitFor(() => {
      expect(
        rendered
          .querySelector('.table-summary-cell')
