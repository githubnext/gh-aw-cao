...
describe('renderTableRegion', () => {
  it('renders headers and provided body rows', () => {
    const rendered = renderTableRegion({
      ...
      bodyRows: [h('tr', null, h('td', null, '1001'), h('td', null, 'completed'))],
    })
  ...
  it('filters rows and announces the visible result count', () => {
    const rendered = renderTableRegion({
      ...
      bodyRows: [h('tr', null, h('td', null, 'alpha'), h('td', null, 'success')), h('tr', null, h('td', null, 'bravo'), h('td', null, 'failure'))],
    })
  ...
  it('ports report-style facets, URL state, and progressive disclosure generically', () => {
    const rows = Array.from({ length: 60 }, (_, index) =>
      h('tr', null, h('td', null, `workflow-${index + 1}`), h('td', null, index % 2 === 0 ? 'review' : 'live')),
    )
    const rendered = renderTableRegion({
      tableClassName: 'custom-table',
  ...
  it('supports report-style filter labels, placeholders, and always-visible facets', () => {
    const rendered = renderTableRegion({
      ...
      bodyRows: [h('tr', null, h('td', null, 'Dependabot'), h('td', null, 'success')), h('tr', null, h('td', null, 'Dependabot'), h('td', null, 'failure'))],
  ...
  it('sorts rows numerically and temporally when a column header is activated', () => {
    ...
    const runValues = () => [...rendered.querySelectorAll('tbody tr')].map((row) => /** @type {HTMLTableRowElement} */ (row).cells[0]?.textContent)
  ...
  it('keeps pagination consistent after sorting', () => {
    const rows = Array.from({ length: 30 }, (_, index) => h('tr', null, h('td', null, String(index + 1))))
    const rendered = renderTableRegion({
      tableClassName: 'custom-table',
      emptyMessage: 'No runs available.',
      colSpan: 1,
    ...
    const visible = [...rendered.querySelectorAll('tbody tr')].filter((row) => !(/** @type {HTMLTableRowElement} */ (row).hidden))
  ...
  it('reveals lazy-list table rows in bounded batches', () => {
    const rows = Array.from({ length: 60 }, (_, index) => h('tr', null, h('td', null, String(index + 1))))
    const rendered = renderTableRegion({
      tableClassName: 'custom-table',
