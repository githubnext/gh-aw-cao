...
import {
  renderLinkedText,
  createEntityAwareCellRenderer,
} from '../../src/components/linked-text.js'
...
describe('linked text helpers', () => {
  it('renders plain text when no safe link is available and an anchor when a safe link exists', () => {
    ...
    expect(linked.getAttribute('href')).toBe(
      'https://github.com/githubnext/gh-aw-cao',
    )
    expect(linked.getAttribute('target')).toBe('_blank')
    expect(linked.getAttribute('rel')).toBe('noopener noreferrer')
    expect(linked.getAttribute('aria-label')).toBe(
      'View githubnext/gh-aw-cao on GitHub',
    )
  ...
  it('renders entity-aware linked table values only for mapped entity fields with safe links', () => {
    ...
    const linkedRepository = /** @type {HTMLElement} */ (
    ...
    const plainRepository = renderEntityAwareCellValue(
      'repository',
      'gh-aw-cao',
      {},
    )
    ...
    expect(linkedRepository.getAttribute('href')).toBe(
      'https://github.com/githubnext/gh-aw-cao',
    )
