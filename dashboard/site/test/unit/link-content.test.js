...
import {
  ...
  findLink,
  renderExternalLink,
  renderExternalLinkOrFallback,
  renderLinkedValue,
  renderOutcomeLink,
  renderSafeLink,
  renderWorkflowRunLink,
  resolveTitleLink,
} from '../../src/components/link-content.js'
...
describe('link content helpers', () => {
  it('renderSafeLink renders a plain-text fallback and an internal or external anchor', () => {
    ...
    const internal = /** @type {HTMLElement} */ (
      renderSafeLink('Detail', {
    ...
    const external = /** @type {HTMLElement} */ (
      renderSafeLink('Run 4', {
  ...
  it('DLS-SAFE-004 finds only safe https links with non-empty labels', () => {
    ...
    expect(
      findLink(
        {
          link: {
            href: 'https://user:secret@example.com/run/1',
