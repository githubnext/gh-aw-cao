      ...
      rows: [
        {
          ...
          'rollout-mode': 'review',
          'safe-output-kind': 'output',
          'safe-output-label': 'Output',
          'safe-output-status': 'success',
          'safe-output-count': 2,
          'observed-at': '2026-09-09T02:00:00Z'
        },
        {
          organization: 'githubnext',
          repository: 'gh-aw-cao',
          ...
          'rollout-mode': 'review',
          'safe-output-kind': 'missing_data',
          'safe-output-label': 'Missing data',
          'safe-output-status': 'warning',
          'safe-output-count': 1,
          'observed-at': '2026-09-09T03:00:00Z'
        }
      ],
      metadata
    }
...
test('the authored Safe Outputs query projects canonical usage records', async ({ page }) => {
  ...
  expect(payload['safe-output-usage'].rows).toEqual([
    expect.objectContaining({ run: '1003', 'safe-output-label': 'Missing data', 'safe-output-count': 1 }),
    expect.objectContaining({ run: '1002', 'safe-output-label': 'Output', 'safe-output-count': 2 })
  ]);
});
