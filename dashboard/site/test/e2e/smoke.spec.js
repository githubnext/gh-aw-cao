...
test('agents page composes declarative marketplace sections with rendered parity', async ({ page }) => {
  const presenterModuleUrl = buildPresenterModuleUrl();
  const documentModel = JSON.parse(readFileSync(new URL('../../dashboard.json', import.meta.url), 'utf8'));
  await page.setViewportSize({ width: 1200, height: 900 });
  await page.setContent(`
  ...
  await expect(agentsPage.locator('[data-agent-marketplace-section="toolbar"]')).toHaveCount(1);
  await expect(agentsPage.locator('[data-agent-marketplace-section="tiles"]')).toHaveCount(1);
  await expect(agentsPage.locator('.agent-marketplace-tile')).toHaveCount(2);
  await expect(agentsPage.locator('.agent-marketplace-tile').first()).toContainText('Zeta Agent');
  await agentsPage.getByLabel('Sort agents').selectOption('name');
  await expect(agentsPage.locator('.agent-marketplace-title').first()).toHaveText('Alpha Agent');
});
