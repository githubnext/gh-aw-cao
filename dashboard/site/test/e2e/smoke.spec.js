...
test('mobile Catch Up completes with persistent Done, Later, Open, and Notifications routing', async ({ page }) => {
  ...
  await firstCard
    .getByRole('button', {
      name: 'Later Investigate mobile failure',
      exact: true,
    })
    .evaluate((button) => {
...
test('control-plane readiness presents operational evidence in one lazy table', async ({ page }) => {
  ...
  const [localStart, localStop] = await page.evaluate(
    (values) =>
      values.map((value) => {
...
test('pie charts match the report layout at medium viewport widths', async ({ page }) => {
  ...
  const heading = page.getByRole('heading', {
    name: 'AI Credit usage by AW repository',
  })
...
test('DLS-VIEW-013 DLS-VIEW-014 DLS-VIEW-015 DLS-SAFE-006 custom views render available, empty, and unavailable states with only context-permitted observations in browser', async ({
  page,
}) => {
  const presenterModuleUrl = buildPresenterModuleUrl()
  ...
  await expect(
    page
      ...
      .filter({ has: page.getByRole('heading', { name: 'Daily Runs' }) })
      .locator('.view-source'),
  ).toHaveCount(0)
