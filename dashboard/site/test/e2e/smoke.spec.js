...
test('DLS-PAGE-014 DLS-PAGE-015 built-in campaigns page renders value, inventory, and campaign activity in browser', async ({ page }) => {
  ...
  const awDoctorLink = awDoctorSummary.getByRole('link', { name: 'View AW Doctor campaign dashboard' });
  await expect(awDoctorLink).toHaveAttribute('href', '#page-campaign-insights?campaign=aw-doctor');
  ...
  await awDoctorLink.click();
  await expect(page).toHaveURL(/#page-campaign-insights\?campaign=aw-doctor$/);
  const awDoctorNavigation = page.getByRole('navigation', { name: 'AW Doctor views' });
  await expect(awDoctorNavigation.getByRole('link', { name: 'Insights', exact: true })).toHaveAttribute('aria-current', 'page');
  await page.evaluate(() => {
