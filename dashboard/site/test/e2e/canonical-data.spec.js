...
function ghAwLogInput() {
  ...
  return {
    ...
    files: paths.map((path) => ({
      path,
      content: readFileSync(join(fixtureRoot, path), 'utf8'),
    })),
  }
}
      ...
      rows: [
        {
          ...
          workflow: '.github/workflows/dashboard.md',
          run,
          'smell-observation-id': `threat-detection:${run}`,
  ...
  await context.route('http://dashboard.test/**', async (route) => {
    ...
    if (pathname === '/' || pathname === '/index.html') {
      await route.fulfill({
        ...
        body: '<main>Canonical data test</main>',
      })
      return
    }
    ...
    if (pathname === '/sources.json') {
      await route.fulfill({
        ...
        body: JSON.stringify(canonicalSources()),
      })
      return
    }
...
test('data worker queries canonical work items and security findings', async ({
  ...
  expect(result['security-findings']).toMatchObject({
    rows: [
      {
        'smell-observation-id': 'threat-detection:12345',
        'smell-severity': 'high',
      },
    ],
    metadata: { 'source-kind': 'canonical-query' },
