...
function isSkippedTransitionError(error) {
  ...
  return transitionError.name === 'AbortError' || (typeof transitionError.message === 'string' && /skip/i.test(transitionError.message))
}
...
export function renderLazyView({ label, headingLevel = 'h3', minHeight = 280, render }) {
  const element = h(
    ...
    h('div', { className: 'dashboard-lazy-view-skeleton', 'aria-hidden': 'true' }, h('span'), h('span'), h('span')),
  )
...
export function enableLazyViews(root) {
  ...
  const lazyViews = [...root.querySelectorAll('[data-lazy-view]')].filter((element) => element instanceof HTMLElement)
