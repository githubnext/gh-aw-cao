 ...
 */
async function executeAction(id, argumentsValue, onOutput) {
  const response = await fetch(endpoint, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id, arguments: argumentsValue })
  });
  if (!response.ok) {
 ...
 */
function commandPreview(action, values) {
  return [
    action.command,
  ...
  for (const action of actions) {
    ...
    const inputs = (action.arguments ?? []).map((argument) => {
      const input = /** @type {HTMLInputElement} */ (h('input', {
        ...
        onChange: (/** @type {Event} */ event) => {
          ...
          command.textContent = commandPreview(action, argumentValues);
        }
      }));
      return {
        argument,
    ...
    const confirm = /** @type {HTMLButtonElement} */ (h('button', {
      ...
      onClick: async () => {
        ...
        output.hidden = false;
        try {
          const result = await executeAction(action.id, argumentValues, ({ data }) => {
    ...
    trigger = /** @type {HTMLButtonElement} */ (h(
      ...
      {
        type: 'button',
        className: settingsPresentation
    ...
    command = h('code', { className: 'cli-action-command' }, commandPreview(action, argumentValues));
    dialog.append(
      h(
        'header',
