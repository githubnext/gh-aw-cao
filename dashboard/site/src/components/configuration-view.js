...
function renderEntry(name, value, path, segments, onChange, depth = 0) {
  ...
  return h(
    ...
    h(
      'div',
      { className: 'configuration-setting-copy' },
...
function renderSettingsEditor(policyDocument) {
  ...
  return h(
    ...
    h(
      'div',
      { className: 'configuration-editor-toolbar' },
      h('div', null, h('strong', null, '.github/workflows/cao.json'), status),
...
export function renderConfigurationView(context) {
  ...
  return h(
    'section',
    { className: 'configuration-view', 'aria-labelledby': headingId },
