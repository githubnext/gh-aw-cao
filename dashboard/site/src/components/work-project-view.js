...
import { formatUtcDateTime, renderCloseButton, renderCountBadge, renderCountHeader, renderEmptyMessage, renderIconSpan } from './ui-primitives.js';
...
function renderBoard(items, section, onUpdate) {
  ...
  const columns = BOARD_COLUMNS.map((column) => {
    ...
    return h(
      ...
      renderCountHeader(h('h4', null, column.title), columnItems.length, `${columnItems.length} work items`),
      h(
        'div',
        ...
        ...groupWorkItems(columnItems, orchestratedPackages).map((group) => group.grouped
          ? h('section', { className: 'work-card-stack', 'aria-label': `${group.label} work` },
            renderCountHeader(h('strong', null, group.label), group.items.length, `${group.items.length} work items`),
