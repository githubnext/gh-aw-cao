  ...
  const renderItems = (filteredItems) => {
    ...
    if (filteredItems.length === 0) {
      viewBody.replaceChildren(
        renderEmptyMessage('No work items match the current filters.', {
          role: 'status',
        }),
      )
      return
...
function renderWorkFilterBar(items, onChange) {
  ...
  const state = renderFacetSelect(
    'State',
    items.map((item) => item.stateLabel),
  )
  const repository = renderFacetSelect(
    'Repository',
    items.map((item) => item.repository),
  )
  const owner = renderFacetSelect(
    'Workflow owner',
  ...
  const mobileFilterToggle = /** @type {HTMLButtonElement} */ (
    h(
      'button',
      {
        type: 'button',
      ...
      renderIconSpan('work-filter-mobile-icon', 'filter', { ariaHidden: true }),
      'Filters',
    )
  )
...
function renderBoard(items, section, onUpdate) {
  ...
  const columns = BOARD_COLUMNS.map((column) => {
    ...
    return h(
      ...
      h(
        ...
        ...groupWorkItems(columnItems, orchestratedPackages).map((group) =>
          group.grouped
            ? h(
                'section',
                {
  ...
  tabs.append(
    ...BOARD_COLUMNS.map((column) => {
      ...
      const button = h(
        ...
        column.title,
        renderCountBadge(count, `${count} work items`),
      )
      return button
    }),
  )
  return h(
    'section',
    {
      className: section.className,
      id: section.id,
      'aria-label': section.landmarkLabel,
    },
    tabs,
    ...columns,
  )
...
function renderTasks(items, section, onUpdate) {
  ...
  const direction = h(
    ...
    {
      type: 'button',
      className: 'work-task-sort-direction',
      'aria-label': 'Sort descending',
      title: 'Sort descending',
    },
    renderIconSpan('work-task-sort-icon', 'arrow-down', { ariaHidden: true }),
  )
  let descending = true
  const renderRows = () => {
  ...
  const sortableHeader = (field, label) =>
    h(
      ...
      label,
      renderIconSpan('work-task-header-sort-icon', 'triangle-down', {
        ariaHidden: true,
      }),
    )
  ...
  const settingsSheet = h(
    'div',
    { className: 'work-task-settings-sheet' },
    h(
      'div',
      { className: 'work-task-settings-panel' },
      h(
        'header',
      ...
      h(
        'fieldset',
        { className: 'work-mobile-field-settings' },
        h('legend', null, 'Fields shown'),
        ...fieldOptions.map(({ value, label, checked }) =>
          h(
            'label',
            null,
            h('input', {
  ...
  const settingsToggle = /** @type {HTMLButtonElement} */ (
    h(
      'button',
      {
        type: 'button',
      ...
      renderIconSpan('work-task-settings-icon', 'filter', { ariaHidden: true }),
      'Fields & sort',
    )
  )
  ...
  return h(
    ...
    h(
      'div',
      { className: 'work-task-viewbar' },
      h(
        'div',
        { className: 'work-task-view-name' },
        renderIconSpan('work-task-view-icon', 'table', { ariaHidden: true }),
    ...
    h(
      'div',
      { className: 'work-task-scroll' },
      h(
        'div',
        { className: 'work-task-table-header', role: 'row' },
...
function renderRoadmap(items, section, onUpdate) {
  ...
  const zoomMenu = h('div', {
    className: 'work-roadmap-zoom-menu',
    role: 'menu',
  ...
  const previousPeriod = h(
    ...
    renderIconSpan('work-roadmap-period-icon', 'chevron-left', {
      ariaHidden: true,
    }),
  )
  const nextPeriod = h(
    ...
    renderIconSpan('work-roadmap-period-icon', 'chevron-right', {
      ariaHidden: true,
    }),
  )
  const mobilePeriodControls = h(
    'div',
    {
      className: 'work-roadmap-mobile-controls',
      'aria-label': 'Visual timeline period',
    },
    previousPeriod,
    mobilePeriod,
    nextPeriod,
  ...
  const visualToggle = /** @type {HTMLButtonElement} */ (
    h(
      ...
      renderIconSpan('work-roadmap-visual-icon', 'project-roadmap', {
        ariaHidden: true,
      }),
      'Visual',
    )
  ...
  const renderTimeline = () => {
    ...
    const scroll = h(
      ...
      h(
        ...
        h(
          'div',
          { className: 'work-roadmap-calendar' },
          h('div', { className: 'work-roadmap-corner' }, 'Title'),
          h(
            'div',
            {
              className: 'work-roadmap-calendar-grid',
              'aria-label': 'Roadmap calendar',
            },
            h(
              'div',
              { className: 'work-roadmap-quarters' },
            ...
            h(
              'div',
              { className: 'work-roadmap-periods' },
              ...periods.map((period) =>
                h(
                  'span',
                  {
  ...
  zoomMenu.append(
    ...zoomLevels.map((level) =>
      h(
        ...
        renderIconSpan('work-roadmap-zoom-check', 'check', {
          ariaHidden: true,
        }),
        titleCase(level),
      ),
  ...
  root = h(
    ...
    h(
      'div',
      { className: 'work-roadmap-toolbar' },
      h(
        'div',
        null,
        renderIconSpan('work-roadmap-toolbar-icon', 'project-roadmap', {
          ariaHidden: true,
        }),
...
function decorateMobileWorkItem(element, item, choices, onUpdate, variant = 'board') {
  ...
  if (variant === 'roadmap') {
    element
      .querySelector('.work-roadmap-label')
      ?.append(
        h(
          'div',
  ...
  const updateSelect = (label, value, values, property) => {
    ...
    return h(
      'label',
      { className: 'work-mobile-detail-control' },
      h('span', null, label),
      h(
        'select',
        {
  ...
  dialog.append(
    ...
    h(
      ...
      h(
        'section',
        {
          className: 'work-mobile-quick-update',
          'aria-label': `Quick update ${item.name}`,
        },
  ...
  const detailsButton = h(
    ...
    'Details',
    renderIconSpan('work-mobile-details-icon', 'chevron-right', {
      ariaHidden: true,
    }),
  )
...
function timelineDateTicks(extents, range) {
  ...
  if (range === 'day' || range === 'week') {
    ...
    for (let time = extents.start; time < extents.stop; time += step)
      ticks.push({
        time,
  ...
  while (cursor < extents.stop) {
    ...
    for (const day of days) {
      ...
      if (time >= extents.start && time < extents.stop)
        ticks.push({
          time,
          position: ((time - extents.start) / extents.duration) * 100,
          label: String(day),
...
function timelinePeriods(extents, range) {
  if (range === 'day')
    return timelineFixedPeriods(extents, 6 * 3_600_000, (time) =>
  ...
  if (range === 'week')
    return timelineFixedPeriods(extents, 86_400_000, (time) =>
      new Intl.DateTimeFormat('en-US', {
        weekday: 'short',
        day: 'numeric',
        timeZone: 'UTC',
...
function timelineContextBands(extents, range) {
  if (range === 'day')
    return timelineFixedPeriods(extents, extents.duration, (time) =>
      new Intl.DateTimeFormat('en-US', {
        weekday: 'long',
        month: 'short',
        day: 'numeric',
...
function groupWorkItems(items, orchestratedPackages) {
  ...
  for (const item of items) {
    ...
    const group = groups.get(key) ?? {
      key,
      label: item.packageName || item.name,
      grouped,
      items: [],
    }
