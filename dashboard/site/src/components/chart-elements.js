...
const SWIMLANE_FAILURES = new Set([
  'failure',
  'startup-failure',
  'stale',
  'timed-out',
...
export function chartSeriesClassName(name, index) {
  ...
  if (
    SEMANTIC_SERIES_PHRASES.failure.has(normalized) ||
    hasMatchingTerm(terms, SEMANTIC_SERIES_TERMS.failure)
  ) {
    meaning = 'failure'
  } else if (
    SEMANTIC_SERIES_PHRASES.waiting.has(normalized) ||
    ((terms.has('approval') || terms.has('review')) &&
      (terms.has('required') ||
        terms.has('awaiting') ||
        terms.has('pending') ||
        terms.has('waiting'))) ||
  ...
  return meaning
    ? `${paletteClass} chart-series-semantic-${meaning}`
    : paletteClass
}
  ...
  const legendItems =
    chartType === 'scatter'
      ? [
          ...series,
          {
            name: 'Dashed lines show scale guides',
            className: 'chart-grid-key',
          },
        ]
      : series
...
function pieChartSegmentPath(startFraction, endFraction, separated = false) {
  ...
  const gap =
    separated && sweep < 1 - Number.EPSILON
  ...
  const pointAt = (fraction) => {
    ...
    return [
      Number(
        (PIE_CHART_CENTER + Math.cos(angle) * PIE_CHART_RADIUS).toFixed(4),
      ),
      Number(
        (PIE_CHART_CENTER + Math.sin(angle) * PIE_CHART_RADIUS).toFixed(4),
      ),
    ]
  }
    ...
    ([label, value]) => {
      ...
      return [
        ...
        h(
          'small',
          null,
          total > 0 ? formatCoveragePercent(value / total) : '0%',
        ),
      ]
    },
    { 'data-chart-legend': 'visual' },
...
export function renderChartWidget(
  ...
  const pieData =
    chartType === 'pie' ? (pieSummary ?? pieChartEntries(points)) : null
  const entryCount = pieData ? pieData.entries.length : points.length
  const minimumEntries = ['heatmap', 'pie', 'scatter'].includes(chartType)
    ? 1
    : 2
  ...
  if (chartType === 'pie') {
    const { entries, total } =
      /** @type {{ entries: Array<[string, number]>, total: number }} */ (
        pieData
      )
    ...
    return renderChartWidgetShell(
      ...
      h(
        'svg',
        {
          viewBox: '0 0 42 42',
          role: 'img',
          'aria-label': `Pie chart: ${entries.map(([label, value]) => `${label} ${formatNumber(value, unit)}`).join(', ') || 'no data'}`,
        },
        h('circle', {
        ...
        ...entries.map(([label, value], index) => {
          ...
          const endFraction =
            safeTotal > 0 ? cumulativeValue / safeTotal : startFraction
          const segmentLabel = `${label}: ${formatNumber(value, unit)}`
          const midpoint =
            ((startFraction + endFraction) / 2) * Math.PI * 2 - Math.PI / 2
          const tooltipWidth = Math.min(
            40,
            Math.max(18, segmentLabel.length * 1.25 + 5),
          )
          const tooltipX = Math.min(
            Math.max(21 + Math.cos(midpoint) * 14 - tooltipWidth / 2, 1),
            41 - tooltipWidth,
          )
          const tooltipY = Math.min(
            Math.max(21 + Math.sin(midpoint) * 14 - 9, 1),
            34,
          )
          const segment = h(
            'g',
            ...
            renderChartPointTooltip({
              ...
              textLengthAttrs:
                tooltipWidth === 40
                  ? { textLength: 35, lengthAdjust: 'spacingAndGlyphs' }
                  : null,
              label: segmentLabel,
            }),
          )
        ...
        h(
          'text',
          {
            className: 'pie-chart-total-value',
            x: 21,
            y: 20,
            'text-anchor': 'middle',
            'aria-hidden': 'true',
          },
          formatNumber(total, unit, false),
        ),
        h(
          'text',
          {
            className: 'pie-chart-total-label',
            x: 21,
            y: 25.5,
            'text-anchor': 'middle',
            'aria-hidden': 'true',
  ...
  if (chartType === 'histogram') {
    ...
    const xTicks = Array.from(
      ...
      (_, index) =>
        xBoundaries[
          Math.round(
            (index * (xBoundaries.length - 1)) / Math.max(xTickCount - 1, 1),
          )
        ],
    )
    ...
    return renderChartWidgetShell(
      ...
      h(
        'svg',
        {
          viewBox: '0 0 100 42',
          role: 'img',
          'aria-label': `Histogram with ${bins.length} automatically calculated bins`,
        },
        ...[
          [4, maximum],
          [21, maximum / 2],
          ...
          h('line', {
            className: 'histogram-chart-grid',
            x1: plotStart,
            y1: y,
            x2: 100,
            y2: y,
        ...
        ...bins.map((bin, index) => {
          ...
          const mark = h(
            ...
            renderChartPointTooltip({
              ...
              textLengthAttrs:
                label.length > 22
                  ? { textLength: 36, lengthAdjust: 'spacingAndGlyphs' }
                  : null,
              label,
            }),
          )
  ...
  if (chartType === 'line' || chartType === 'dot' || chartType === 'scatter') {
    ...
    const renderedPointLimit = Math.max(
      2,
      Math.floor(MAX_RENDERED_LINE_POINTS / Math.max(groupedSeries.length, 1)),
    )
    const hasWindowHighlight = points.some(
      (point) => typeof point.highlighted === 'boolean',
    )
    const showInteractivePoints = points.length <= MAX_INTERACTIVE_LINE_POINTS
    const seriesClassNames = new Map(
      series.map((item) => [item.name, item.className]),
    )
    const xValues = [...new Set(points.map((point) => point.x))]
    const xIndexes = new Map(xValues.map((value, index) => [value, index]))
    const parsedTimes = isScatterChart
      ? xValues.map((value) => Date.parse(value))
      : []
    ...
    const gridLines = [4, 21, 38].map((y) =>
      h('line', { className: 'line-chart-grid', x1: 0, y1: y, x2: 100, y2: y }),
    )
    ...
    return renderChartWidgetShell(
      ...
      h(
        ...
        ...referenceLines.map(({ seriesName, value }) =>
          h('line', {
        ...
        ...groupedSeries.flatMap(([seriesName, seriesPoints], seriesIndex) => {
          const seriesClassName =
            seriesClassNames.get(seriesName) ?? 'chart-series-1'
          const coordinates = seriesPoints.map((point) => {
            ...
            const x =
              ...
              Number.isFinite(pointTime)
                ? ((pointTime - minimumTime) / (maximumTime - minimumTime)) *
                  100
                : xValues.length < 2
                  ? 50
                  : (xIndex / (xValues.length - 1)) * 100
            const y = 38 - (Math.max(0, toNumber(point.y)) / maximum) * 34
            return { point, x, y }
          })
          const renderedCoordinates = sampleLineCoordinates(
            coordinates,
            renderedPointLimit,
          )
          const highlightedCoordinates = hasWindowHighlight
            ? sampleLineCoordinates(
                coordinates.filter(({ point }) => point.highlighted),
                renderedPointLimit,
              )
            : []
          return [
            ...
            ...(showInteractivePoints
              ? coordinates.map(({ point, x, y }, pointIndex) =>
                  h(
                    'g',
                    {
      ...
      hasWindowHighlight
        ? h(
            'div',
            { className: 'chart-window-key', 'aria-label': 'Chart window key' },
            h(
              'span',
              { className: 'chart-window-context' },
              'Previous context',
            ),
      ...
      xValues.length > 0
        ? h(
            'div',
            {
              className: 'chart-axis timeline-chart-axis',
              'data-chart-axis': chartType,
            },
            ...timelineTicks.map((value) =>
              h('span', { title: value }, formatTimelineTick(value)),
            ),
          )
        : null,
    )
  ...
  const barWidth =
    points.length > 0 ? Math.min(14, (plotWidth * 0.8) / points.length) : 14
  const seriesClassNames = new Map(
    series.map((item) => [item.name, item.className]),
  )
  ...
  return renderChartWidgetShell(
    ...
    h(
      'svg',
      {
        viewBox: '0 0 100 42',
        role: 'img',
        'aria-label': `Bar chart with ${points.length} bars`,
      },
      h(
        'g',
        {
        ...
        ...yTicks.flatMap((value) => {
          ...
          return [
            ...
            h(
              'text',
              { x: BAR_CHART_LEFT - 1.5, y: y + 1, 'text-anchor': 'end' },
      ...
      ...points.map((point, index) => {
        const x =
          BAR_CHART_LEFT +
          ((index + 0.5) / Math.max(points.length, 1)) * plotWidth -
          barWidth / 2
        const height = Math.max(
          1,
          (Math.max(0, toNumber(point.y)) / maximum) * BAR_CHART_HEIGHT,
        )
        return h(
          'rect',
          {
...
function sampleLineCoordinates(coordinates, limit) {
  ...
  for (let bucket = 0; bucket < bucketCount; bucket += 1) {
    ...
    for (let index = start + 1; index < end; index += 1) {
      if (coordinates[index].y < coordinates[minimumIndex].y)
        minimumIndex = index
      if (coordinates[index].y > coordinates[maximumIndex].y)
        maximumIndex = index
    }
...
function renderHeatmapChart(points, valueLabel, unit) {
  const columns = [...new Set(points.map((point) => point.x))].sort(
    (left, right) => left.localeCompare(right),
  )
  const rows = [
    ...new Set(points.map((point) => point.color ?? 'unknown')),
  ...
  const cells = new Map(
    points.map((point) => [
      JSON.stringify([point.x, point.color ?? 'unknown']),
      point,
    ]),
  )
  const values = points
  ...
  return renderChartWidgetShell(
    ...
    h(
      'div',
      {
        className: 'heatmap-scroll-region',
        tabIndex: 0,
        role: 'region',
          ...
          ...rows.map((row) =>
            h(
              ...
              ...columns.map((column) => {
                ...
                const intensity =
                  span > 0 ? 12 + ((value - minimum) / span) * 28 : 26
                return h(
                  'td',
                  {
...
function renderSwimlaneChart(points, timeRange) {
  ...
  const counts = Object.fromEntries(
    SWIMLANE_DEFINITIONS.map(([lane]) => [lane, 0]),
  )
  let plottedCount = 0
  ...
  for (let index = 0; index < points.length; index += 1) {
    ...
    laneIndexes[index] = SWIMLANE_DEFINITIONS.findIndex(
  ...
  const ticks = Array.from(
    { length: 4 },
    (_, index) => start + (span * index) / 3,
  )
  /** @param {number} timestamp */
  const xCoordinate = (timestamp) =>
    SWIMLANE_START_X +
    Math.min(1, Math.max(0, (timestamp - start) / span)) *
      (SWIMLANE_END_X - SWIMLANE_START_X)
  const sectionsByLane = buildSwimlaneSections(
    points,
    timestamps,
    laneIndexes,
    xCoordinate,
      ...
      ...SWIMLANE_DEFINITIONS.flatMap(([lane, label], laneIndex) => {
        ...
        return [
          ...
          ...(sectionsByLane.get(lane) ?? []).map((section) =>
            renderSwimlaneSection(section, y),
          ),
        ]
      }),
      h('line', {
        className: 'swimlane-axis',
        x1: 25,
        y1: 54,
        x2: 117,
        y2: 54,
...
function buildSwimlaneSections(points, timestamps, laneIndexes, xCoordinate) {
  ...
  const binsByLane = new Map(
    SWIMLANE_DEFINITIONS.map(([lane]) => [lane, Array(binCount).fill(null)]),
  )
  for (let pointIndex = 0; pointIndex < points.length; pointIndex += 1) {
    ...
    const index = Math.min(
      binCount - 1,
      Math.max(0, Math.floor((x - SWIMLANE_START_X) / sectionWidth)),
    )
    const bins = /** @type {NonNullable<ReturnType<typeof binsByLane.get>>} */ (
      binsByLane.get(lane)
    )
    ...
    if (bin) {
      ...
      bins[index] = {
        ...
        first: timestamp,
        last: timestamp,
        point: { ...points[pointIndex], lane, timestamp },
      }
    }
  }
...
function swimlaneTooltipLines(point) {
  ...
  const branch = String(
    source.branch ?? source.ref ?? source['head-branch'] ?? '',
  ).trim()
...
function swimlaneConclusion(value) {
  ...
  return SWIMLANE_DEFINITIONS.some(([lane]) => lane === conclusion)
    ? conclusion
    : null
}
...
function lineChartPointSize(pointCount) {
  const progress = Math.min(
    1,
    Math.max(0, (pointCount - 2) / (MIN_RADIUS_POINT_COUNT - 2)),
  )
  return (
    MAX_LINE_POINT_SIZE - progress * (MAX_LINE_POINT_SIZE - MIN_LINE_POINT_SIZE)
  )
}
...
function dotChartPointRadius(pointCount) {
  ...
  return (
    MAX_DOT_POINT_RADIUS -
    progress * (MAX_DOT_POINT_RADIUS - MIN_DOT_POINT_RADIUS)
  )
}
...
function lineChartTimelineTicks(values) {
  ...
  return Array.from(
    { length: tickCount },
    (_, index) =>
      values[Math.round((index / (tickCount - 1)) * (values.length - 1))],
  )
}
...
function scatterChartTimeAxisTicks(values, minimum, maximum) {
  ...
  if (tickCount === 1 || maximum === minimum)
  ...
  return Array.from({ length: tickCount }, (_, index) =>
    new Date(
      minimum + (index / (tickCount - 1)) * (maximum - minimum),
    ).toISOString(),
  )
}
...
function sampledIndexes(valueCount, maximumCount) {
  ...
  return Array.from({ length: tickCount }, (_, index) =>
    Math.round((index / (tickCount - 1)) * (valueCount - 1)),
  )
}
