...
function pieChartSegmentPath(startFraction, endFraction, separated = false) {
  ...
  const gap = separated && sweep < 1 - Number.EPSILON ? Math.min(PIE_CHART_SEGMENT_GAP, sweep * 0.2) : 0
...
export function renderChartWidget(chartType, points, series, pieSummary = null, totalLabel = 'Total', unit = null, timeRange = null, referenceField = null) {
  ...
  if (entryCount < minimumEntries && chartType !== 'swimlane') {
    return renderChartWidgetShell(
      chartType,
      null,
      renderEmptyMessage(entryCount === 0 ? 'No data is available for this visualization.' : 'Not enough data to show this visualization.', { role: 'status' }),
    )
  }
  ...
  if (chartType === 'histogram') {
    ...
    const xBoundaries = bins.length > 0 ? [...bins.map((bin) => bin.lower), bins[bins.length - 1].upper] : []
    const xTickCount = Math.min(xBoundaries.length, MAX_HISTOGRAM_X_TICKS)
    const xTicks = Array.from({ length: xTickCount }, (_, index) => xBoundaries[Math.round((index * (xBoundaries.length - 1)) / Math.max(xTickCount - 1, 1))])
    ...
    return renderChartWidgetShell(
      ...
      bins.length > 0 ? h('div', { className: 'chart-axis', 'data-chart-axis': 'histogram' }, ...xTicks.map((value) => h('span', null, value))) : null,
    )
  }
  ...
  if (chartType === 'line' || chartType === 'dot' || chartType === 'scatter') {
    ...
    const timelineTicks = isScatterChart ? scatterChartTimeAxisTicks(parsedTimes, minimumTime, maximumTime) : lineChartTimelineTicks(xValues)
    let maximum = 1
    for (const point of points) {
    ...
    const referenceLines =
      isDotChart && referenceField
        ? groupedSeries.flatMap(([seriesName, seriesPoints]) =>
            [...new Set(seriesPoints.map((point) => toNumber(point.source?.[referenceField])).filter(Number.isFinite))].map((value) => ({ seriesName, value })),
          )
        : []
      ...
      h(
        ...
        ...groupedSeries.flatMap(([seriesName, seriesPoints], seriesIndex) => {
          ...
          const coordinates = seriesPoints.map((point) => {
            ...
            const x =
              isScatterChart && maximumTime > minimumTime && Number.isFinite(pointTime)
    ...
    h(
      ...
      h(
        ...
        ...xTickIndexes.map((index) => {
          ...
          return h('text', { x, y: 41.5, 'text-anchor': 'middle', title: point.x }, h('title', null, point.x), compactAxisLabel(point.x))
        }),
      ),
    ),
  )
  ...
  return renderChartWidgetShell(
    ...
    h(
      ...
      h(
        ...
        h('thead', null, h('tr', null, h('td', { 'aria-hidden': 'true' }), ...columns.map((column) => h('th', { scope: 'col' }, column)))),
        h(
          'tbody',
          null,
          ...rows.map((row) =>
...
function renderSwimlaneChart(points, timeRange) {
  ...
  if (plottedCount === 0) {
    return renderChartWidgetShell('swimlane', null, renderEmptyMessage('No workflow runs to show.', { role: 'status' }))
  }
  ...
  const xCoordinate = (timestamp) => SWIMLANE_START_X + Math.min(1, Math.max(0, (timestamp - start) / span)) * (SWIMLANE_END_X - SWIMLANE_START_X)
...
function swimlaneTooltipLines(point) {
  ...
  const lines = [formatSwimlaneTooltipTime(point.timestamp), `Conclusion: ${point.lane}`]
...
function formatSwimlaneAxisTime(instant, span) {
  return new Intl.DateTimeFormat(
    'en',
    span <= 86_400_000 ? { hour: 'numeric', minute: '2-digit', timeZone: 'UTC' } : { month: 'short', day: 'numeric', timeZone: 'UTC' },
...
function lineChartTimelineTicks(values) {
  ...
  return Array.from({ length: tickCount }, (_, index) => values[Math.round((index / (tickCount - 1)) * (values.length - 1))])
}
...
function scatterChartTimeAxisTicks(values, minimum, maximum) {
  ...
  return Array.from({ length: tickCount }, (_, index) => new Date(minimum + (index / (tickCount - 1)) * (maximum - minimum)).toISOString())
}
...
function sampledIndexes(valueCount, maximumCount) {
  ...
  return Array.from({ length: tickCount }, (_, index) => Math.round((index / (tickCount - 1)) * (valueCount - 1)))
}
