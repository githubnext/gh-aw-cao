...
export function renderNotificationsInbox(rows, sources = {}) {
  ...
  const previousPage = /** @type {HTMLButtonElement} */ (h('button', {
    type: 'button', className: 'notifications-page-button', 'aria-label': 'Previous notifications'
  }, octicon('chevron-left')));
  const nextPage = /** @type {HTMLButtonElement} */ (h('button', {
    type: 'button', className: 'notifications-page-button', 'aria-label': 'Next notifications'
  ...
  const render = () => {
    ...
    const pageStart = pageIndex * NOTIFICATIONS_PAGE_SIZE;
    currentVisible = visible.slice(pageStart, pageStart + NOTIFICATIONS_PAGE_SIZE);
    count.textContent = visible.length <= NOTIFICATIONS_PAGE_SIZE
      ? `${visible.length} notification${visible.length === 1 ? '' : 's'}`
      : `Showing ${pageStart + 1}–${pageStart + currentVisible.length} of ${visible.length} notifications`;
    previousPage.hidden = visible.length <= NOTIFICATIONS_PAGE_SIZE;
    previousPage.disabled = pageIndex === 0;
    nextPage.hidden = visible.length <= NOTIFICATIONS_PAGE_SIZE;
  ...
  previousPage.addEventListener('click', () => { pageIndex = Math.max(0, pageIndex - 1); render(); });
  nextPage.addEventListener('click', () => { pageIndex += 1; render(); });
  ...
  const main = h('div', { className: 'notifications-main' },
      ...
      h('div', { className: 'notifications-selection-bar' },
        ...
        count,
        h('span', { className: 'notifications-pagination' }, previousPage, nextPage),
        bulkDone),
      list);
