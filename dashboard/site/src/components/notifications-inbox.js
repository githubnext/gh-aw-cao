...
export function renderNotificationsInbox(rows, sources = {}) {
  ...
  const count = h('span', {
    className: 'notifications-result-count',
    'aria-live': 'polite',
  })
  ...
  const all = h(
    'button',
    {
      type: 'button',
      onClick: () => {
        search.value = search.value.replace(stateQuery, '').trim()
        render()
      },
    },
    'All',
  ...
  return h(
    'div',
    {
      className: `notifications-inbox${stories.length > 0 ? ' has-notifications' : ' is-clear'}`,
    },
    health,
    main,
  )
...
function renderOperationalPulse(attentionRows, sources, showNotifications) {
  ...
  const range = /** @type {HTMLSelectElement} */ (
    h(
      'select',
      { 'aria-label': 'Catch-up interval' },
      h(
        'option',
        {
          value: 'since',
          disabled: !lastCatchUp,
          selected: defaultRange === 'since',
        },
        'Since last catch-up',
      ),
  ...
  markCaughtUp.addEventListener('click', () => {
    try {
      globalThis.window?.localStorage.setItem(CATCH_UP_STORAGE_KEY, String(end))
    } catch {
      /* The briefing remains usable without persistence. */
    }
...
function renderCatchUpContent(
  ...
  return h(
    ...
    h(
      'section',
      {
        className: 'home-catchup-stories',
        'aria-labelledby': 'home-catchup-stories-title',
      },
      h(
        'header',
        null,
    ...
    h(
      'section',
      {
        className: 'home-catchup-mobile',
        'aria-labelledby': 'home-catchup-mobile-title',
      },
      h(
        'header',
        null,
        ...
        h(
          'span',
          {
            className: 'home-catchup-mobile-progress',
            'aria-live': 'polite',
            tabindex: '-1',
          },
          size > 0
...
function renderOutcomeMomentum(
  ...
  return h(
    ...
    h(
      'svg',
      {
        className: 'home-momentum-chart',
        viewBox: '0 0 520 126',
        role: 'img',
        'aria-label': `${delivered} delivered outcomes and ${outcomes.filter((row) => row['outcome-state'] === 'pending').length} pending outcomes`,
      },
      ...[24, 54, 84, 114].map((y) =>
        h('line', {
          x1: 8,
      ...
      ...days.flatMap((day, index) => {
        ...
        return [
          h('rect', {
            x,
            y: 114 - deliveredHeight,
            width: 30,
            height: Math.max(0, deliveredHeight),
            rx: 3,
            className: 'home-chart-delivered',
          }),
          h('rect', {
            x,
            y: 114 - deliveredHeight - pendingHeight,
            width: 30,
            height: Math.max(0, pendingHeight),
            rx: 3,
            className: 'home-chart-pending',
          }),
        ]
      }),
    ),
  ...
  return h(
    ...
    h(
      ...
      h(
        'div',
        {
          className: 'home-work-ring',
          style: `--running:${(running / total) * 360}deg;--review:${((running + pending) / total) * 360}deg`,
        },
        h('strong', null, String(running)),
        h('span', null, 'running'),
      ),
      h(
        'dl',
...
function valueSeries(rows, start, end) {
  const buckets = timeBuckets(start, end, 10).map((bucket) => ({
    ...bucket,
    values: /** @type {number[]} */ ([]),
  }))
...
function renderCatchUpStory(story, processStory) {
  ...
  const action = (label, icon, bucket) =>
    h(
      'button',
      {
        type: 'button',
  ...
  return h(
    'article',
    { className: 'home-catchup-story' },
    h(
      story.deepLink ? 'a' : 'span',
      {
...
function renderMobileCatchUpStory(story, processStory) {
  ...
  const card = h(
    'article',
    { className: 'home-catchup-mobile-card' },
    h(
      story.deepLink ? 'a' : 'div',
      {
...
function renderNotification(row, state, selected, bulkDone, render) {
  ...
  const action = (label, icon, active, change) =>
    h(
      'button',
      {
        type: 'button',
        className: `notifications-icon-button${active ? ' active' : ''}`,
        title: label,
        'aria-label': label,
