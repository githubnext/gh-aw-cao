...
function requiresHuman(row) {
  ...
  return !['agent', 'ai agent', 'ai-agent', 'scheduler', 'coordinator', 'orchestrator', 'worker'].includes(actor);
}

/** @param {unknown} actor */
function actorLabel(actor) {
  const value = String(actor || '').trim();
  if (['agent', 'ai-agent'].includes(value.toLowerCase())) return 'AI agent';
  if (value.toLowerCase() === 'orchestrator') return 'Coordinator';
...
function renderNotification(row, state, selected, bulkDone, render) {
  ...
  const attentionState = done ? 'acknowledged' : unread ? 'unread' : 'read';
  ...
  const details = [
    ...
    ['Expected actor', actorLabel(row['expected-actor'])],
  ...
  return h('li', {
    ...
    h('span', { className: 'notification-actions' },
      ...
      action(done ? 'Remove local acknowledgement' : 'Acknowledge locally (does not resolve work)', done ? 'inbox' : 'check-circle', done, () => {
