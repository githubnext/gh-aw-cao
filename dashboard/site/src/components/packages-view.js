...
export function renderPackagesView(sources, pageId = 'packages') {
  return renderPackagesModeShell({
    ...
    sections: [
      {
        id: 'utilization',
        render: (mode) => renderPackageUtilization(sources, mode),
      },
  ...
  return h(
    ...
    h(
      'div',
      {
        ...
        role: 'region',
        'aria-labelledby': `${headingId}-caption`,
        tabIndex: 0,
      },
      h(
        'table',
...
function summarizePackageActivity(packages, sources, mode) {
  ...
  for (const row of activity.rows) {
    ...
    runDetails.set(runKey, {
      packageKey,
      mode: String(row['rollout-mode'] ?? 'unknown'),
    })
    summary.runs += 1
...
function renderUtilizationCard(entry, utilization, available, completeness) {
  ...
  const coverage = !available ? '' : completenessCaveat(completeness, 'usage')
  const ariaLabel =
    ratio === null
...
export function renderRunTrend(sources, mode = 'all') {
  ...
  const series = {
    successful: cumulativeCounts(
      trendDays,
      runs.filter((row) => row['run-conclusion'] === 'success'),
    ),
    failed: cumulativeCounts(
      trendDays,
      runs.filter((row) => isFailureConclusion(row['run-conclusion'])),
    ),
    cancelled: cumulativeCounts(
      trendDays,
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h('text', { x: 8, y: 204 }, '0'),
        h('polyline', {
          className: 'package-chart-successful',
          points: trendPoints(series.successful, maximum),
        }),
        h('polyline', {
          className: 'package-chart-failed',
          points: trendPoints(series.failed, maximum),
        }),
        h('polyline', {
          className: 'package-chart-cancelled',
          points: trendPoints(series.cancelled, maximum),
        }),
      ...
      return /** @type {SVGElement} */ (
        /** @type {unknown} */ (
          h(
            'g',
            {
              className: 'package-chart-point',
              tabIndex: 0,
              role: 'img',
              'aria-label': label,
            },
            h('rect', {
              className: 'package-point-hit',
              x: x - 12,
              y: 40,
              width: 24,
              height: 170,
            ...
            h(
              'g',
              {
                className: 'package-point-tooltip',
                transform: `translate(${tooltipX} 44)`,
                'aria-hidden': 'true',
              },
...
function renderPackageIdentityLink(entry, nameTag) {
  return renderIdentityLink({
    href: packageInsightsHref(entry.id),
    icon: entry.icon,
    label: entry.name,
    labelTag: nameTag,
  })
...
function summarizeUtilization(packages, usage, mode) {
  ...
  for (const run of runs.values()) {
    const total = totals.get(run.packageKey) ?? {
      used: 0,
      allowed: 0,
      reportedRuns: 0,
    }
