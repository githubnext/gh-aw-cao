...
export function renderAgentMarketplaceView(context) {
  ...
  const status = /** @type {HTMLSelectElement} */ (
    h(
      ...
      h(
        'option',
        { value: 'smells' },
        `Smells (${agents.filter((agent) => agentSmellReasons(agent).length > 0).length})`,
      ),
      h(
        'option',
        { value: 'disabled' },
        `Disabled (${agents.filter((agent) => agent.state === 'disabled').length})`,
      ),
      h(
        'option',
        { value: 'slow' },
        `Slow (${agents.filter((agent) => agent.slow).length})`,
      ),
      h(
        'option',
        { value: 'stale' },
  ...
  let activeKind = agents.some((agent) => agent.kind === 'package')
    ? 'package'
    : 'all'
  ...
  const render = () => {
    ...
    for (const button of facetButtons) {
      if (button.dataset.facet === activeKind)
  ...
  const facets = h(
    ...
    facetButton(
      ...
      'package',
      'package',
      agents.filter((agent) => agent.kind === 'package').length,
    ),
    facetButton(
      'Standalone workflows',
      'standalone',
      'workflow',
      agents.filter((agent) => agent.kind === 'standalone').length,
    ),
    ...
    h(
      'select',
      {
        ...
        onChange: (/** @type {Event} */ event) => {
          sortOrder = /** @type {HTMLSelectElement} */ (event.currentTarget)
            .value
          render()
        },
      },
  ...
  return h(
    ...
    agents.length > 0
      ...
      : h(
          'p',
          { role: 'status' },
          'No operation packages or standalone workflows are available in the selected scope.',
        ),
  )
}
...
function renderAgentTile(agent) {
  ...
  const badges = [
    ...
    agent.state === 'disabled'
      ? h('span', { className: 'agent-badge agent-badge-disabled' }, 'Disabled')
      : null,
    agent.slow
      ? h('span', { className: 'agent-badge agent-badge-warning' }, 'Slow')
      : null,
    agent.stale
      ? h('span', { className: 'agent-badge agent-badge-stale' }, 'Stale')
      : null,
  ]
  const indicators = [
    agent.state === 'disabled'
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h(
          'span',
          { className: 'agent-marketplace-description' },
          agent.description,
        ),
...
function agentDetailHref(agent) {
  if (agent.kind === 'package')
    return `#page-package-detail?package=${encodeURIComponent(agent.id.replace(/^package:[^:]+:/, ''))}`
  const workflowKey = agent.workflowKeys[0]
  return workflowKey
    ? `#page-workflow-runtime?workflow=${encodeURIComponent(workflowKey)}`
    : '#page-agents'
}
...
export function smellMark() {
  return h(
    'span',
    { className: 'agent-smell-mark', 'aria-hidden': 'true' },
    octicon('copilot'),
    scentLines(),
  )
}
 ...
 */
export function agentSmellNotifications(
  workflows,
  assignments,
  securityObservations = [],
  smellObservations = [],
) {
  const legacySecurityObservations =
    smellObservations.length > 0 ? [] : securityObservations
  return catalogAgents(
    workflows,
    assignments,
    legacySecurityObservations,
    smellObservations,
  ).flatMap((agent) => {
    ...
    const repositoryHref =
      agent.repositoryLink?.externalHref ?? agent.repositoryLink?.href
    ...
    return [
      {
        ...
        'age-seconds': Number.isFinite(observedAt)
...
export function smellObservationNotifications(rows, options) {
  return rows.map((row) => {
    ...
    const navigationHref =
      options.signalType === 'workflow-smell' &&
      qualifiedRepository !== 'Unknown owner' &&
      workflow
    ...
    return {
      ...
      reason:
        textValue(row['smell-summary']) ||
        textValue(row['smell-evidence']) ||
        'Review the supporting evidence.',
      action:
        textValue(row['smell-recommendation']) ||
...
function catalogAgents(
    ...
    const agents = [
      ...assignments
        .reduce((byId, agent) => {
          ...
          if (
            !existing ||
            agent.totalRuntimeSeconds > existing.totalRuntimeSeconds
          )
  ...
  for (const workflow of workflows) {
    ...
    const key = packaged
      ? `package:${repositoryOwner(workflow)}:${packageId}`
  ...
  return [...groups]
    .map(([key, members]) => {
      const primary =
        members.find(
          (member) => textValue(member['workflow-role']) === 'orchestrator',
        ) ?? members[0]
      const memberKeys = new Set(members.map(workflowKey))
      const runtime = assignments.filter((assignment) =>
        memberKeys.has(assignment.workItemId.replace(/:run:.+$/, '')),
      )
      const totalRuntimeSeconds = runtime.reduce(
        (total, assignment) => total + assignment.totalRuntimeSeconds,
        0,
      )
      const runCount = runtime.reduce(
        (total, assignment) => total + assignment.runCount,
        0,
      )
      const observed =
        runtime
      ...
      const orchestratorCount = members.filter(
        (member) => textValue(member['workflow-role']) === 'orchestrator',
      ).length
      const workerCount = members.filter(
        (member) => textValue(member['workflow-role']) === 'worker',
      ).length
      ...
      return {
        id: key,
        name: packageEntry
          ? textValue(primary['package-name']) ||
            textValue(primary['workflow-name'])
          : textValue(primary['workflow-name']) || textValue(primary.workflow),
        icon:
          textValue(primary['package-icon']) ||
          (packageEntry ? 'package' : 'workflow'),
        description:
        ...
        featured:
          packageEntry &&
          primary['inventory-ready'] === true &&
          Number(primary['package-rollout-percent']) === 100,
        slow:
          runCount > 0 &&
          totalRuntimeSeconds / runCount >= LONG_RUNNING_SECONDS,
        stale:
          runtime.length > 0 && runtime.every((assignment) => assignment.stale),
        ...
        members: members.map((member) => ({
          name:
            textValue(member['workflow-name']) || textValue(member.workflow),
...
function workflowPermissions(workflow) {
  const permissions =
    workflow['gh-aw-metadata'] && typeof workflow['gh-aw-metadata'] === 'object'
      ? /** @type {Record<string, unknown>} */ (workflow['gh-aw-metadata'])
          .permissions
      : null
...
function normalizeAgent(row) {
  const runtime = Number(
    row['total-runtime-seconds'] ?? row['total-run-time-seconds'],
  )
  const totalRuntimeSeconds = Number.isFinite(runtime)
    ? Math.max(0, runtime)
    : 0
  const observed = String(row['last-observed-at'] ?? row['observed-at'] ?? '')
  const ageHours = observed
    ? (Date.now() - Date.parse(observed)) / 3_600_000
    : Number.POSITIVE_INFINITY
  return {
    id:
      textValue(row['agent-id']) ||
      textValue(row['agent-name']) ||
      'unknown-agent',
    name: textValue(row['agent-name']) || 'Unknown agent',
    icon: textValue(row['agent-icon']) || 'copilot',
    description:
      textValue(row['agent-description']) ||
      'Agent assignment and runtime telemetry.',
    ...
    slow:
      Boolean(row['long-running']) ||
      (Number(row['run-count']) > 0 &&
        totalRuntimeSeconds / Number(row['run-count']) >= LONG_RUNNING_SECONDS),
    stale:
      Boolean(row.stale) ||
  ...
  return [
    ...new Set(
      observations
        ...
        .map(
          (row) => `${textValue(row['security-signal']) || 'Threat'} detected`,
        ),
    ),
  ]
}
...
function smellReasons(
  ...
  const structured = smellObservations
    ...
    .map((row) => {
      const name =
        textValue(row['smell-name']) ||
        textValue(row['smell-id']) ||
        'Agent smell'
  ...
  return [
    ...new Set([
      ...structured,
      ...(smellObservations.length > 0
        ? []
        : threatReasons(agentWorkflowKeys, securityObservations)),
    ]),
  ]
}
    ...
    sort === 'name'
      ...
      : sort === 'status'
        ? left.state.localeCompare(right.state) ||
          right.totalRuntimeSeconds - left.totalRuntimeSeconds
        : right.totalRuntimeSeconds - left.totalRuntimeSeconds ||
          left.name.localeCompare(right.name)
}
