...
function renderRuntimeMetrics(context, workflow, runs, usage) {
  ...
  return h(
    'section',
    {
      className: 'repository-workflow-summary workflow-runtime-summary',
  ...
  return h(
    ...
    h(
      ...
      renderChartWidget(
        ...
        [],
        {
          entries: /** @type {Array<[string, number]>} */ (entries),
          total: health.total,
        },
        'runs',
      ),
  ...
  return h(
    ...
    h(
      'div',
      {
        ...
        role: 'group',
        'aria-label': 'Operational-value summary',
      },
...
function renderValueHistoryPanel({ className, headingId, heading, description, body }) {
  return h(
    'section',
    {
      className: `value-history-panel ${className}`,
      'aria-labelledby': headingId,
    },
...
function renderValueHistory(observations) {
  ...
  if (weekly.length > 0) {
    const primaryPoints = weekly.flatMap((week, index) => [
      ...
      {
        x: formatWeek(week.weekStart),
        y: rollingMean(weekly, index),
        color: '4-week rolling mean',
      },
    ])
    const primarySeries = [
...
function primaryChangeSeries(weekly) {
  ...
  const points = weekly.map((week) => ({
    weekStart: week.weekStart,
    change: week.value - first,
  }))
  return [
    {
  ...
  return h(
    ...
    h(
      'svg',
      {
        viewBox: '0 0 100 46',
        role: 'img',
        'aria-label': `Outcome change: ${series.map((item) => `${item.name} ${formatPointChange(item.latestChange)}`).join(', ')}`,
      },
      h('rect', {
        className: 'diagnostic-gain-zone',
        x: 10,
        y: 4,
        width: 88,
        height: 17,
      ...
      h('rect', {
        className: 'diagnostic-loss-zone',
        x: 10,
        y: 21,
        width: 88,
        height: 17,
      ...
      ...grid.flatMap((change) => {
        ...
        return [
          h('line', {
            className: change === 0 ? 'diagnostic-baseline' : 'line-chart-grid',
            x1: 10,
            y1: y,
            x2: 98,
            y2: y,
          ...
          h(
            'text',
            {
              className: 'diagnostic-axis-label',
              x: 8,
              y: y + 1.5,
              'text-anchor': 'end',
            },
            formatPointChange(change, false),
          ),
        ]
      }),
        ...
        return [
          ...
          ...coordinates.map((point) =>
            h(
              'circle',
              {
                className: `diagnostic-point ${className}`,
                cx: point.x,
                cy: point.y,
                r: 0.55,
                tabIndex: 0,
...
function diagnosticSeries(observations) {
  ...
  return [...definitions.values()].flatMap((definition) => {
    ...
    const points = weekly.map((week) => ({
      weekStart: week.weekStart,
      change: (week.value - first) * direction,
    }))
    return [
      {
...
function weeklyAttainment(observations) {
  return [...groupObservationsByWeek(observations)]
    .sort(([left], [right]) => left.localeCompare(right))
    .flatMap(([weekStart, rows]) => {
...
function renderObservationTable(observations) {
  return h(
    'div',
    {
      ...
      role: 'region',
      tabIndex: 0,
      'aria-label': 'Workflow operational-value observations',
    },
    h(
      'table',
      null,
