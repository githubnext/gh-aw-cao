...
export function renderInsightsOverview(context) {
      ...
      ? [
          {
            x: observed,
            y: value,
            color: String(row['operational-value-definition'] || 'Attainment'),
            key: `value-${index}`,
            source: row,
          },
        ]
  ...
  return h(
    'section',
    { className: 'insights-overview', 'aria-label': context.title },
    h(
      'section',
      {
        className: 'insights-value-lead',
        'aria-labelledby': 'insights-value-title',
      },
      h(
        'div',
    ...
    h(
      ...
      renderLazyPanel('Execution health', () =>
        insightPanel(
          'Execution health',
          'Recent completed workflow-run conclusions.',
      ...
      renderLazyPanel('Threat detection', () =>
        insightPanel(
          'Threat detection',
    ...
    renderLazyPanel(
      ...
      () =>
        h(
          'section',
          {
            className: 'insights-experiment-band',
            'aria-labelledby': 'insights-experiments-title',
          },
          h(
            'div',
            { className: 'insights-section-heading' },
...
function renderValueSeriesSelector(points, series, chartHost) {
  ...
  return h(
    'details',
    { className: 'insights-series-selector' },
    h('summary', null, h('span', null, 'Series'), count),
    h(
      'div',
      { className: 'insights-series-menu' },
      h(
        'div',
        { className: 'insights-series-actions' },
...
function dailyPoints(rows, dateField, value, series) {
  ...
  for (const row of rows) {
    ...
    totals.set(key, {
      x: `${day}T00:00:00Z`,
      y: (totals.get(key)?.y ?? 0) + amount,
      color,
    })
  }
  return [...totals.values()].map((point, index) => ({
    ...point,
    key: `daily-${index}`,
  }))
}
