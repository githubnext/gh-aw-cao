...
function renderOutcomeDiscussionSection(outcome) {
  ...
  return h(
    ...
    h(
      ...
      h(
        'div',
        { className: 'post-avatar', 'aria-hidden': 'true' },
        octicon('mark-github'),
      ),
      h(
        'div',
...
function renderOutcomeMetadataSection(outcome) {
  const sourceLink =
    findLink(outcome, 'external-link') ??
  ...
  const workflowName =
    text(outcome['workflow-name']) ||
    text(outcome.workflow) ||
    'Unknown workflow'
  ...
  return h(
    ...
    renderMetadataSection(
      'Disposition',
      renderStatusBadge(titleCase(text(outcome['outcome-state']))),
    ),
    ...
    renderMetadataSection(
      'Mode',
      renderModeBadge(titleCase(text(outcome['rollout-mode']))),
    ),
    renderMetadataSection(
      'Category',
      h('p', null, titleCase(text(outcome['outcome-category']))),
    ),
    renderMetadataSection(
      'Workflow',
      h(
...
function copySafeAttributes(source, target) {
  const safeClasses = [...source.classList].filter((name) =>
    ALLOWED_MARKDOWN_CLASSES.has(name),
  )
  ...
  if (source.tagName === 'A') {
    ...
    source.getAttribute('type') === 'checkbox'
  ) {
