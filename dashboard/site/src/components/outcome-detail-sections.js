  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h('p', null, 'published ', formatUtcDateTime(outcome['published-at']), ' · updated ', formatUtcDateTime(outcome['observed-at'])),
      ),
    ),
    h(
      'div',
...
function renderOutcomeMetadataSection(outcome) {
  const sourceLink = findLink(outcome, 'external-link') ?? findLink(outcome, 'issue-link') ?? findLink(outcome, 'pull-request-link')
  ...
  return h(
    ...
    ...(text(outcome['outcome-warning']) === 'Warning' ? [renderMetadataSection('Warning', renderStatusBadge('Warning'))] : []),
    ...
    renderMetadataSection('Workflow', h('p', null, renderExternalLinkOrFallback(workflowLink, workflowName, workflowName))),
...
const DROPPED_MARKDOWN_TAGS = new Set(['BUTTON', 'EMBED', 'FORM', 'IFRAME', 'MATH', 'OBJECT', 'OPTION', 'SCRIPT', 'SELECT', 'STYLE', 'SVG', 'TEXTAREA'])
...
function sanitizedMarkdownNodes(html) {
  ...
  return [...parsed.body.childNodes].map(cloneSafeMarkdownNode).filter((node) => node !== null)
}
...
export function isOutcomeDetailSectionConfig(value) {
  return isPlainObject(value) && typeof value.body === 'string' && (value.body === 'discussion' || value.body === 'metadata')
}
