...
export function renderPackagesModeShell(options) {
  ...
  const content = h('div', {
    className: 'packages-mode-content',
    id: panelId,
    role: 'tabpanel',
  })
  ...
  function renderMode() {
    ...
    content.setAttribute(
      'aria-labelledby',
      `${options.pageId}-${selectedMode}-tab`,
    )
    content.replaceChildren(
      ...options.sections.map((section) => section.render(selectedMode)),
    )
    content.dispatchEvent(
      new CustomEvent('package-mode-change', {
        bubbles: true,
...
export function isPackageActivityMode(value) {
  return (
    typeof value === 'string' &&
    MODES.includes(/** @type {PackageActivityMode} */ (value))
  )
}
