...
import { batch, derived, effect } from '../reactive.js';
import { clearSources, publishSource, requestSource, sourceState } from '../source-store.js';
import { createFactoryOverviewElements } from './factory-overview-elements.js';
...
export function renderFactoryOverview(context) {
  ...
  const elements = createFactoryOverviewElements({
  ...
  return h(
    ...
    elements.renderIntroduction(sources, metrics),
    elements.renderFloor({
      sources,
      metrics,
      label: pluralLabelResolver(context.elementConfig),
      animateNumbers: context.elementConfig?.animate === 'number'
    })
  );
}
