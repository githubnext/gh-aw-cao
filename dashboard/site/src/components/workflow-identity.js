...
export function renderWorkflowIdentity(workflow) {
  ...
  const sourceLink = link ? { href: link.externalHref ?? link.href, label: 'View authored workflow' } : null
  return h(
    'section',
    { className: 'workflow-identity', 'aria-label': 'Workflow identity' },
    h('div', null, renderWorkflowBadges(workflow), h('p', null, h('code', null, text(workflow.workflow)))),
