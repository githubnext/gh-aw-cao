/**
 * Dashboard sidebar markup and interactions.
 */
...
/**
 * @typedef {{ id: string, title?: string, ['navigation-label']?: string, icon?: string }} SidebarPage
 */

/**
 * @param {SidebarPage[]} pages
 * @param {string} title
 * @param {Array<{ label?: string, pages?: string[], experimental?: boolean }> | undefined} navigation
 * @returns {HTMLElement}
 */
export function renderDashboardSidebar(pages, title, navigation) {
...
/**
 * Restores sidebar state and owns all sidebar-scoped event handling.
 * @param {HTMLElement} root
 */
export function enableDashboardSidebar(root) {
  const appShell = root.querySelector('.app-shell');
  const toggle = root.querySelector('.sidebar-toggle');
  if (appShell instanceof HTMLElement && toggle instanceof HTMLButtonElement) {
  ...
  const mobileMenu = root.querySelector('.mobile-nav-menu');
  if (mobileMenu instanceof HTMLDetailsElement) {
    enableDetailsMenuDismissal(root, mobileMenu, '[data-mobile-nav-page-id]');
  }
}
