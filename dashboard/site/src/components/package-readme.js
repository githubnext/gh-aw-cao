...
export function renderPackageReadme({ packageId, packageName, workflows }) {
  const primary =
    ...
    {}
  const description =
    value(primary['package-description']) ||
    `Operational workflows in the ${packageName} package.`
  const markdown = value(primary['package-readme'])
  const repositoryUrl = ownerUrl(primary)
  const readmeUrl = repositoryFileUrl(
    primary,
    value(primary['package-readme-path']),
  )
  ...
  return h(
    ...
    h(
      'header',
      { className: 'package-marketplace-header' },
      h(
        'div',
        { className: 'package-marketplace-identity' },
        h(
          'span',
          { className: 'package-marketplace-icon', 'aria-hidden': 'true' },
          octicon(value(primary['package-icon']) || 'package'),
        ),
        h(
          'div',
          null,
          h(
            'div',
            { className: 'package-marketplace-title' },
      ...
      h(
        'div',
        { className: 'package-marketplace-actions' },
        readmeUrl ? resourceLink(readmeUrl, 'Read README', 'book') : null,
        repositoryUrl
          ? resourceLink(repositoryUrl, 'View source', 'mark-github')
          : null,
      ),
    ),
    h(
      'div',
      { className: 'package-readme-layout' },
      h(
        'article',
        {
          className: 'package-readme markdown-body',
          'aria-label': `${packageName} README`,
        },
        ...(markdown
          ? renderMarkdownBlocks(markdown, primary)
          : [
              h('h2', null, packageName),
              h('p', null, description),
              h(
                ...
                'Package README content is unavailable in this inventory.',
              ),
            ]),
      ),
      h(
        'aside',
        {
          className: 'package-readme-about',
          'aria-label': `${packageName} package information`,
        },
        h(
          'section',
          null,
          ...
          h(
            ...
            h(
              ...
              null,
              h('dt', null, 'Workflows'),
              h('dd', null, String(workflows.length)),
            ),
            h(
              'div',
              null,
              h('dt', null, 'Owner'),
              h('dd', null, owner(primary)),
            ),
            mode
              ? h(
                  'div',
                  ...
                  h(
                    'dd',
                    { className: `package-rollout package-rollout-${mode}` },
                    mode,
                  ),
                )
              : null,
        ...
        h(
          'details',
          { className: 'package-readme-resources' },
          h(
            'summary',
            null,
            ...
            h(
              'span',
              { className: 'package-readme-resources-hint' },
              'Show details',
            ),
          ),
          h(
            ...
            null,
            readmeUrl
              ? h('li', null, resourceLink(readmeUrl, 'README', 'book'))
              : null,
            repositoryUrl
              ? h(
                  'li',
                  null,
                  resourceLink(repositoryUrl, 'Source repository', 'repo'),
                )
              : null,
          ),
        ),
...
function resourceLink(href, label, icon) {
  return h(
    'a',
    externalAnchorAttrs(href, label),
    octicon(icon),
    h('span', null, label),
  )
}
...
function renderMarkdownBlocks(markdown, source) {
  ...
  for (let index = 0; index < lines.length;) {
    ...
    if (fence) {
      ...
      nodes.push(
        h(
          ...
          h(
            'code',
            { className: fence[1] ? `language-${fence[1]}` : '' },
            body.join('\n'),
          ),
        ),
      )
    ...
    if (heading) {
      nodes.push(
        h(`h${heading[1].length}`, null, ...renderInline(heading[2], source)),
      )
      index += 1
      continue
    }
        ...
        h(
          ...
          h(
            ...
            h(
              ...
              null,
              ...headers.map((cell) =>
                h('th', { scope: 'col' }, ...renderInline(cell, source)),
              ),
            ),
          ),
          h(
            ...
            ...rows.map((row) =>
              h(
                ...
                null,
                ...row.map((cell) =>
                  h('td', null, ...renderInline(cell, source)),
                ),
              ),
            ),
          ),
    ...
    if (/^\s*>/.test(line)) {
      ...
      nodes.push(
        h(
          'blockquote',
          null,
          h('p', null, ...renderInline(quote.join(' '), source)),
        ),
      )
      continue
    }
...
function renderInline(text, source) {
  ...
  return parts.map((part) => {
    if (part.startsWith('`') && part.endsWith('`'))
      return h('code', null, part.slice(1, -1))
    if (part.startsWith('**') && part.endsWith('**'))
      return h('strong', null, part.slice(2, -2))
    const link = part.match(/^\[([^\]]+)\]\(([^)]+)\)$/)
    if (!link) return document.createTextNode(part)
    const href = readmeHref(link[2], source)
    return href
      ? h('a', externalAnchorAttrs(href, link[1]), link[1])
...
function readmeHref(href, source) {
  ...
  try {
    ...
    return resolved.origin === base.origin &&
      resolved.pathname.startsWith(`${new URL(repositoryUrl).pathname}/`)
      ? resolved.href
      : ''
  } catch {
    return ''
...
function owner(source) {
  return (
    [value(source.organization), value(source.repository)]
      .filter(Boolean)
      .join('/') || 'Unknown'
  )
}
