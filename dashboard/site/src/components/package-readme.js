  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h('div', null, h('div', { className: 'package-marketplace-title' }, h('h2', null, packageName), h('span', null, 'Package')), h('p', null, description)),
      ),
      h(
        'div',
    ...
    h(
      ...
      h(
        ...
        h(
          ...
          h('summary', null, h('span', null, 'Resources'), h('span', { className: 'package-readme-resources-hint' }, 'Show details')),
          h(
            'ul',
            null,
...
function startsBlock(lines, index) {
  const line = lines[index]
  return /^\s*(?:```|#{1,6}\s|>|[-*+] |\d+\. )/.test(line) || /^\s*(?:---+|___+|\*\*\*+)\s*$/.test(line) || isTableHeader(lines, index)
}
...
function readmeHref(href, source) {
  ...
  try {
    ...
    return resolved.origin === base.origin && resolved.pathname.startsWith(`${new URL(repositoryUrl).pathname}/`) ? resolved.href : ''
  } catch {
    return ''
  }
}
