 ...
 */
export function clusterScatterPoints(
  points,
  maximumClusters = MAX_RENDERED_SCATTER_POINTS,
) {
  ...
  const series = [...grouped.entries()].sort(([left], [right]) =>
    left.localeCompare(right),
  )
  const quotas = allocateClusterQuotas(
    series.map(([, entries]) => entries.length),
    limit,
  )

  return series.flatMap(([name, entries], seriesIndex) => {
    const sorted = [...entries].sort(
      (left, right) => Date.parse(left.x) - Date.parse(right.x),
    )
...
function allocateClusterQuotas(sizes, limit) {
  ...
  shares.sort(
    (left, right) =>
      (right.exact % 1) - (left.exact % 1) || left.index - right.index,
  )
  while (remaining > 0) {
