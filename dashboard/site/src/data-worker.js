...
async function queryLiveDashboard(requested, context, requestContext) {
  ...
  const canonicalPayload = await queryCanonicalViewSources(indexedDB, liveDashboard.logicalSources, liveDashboard.generation, [...requested])
...
function dashboardContext(value) {
  ...
  return {
    githubUrlBase: typeof context.githubUrlBase === 'string' && context.githubUrlBase ? context.githubUrlBase : 'https://github.com',
...
export function processDataRequest(request) {
  if (request?.operation === 'query-canonical-dashboard') {
    ...
    return queryLiveDashboard(requested, context, /** @type {{ githubUrlBase?: string, dashboardRepository?: string | null }} */ (request.context ?? {}))
  }
  if (request?.operation === 'load-canonical-dashboard') {
    ...
    if (typeof globalThis.location?.origin === 'string' && globalThis.location.origin !== 'null' && sourceUrl.origin !== globalThis.location.origin) {
