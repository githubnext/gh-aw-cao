...
function requestedSourceNames(sourceNames) {
  if (
    ...
    throw new TypeError(
      'Canonical dashboard source names must be an array of strings.',
    )
  }
...
function pageScopedSources(sources, requested) {
  return Object.fromEntries(
    Object.entries(sources).filter(([name]) => requested.has(name)),
  )
}
...
async function queryLiveDashboard(requested, context, requestContext) {
  if (!liveDashboard)
    throw new Error('Canonical dashboard data has not been loaded.')
  ...
  const querySources = [...requested].some((name) =>
    name.startsWith('data-health-'),
  )
    ? {
        ...derivedSources,
        ...deriveDataHealthSources(derivedSources, requestContext),
      }
    : derivedSources
  return deriveDashboardLinkSources(
    pageScopedSources(querySources, requested),
    context,
  )
}
...
function dashboardContext(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new TypeError(
      'Canonical dashboard queries require a dashboard context.',
    )
  }
  const context = /** @type {{ githubUrlBase?: unknown, pages?: unknown }} */ (
    value
  )
  ...
  return {
    githubUrlBase:
      typeof context.githubUrlBase === 'string' && context.githubUrlBase
        ? context.githubUrlBase
        : 'https://github.com',
    pages: /** @type {import('./inferred-sources.js').DashboardPage[]} */ (
      context.pages
    ),
  }
}
...
export function processDataRequest(request) {
  if (request?.operation === 'query-canonical-dashboard') {
    ...
    return queryLiveDashboard(
      requested,
      context,
      /** @type {{ githubUrlBase?: string, dashboardRepository?: string | null }} */ (
        request.context ?? {}
      ),
    )
  }
  if (request?.operation === 'load-canonical-dashboard') {
    ...
    if (!['http:', 'https:'].includes(sourceUrl.protocol)) {
      throw new TypeError(
        'Canonical dashboard source URL must use HTTP or HTTPS.',
      )
    }
    if (
  ...
  if (request?.operation === 'cluster-scatter-points') {
    ...
    if (!Number.isSafeInteger(limit) || limit <= 0) {
      throw new TypeError(
        'Scatter clustering requests require a positive integer limit.',
      )
    }
  ...
  if (request?.operation === 'canonicalize-dashboard-sources') {
    ...
    if (adapted.generation !== request.generation) {
      throw new TypeError(
        'Canonical source generation changed during processing.',
      )
    }
