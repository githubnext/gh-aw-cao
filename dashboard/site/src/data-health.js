...
export function deriveDataHealthSources(sources, context = {}) {
  ...
  const collectorState = ['failed', 'partial', 'unknown', 'complete'].find((state) => collectionRows.some((row) => row.state === state)) ?? 'unknown'
  const compatibilityGaps = compatibilityRows.filter((row) => row.compatibility !== 'compatible').length
  const githubUrlBase = typeof context.githubUrlBase === 'string' && context.githubUrlBase.length > 0 ? context.githubUrlBase.replace(/\/+$/, '') : null
  const dashboardRepository = typeof context.dashboardRepository === 'string' && context.dashboardRepository.length > 0 ? context.dashboardRepository : null
  const activityLink =
...
export function deriveDataHealthCalloutSources(sources) {
  ...
  const metadata = combineSourceMetadata(Object.values(sources))
  return {
    'data-health-collections': healthSource('data-health-collections', collectionDiagnostics(sources), metadata),
    'data-health-reconciliation': healthSource('data-health-reconciliation', reconciliationRows, metadata),
    'data-health-coverage': healthSource('data-health-coverage', coverageRows, metadata),
  }
}
...
function sourceDiagnostic(name, source) {
  ...
  const populatedCells = rows.reduce((total, row) => total + [...fields].filter((field) => hasValue(row?.[field])).length, 0)
...
function inferShape(value, openContainers, depth = 0) {
  if (Array.isArray(value)) {
    ...
    try {
      ...
      return {
        kind: 'array',
        element: elementShapes.length > 0 ? mergeShapes(elementShapes) : { kind: 'primitive', type: 'unknown' },
      }
    } finally {
...
function compatibilityDiagnostics(workflows) {
  ...
  return (workflows?.rows ?? []).map((row) => {
    ...
    const missingCurrentFields =
      producer && currentVersion && producer.major === currentVersion.major && compareVersion(producer, currentVersion) >= 0 && !hasValue(row['gh-aw-metadata'])
        ? ['gh-aw-metadata']
        : []
...
function reconcile(contract, sources) {
  ...
  const state = childSource.metadata?.availability === 'unavailable' ? 'missing' : observed === expected ? 'complete' : 'partial'
