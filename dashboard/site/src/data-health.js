...
export const DOMAIN_DEPENDENCIES = Object.freeze({
  inventory: {
    required: ['repositories', 'workflows'],
    optional: ['organizations'],
  },
  runtime: {
    required: ['workflows', 'runs'],
    optional: ['run-performance', 'job-performance'],
  },
  cost: { required: ['runs', 'usage'], optional: [] },
  models: {
    required: ['workflows', 'usage'],
    optional: ['mcp-calls', 'mcp-servers'],
  },
  detection: {
    required: ['runs', 'detection-observations'],
    optional: ['security-observations'],
  },
  firewall: {
    required: ['runs', 'firewall-observations'],
    optional: ['firewall-policy-rules'],
  },
  'safe-outputs': {
    required: ['runs', 'safe-output-performance'],
    optional: ['findings'],
  },
  outcomes: {
    required: ['safe-output-performance', 'outcomes'],
    optional: ['findings', 'operational-values'],
  },
  configuration: {
    required: ['configuration-policy'],
    optional: ['configuration-summary', 'configuration-actions'],
  },
})
...
const COVERAGE_CONTRACTS = Object.freeze([
  ...
  { area: 'Run queries', source: 'runs', denominator: 'metadata' },
  {
    ...
    expectedKey: 'run-records-expected',
    observedKey: 'run-records-observed',
  },
  ...
  {
    area: 'Firewall evidence',
    source: 'firewall-observations',
    parent: 'runs',
  },
  {
    area: 'Safe Outputs evidence',
    source: 'safe-output-performance',
    parent: 'runs',
  },
  {
    area: 'Outcome reconciliation',
    source: 'outcomes',
    parent: 'safe-output-performance',
  },
])

const RECONCILIATION_CONTRACTS = Object.freeze([
  {
    relationship: 'Inventory → workflows',
    parent: 'repositories',
    child: 'workflows',
    parentKey: repositoryKey,
    childKey: repositoryKey,
    domain: 'inventory',
  },
  {
    relationship: 'Workflows → run queries',
    parent: 'workflows',
    child: 'runs',
    parentKey: workflowKey,
    childKey: workflowKey,
    domain: 'runtime',
  },
  {
    relationship: 'Runs → usage',
    parent: 'runs',
    child: 'usage',
    parentKey: runKey,
    childKey: runKey,
  ...
  {
    relationship: 'Runs → detection',
    parent: 'runs',
    child: 'detection-observations',
    parentKey: runKey,
    childKey: runKey,
    domain: 'detection',
  },
  {
    relationship: 'Runs → firewall',
    parent: 'runs',
    child: 'firewall-observations',
    parentKey: runKey,
    childKey: runKey,
    domain: 'firewall',
  },
  {
    relationship: 'Runs → Safe Outputs',
    parent: 'runs',
    child: 'safe-output-performance',
    parentKey: runKey,
    childKey: runKey,
    domain: 'safe-outputs',
  },
  {
    relationship: 'Safe Outputs → outcomes',
    parent: 'safe-output-performance',
    child: 'outcomes',
    parentKey: runKey,
    childKey: runKey,
...
export function deriveDataHealthSources(sources, context = {}) {
  const sourceRows = Object.entries(sources).map(([name, source]) =>
    sourceDiagnostic(name, source),
  )
  const fieldRows = Object.entries(sources).flatMap(([name, source]) =>
    fieldDiagnostics(name, source),
  )
  const fileRows = Object.entries(sources)
    .map(([name, source]) => loadedFileDiagnostic(name, source))
    .sort(
      (left, right) =>
        right.size - left.size || left.file.localeCompare(right.file),
    )
  const totalSize = fileRows.reduce((total, row) => total + row.size, 0)
  const schemaRows = Object.entries(sources).map(([name, source]) =>
    schemaDiagnostic(name, source),
  )
  const compatibilityRows = compatibilityDiagnostics(sources.workflows)
  const reconciliationRows = RECONCILIATION_CONTRACTS.map((contract) =>
    reconcile(contract, sources),
  )
  const coverageRows = COVERAGE_CONTRACTS.map((contract) =>
    coverageDiagnostic(contract, sources, reconciliationRows),
  )
  const collectionRows = collectionDiagnostics(sources)
  const domainRows = Object.entries(DOMAIN_DEPENDENCIES).map(
    ([domain, dependencies]) =>
      domainDiagnostic(
        domain,
        dependencies,
  ...
  const affectedDomains = domainRows
    .filter((row) => row.confidence !== 'trusted')
    .map((row) => row.domain)
  const repositoryCoverage = coverageRows.find(
    (row) => row.area === 'Repositories',
  )
  const collectorState =
    ['failed', 'partial', 'unknown', 'complete'].find((state) =>
      collectionRows.some((row) => row.state === state),
    ) ?? 'unknown'
  const compatibilityGaps = compatibilityRows.filter(
    (row) => row.compatibility !== 'compatible',
  ).length
  const githubUrlBase =
    typeof context.githubUrlBase === 'string' &&
    context.githubUrlBase.length > 0
      ? context.githubUrlBase.replace(/\/+$/, '')
      : null
  const dashboardRepository =
    typeof context.dashboardRepository === 'string' &&
    context.dashboardRepository.length > 0
      ? context.dashboardRepository
      : null
  const activityLink =
    githubUrlBase && dashboardRepository
      ? {
          href: `${githubUrlBase}/${dashboardRepository}/actions/workflows/activity.yml`,
          label: 'CAO Activity',
        }
      : null

  return {
    'data-health-summary': healthSource(
      'data-health-summary',
      [
        {
          ...
          'affected-domains':
            affectedDomains.length > 0 ? affectedDomains.join(', ') : 'None',
          ...
          'scope-coverage':
            repositoryCoverage?.['coverage-percent'] ?? 'Unknown',
          'collector-state': collectorState,
          'compatibility-gaps':
            compatibilityRows.length === 0 ? 'Unknown' : compatibilityGaps,
          sources: sourceRows.length,
          'available-sources': sourceRows.filter(
            (row) => row.availability === 'available',
          ).length,
          rows: sourceRows.reduce((total, row) => total + row.rows, 0),
          fields: fieldRows.length,
          'total-size': formatBytes(totalSize),
          'populated-cells': sourceRows.reduce(
            (total, row) => total + row['populated-cells'],
            0,
          ),
          'empty-cells': sourceRows.reduce(
            (total, row) => total + row['empty-cells'],
            0,
          ),
    ...
    'data-health-domains': healthSource(
      'data-health-domains',
      domainRows,
      metadata,
    ),
    ...
    'data-health-compatibility': healthSource(
      'data-health-compatibility',
      compatibilityRows,
      metadata,
    ),
    'data-health-reconciliation': healthSource(
      'data-health-reconciliation',
      reconciliationRows,
      metadata,
    ),
    'data-health-coverage': healthSource(
      'data-health-coverage',
      coverageRows,
      metadata,
    ),
    'data-health-sources': healthSource(
      'data-health-sources',
      sourceRows,
      metadata,
    ),
    'data-health-fields': healthSource(
      'data-health-fields',
      fieldRows,
      metadata,
    ),
    'data-health-files': healthSource('data-health-files', fileRows, metadata),
    'data-health-schema': healthSource(
      'data-health-schema',
      schemaRows,
      metadata,
    ),
...
function sourceDiagnostic(name, source) {
  ...
  const populatedCells = rows.reduce(
    (total, row) =>
      total + [...fields].filter((field) => hasValue(row?.[field])).length,
    0,
  )
  ...
  return {
    ...
    'field-coverage':
      fields.size === 0
        ? 'Unknown'
        : formatPercent(populatedFields.length / fields.size),
    'cell-coverage':
      cells === 0 ? 'Unknown' : formatPercent(populatedCells / cells),
    status: confidence,
    availability,
    completeness,
    freshness,
...
function schemaDiagnostic(name, source) {
  const rows = Array.isArray(source?.rows)
    ? source.rows.slice(0, MAX_SCHEMA_SAMPLE_ROWS)
    : []
...
function inferShape(value, openContainers, depth = 0) {
  if (Array.isArray(value)) {
    ...
    try {
      const elementShapes = value.map((item) =>
        inferShape(item, openContainers, depth + 1),
      )
      return {
        kind: 'array',
        element:
          elementShapes.length > 0
  ...
  if (value !== null && typeof value === 'object') {
    ...
    try {
      ...
      for (const [key, propertyValue] of Object.entries(value)) {
        properties[key] = {
          shape: inferShape(propertyValue, openContainers, depth + 1),
          optional: false,
        }
      }
...
function mergeShapes(shapes) {
  ...
  if (kinds.size === 1 && kinds.has('primitive')) {
    const types = [
      ...new Set(
        shapes.map((shape) => /** @type {{ type: string }} */ (shape).type),
      ),
    ].sort()
    const knownTypes = types.filter((type) => type !== 'unknown')
    return {
      kind: 'primitive',
      type: (knownTypes.length > 0 ? knownTypes : types).join(' | '),
    }
  }
  if (kinds.size === 1 && kinds.has('array')) {
    const elementShapes = /** @type {Array<{ element: Shape }>} */ (shapes).map(
      (shape) => shape.element,
    )
  ...
  if (kinds.size === 1 && kinds.has('object')) {
    const objectShapes =
      /** @type {Array<{ properties: Record<string, { shape: Shape, optional: boolean }>}>} */ (
        shapes
      )
    const keys = [
      ...new Set(
        objectShapes.flatMap((shape) => Object.keys(shape.properties)),
      ),
    ].sort()
    /** @type {Record<string, { shape: Shape, optional: boolean }>} */
    const properties = {}
    for (const key of keys) {
      const observed = objectShapes
        .map((shape) => shape.properties[key])
        .filter((property) => property !== undefined)
      properties[key] = {
        shape: mergeShapes(observed.map((property) => property.shape)),
        optional:
          observed.length < objectShapes.length ||
          observed.some((property) => property.optional),
      }
    }
    return { kind: 'object', properties }
  }
  ...
  for (const shape of shapes) {
    if (
      !distinctOptions.some(
        (option) => formatShape(option, 0) === formatShape(shape, 0),
      )
    )
      distinctOptions.push(shape)
  }
  return distinctOptions.length === 1
    ? distinctOptions[0]
    : { kind: 'union', options: distinctOptions }
}
...
function formatShape(shape, depth) {
  ...
  if (shape.kind === 'union')
    return shape.options.map((option) => formatShape(option, depth)).join(' | ')
  if (shape.kind === 'array')
    return `${formatShape(shape.element, depth + 1)}[]`
  ...
  if (keys.length > visibleKeys.length)
    fields.push(`… +${keys.length - visibleKeys.length} more`)
  return `{ ${fields.join(', ')} }`
}
  ...
  return fields.map((field) => {
    ...
    return {
      ...
      coverage:
        rows.length === 0
          ? 'Unknown'
          : formatPercent(populated.length / rows.length),
...
export function evidenceConfidence({
  ...
  if (availability === 'unavailable' || collectionState === 'failed')
    return 'insufficient'
  if (
    [
      availability,
      ...
      compatibility,
      collectionState,
    ].some((value) => value === 'unknown' || value === undefined)
  )
    return 'unknown'
  if (
...
function domainDiagnostic(
  ...
  const required = dependencies.required.map((name) =>
    sourceRows.find((row) => row.source === name),
  )
  const missing = dependencies.required.filter((_, index) => !required[index])
  const affectedCompatibility = compatibilityRows.filter(
    (row) =>
      dependencies.required.includes(row.source) &&
      row.compatibility !== 'compatible',
  )
  const affectedReconciliation = reconciliationRows.filter(
    (row) =>
      row.domain === domain &&
      row.state !== 'complete' &&
  ...
  let reason =
    'Required evidence is available, complete, fresh, compatible, and consistent.'
  if (missing.length > 0 || required.some((row) => row?.status === 'unknown')) {
    confidence = 'unknown'
    reason = `Required evidence state is unknown: ${
      missing.length > 0
        ? missing.join(', ')
        : required
    ...
    required.some((row) => row?.status === 'insufficient') ||
    affectedCompatibility.some((row) => row.compatibility === 'unsupported')
  ) {
    confidence = 'insufficient'
    reason = 'A critical evidence source is unavailable or unsupported.'
  } else if (
    required.some((row) => row?.status === 'degraded') ||
    affectedCompatibility.length > 0 ||
    affectedReconciliation.length > 0
  ) {
    confidence = 'degraded'
  ...
  return {
    ...
    'next-action':
      confidence === 'trusted'
        ? 'No action required.'
        : `Investigate ${domain} evidence diagnostics.`,
  }
}
...
function overallConfidence(domains) {
  ...
  if (insufficient)
    return {
      ...
      action: insufficient['next-action'],
    }
  ...
  if (degraded)
    return {
      ...
      action: degraded['next-action'],
    }
  return {
    state: 'trusted',
    reason: 'All critical evidence domains are trusted.',
...
function coverageDiagnostic(contract, sources, reconciliationRows) {
  ...
  const metadata = source?.metadata ?? {}
  let expected = finiteCount(
    metadata[contract.expectedKey ?? 'coverage-expected'],
  )
  let observed = finiteCount(
    metadata[contract.observedKey ?? 'coverage-observed'],
  )
  if (contract.parent) {
    const suffix =
      `→ ${contract.area.replace(' evidence', '').replace(' telemetry', '')}`.toLowerCase()
    const relationship = reconciliationRows.find((row) =>
      String(row.relationship).toLowerCase().endsWith(suffix),
    )
    expected = relationship
      ? finiteCount(relationship.expected)
      : uniqueKeys(sources[contract.parent]?.rows, runKey).size
    observed = relationship
      ? finiteCount(relationship.observed)
      : uniqueKeys(source?.rows, runKey).size
  }
  const coverage =
    expected === null
      ? null
      : expected === 0
        ? 100
        : Math.round((Math.min(observed ?? 0, expected) / expected) * 100)
  ...
  const horizonComplete =
    requestedStart && requestedEnd
      ? Boolean(
          observedStart &&
          observedEnd &&
          Date.parse(observedStart) <= Date.parse(requestedStart) &&
          Date.parse(observedEnd) >= Date.parse(requestedEnd),
        )
      : null
  return {
    area: contract.area,
    ...
    missing:
      expected === null || observed === null
        ? 'Unknown'
        : Math.max(0, expected - observed),
    'coverage-percent': coverage === null ? 'Unknown' : `${coverage}%`,
    state:
      expected === null || horizonComplete === false
        ? 'unknown'
        : coverage === 100
          ? 'complete'
          : 'partial',
    reason:
      expected === null
        ...
        : horizonComplete === false
          ...
          : coverage === 100
            ? 'Authoritative scope was fully observed.'
            : 'Some expected evidence was not observed.',
    'requested-horizon':
      requestedStart && requestedEnd
        ? `${requestedStart} — ${requestedEnd}`
        : 'Unknown',
...
function collectionDiagnostics(sources) {
  const sourceDiagnostics = Object.entries(sources).map(([name, source]) => {
    ...
    return {
      ...
      progress:
        metadata['collection-progress'] ??
        (state === 'complete' ? 'Complete' : 'Unknown'),
      'collector-completed-at':
        metadata['collector-completed-at'] ?? metadata['retrieved-at'] ?? '',
      'retrieved-at': metadata['retrieved-at'] ?? '',
      'source-as-of': metadata['as-of'] ?? '',
      'evidence-horizon':
        metadata['coverage-start'] && metadata['coverage-end']
          ? `${metadata['coverage-start']} — ${metadata['coverage-end']}`
          : 'Unknown',
      fallback: fallback ? 'stale snapshot' : 'none',
      'snapshot-age':
        metadata['snapshot-age-seconds'] == null
          ? 'Unknown'
          : `${metadata['snapshot-age-seconds']} seconds`,
      provenance: metadata['source-id'] ?? name,
      'technical-detail': '',
      reason:
        metadata['collection-reason'] ??
        confidenceReason({
          availability,
          completeness,
  ...
  const retainedDiagnostics = (sources['coverage-diagnostics']?.rows ?? []).map(
    (row) => ({
      operation: row.title || row.kind || 'coverage-diagnostic',
      source: 'coverage-diagnostics',
      state: 'partial',
      'failure-class': String(row.kind ?? '').includes('rate-limit')
        ? 'rate-limit'
        : 'collection',
      progress: 'Partial',
      'collector-completed-at':
        sources['coverage-diagnostics']?.metadata?.['retrieved-at'] ?? '',
      'retrieved-at':
        sources['coverage-diagnostics']?.metadata?.['retrieved-at'] ?? '',
      'source-as-of':
        sources['coverage-diagnostics']?.metadata?.['as-of'] ?? '',
      'evidence-horizon': 'Unknown',
      fallback:
        row['snapshot-age-seconds'] === '' ||
        row['snapshot-age-seconds'] == null
          ? 'none'
          : 'stale snapshot',
      'snapshot-age':
        row['snapshot-age-seconds'] === '' ||
        row['snapshot-age-seconds'] == null
          ? 'Unknown'
          : `${row['snapshot-age-seconds']} seconds`,
      provenance:
        row.endpoint ||
        sources['coverage-diagnostics']?.metadata?.['source-id'] ||
...
function compatibilityDiagnostics(workflows) {
  const currentVersion =
    workflows?.rows
      ?.map((row) => parseVersion(row['gh-aw-current-version']))
      .find(Boolean) ?? null
  return (workflows?.rows ?? []).map((row) => {
    const producer = parseVersion(row['gh-aw-version'])
    const missingBaseFields = ['organization', 'repository', 'workflow'].filter(
    ...
    if (producer && currentVersion && producer.major !== currentVersion.major) {
      ...
      compatibility = 'limited'
      reason =
        'Supported legacy producer; newer optional telemetry may be absent.'
    } else if (producer) {
      compatibility = 'compatible'
    ...
    if (missingFields.length > 0 && compatibility === 'compatible') {
      compatibility = 'limited'
      reason =
        'A required field is unexpectedly missing from a compatible producer.'
    }
    return {
      source: 'workflows',
      ...
      'affected-domain':
        compatibility === 'compatible'
          ? 'None'
          : 'runtime, cost, models, security',
      reason,
      'next-action':
        compatibility === 'compatible'
          ? 'No action required.'
          : 'Investigate producer compatibility.',
    }
  })
}
...
function reconcile(contract, sources) {
  ...
  if (
    !parentSource ||
    !childSource ||
    parentSource.metadata?.availability === 'unavailable'
  ) {
    return reconciliationRow(
      ...
      null,
      'unknown',
      'Authoritative parent evidence is unavailable.',
    )
  }
  ...
  if (parentSource.metadata?.completeness === 'unknown') {
    return reconciliationRow(
      ...
      null,
      'unknown',
      'Authoritative denominator is unknown.',
    )
  }
  const observedKeys = uniqueKeys(childSource.rows, contract.childKey)
  const observed = [...expectedKeys].filter((key) =>
    observedKeys.has(key),
  ).length
  const expected = expectedKeys.size
  const state =
    childSource.metadata?.availability === 'unavailable'
      ? 'missing'
      : observed === expected
        ? 'complete'
        : 'partial'
  return reconciliationRow(
    ...
    state,
    expected === 0
      ? 'No eligible parent records exist.'
      : observed === expected
        ? 'Stable identifiers reconcile.'
        : 'Expected related records are missing.',
  )
}
...
function reconciliationRow(contract, expected, observed, state, reason) {
  ...
  return {
    ...
    'next-action':
      state === 'complete'
        ? 'No action required.'
        : `Investigate ${contract.relationship} reconciliation.`,
  }
}

/** @param {Record<string, any>} row */
function repositoryKey(row) {
  return row?.organization && row?.repository
    ? `${row.organization}/${row.repository}`
    : ''
}
...
function runKey(row) {
  const repository = repositoryKey(row)
  return repository && row?.run != null && row.run !== ''
    ? `${repository}:${row.run}`
    : ''
}
...
function aggregateAxis(rows, axis) {
  ...
  if (values.includes('partial') || values.includes('stale'))
    return axis === 'freshness' ? 'stale' : 'partial'
