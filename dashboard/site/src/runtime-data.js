...
function deriveEpisodeSummary(model, runsMetadata) {
  ...
  return [
    ...
    {
      label: 'Run window',
      value: `${runsMetadata?.completeness === 'complete' ? 'Complete' : 'Partial'} ${formatCount(windowHours)}h`,
    },
  ...
  return [
    {
      ...
      value: total > 0 ? formatPercent(activated / total) : 'Not observed',
    },
...
function formatWorkerDispatches(workers) {
  return workers.size > 0
    ? [...workers]
        ...
        .map(([worker, count]) => `${worker}: ${formatCount(count)}`)
        .join(', ')
    : '—'
}
...
function combinedMetadata(sources) {
  return (
    sources.runs?.metadata ??
    sources.workflows?.metadata ?? {
