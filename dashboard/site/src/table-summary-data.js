...
const RUN_SUMMARY_LABELS = new Set([
  'run',
  'run link',
  'workflow run',
  'workflow runs',
...
function histogramBinCountForSampleSize(sampleSize) {
  return sampleSize > 0
    ? Math.min(
        sampleSize,
        Math.ceil(Math.log2(sampleSize) + 1),
        MAX_AUTOMATIC_HISTOGRAM_BINS,
      )
    : 0
}
...
function summarizeTableColumn(column) {
  if (
    ['outcome-link', 'run-link', 'evidence-link'].includes(
      column.display ?? '',
    ) ||
    !SUMMARY_TYPES.has(String(column.type ?? ''))
  ) {
    return { kind: 'none' }
  }
...
export function binHistogramValues(values, binCount) {
  ...
  const count = Math.max(
    1,
    Math.min(Math.floor(binCount ?? automaticBinCount), sampleSize),
  )
