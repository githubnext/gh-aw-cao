  ...
  for (const candidate of workflows.rows) {
    ...
    observations.push({
      ...
      data: {
        ...
        ghAwVersionLabel: row['gh-aw-version-label'],
        ghAwUpdateState: row['gh-aw-update-state'] ?? 'unknown',
        ghAwMetadata: row['gh-aw-metadata'],
        ghAwManifest: row['gh-aw-manifest'],
        ...
        admissionStatus: row['admission-status'],
        admissionReason: row['admission-reason'],
        campaignAiCreditAllowance: row['campaign-aic-allowance'],
        campaignWorkerCount: row['campaign-worker-count'],
        campaignInventoryWarnings: row['campaign-inventory-warnings'],
        inventoryReady: row['inventory-ready']
      }
    });
  }
  ...
  for (const candidate of runs.rows) {
    ...
    observations.push({
      ...
      data: {
        ...
        actionMinutes: row['action-minutes'],
        githubApiCalls: row['github-api-calls'],
        safeItemsCount: row['safe-items-count'],
        errorCount: row['error-count'],
        admissionStatus: row['admission-status'],
        admissionReason: row['admission-reason'],
        failureJob: row['failure-job'],
        failureMessage: row['failure-message'],
        failureStep: row['failure-step'],
        failureDetail: row['failure-detail'] ?? row['failure-message'] ?? row['failure-step'] ?? `Run ${githubRunId}`,
        resource: row.resource,
        resourceResetAt: row['resource-reset-at'],
        resourceWaitHours: row['resource-wait-hours'],
        ...
        agentRuntime: row['agent-runtime'],
        firewallVersion: row['firewall-version'],
        gatewayVersion: row['gateway-version'],
        data: row.data,
        logsPayload: row['logs-payload']
      }
    });
  }
  ...
  for (const { kind, document } of runLinkedSources) {
    ...
    observations.push({
      ...
      data: {
        ...
        mcpServerVersion: row['mcp-server-version'],
        mcpProtocolVersion: row['mcp-protocol-version'],
        mcpTool: row['mcp-tool'],
        responseBytes: row['response-bytes'],
