...
export function adaptGhAwLogs(input) {
  ...
  const sessionSourceId = `${canonicalRunId}:run-observed`;
...
export function adaptCachedGhAwJsonl(content) {
  ...
  for (const [index, line] of content.split(/\r?\n/).entries()) {
    ...
    observations.push(
      ...
      { kind: 'session', source: OBSERVATION_SOURCE, sourceId: sessionSourceId, observedAt, data: { id: session, runId: canonicalRunId, kind: 'run-observation', status: optionalString(run.status) ?? 'unknown', startedAt: observedAt, completedAt: run.status === 'completed' ? observedAt : null } },
