...
export function ingestNormalizedJson(indexedDB, input, options) {
  return serializeIngestion(indexedDB, async () => {
    ...
    try {
      ...
      if (![CANONICAL_SCHEMA_VERSION, 17, 16, 15, 14, 13, 12].includes(Number(payload.schemaVersion))) {
    ...
    try {
      ...
      const accept = async (line) => {
        ...
        if (!header) {
          ...
          if (![CANONICAL_SCHEMA_VERSION, 17, 16, 15, 14, 13, 12].includes(schemaVersion)) {
