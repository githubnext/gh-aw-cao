...
const index = JSON.parse(
  await readFile(resolve(corpusDirectory, 'index.json'), 'utf8'),
)
...
for (const entry of index.examples) {
  ...
  assert.equal(
    entry.metadata,
    `examples/${entry.id}.json`,
    `${entry.id}: metadata path must match id`,
  )
  assert.equal(
    entry.dashboard,
  ...
  const metadata = JSON.parse(
    await readFile(resolve(corpusDirectory, entry.metadata), 'utf8'),
  )
  assert.equal(
    metadata.schemaVersion,
    1,
    `${entry.id}: metadata must use schemaVersion 1`,
  )
  ...
  for (const field of [
    ...
    assert.ok(
      Array.isArray(metadata.task[field]),
      `${entry.id}: task.${field} must be an array`,
    )
  }
  ...
  for (const field of [
    ...
    assert.ok(
      ...
      `${entry.id}: operationalValue.${field} is required`,
    )
  }
  assert.equal(
    value.metric.range,
    '[0,1]',
    `${entry.id}: operational value range must be [0,1]`,
  )
  assert.ok(
  ...
  if (value.baseline.mode === 'attainment-only') {
    assert.equal(
      ...
      null,
      `${entry.id}: attainment-only baseline value must be null`,
    )
    assert.equal(
      value.baseline.cutoff,
      null,
      `${entry.id}: attainment-only baseline cutoff must be null`,
    )
  } else {
    assert.ok(
    ...
    assert.ok(
      value.baseline.cutoff,
      `${entry.id}: comparable baseline cutoff is required`,
    )
  }

  assert.equal(
    metadata.dashboard,
    `${entry.id}.dashboard.yml`,
    `${entry.id}: dashboard mismatch`,
  )
  const dashboardSource = await readFile(
    resolve(examplesDirectory, metadata.dashboard),
    'utf8',
  )
...
console.log(
  `Validated ${index.examples.length} generate-dashboard-ir corpus example(s).`,
)
