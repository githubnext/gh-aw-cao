...
async function main() {
  ...
  const dead = renderers.filter((name) => !references.has(name));
  if (dead.length > 0) {
    for (const name of dead) {
      console.error(`${relative(repositoryRoot, rendererPath)}: ELEMENT_RENDERERS contains unreferenced element "${name}".`);
    }
    process.exitCode = 1;
    return;
  }

  console.log(`Validated ${renderers.length} element renderers across ${dashboardPaths.length} dashboard.json file(s).`);
}
