...
export async function buildDashboardSite({
  ...
  if (
    !controlSettings ||
    typeof controlSettings !== 'object' ||
    Array.isArray(controlSettings)
  ) {
  ...
  await Promise.all([
    ...
    cp(
      new URL('dashboard.json', siteRoot),
      join(destinationPath, 'dashboard.json'),
    ),
    cp(new URL('src', siteRoot), join(destinationPath, 'src'), {
      recursive: true,
    }),
  ])
  ...
  await writeFile(
    indexPath,
    configureSite(await readFile(indexPath, 'utf8'), controlSettings),
  )
  ...
  for (const packageName of Object.keys(
    ...
    await access(source)
      .then(() => packageDashboards.push(source))
      .catch((error) => {
  ...
  for (const page of dashboard.dashboard?.pages ?? []) {
    if (
      ...
      !page.id ||
      requiresHashQueryParameter(page)
    )
      continue
...
function requiresHashQueryParameter(page) {
  ...
  return (
    typeof route === 'object' &&
    route !== null &&
    typeof route['hash-query-parameter'] === 'string'
  )
}
...
async function main([destination, settingsPath]) {
  ...
  const controlSettings = JSON.parse(
    await readFile(resolve(settingsPath), 'utf8'),
  )
