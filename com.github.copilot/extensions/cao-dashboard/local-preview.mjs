...
export async function startLocalDashboardPreview({
  ...
  if (typeof localServer.startDashboardServer !== "function") {
    throw new Error(
      ".github/aw/dashboard/local-server.mjs does not export startDashboardServer.",
    );
  }
...
async function findLocalServer(workingDirectory) {
  ...
  const candidates = [
    ...
    resolve(extensionDirectory, "../../../.github/aw/dashboard/local-server.mjs"),
  ];
  ...
  throw new Error(
    "Could not find .github/aw/dashboard/local-server.mjs in the workspace or installed plugin.",
  );
}
