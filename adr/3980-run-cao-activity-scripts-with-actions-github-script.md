...
- New files: `.github/cao/activity/actions-context.mjs`, `.github/cao/activity/local-runner.mjs`, `.github/cao/activity/run-activity.mjs`, `.github/cao/activity/action.yml`, `.github/cao/activity/README.md`, `.github/cao/activity/.env.example` — introducing an Actions-context module, a local runner, an activity entrypoint runner, an action definition, and documentation/example configuration for local debugging.
- Modified: `.github/cao/activity/actions-log.mjs`, `.github/cao/activity/github-telemetry.mjs`, `.github/cao/activity/index.mjs`, `.github/cao/activity/logs.mjs` — adapted to the new entrypoint/logging contract.
...
- `.github/cao/dashboard/report/activity-collectors.mjs`, `.github/cao/dashboard/report/control-settings.mjs`, `.github/cao/dashboard/report/inventory.mjs` — modest adjustments, presumably to remain compatible with the updated activity script contract.
...
To support this, activity-related scripts now export a `main(actions, args)` entrypoint contract, expose the supplied Actions APIs on `globalThis`, and route logging through `core` when available (falling back to console-based logging otherwise). A new `.github/cao/activity/actions-context.mjs` module packages this shared Actions-context, and `.github/cao/activity/action.yml` defines the packaged action. A new `.github/cao/activity/local-runner.mjs`, backed by `@github/local-action` Toolkit shims, supports running the same scripts outside Actions via `local-action` or direct Node invocation, with `.github/cao/activity/README.md` and `.github/cao/activity/.env.example` documenting the local debugging workflow and example environment configuration. `.github/cao/activity/run-activity.mjs` implements the entrypoint runner used in both contexts.
...
- Logging is unified through `core` when available, with console fallback retained, per changes in `.github/cao/activity/actions-log.mjs`.
- Local Node debugging is preserved via `.github/cao/activity/local-runner.mjs` and `@github/local-action` Toolkit shims, so contributors are not forced to run scripts only inside Actions to test them (per PR body and new README documentation).
...
- Downstream dashboard report modules (`activity-collectors.mjs`, `control-settings.mjs`, `inventory.mjs`) required corresponding adjustments, indicating the entrypoint contract change has ripple effects beyond the `.github/cao/activity/` directory. Full scope of any other affected consumers: Not inferable from current pull request evidence.
