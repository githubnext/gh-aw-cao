...
The dashboard's local server (`.github/aw/dashboard/local-server.mjs`) previously produced a single
...
- `.github/aw/dashboard/site/src/source-loader.js` — loads sources incrementally via the manifest.
- `.github/aw/dashboard/site/src/overview-data.js` — consumes incrementally loaded source data.
- `.github/aw/dashboard/site/src/components/notifications-inbox.js` — renders overview notifications
  in viewport-sized batches rather than all at once.
- `.github/aw/dashboard/site/index.html` and `.github/aw/dashboard/site/src/styles.js` — supporting UI changes.
...
- The dashboard site now depends on a two-step fetch protocol (manifest, then per-source
  ...
  `.github/aw/dashboard/local-server.mjs` and `source-loader.js` that must stay in sync (manifest
